...
class Session:
    ...
    def __init__(
        ...
        self._backend: PersistenceBackendProtocol = backend or resolve_backend(
    ...
    def _deserialize_to_model(
        self, model_class: Type[T], row_data: Dict[str, Any]
    ) -> T:
        """
        ...
        try:
            ...
            logger.error(
                f"Error deserializing {model_class.__name__} from row data: {e}"
            )
            logger.error(f"Row data: {original_row_data}")
            raise

    @staticmethod
    def _map_sql_columns_to_attributes(
        model_class: Type[T], row_data: Dict[str, Any]
    ) -> Dict[str, Any]:
