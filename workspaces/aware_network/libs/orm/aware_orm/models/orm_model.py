...
class ORMModel(QueryMixin, CRUDMixin, RelationshipMixin, BranchMixin):
    ...
    2. QueryMixin: QuerySpec-backed read helpers and query builder
    ...
    Usage:
        ...
        # Query methods (via QueryMixin)
        user = await User.by_id(user_id)
        users = await User.where(status="active").limit(10).all()
        user = await User.one(email="alice@example.com")
    ...
    async def push(self) -> None:
        ...
        try:
            ...
            logger.error(
                f"Error in enhanced push for {self.__class__.__name__} {self.id}: {e}"
            )
            raise
    ...
    async def refresh_from_db(self) -> Self:
        ...
        # Query by id without using the identity map.
        refreshed = await self.__class__.by_id(self.id, cache_valid=False)
        ...
        if refreshed:
            ...
            logger.warning(
                f"No data found when refreshing {self.__class__.__name__} {self.id}"
            )
    ...
    def __str__(self) -> str:
        ...
        branch_info = (
            f" (branch: {self.get_branch_id()})" if not self.is_main_branch else ""
        )
