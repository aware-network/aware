...
def register_relationship_metadata(metadata: RelationshipMetadata) -> None:
    ...
    if metadata.source_attribute:
        _relationship_metadata_by_source[
            (metadata.source_class_fqn, metadata.source_attribute)
        ] = metadata
    if metadata.target_attribute:
        _relationship_metadata_by_target[
            (metadata.target_class_fqn, metadata.target_attribute)
        ] = metadata
...
def install_relationship_metadata_from_payload(
    ...
    *,
    ...
    if not relationship_strategies:
        if strict:
            raise BundleInstallError(
                "Environment bundle missing relationship strategies manifest"
            )
        return RelationshipStrategiesInstallResult(
            installed=0, missing_relationships=[], missing_classes=[]
        )
    ...
    try:
        ...
        if strict:
            raise BundleInstallError(
                f"Relationship strategies manifest is not valid JSON: {exc}"
            ) from exc
        return RelationshipStrategiesInstallResult(
            installed=0, missing_relationships=[], missing_classes=[]
        )
    if not isinstance(payload, dict):
        if strict:
            raise BundleInstallError(
                "Relationship strategies manifest root must be an object"
            )
        return RelationshipStrategiesInstallResult(
            installed=0, missing_relationships=[], missing_classes=[]
        )
    ...
    for entry in relationships:
        ...
        metadata = RelationshipMetadata(
            ...
            loading_strategy=(
                str(entry["loading_strategy"])
                if entry.get("loading_strategy") is not None
                else None
            ),
            ...
            relationship_type=(
                ...
                if entry.get("relationship_type") is not None
...
def _extract_join_attr(
    join_keys: Iterable[dict[str, Any]], *, owner: str
) -> str | None:
