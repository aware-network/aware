"""Install SQL runtime metadata from a bindings payload.

This module is intentionally separate from:
- ORM graph binding (see `graph_binding.py`)
- bindings payload adapter (see `bundle_binding.py`)

Contract:
- reads SQL mapping payloads from bindings JSON, under each binding entry:
  - `class_fqn: str`
...
def install_sql_metadata_from_bindings_payload(
    ...
    *,
    ...
    if not bindings:
        if strict:
            raise BundleInstallError(
                "Environment bundle missing bindings manifest (for SQL metadata install)"
            )
    ...
    try:
        ...
        if strict:
            raise BundleInstallError(
                f"Bindings manifest is not valid JSON: {exc}"
            ) from exc
        return SQLMetadataInstallResult(installed=0, missing_classes=[])
            ...
            parsed.append(
                SQLMappingEntry(
                    attribute_name=(
                        ...
                        if item.get("attribute_name") is not None
                        else None
                    ),
                    table_schema=(
                        str(item["table_schema"])
                        if item.get("table_schema") is not None
                        else None
                    ),
                    table_name=(
                        str(item["table_name"])
                        if item.get("table_name") is not None
                        else None
                    ),
                    column_name=(
                    ...
                    fk_owner=(
                        str(item["fk_owner"])
                        if item.get("fk_owner") is not None
                        else None
                    ),
    ...
    if strict and missing_classes:
        raise BundleInstallError(
            f"Missing SQL metadata for bound classes: {sorted(set(missing_classes))}"
        )

    return SQLMetadataInstallResult(
        installed=installed, missing_classes=missing_classes
    )


def _build_sql_runtime_metadata(
    class_config_id: UUID, entries: Iterable[SQLMappingEntry]
) -> SQLRuntimeMetadata:
    ...
    for entry in entries:
        ...
        if entry.fk_columns:
            ...
            if attr not in column_by_attribute:
                source_fk = next(
                    (fk for fk in entry.fk_columns if fk.get("owner") == "source"), None
                )
