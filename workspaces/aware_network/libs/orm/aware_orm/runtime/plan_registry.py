...
def load_plan_registry_from_payload(
    plan_registry: bytes | None,
) -> GraphSQLPlanRegistry | None:
    ...
    return GraphSQLPlanRegistry(
        planner_version=planner_version, plans_by_language=plans_by_language
    )
...
def _parse_plan_descriptor(entry: dict[str, Any]) -> PlanDescriptor:
    ...
    steps: tuple[PlanStepDescriptor, ...] = tuple(
        PlanStepDescriptor(
            ...
            via_relationship_id=(
                str(step["via_relationship_id"])
                if step.get("via_relationship_id") is not None
                else None
            ),
            uses_collection=bool(step.get("uses_collection")),
            join_condition=(
                str(step["join_condition"])
                if step.get("join_condition") is not None
                else None
            ),
    ...
    return PlanDescriptor(
        ...
        plan_hash=(
            str(entry["plan_hash"]) if entry.get("plan_hash") is not None else None
        ),
        steps=steps,
    )
