"""Bind ORM graph entities to ORM classes from a bindings payload.

This is an adapter:
- reads a bindings payload (JSON) and the ORM graph binding snapshot (msgpack)
...
Those stay on their own explicit package-artifact payload loaders.
"""
    ...
    *,
    ...
    if not bindings:
        ...
        return BindingInstallResult(
            bound_count=0, missing_classes=[], planner_version=None
        )
    ...
    try:
        result = bind_entities_by_fqn(
            bindings=pairs, entity_index=entity_index, strict=strict
        )
...
def _parse_binding_manifest(payload: bytes) -> BindingManifest:
    ...
    return BindingManifest(
        version=version, planner_version=planner_version, bindings=bindings
    )
