    ...
    def __post_init__(self) -> None:
        ...
        for predicate in predicates:
            if not _is_predicate(predicate):
                raise TypeError(
                    f"Unsupported QuerySpec predicate: {type(predicate).__name__}"
                )
...
class QueryOrder:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.direction, SortOrder):
            object.__setattr__(
                self, "direction", SortOrder(str(self.direction).lower())
            )
...
class QuerySpec:
    ...
    def __post_init__(self) -> None:
        if self.where is not None and not _is_predicate(self.where):
            raise TypeError(
                f"Unsupported QuerySpec where predicate: {type(self.where).__name__}"
            )
...
def _group_or_single(
    op: Literal["and", "or"], predicates: Iterable[Predicate]
) -> Predicate | None:
...
def _is_predicate(value: object) -> bool:
    return isinstance(value, _FILTER_PREDICATE_TYPES) or isinstance(
