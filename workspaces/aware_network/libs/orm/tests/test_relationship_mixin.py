...
def _attr(name: str) -> AttributeConfig:
    return AttributeConfig(
        name=name, owner_key=name, type_descriptor=_uuid_descriptor()
    )
...
class TestCanonicalFkPropagation:
    def _bind_class_config_for_child(self) -> None:
        ...
        cc = ClassConfig(
            ...
            class_config_attribute_configs=[
                ClassConfigAttributeConfig(
                    class_config_id=uuid4(), attribute_config=ref_attr
                ),
                ClassConfigAttributeConfig(
                    class_config_id=uuid4(), attribute_config=fk_attr
                ),
            ],
        )
    ...
    def test_existing_model_instance_revalidation_is_passthrough(self):
        self._bind_class_config_for_child()
        child = ChildModel(parent={"id": uuid4()})
        assert ChildModel.model_validate(child) is child
