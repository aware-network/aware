...
def _orders_metadata() -> SQLRuntimeMetadata:
    return SQLRuntimeMetadata(
        ...
        persisted_attributes=frozenset(
            {"id", "status", "total", "customer_id", "note"}
        ),
...
def test_query_spec_compiles_boolean_grouping_sorting_and_pagination() -> None:
    ...
    assert (
        "WHERE ((status = $1 OR status = $2) AND total >= $3 AND note IS NOT NULL)"
        in sql
    )
...
def test_query_spec_does_not_expose_flat_filter_compatibility_adapter() -> None:
    assert not hasattr(QuerySpec, "from_filters")
...
def test_query_spec_count_uses_where_and_ignores_ordering_and_pagination() -> None:
    ...
    assert (
        sql
        == "SELECT COUNT(*) as count FROM public.orders WHERE (status = $1 OR status = $2)"
    )
...
def test_query_spec_relation_path_unsupported_operator_fails_explicitly() -> None:
    ...
    spec = QuerySpec(
        where=RelationPathFilter(
            path="customer", field="id", operator="like", value="%east%"
        ),
    )
