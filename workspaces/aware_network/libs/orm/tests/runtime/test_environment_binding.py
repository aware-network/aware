...
def test_install_bindings_from_payload_binds_canonical_class_config(
    tmp_path: Path,
    identity_key: str,
) -> None:
    graph = _build_graph()
    ...
    graph_binding_snapshot = (
        dump_orm_graph_binding_snapshot_msgpack_from_object_config_graph(
            object_config_graph=graph
        )
    )
    bindings_payload = json.dumps(bindings).encode("utf-8")

    with ORMModelRegistry.temporary_clear():
        ORMModelRegistry.register_class_stub(UserModel)
        res = install_bindings_from_payload(
            bindings=bindings_payload,
            orm_graph_binding_snapshot_bytes=graph_binding_snapshot,
            strict=True,
        )
...
def test_install_bindings_from_payload_uses_orm_graph_artifact_metadata(
    tmp_path: Path,
) -> None:
    graph = _build_graph()
    user_cls = next(
