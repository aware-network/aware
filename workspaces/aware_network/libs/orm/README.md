...
serialized query plans. Generated-model reads should use the builder, which
compiles to `QuerySpec` internally:

```python
rows = await Order.query().where(Order.f.status.eq("active")).all()
```

Most service code should not construct `QuerySpec` directly. Prefer generated
model helpers and the query builder. `Model.query(...)` is builder-only; it does
not accept positional `QuerySpec` execution. Backends can implement the
structured `execute_query_spec` hook; otherwise ORM sessions use metadata-bound
SQL generation as a fallback.
...
The retired SQL grammar/bootstrap package source is absent from the ORM package.
Public callers should use `aware_orm.db` and `aware_orm.runtime`.
...
Retired generated-model helpers such as `get_by_id(...)`, `get(...)`,
`get_list(...)`, `find(...)`, `find_all(...)`, `batch_get(...)`,
`exists(...)`, and class-level `count(...)` have been removed. Do not recreate
them in service ontology-replica or product service code; they can route through
generated SQL strings or implicit eager GraphSQL.
...
- repo-only docs, tests, scripts, and caches are excluded from public artifacts;
- retired bootstrap package source remains absent from source and artifacts;
