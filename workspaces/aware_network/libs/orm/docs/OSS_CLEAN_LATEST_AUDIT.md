...
| --- | --- | --- |
| `aware_orm.bootstrap` | Removed legacy kernel/workspace bootstrap adapter | The old SQL grammar/bootstrap CLI rail was retired from source after external consumers moved to current DB/test helpers. Public and monorepo callers should use `aware_orm.db` or `aware_orm.testing` as appropriate. |
| `aware_orm.runtime.bundle_runtime_install` | Removed Environment bundle rail | Package-local ontology artifacts are the public rail; the retired fail-closed stub is absent from source. |
...
PyPI artifacts must not ship retired bootstrap adapters, repo-only docs/tests/
scripts, or local caches. The retired `aware_orm.bootstrap` source package is
absent before packaging, and the sdist/wheel policy is explicit in
`pyproject.toml`:

- include package source, README, changelog, license, and `pyproject.toml`;
- exclude `docs`, `tests`, `scripts`, `.pytest_cache`, `__pycache__`, and
  `*.pyc`;
- guard that `aware_orm.bootstrap` source/import/artifact exposure does not
  return.
...
4. The legacy `aware_orm.bootstrap` package was removed from source after its
   remaining test-container helper consumers moved to `aware_orm.testing`.
...
6. The deprecated-surface removal pass removed the public `bootstrap` extra and
   deleted the remaining stale historical docs from the ORM docs tree; the later
   bootstrap-removal pass deleted the retired `aware_orm.bootstrap` source
   package itself.
...
- Built sdist excludes `docs/`, `tests/`, `scripts/`, `.pytest_cache`,
  `__pycache__`, and `*.pyc`; public-boundary checks also assert that the
  retired `aware_orm.bootstrap` source package and import are absent.
...
has been removed; it is not a public or monorepo DB boot API.
