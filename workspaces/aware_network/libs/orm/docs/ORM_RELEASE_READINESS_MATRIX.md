...
| Legacy bootstrap adapter/source | Removed from source | `aware_orm.bootstrap` source is absent; import and artifact gates guard that the retired package does not return. |
...
| 20 | Build artifact audit | Done | `uv build --package aware-orm` from the selected-kernel path produces a narrow wheel and sdist: package source plus README/changelog/license/metadata; retired bootstrap source is absent before packaging, while repo-only docs/tests/scripts and caches are excluded. | `docs/issues/2026/06/01/fb-2026-06-01-aware-orm-deprecated-surface-removal-v0.md` |
...
- `aware_orm.bootstrap` source is absent and import probes guard against its
  return; repo docs, tests, scripts, and caches are excluded from public
