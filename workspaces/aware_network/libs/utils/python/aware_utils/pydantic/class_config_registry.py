...
_package_class_config_payloads_by_key: dict[
    tuple[str, str], dict[str, RegisteredClassConfigPayload]
] = {}
...
def register_class_configs_from_binding_snapshot_bytes(
    ...
    *,
    ...
    entries = _decode_class_config_payload_entries(
        snapshot_bytes=snapshot_bytes,
        source=source,
    )
    with _lock:
        for entry in entries:
            _class_config_payloads_by_id[entry.class_config_id] = entry
    return len(entries)


def _decode_class_config_payload_entries(
    *,
    snapshot_bytes: bytes,
    source: str | None,
) -> tuple[RegisteredClassConfigPayload, ...]:
    try:
    ...
    if not isinstance(decoded, dict):
        raise TypeError(
            f"Binding snapshot must decode to dict, got {type(decoded).__name__}"
        )
...
def register_pydantic_package_class_configs(
    *,
    ...
    entries = _load_pydantic_package_class_config_payloads(
...
def get_pydantic_package_class_config_payload(
    *,
    package_prefix: str,
    class_config_id: str,
    artifacts_dir: str = DEFAULT_ARTIFACTS_DIR,
    ...
    """Return one package-owned ClassConfig payload without validating all entries."""
    normalized_package_prefix = package_prefix.strip()
    normalized_class_config_id = class_config_id.strip()
    if not normalized_package_prefix or not normalized_class_config_id:
        return None
    entries_by_id = _load_pydantic_package_class_config_payloads_by_id(
        package_prefix=normalized_package_prefix,
        artifacts_dir=artifacts_dir,
        strict=strict,
    )
    entry = entries_by_id.get(normalized_class_config_id)
    return dict(entry.payload) if entry is not None else None
...
def iter_pydantic_package_class_config_payloads(
    *,
    ...
    return tuple(
        _load_pydantic_package_class_config_payloads(
            package_prefix=package_prefix,
            artifacts_dir=artifacts_dir,
            strict=strict,
        )
...
def _load_pydantic_package_class_config_payloads(
    *,
    package_prefix: str,
    artifacts_dir: str,
    strict: bool,
    ...
    return tuple(sorted(entries_by_id.values(), key=lambda item: item.class_config_id))


def _load_pydantic_package_class_config_payloads_by_id(
    *,
    package_prefix: str,
    artifacts_dir: str,
    strict: bool,
) -> dict[str, RegisteredClassConfigPayload]:
    cache_key = (package_prefix, artifacts_dir)
    with _lock:
        cached = _package_class_config_payloads_by_key.get(cache_key)
    ...
    entries_by_id = {entry.class_config_id: entry for entry in entries}
    with _lock:
        _package_class_config_payloads_by_key[cache_key] = dict(entries_by_id)
