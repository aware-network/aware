...
def test_sql_materialization_skips_inline_value_only_source_files_and_prunes_stale(
    tmp_path: Path,
) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)
    transport_code = _build_code(
        tmp_path,
        "transport.aware",
        """
class ExperienceViewInvocationActionRequest : inline_value {
    experience_name String
    request_payload JsonObject = {}
}
""".strip(),
    )
    record_code = _build_code(
        tmp_path,
        "record.aware",
        """
...
""".strip(),
    ...
    ns, _domains = _ns(
        fqn_prefix="pkg",
        namespace="default",
        code_ids=[transport_code.id, record_code.id],
    )
    graph = build_object_config_graph_from_code(
        name="inline-skip",
        description="inline-skip",
        fqn_prefix="pkg",
        file_codes=[
            ("transport.aware", transport_code),
            ("record.aware", record_code),
        ],
        namespace_by_code_id=ns,
    ).graph
    MetaLanguagePluginRegistry.register(SQL_META_PLUGIN)
    ...
    output_root.mkdir()
    stale_transport_sql = output_root / "transport.sql"
    stale_transport_sql.write_text(
    ...
    generated_paths = {generated.path for generated in result.generated_files}
    assert Path("record.sql") in generated_paths
    assert Path("transport.sql") not in generated_paths
    assert (output_root / "record.sql").is_file()
    assert not stale_transport_sql.exists()
