...
class SQLRenderer(ObjectConfigGraphRendererLanguage):
    ...
    def emit_file(
        ...
        body = self.render_source_artifact(
            enums=tuple(enums),
    ...
    def render_source_artifact(
        ...
        *,
        ...
        """Render the same full-file SQL source artifact used by materialization."""

        persistent_classes = tuple(cls for cls in classes if cls.value_mode == ClassValueMode.graph_ref)
        ordered_enums = tuple(sorted(enums, key=lambda e: e.name))
        ordered_classes = tuple(sorted(persistent_classes, key=lambda c: c.name))

        if not ordered_enums and not ordered_classes:
            return ""

        lookup: dict[UUID, ClassConfig] = dict(class_lookup or {})
        for cls in ordered_classes:
            _ = lookup.setdefault(cls.id, cls)

        ddl: list[str] = []
        ddl.append("-- coverage:ignore-file\n")
        ddl.append("-- GENERATED CODE - DO NOT MODIFY BY HAND\n\n")

        for enum in ordered_enums:
            ddl.append(self._emit_enum(enum))
            ddl.append("\n")

        for cls in ordered_classes:
            ddl.append(self._emit_table(cls, class_lookup=lookup))
            ddl.append(self._emit_indexes_for_table(cls))
            ddl.append("\n")

        return "".join(ddl).rstrip() + "\n"

    def render_class_source_artifact(
        self,
        cls: ClassConfig,
        *,
        class_lookup: dict[UUID, ClassConfig] | None = None,
    ) -> str:
        """Render one class as a normal generated SQL source artifact."""

        return self.render_source_artifact(
            classes=(cls,),
            class_lookup=class_lookup,
        )
