    ...
    def is_structural(
        ...
        if file_content:
            ...
            structural_count = sum(
                1 for pattern in structural_patterns if pattern in content_upper
            )
            non_structural_count = sum(
                1 for pattern in non_structural_patterns if pattern in content_upper
            )
...
_SQL_TYPE_DESCRIPTOR_ADAPTER = SqlTypeDescriptorAdapter(
    parser=_SQL_TYPE_PARSER, primitive_codec=_SQL_PRIMITIVE_CODEC
)
