...
class DartPackageStrategy(ObjectConfigGraphPackageStrategy):
    ...
    def _render_pubspec(
        ...
        *,
        ...
        package_dependency_mode = _dart_package_dependency_mode(metadata)
        ...
        missing_runtime_defaults: list[tuple[str, str]] = []
        for dependency in dart_generated_runtime_dependency_specs(
            source="dart.package_strategy.fallback"
        ):
            dependency_name = (
                dependency.package_manager_name or dependency.name
            ).strip()
            if self._dependency_declared(deps, dependency_name):
                continue
            missing_runtime_defaults.append(
                (
                    dependency_name,
                    render_materialization_package_dependency(dependency),
                )
            )
        missing_dev_defaults: list[tuple[str, str]] = []
        for dependency in dart_generated_dev_dependency_specs(
            source="dart.package_strategy.fallback"
        ):
            dependency_name = (
                dependency.package_manager_name or dependency.name
            ).strip()
            if self._dependency_declared(dev_deps, dependency_name):
                continue
            missing_dev_defaults.append(
                (
                    dependency_name,
                    render_materialization_package_dependency(dependency),
                )
            )
        _handle_missing_dart_package_dependencies(
            dependency_names=tuple(
                name for name, _ in (*missing_runtime_defaults, *missing_dev_defaults)
            ),
            mode=package_dependency_mode,
        )
        if package_dependency_mode != "strict":
            deps.extend(rendered for _, rendered in missing_runtime_defaults)
            dev_deps = [*dev_deps, *(rendered for _, rendered in missing_dev_defaults)]
        ...
        if not self._dependency_declared(deps, DART_AWARE_MODEL_HELPERS_PACKAGE_NAME):
            _handle_missing_dart_package_dependencies(
                dependency_names=(DART_AWARE_MODEL_HELPERS_PACKAGE_NAME,),
                mode=package_dependency_mode,
            )
            if package_dependency_mode == "strict":
                raise AssertionError("unreachable strict package dependency fallback")
            ...
            deps.append(
                render_materialization_package_dependency(
                    dart_path_dependency_spec(
                        name=DART_AWARE_MODEL_HELPERS_PACKAGE_NAME,
                        path=Path(rel).as_posix(),
                        source="dart.package_strategy.fallback",
                    )
                )
        ...
        if not (
            is_api_package or is_ontology_dto_package
        ) and not self._dependency_declared(deps, DART_AWARE_API_PACKAGE_NAME):
            _handle_missing_dart_package_dependencies(
                dependency_names=(DART_AWARE_API_PACKAGE_NAME,),
                mode=package_dependency_mode,
            )
            if package_dependency_mode == "strict":
                raise AssertionError("unreachable strict package dependency fallback")
            ...
            deps.append(
                render_materialization_package_dependency(
                    dart_path_dependency_spec(
                        name=DART_AWARE_API_PACKAGE_NAME,
                        path=Path(rel).as_posix(),
                        source="dart.package_strategy.fallback",
                    )
                )
...
def _dart_package_dependency_mode(metadata: dict[str, object]) -> str:
    raw_mode = (
        str(
            metadata.get("package_dependencies_mode")
            or metadata.get("package_dependency_mode")
            or ""
        )
        .strip()
        .lower()
    )
    if raw_mode in {"fallback", "warn", "strict"}:
        return raw_mode
    strict_value = DartPackageStrategy._metadata_bool(
        metadata.get("strict_package_dependencies")
    )
    if strict_value:
        return "strict"
    warn_value = DartPackageStrategy._metadata_bool(
        metadata.get("warn_hidden_package_dependencies")
    )
    if warn_value:
        return "warn"
    return "fallback"
...
def _handle_missing_dart_package_dependencies(
    *,
    dependency_names: tuple[str, ...],
    mode: str,
) -> None:
    names = tuple(dict.fromkeys(name for name in dependency_names if name))
    if not names:
        return
    details = ", ".join(names)
    if mode == "strict":
        raise ValueError(
            "Dart package dependencies must be declared explicitly; missing: " + details
        )
    if mode == "warn":
        warnings.warn(
            "Dart package strategy filled undeclared package dependencies: " + details,
            RuntimeWarning,
            stacklevel=3,
        )
