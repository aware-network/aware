...
from aware_code.language.contracts import (
    CodeNamespaceEntry,
    CodeNamespaceGroup,
...
class DartAwareModelsLayout(CodeLanguagePluginLayout):
    ...
    Interprets aware_models paths like:
    <root>/aware_models/lib/<group>/<entry>/... ->
      group = <group>, entry = <entry>
      group_path = <root>/aware_models/lib/<group>
      entry_path = <entry>
    """
    ...
    def extract_namespace_groups(
        ...
        for rel_path in file_paths:
            ...
            if root is None:
                continue
            idx, _root_segment = root

            # Require explicit aware_models/lib structure
            # Need at least: root/aware_models/lib/<group>/<entry>
            if len(parts) < idx + 5:
                continue
            if parts[idx + 1] != "aware_models" or parts[idx + 2] != "lib":
                continue

            group_name = parts[idx + 3]
            raw_entry_name = parts[idx + 4]

            # Normalize known folder aliases for Dart layout without changing the path.
            # Only apply for meta: class_ -> class. Do NOT alias identity:auth (kept explicit).
            entry_name = self._normalize_namespace_entry_name(
                group_name, raw_entry_name
            )

            group_path = "/".join(parts[: idx + 4])

            group_paths[group_name] = group_path
            if group_name not in groups_map:
                groups_map[group_name] = set()
            groups_map[group_name].add(entry_name)

        result: list[CodeNamespaceGroup] = []
        for group_name, entry_set in groups_map.items():
            group_path = group_paths.get(group_name, "")
            # Keep the on-disk folder as the path, but expose normalized entry names.
            # Since we only normalize meta/class_ -> meta/class, the folder stays
            # 'class_' while the logical entry is 'class'. For other entries, name == path.
            entries = [
                CodeNamespaceEntry(
                    name=entry,
                    path=(
                        "class_"
                        if (group_name == "meta" and entry == "class")
                        else entry
                    ),
                )
    ...
    def _normalize_namespace_entry_name(self, group: str, entry: str) -> str:
        """
        Normalize Dart-specific folder names to canonical namespace entry names without changing paths.

        - meta: class_ -> class
        - all others unchanged (e.g., identity:auth stays as 'auth')
        """
        if group == "meta" and entry == "class_":
            return "class"
        return entry
