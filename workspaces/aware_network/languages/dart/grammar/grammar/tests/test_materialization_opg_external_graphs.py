...
def test_materialization_opg_registers_external_graph_materializations(
    tmp_path: Path,
) -> None:
    ...
    with disable_autobind():
        ...
        external_graph.object_config_graph_nodes = [
            make_class_node(
                object_config_graph_id=external_graph.id, class_config=external_cls
            )
        ]
        ...
        graph.object_config_graph_nodes = [
            make_class_node(object_config_graph_id=graph.id, class_config=root_cls)
        ]
    ...
    assert (
        "package:aware_content/_aware/materialization/materializers_opg.dart"
        in dart_source
    )
...
def test_materialization_opg_uses_external_owner_import_for_external_root(
    tmp_path: Path,
) -> None:
    layout = DartLayoutStrategyTemplateMixin(
        base_dir=tmp_path,
    ...
    with disable_autobind():
        ...
        external_graph.object_config_graph_nodes = [
            make_class_node(
                object_config_graph_id=external_graph.id, class_config=external_root_cls
            )
        ]
    ...
    assert (
        "package:aware_environment_ontology/default/code_module.dart" not in dart_source
    )
