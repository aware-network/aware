...
def test_dart_package_strategy_warns_for_undeclared_generated_dependencies(
    tmp_path: Path,
) -> None:
    strategy = DartPackageStrategy(base_dir=tmp_path)
    spec = ObjectConfigGraphPackageSpec(
        ...
        version="0.1.0",
        dependencies=list(_DECLARED_WORKSPACE_DEPS),
        metadata={"package_dependencies_mode": "warn"},
    )

    with pytest.warns(RuntimeWarning, match="freezed_annotation"):
        pubspec = strategy._render_pubspec(
            "aware_meta_ontology", spec, package_root=tmp_path
        )

    assert "freezed_annotation: ^3.0.0" in pubspec
    assert "json_annotation: ^4.9.0" in pubspec
    assert "uuid: ^4.5.1" in pubspec
    assert "build_runner: ^2.4.11" in pubspec
    assert "freezed: ^3.0.0" in pubspec
    assert "json_serializable: ^6.8.0" in pubspec


def test_dart_package_strategy_strict_mode_requires_declared_generated_dependencies(
    tmp_path: Path,
) -> None:
    strategy = DartPackageStrategy(base_dir=tmp_path)
    spec = ObjectConfigGraphPackageSpec(
        ...
        version="0.1.0",
        dependencies=list(_DECLARED_WORKSPACE_DEPS),
        metadata={"package_dependencies_mode": "strict"},
    )

    with pytest.raises(ValueError, match="freezed_annotation"):
        strategy._render_pubspec("aware_meta_ontology", spec, package_root=tmp_path)
