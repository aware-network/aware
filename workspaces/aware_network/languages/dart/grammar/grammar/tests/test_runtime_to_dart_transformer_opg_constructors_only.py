...
from dart_grammar.transformers.runtime_to_dart_transformer import (
    RuntimeToDartTransformer,
)
...
def _namespace_by_code_id(
    *, fqn_prefix: str, namespace: str, code_ids: list[UUID]
...
def test_runtime_to_dart_transformer_drops_non_opg_constructors(tmp_path: Path) -> None:
    ...
    assert any(
        (l.function_config.name == "signup")
        for l in identity_cc.class_config_function_configs
    )
    assert any(
        (l.function_config.name == "create_human")
        for l in human_cc.class_config_function_configs
    )
    signup_link = next(
        l
        for l in identity_cc.class_config_function_configs
        if l.function_config.name == "signup"
    )
    ...
    identity_names = [
        l.function_config.name for l in identity_cc.class_config_function_configs
    ]
    human_names = [
        l.function_config.name for l in human_cc.class_config_function_configs
    ]
    ...
    function_names_after = [
        ...
        for n in ocg.object_config_graph_nodes
        if n.type == ObjectConfigGraphNodeType.function
