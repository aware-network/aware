...
def test_ontology_dto_policy_keeps_internal_relationships_and_external_dto_imports(
    ...
    graph = ObjectConfigGraph(
        ...
        object_config_graph_nodes=[
            ...
            make_class_node(
                object_config_graph_id=parent.id, class_config=internal_child
            ),
        ],
    )
    ...
    renderer.import_overrides = {
        str(
            external_child.id
        ): "package:aware_content_ontology_dto/chain/content_chain_model.dart"
    }
    ...
    assert (
        "@Default(const []) List<ConversationMessage> conversationMessages"
        in dart_source
    )
