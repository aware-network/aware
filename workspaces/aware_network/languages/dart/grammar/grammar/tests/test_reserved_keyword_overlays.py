...
def test_reserved_keyword_overlays_generate_dart_enum_option_overlays(
    tmp_path: Path,
) -> None:
...
""".strip(),
    ...
    dart_overlay = next(
        (
            ...
            if ov.language == CodeLanguage.dart
        ),
        None,
    )
    assert (
        dart_overlay is not None
    ), "Expected a Dart overlay to be generated for reserved enum options"
    ...
    overlays_by_opt_id = {
        o.enum_option_id: o for o in dart_overlay.enum_option_overlays
    }
