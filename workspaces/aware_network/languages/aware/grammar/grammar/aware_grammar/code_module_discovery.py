...
class AwareCodeModuleDiscovery(CodeModuleDiscovery):
    ...
    def get_module_name(self, module_path: Path, workspace_root: Path) -> str:
        return self._resolve_module(
            path=module_path, workspace_root=workspace_root
        ).module_id
    ...
    @override
    def get_metadata(
        self, module_path: Path, workspace_root: Path
    ) -> dict[str, object]:
        return self._resolve_module(
            path=module_path, workspace_root=workspace_root
        ).to_metadata()
    ...
    def _resolve_module(
        self, *, path: Path, workspace_root: Path
    ) -> _ResolvedAwareModule:
        ...
        for package_spec in module_spec.packages:
            ...
            source_root_rel = self._relative_to_workspace(
                path=source_root, workspace_root=workspace_root
            )
            ...
            packages.append(
                _ResolvedAwarePackage(
                    ...
                    package_root=self._relative_to_workspace(
                        path=package_root, workspace_root=workspace_root
                    ),
        ...
        resolved = _ResolvedAwareModule(
            ...
            module_toml_path=self._relative_to_workspace(
                path=module_toml_path, workspace_root=workspace_root
            ),
        ...
        *,
        ...
        for pattern in include_paths:
            ...
            try:
                ...
                logger.debug(
                    f"Failed to glob {raw_pattern!r} under {sources_root}: {exc}"
                )
                continue
        ...
        for pattern in exclude_paths:
            ...
            try:
                ...
                logger.debug(
                    f"Failed to glob exclude {raw_pattern!r} under {sources_root}: {exc}"
                )
                continue
            for candidate in matches:
            ...
            try:
                owned_paths.append(
                    self._relative_to_workspace(
                        path=candidate, workspace_root=workspace_root
                    )
                )
            except ValueError:
                logger.debug(
                    f"Skipping Aware source outside workspace root: {candidate}"
                )
                continue
