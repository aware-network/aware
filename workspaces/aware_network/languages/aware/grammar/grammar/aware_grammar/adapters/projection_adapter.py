...
class AwareProjectionAdapter(CodeSectionProjectionAdapter[Node]):
    ...
    def get_views(self, projection_node: CodeNode[Node]) -> list[ProjectionViewSpec[Node]]:
        ...
        for view in view_nodes:
            keyword_node = view.child_by_field_name("keyword")
            keyword = self._node_text(keyword_node).strip()
            if keyword != "observable":
                raise ValueError(
                    "Projection observable declarations must use `observable`; "
                    f"legacy keyword {keyword!r} is not supported"
                )
            key_node = view.child_by_field_name("view_key")
            kind_node = view.child_by_field_name("kind")
            body_node = view.child_by_field_name("body")
            if key_node is None or body_node is None:
                continue

            key_raw = self._node_text(key_node).strip()
            kind = self._node_text(kind_node).strip().lower() if kind_node is not None else "instance"
            if kind not in {"construct", "instance"}:
                raise ValueError("Projection observable kind must be one of " f"{{'construct','instance'}}, {kind!r}")
            invalid_body_children = [
                child.type for child in body_node.named_children if child.type not in {"comment", "literal"}
            ]
            if invalid_body_children:
                raise ValueError(
                    "Projection observable bodies only support doc/comment "
                    "content in v0; nested observables and view declarations "
                    f"are invalid (observable={key_raw!r}, "
                    f"children={invalid_body_children!r})"
                )
            ...
            out.append(
                ProjectionViewSpec(
                    ...
                    full_key=key_raw,
                    kind_node=CodeNode(
                        node=kind_node or key_node,
                        byte_start=(kind_node or key_node).start_byte,
                        byte_end=(kind_node or key_node).end_byte,
                    ),
                    kind=kind,
                    is_default=False,
                    body_node=CodeNode(
