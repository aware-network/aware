...
def _fk_attr_for_relationship(
    ...
    *,
    ...
    owner = (
        ...
        if fk_owner_side == ClassConfigRelationshipDirection.forward
        else None
    )
...
def _assert_positions_are_dense_and_deterministic(
    classes: Iterable[ClassConfig],
) -> None:
    for cls in classes:
        positions: list[int] = []
        for acc in cls.class_config_attribute_configs:
        ...
        assert sorted(positions) == list(
            range(len(positions))
        ), f"{cls.name} positions not dense: {sorted(positions)}"
...
def test_fk_required_when_lazy_by_default(
    ...
    rel = _relationship_for_source_attr(
        transformed, source_class="Parent", forward_attr="child"
    )
...
def test_cross_ocg_relationship_synthesizes_fk_with_external_graph_mapping(
    tmp_path: Path,
) -> None:
    """
    Regression:
...
""".lstrip()
    ...
    assert (
        rel is not None
    ), "Expected cross-OCG relationship for ObjectInstanceGraphBranch.branch"
...
def test_fk_optional_when_forward_side_is_eager(
    ...
    config = ClassConfigRelationshipSideLoadingConfig(
        entries=[
            ClassConfigRelationshipSideLoadingEntry(
                namespace="test_domain.test_schema",
                class_name="Parent",
                attribute="child",
...
""".lstrip()
    graph, namespace_by_code_id = _build_graph_from_aware_source(
        tmp_path=tmp_path, source=src
    )
    config = ClassConfigRelationshipSideLoadingConfig(
        entries=[
            ClassConfigRelationshipSideLoadingEntry(
...
""".lstrip()
    ...
    rel = _relationship_for_source_attr(
        transformed, source_class="Department", forward_attr="employees"
    )
