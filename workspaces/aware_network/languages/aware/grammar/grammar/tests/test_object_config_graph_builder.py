...
def _build_test_namespace_by_code_id(
    *,
    ...
    namespace: str = "test_domain.test_schema",
...
def test_ocg_builder_is_ssot_driven(sample_files: dict[str, str]) -> None:
    ...
    namespace_by_code_id = _build_test_namespace_by_code_id(
        package_name="test_pkg", codes=codes
    )
    ...
    for rel in m2m_rels:
        ...
        assoc_names.add(
            next(
                (c.name for c in classes.values() if c.id == assoc.class_config_id), ""
            )
        )
...
def test_cross_ocg_reference_emits_cross_map(sample_files: dict[str, str]) -> None:
    ...
    dep_ns = _build_test_namespace_by_code_id(
        ...
        codes=[("dep", dep_code)],
        namespace="dep_domain.dep_schema",
    )
    ...
    local_ns = _build_test_namespace_by_code_id(
        ...
        namespace="main_domain.main_schema",
    )
...
def test_ocg_builder_user_post(sample_files: dict[str, str]) -> None:
    ...
    with pytest.raises(
        ValueError,
        match="Standalone/global functions are not modeled in canonical ObjectConfigGraph",
    ):
...
""".lstrip()
    ...
    bar = classes["Bar"]
    assert any(
        r.class_config_id == bar.id and r.target_class_config_id == foo.id for r in rels
    )


def test_ocg_builder_ignores_import_without_alias_for_resolution(
    tmp_path: Path,
) -> None:
    """
...
def test_relationship_type_matrix(sample_files: dict[str, str]) -> None:
    ...
    classes = _classes_by_name(graph)
    assert set(
        ["User", "Post", "Image", "Group", "Membership", "UserGroupEdge"]
    ).issubset(set(classes.keys()))
    ...
    assoc_name = id_to_name.get(
        str(m2m.class_config_relationship_association_edge.class_config_id)
    )
    ...
    def _source_attr_name_for_rel(
        source_name: str, target_name: str, rel_type: ClassConfigRelationshipType
    ) -> str:
        if (source_name, target_name, rel_type) == (
            "User",
            "Post",
        ...
        raise AssertionError(
            f"Unhandled relationship triple: {(source_name, target_name, rel_type)}"
        )
