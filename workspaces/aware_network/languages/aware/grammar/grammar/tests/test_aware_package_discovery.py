...
def _register_manifest_semantic_providers() -> None:
    ...
    for provider_key, contract in _TEST_SEMANTIC_CONTRACTS.items():
        module_name = f"{__name__}.{provider_key}_semantic_contract"
        _install_semantic_contract_module(module_name=module_name, contract=contract)
        AwareModulePluginRegistry.register(
            AwareModulePlugin(
                provider_key=provider_key,
                semantic_contract_module=module_name,
            ),
        )
    ...
    return ModuleSemanticContract(
        ...
        manifest_resolution=(
            ModuleSemanticManifestResolutionDescriptor(
                ...
                loader_name="_load_test_toml_manifest",
                semantic_root_kind=semantic_root_kind,
                semantic_package_kind=package_kind,
                copy_code_package_metadata_keys=copy_metadata_keys,
            ),
        ),
    )


_TEST_SEMANTIC_CONTRACTS = {
    "aware_environment": _semantic_contract(
        provider_key="aware_environment",
        semantic_owner="aware_environment.profile",
        manifest_kind="aware_environment_profile_toml",
        filename="aware.environment.profile.toml",
        contract="aware.environment_profile",
        semantic_root_kind="environment_profile",
        package_kind="environment_profile",
        copy_metadata_keys=("environment_handle", "profile_key"),
    ),
    "aware_economy": _semantic_contract(
        provider_key="aware_economy",
        semantic_owner="aware_economy.package",
        manifest_kind="aware_economy_toml",
        filename="aware.economy.toml",
        contract="aware.economy",
        semantic_root_kind="economy",
        package_kind="economy",
        copy_metadata_keys=(),
    ),
    "aware_skill": _semantic_contract(
        provider_key="aware_skill",
        semantic_owner="aware_skill.package",
        manifest_kind="aware_skill_toml",
        filename="aware.skill.toml",
        contract="aware.skill",
        semantic_root_kind="skill",
        package_kind="skill",
        copy_metadata_keys=(),
    ),
}
...
def _install_semantic_contract_module(
    *,
    module_name: str,
    contract: ModuleSemanticContract,
) -> None:
    module = ModuleType(module_name)
    module.AWARE_MODULE_SEMANTIC_CONTRACT = contract
    sys.modules[module_name] = module


def _load_test_toml_manifest(*, toml_path: Path) -> SimpleNamespace:
...
def _load_test_workspace_registry(
    *,
    workspace_root: Path,
    ...
    if not workspace_toml.is_file():
        return None
    with workspace_toml.open("rb") as file:
        workspace_payload = tomllib.load(file)
    workspace_spec = workspace_payload.get("workspace", {})
    direct_manifest_paths: list[Path] = []
    for key in (
        "environments",
        "apis",
        "economies",
        ...
        for raw_path in workspace_spec.get(key, ()) or ():
            if isinstance(raw_path, str) and raw_path.strip():
                direct_manifest_paths.append((workspace_root / raw_path).resolve())

    module_package_manifest_paths: list[Path] = []
    for module_spec in workspace_spec.get("modules", ()) or ():
        ...
        if not module_toml.is_file():
            continue
        with module_toml.open("rb") as file:
            module_payload = tomllib.load(file)
        for package_spec in module_payload.get("packages", ()) or ():
            if not isinstance(package_spec, dict):
                continue
            manifest = package_spec.get("manifest")
            if isinstance(manifest, str) and manifest.strip():
                module_package_manifest_paths.append((module_root / manifest).resolve())

    return SimpleNamespace(
        source="workspace_toml",
        entries=tuple(
            SimpleNamespace(resolved_path=path) for path in direct_manifest_paths
        ),
        module_package_manifest_paths=tuple(module_package_manifest_paths),
    )
...
def test_aware_package_discovery_fails_closed_for_undeclared_nested_workspace_package(
    ...
    discovery = SemanticContractCodePackageDiscovery(
        workspace_registry_loader=_load_test_workspace_registry,
    )
    ...
    plugin = build_aware_code_language_plugin(package_discovery=discovery)
    packages = plugin.discover_packages(file_tree={}, workspace_root=tmp_path)
