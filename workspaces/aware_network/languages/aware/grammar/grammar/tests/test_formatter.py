...
def test_formatter_formats_projection_observable() -> None:
    text = (
        "projection Identity {\n"
        "root identity.Identity\n"
        "observable welcome construct { }\n"
        "observable profile.home { }\n"
        "}\n"
    )
    formatted = format_aware_source(text=text, indent_size=4)
    assert formatted == (
...
def test_formatter_preserves_experience_api_view_mount() -> None:
    text = (
        "experience CodePackage on aware_code.package.CodePackage{\n"
        "observable codes{\n"
        "view selector.v1 default api_view code.package_selector{\n"
        '"""Selector."""\n'
        "}\n"
        "}\n"
        "}\n"
...
def test_formatter_formats_api_endpoint_contract_blocks() -> None:
    text = (
        "api home_devices{\n"
        "view door_panel on aware_home.Home.security state aware_home_api.DoorPanel{\n"
        "stream snapshot;\n"
        "endpoint lock_door home_devices.lock_door.lock_door\n"
        "}\n"
