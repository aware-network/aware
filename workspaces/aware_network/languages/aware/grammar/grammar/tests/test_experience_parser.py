...
experience home_story on aware_home.home.Home {
    ...
    observable security {
        view door default api_view code.editor {}
    }
...
"""
    ...
    assert _text(source_bytes, view_defs[0].child_by_field_name("state_model")) == (
        "aware_home.home.Home"
    )
    assert _text(source_bytes, view_defs[1].child_by_field_name("api_view")) == (
        "code.editor"
    )
    assert _text(source_bytes, view_defs[0].child_by_field_name("state_provider")) == (
        "aware_home.views.home_state"
    )
    assert view_defs[0].child_by_field_name("api_view") is None
    assert view_defs[1].child_by_field_name("state_model") is None
