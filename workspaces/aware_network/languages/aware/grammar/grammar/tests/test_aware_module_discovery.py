...
def test_aware_module_discovery_resolves_manifest_metadata(tmp_path: Path) -> None:
    ...
    package = packages[0]
    assert package["module_package_manifest"] == "structure/ontology/aware.toml"
    assert "aware_toml_path" not in package
    ...
    assert "semantic_bindings" not in package


def test_aware_plugin_discovers_modules_from_manifest_without_file_tree(
    tmp_path: Path,
) -> None:
    module_root, owned_rel_path, non_owned_rel_path = _create_module_fixture(tmp_path)
...
def test_aware_module_discovery_reports_empty_owned_paths_without_fallback(
    tmp_path: Path,
) -> None:
    discovery = AwareCodeModuleDiscovery()
