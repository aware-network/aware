...
_AWARE_ORM_DEPENDENCY = "aware-orm"
...
class PythonPackageStrategy(ObjectConfigGraphPackageStrategy):
    ...
    def build_into(
        ...
        *,
        ...
        self._write_text_if_changed(readme_path, self._render_readme(package_name, spec.description, spec))
        ...
        python_module_files = [
            p for p in files_written if p.suffix == ".py" and import_root in p.parents and p.name != "__init__.py"
        ]
    ...
    def _remove_stale_api_dto_python_modules(
        ...
        *,
        ...
        for directory in sorted(
            (path for path in import_root.rglob("*") if path.is_dir() and path.name != "_aware"),
    ...
    def _remove_stale_api_public_package_local_models(
        ...
        *,
        ...
        if models_dir.exists():
            for existing in sorted(models_dir.rglob("*"), key=lambda path: len(path.parts), reverse=True):
    ...
    def _ensure_package_inits(self, current: Path, stop: Path, files_written: list[Path]) -> None:
        ...
        while True:
            ...
            if not init_path.exists():
                ...
                self._write_text_if_changed(init_path, "# Auto-generated by Aware package strategy.\n")
    ...
    def _write_root_init(
        ...
        policy = self.policy if isinstance(self.policy, PythonPackagePolicy) else PythonPackagePolicy.orm_default()
    ...
    def _render_api_public_package_readme(
        ...
        *,
        ...
        content = _scrub_public_api_client_text(description or f"Generated API client package for {package_name}.")
    ...
    def _render_pyproject(
        ...
        *,
        ...
        deps = self._filter_pyproject_dependencies(
            dependencies=self._pyproject_dependencies_for_policy(
    ...
    def _filter_pyproject_dependencies(
        ...
        *,
        ...
        prunable_dependency_roots = (
            _metadata_prunable_dependency_import_roots(metadata=metadata) or metadata_dependency_roots
        )
    ...
    def _pyproject_dependencies_for_policy(
        self,
        *,
        dependencies: list[str],
    ) -> list[str]:
        policy = self.policy if isinstance(self.policy, PythonPackagePolicy) else PythonPackagePolicy.orm_default()
        if not policy.install_runtime_artifacts:
            return dependencies
        if any(_dependency_distribution_name(dependency) == _AWARE_ORM_DEPENDENCY for dependency in dependencies):
            return dependencies
        return [*dependencies, _AWARE_ORM_DEPENDENCY]
...
def _metadata_dependency_import_roots(*, metadata: dict[str, object]) -> frozenset[str]:
    ...
    if not isinstance(raw_dependency_roots, list):
        return frozenset()
    return frozenset(
        root.strip().replace("-", "_") for root in raw_dependency_roots if isinstance(root, str) and root.strip()
    )


def _metadata_prunable_dependency_import_roots(*, metadata: dict[str, object]) -> frozenset[str]:
...
def _read_bootstrap_dependency_import_roots(*, import_root_path: Path) -> frozenset[str]:
    ...
    if not isinstance(dependency_roots, list):
        return frozenset()
    return frozenset(
        root.strip().replace("-", "_") for root in dependency_roots if isinstance(root, str) and root.strip()
    )
