...
PYTHON_ORM_ATTRIBUTE_TYPE_EVIDENCE_ONLY_DIAGNOSTIC = (
...
PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_TARGET_RELATIVE_PATH_MISSING_DIAGNOSTIC = (
...
PYTHON_ORM_ATTRIBUTE_TYPE_UNSUPPORTED_REASON = (
    "meta_python_orm_attribute_type_requires_renderable_primitive_type_descriptor"
)
PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_UNSUPPORTED_REASON = (
...
class PythonOrmRuntimeGeneratedDeltaRenderer(
    ...
    def supports_generated_materialization_delta(
        ...
        return (
            supports_python_orm_class_generated_delta(request)
            or supports_python_orm_enum_generated_delta(request)
    ...
    def render_generated_materialization_delta(
        ...
        if not self.supports_generated_materialization_delta(request):
            return MetaLanguageGeneratedMaterializationDeltaRenderResult.unhandled(
                reason="python_orm_runtime_generated_delta_operation_not_supported",
            )
        if supports_python_orm_class_generated_delta(request):
            return render_python_orm_class_generated_delta(request)
        if supports_python_orm_enum_generated_delta(request):
            return render_python_orm_enum_generated_delta(request)
        if supports_python_orm_function_generated_delta(request):
            return render_python_orm_function_generated_delta(request)
        if supports_python_orm_relationship_generated_delta(request):
            return render_python_orm_relationship_generated_delta(request)
        operation = request.operation
        context = _context_with_defaults(request.context)
        event_key = _world_change_event_key(operation=operation)
        ...
        if operation.ontology_subject_kind == "attribute_membership":
            ...
            reason = (
                "python_orm_runtime_attribute_membership_generated_delta_not_required"
            )
            ...
            reason = "python_orm_runtime_attribute_field_generated_delta_rendered"
        elif _attribute_default_value_changed(operation=operation):
            result = _attribute_default_value_result(
                operation=operation,
                context=context,
                event_key=event_key,
                target=target,
            ...
            reason = (
                "python_orm_runtime_attribute_default_value_generated_delta_rendered"
            )
        else:
            result = _attribute_type_result(
                operation=operation,
                context=context,
                event_key=event_key,
                target=target,
            )
            reason = "python_orm_runtime_attribute_type_generated_delta_rendered"
...
def _context_with_defaults(
    ...
    return MetaLanguageGeneratedMaterializationDeltaContext(
        ...
        product_intent=(
            ...
            or PYTHON_ORM_GENERATED_MATERIALIZATION_PRODUCT_INTENT
        ),
...
def _delta_request(
    *,
    ...
    field_key = _python_orm_attribute_field_key(operation=operation)
    renderer_key = _renderer_key_for_operation(operation=operation)
...
def _attribute_type_result(
    *,
    ...
    if not _attribute_type_changed(operation=operation):
        return CodeGeneratedMaterializationDeltaResult(
            ...
            skipped_targets=[
                CodeGeneratedMaterializationSkippedTarget(
                    target=target,
                    reason=PYTHON_ORM_ATTRIBUTE_TYPE_NOT_REQUIRED_REASON,
                    event_refs=[event_key],
                )
            ],
            metadata=_json_object(
    ...
    if replacement_text is None:
        return CodeGeneratedMaterializationDeltaResult(
            ...
            skipped_targets=[
                CodeGeneratedMaterializationSkippedTarget(
                    target=target,
                    reason=PYTHON_ORM_ATTRIBUTE_TYPE_UNSUPPORTED_REASON,
                    event_refs=[event_key],
                    metadata=_json_object(
                        {
                            "operation_key": operation.operation_key,
    ...
    entry = CodeGeneratedMaterializationDeltaEntry(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_generated_delta_entry",
                "operation_key": operation.operation_key,
                "mode_reason": (
    ...
    return CodeGeneratedMaterializationDeltaResult(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_generated_delta_result",
                "operation_key": operation.operation_key,
...
def _attribute_default_value_result(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    event_key: str,
    ...
    if replacement_text is None:
        return CodeGeneratedMaterializationDeltaResult(
            ...
            skipped_targets=[
                CodeGeneratedMaterializationSkippedTarget(
                    target=target,
                    reason=PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_UNSUPPORTED_REASON,
                    event_refs=[event_key],
                    metadata=_json_object(
                        {
                            "operation_key": operation.operation_key,
    ...
    entry = CodeGeneratedMaterializationDeltaEntry(
        ...
        target=target,
        section_delta=(
            _attribute_default_value_section_delta(
                operation=operation,
                context=context,
                target=target,
                event_key=event_key,
...
def _attribute_structural_result(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    event_key: str,
    ...
    if operation.operation_family == "create":
        delta_evidence = _attribute_create_delta_evidence(
            operation=operation,
            context=context,
            target=target,
            event_key=event_key,
    ...
    diagnostics = (
        ()
        if has_delta
        else _attribute_structural_diagnostics(
            operation=operation,
            target=target,
        )
    )
    ...
    after_hash = delta_evidence.after_hash if delta_evidence is not None else None
    renderer_operation = CodeGeneratedRendererDeltaOperation(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_attribute_field_operation",
                "operation_key": operation.operation_key,
                "operation_family": operation.operation_family,
                "provider_operation_type": operation.provider_operation_type,
                "mode_reason": (
...
def _attribute_type_section_delta(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    target: CodeGeneratedMaterializationTargetRef,
    ...
    return CodeSectionDeltaSet(
        ...
        entries=[
            CodeSectionDeltaEntry(
                ...
                section_ref=CodeSectionRef(
                    ...
                    language=context.target_language or "python",
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_section_delta_set",
                "operation_key": operation.operation_key,
                "renderer_key": PYTHON_ORM_ATTRIBUTE_TYPE_RENDERER_KEY,
                "renderer_profile": context.renderer_profile,
            }
...
def _attribute_default_value_section_delta(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    target: CodeGeneratedMaterializationTargetRef,
...
def _attribute_create_delta_evidence(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    target: CodeGeneratedMaterializationTargetRef,
...
def _attribute_delete_delta_evidence(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    target: CodeGeneratedMaterializationTargetRef,
...
def _attribute_field_grammar_anchor_render_delta(
    *,
    ...
    span_context = MetaGeneratedMaterializationTextSpanContext(
        ...
        relative_path=relative_path,
        language=_code_language(context.target_language or "python"),
    ...
    return meta_generated_materialization_correlated_text_span_render_delta(
        ...
        replacements=[
            meta_generated_materialization_text_span_replacement(
                ...
                metadata=_json_object(
                    {
                        "source": "python_grammar.python_orm_attribute_field_span",
                        "operation_key": operation.operation_key,
                        "attribute_name": _attribute_name(operation=operation),
...
def _attribute_field_graph_selector(
    *,
    ...
    return CodeGraphFieldSelector(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_attribute_field_selector",
                "operation_key": operation.operation_key,
                "attribute_name": attribute_name,
                "owner_key": owner_key,
            }
...
def _attribute_structural_diagnostics(
    *,
    operation: MetaProviderDeltaTypedOperation,
    target: CodeGeneratedMaterializationTargetRef,
) -> tuple[str, ...]:
    ...
    if _python_field_text(operation=operation) is None and (
        operation.operation_family in {"create", "update"}
    ):
...
def _target_ref(
    *,
    ...
    raw_relative_path = (
        ...
        or context.relative_path_for_owner(owner_key)
    ...
    return CodeGeneratedMaterializationTargetRef(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_target_ref",
                "operation_key": operation.operation_key,
                "owner_key": owner_key,
...
def _anchor_ref(
    *,
    ...
    return CodeGeneratedRendererAnchorRef(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_attribute_type_anchor",
                "operation_key": operation.operation_key,
                "semantic_key": operation.semantic_key,
            }
        ),
...
def _field_anchor_ref(
    *,
    ...
    return CodeGeneratedRendererAnchorRef(
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_runtime_attribute_field_anchor",
                "operation_key": operation.operation_key,
                "semantic_key": operation.semantic_key,
            }
        ),
...
def _attribute_structural_required(
    *,
    operation: MetaProviderDeltaTypedOperation,
) -> bool:
...
def _attribute_type_update_requires_field_delta(
    *,
    ...
    if (
        ...
        or _python_type_annotation(operation=operation, payload=operation.baseline)
        is None
    ):
        return False
...
def _renderer_key_for_operation(
    *,
    operation: MetaProviderDeltaTypedOperation,
) -> str:
    if _attribute_structural_required(operation=operation):
        return PYTHON_ORM_ATTRIBUTE_FIELD_RENDERER_KEY
    if _attribute_default_value_changed(operation=operation):
        return PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_RENDERER_KEY
    return PYTHON_ORM_ATTRIBUTE_TYPE_RENDERER_KEY
...
def _descriptor_is_decimal_collection(descriptor: Mapping[str, object]) -> bool:
    if optional_text(descriptor.get("kind")) != "collection":
        return False
    children = tuple_mappings(descriptor.get("child_descriptors")) or tuple(
        mapping_value(link.get("child"))
        for link in tuple_mappings(descriptor.get("child_links"))
    )
    return len(children) == 1 and _descriptor_is_decimal(children[0])
...
def _python_field_text(*, operation: MetaProviderDeltaTypedOperation) -> str | None:
    attribute_name = _attribute_name(operation=operation)
    type_text = _python_type_annotation(
        operation=operation,
        payload=operation.current,
    )
