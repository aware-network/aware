...
PYTHON_ORM_CLASS_GENERATED_MATERIALIZATION_PRODUCT_INTENT = ORM_RUNTIME_PRODUCT_INTENT
...
PYTHON_ORM_CLASS_PARENT_ANCHOR_KEY = "python.orm.class.parent_class"
...
PYTHON_ORM_CLASS_PARENT_SPAN_MISSING_DIAGNOSTIC = (
    "meta_python_orm_class_parent_update_generated_materialization_span_missing"
)
PYTHON_ORM_CLASS_PARENT_TEXT_MISSING_DIAGNOSTIC = (
    "meta_python_orm_class_parent_update_generated_materialization_text_missing"
)
...
PYTHON_ORM_CLASS_PARENT_UPDATE_POLICY_MISSING_DIAGNOSTIC = (
    "meta_python_orm_class_parent_update_generated_materialization_policy_missing"
)
...
def _is_authored_aware_sources_root(sources_root: str | None) -> bool:
    normalized_sources_root = _normalized_path(sources_root)
    if normalized_sources_root is None:
        return True
    return normalized_sources_root == "aware" or normalized_sources_root.endswith(
        "/aware"
    )
        ...
        action_bindings=[
            CodeGeneratedMaterializationActionBinding(
                ...
                metadata=_json_object(
                    {
                        "source": (
                            "python_grammar." "python_orm_class_generated_delta_action"
                        ),
                        "operation_key": operation.operation_key,
                    }
                ),
...
def _class_result(
    *,
    ...
    elif _class_parent_changed(operation=operation):
        delta_evidence = _class_parent_delta_evidence(
            operation=operation,
            context=context,
            event_key=event_key,
            target=target,
...
def _class_parent_update_blocked_result(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    event_key: str,
    ...
    return CodeGeneratedMaterializationDeltaResult(
        ...
        skipped_targets=[
            CodeGeneratedMaterializationSkippedTarget(
                target=target,
                reason=PYTHON_ORM_CLASS_PARENT_UPDATE_POLICY_MISSING_DIAGNOSTIC,
                event_refs=[event_key],
                metadata=_json_object(
                    {
                        "operation_key": operation.operation_key,
                        "semantic_key": operation.semantic_key,
                        "parent_class_name": _current_parent_class_name(operation),
                        "previous_parent_class_name": (
                            _baseline_parent_class_name(operation)
                        ),
        ...
        diagnostics=[PYTHON_ORM_CLASS_PARENT_UPDATE_POLICY_MISSING_DIAGNOSTIC],
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_class_parent_update_blocked",
                "operation_key": operation.operation_key,
                "language_plugin_delta_renderer": (
                    PYTHON_ORM_GENERATED_DELTA_RENDERER_NAME
                ),
...
def _class_parent_delta_evidence(
    *,
    operation: MetaProviderDeltaTypedOperation,
    context: MetaLanguageGeneratedMaterializationDeltaContext,
    target: CodeGeneratedMaterializationTargetRef,
    ...
    if (
        ...
        return None
    class_name = _class_name(operation=operation)
    replacement_text = _current_parent_class_name(operation) or "ORMModel"
    expected_parent_name = _baseline_parent_class_name(operation) or "ORMModel"
    source_state = _generated_source_state(context=context, target=target)
    if class_name is None or source_state is None:
        return None
    relative_path, source_text = source_state
    span = _python_class_parent_span(
        source_text=source_text,
        class_name=class_name,
    ...
    graph_selector = CodeGraphFieldSelector(
        ...
        field_path=f"{class_name}.parent_class",
        class_fqn=_class_fqn(operation=operation),
        class_name=class_name,
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_class_parent_selector",
                "operation_key": operation.operation_key,
                "class_name": class_name,
                "parent_class_name": _current_parent_class_name(operation),
                "previous_parent_class_name": _baseline_parent_class_name(operation),
            }
        ),
    )
    span_context = MetaGeneratedMaterializationTextSpanContext(
        ...
        meta_generated_materialization_correlated_text_span_render_delta(
            ...
            replacements=[
                meta_generated_materialization_text_span_replacement(
                    context=span_context,
                    replacement_key=(
                        "aware_meta.python_orm.class.parent:"
                        f"{operation.operation_key}"
                    ),
                    byte_start=byte_start,
                    byte_end=byte_end,
                    ...
                    metadata=_json_object(
                        {
                            "source": (
                                "python_grammar." "python_orm_class_parent_replacement"
                            ),
                            "operation_key": operation.operation_key,
                            "class_name": class_name,
                            "parent_class_name": _current_parent_class_name(
                                operation
                            ),
                            "previous_parent_class_name": (
                                _baseline_parent_class_name(operation)
            ...
            metadata=_json_object(
                {
                    "source": (
                        "python_grammar."
                        "python_orm_class_parent_grammar_anchor_render_delta"
                    ),
                    "operation_key": operation.operation_key,
                    "target_kind": CodeGrammarAnchorRenderTargetKind.text_span.value,
                    "renderer_key": PYTHON_ORM_CLASS_RENDERER_KEY,
    ...
    return _PythonOrmClassDeltaEvidence(
        grammar_anchor_render_delta=grammar_anchor_render_delta,
        anchor=_parent_anchor_ref(operation=operation, context=context),
        content_text=replacement_text,
        before_hash=before_hash,
        after_hash=after_hash,
        mode_reason="python_orm_class_parent_grammar_anchor_render_delta_ready",
    )
...
def _class_create_delta_evidence(
    *,
    ...
    replacement_text = _python_class_text(operation=operation, placement=placement)
            ...
            replacements=[
                meta_generated_materialization_text_span_replacement(
                    ...
                    metadata=_json_object(
                        {
                            "source": (
                                "python_grammar." "python_orm_class_create_replacement"
                            ),
                            "operation_key": operation.operation_key,
                            "class_name": class_name,
                            "renderer_key": PYTHON_ORM_CLASS_RENDERER_KEY,
            ...
            replacements=[
                meta_generated_materialization_text_span_replacement(
                    ...
                    metadata=_json_object(
                        {
                            "source": (
                                "python_grammar." "python_orm_class_delete_replacement"
                            ),
                            "operation_key": operation.operation_key,
                            "class_name": class_name,
                            "renderer_key": PYTHON_ORM_CLASS_RENDERER_KEY,
...
def _guarded_delta_diagnostics(
    *,
    ...
    if _class_structural_create_required(operation=operation):
        ...
        if _current_parent_class_name(operation) is None and (
            _baseline_parent_class_name(operation) is None
        ):
            diagnostics.append(PYTHON_ORM_CLASS_PARENT_TEXT_MISSING_DIAGNOSTIC)
        else:
            diagnostics.append(PYTHON_ORM_CLASS_PARENT_SPAN_MISSING_DIAGNOSTIC)
...
def _python_class_create_insert_span(
    *,
    ...
    if any(node.name == class_name for node in class_nodes):
        return None
    for node in sorted(class_nodes, key=lambda item: item.name):
        if node.name <= class_name:
            continue
        byte_start = _node_start_byte(source_text=source_text, node=node)
        if byte_start is None:
            continue
        return byte_start, byte_start, "", "before_class"
    byte_end = len(source_text.encode("utf-8"))
    return byte_end, byte_end, "", "append"
...
def _python_class_parent_span(
    *,
    ...
    if not isinstance(lineno_value, int):
        return None
    lines = source_text.splitlines(keepends=True)
    line_index = lineno_value - 1
    if line_index < 0 or line_index >= len(lines):
        return None
    line = lines[line_index]
    class_name_index = line.find(class_name)
    if class_name_index < 0:
        return None
    base_start = line.find("(", class_name_index + len(class_name))
    colon_index = line.find(":", class_name_index + len(class_name))
    if base_start < 0 or colon_index < 0 or base_start > colon_index:
        return None
    base_end = line.rfind(")", base_start, colon_index)
    if base_end < 0 or base_end <= base_start:
        return None
    before_text = line[base_start + 1 : base_end]
    if before_text.strip() != expected_parent_name:
        return None
    byte_line_start = len("".join(lines[:line_index]).encode("utf-8"))
    byte_start = byte_line_start + len(line[: base_start + 1].encode("utf-8"))
    byte_end = byte_line_start + len(line[:base_end].encode("utf-8"))
    return byte_start, byte_end, before_text
...
def _parent_anchor_ref(
    *,
    ...
    class_fqn = _class_fqn(operation=operation)
    class_name = _class_name(operation=operation)
    anchor_path = ".".join(part for part in (class_name, "parent_class") if part)
    return CodeGeneratedRendererAnchorRef(
        anchor_key=PYTHON_ORM_CLASS_PARENT_ANCHOR_KEY,
        anchor_path=anchor_path,
        anchor_role="class_parent",
        renderer_key=PYTHON_ORM_CLASS_RENDERER_KEY,
        renderer_profile=context.renderer_profile,
        materialization_source=context.materialization_source,
        target_language=context.target_language,
        section_type="class",
        segment_name="parent_class",
        graph_selector=_json_object(
            {
                "provider_key": PYTHON_ORM_CLASS_GENERATED_MATERIALIZATION_PROVIDER_KEY,
                "semantic_owner": (
        ...
        metadata=_json_object(
            {
                "source": "python_grammar.python_orm_class_parent_anchor",
                "operation_key": operation.operation_key,
                "semantic_key": operation.semantic_key,
                "parent_class_name": _current_parent_class_name(operation),
                "previous_parent_class_name": _baseline_parent_class_name(operation),
...
def _class_parent_changed(*, operation: MetaProviderDeltaTypedOperation) -> bool:
    if (
        operation.ontology_subject_kind != "class"
        or operation.operation_family != "update"
    ):
        return False
    current_parent = _parent_class_signature(payload=operation.current)
    baseline_parent = _parent_class_signature(payload=operation.baseline)
    return any(value is not None for value in (*current_parent, *baseline_parent)) and (
        current_parent != baseline_parent
    )
...
def _current_parent_class_name(
    operation: MetaProviderDeltaTypedOperation,
) -> str | None:
    return _parent_class_name_from_signature(
        _parent_class_signature(payload=operation.current)
    )


def _baseline_parent_class_name(
    operation: MetaProviderDeltaTypedOperation,
) -> str | None:
    return _parent_class_name_from_signature(
        _parent_class_signature(payload=operation.baseline)
    )


def _parent_class_signature(
    *,
    payload: Mapping[str, object],
) -> tuple[str | None, str | None, str | None]:
    signature = _class_signature(payload=payload)
    return (
        optional_text(signature.get("parent_class_id"))
        or optional_text(signature.get("parent_class_config_id")),
        optional_text(signature.get("parent_class_fqn")),
        optional_text(signature.get("parent_class_semantic_key")),
    )


def _parent_class_name_from_signature(
    signature: tuple[str | None, str | None, str | None],
) -> str | None:
    _parent_id, parent_fqn, parent_semantic_key = signature
    if parent_fqn is not None:
        return parent_fqn.rsplit(".", maxsplit=1)[-1] or None
    if parent_semantic_key is not None:
        return _class_name_from_semantic_key(parent_semantic_key)
    return None
