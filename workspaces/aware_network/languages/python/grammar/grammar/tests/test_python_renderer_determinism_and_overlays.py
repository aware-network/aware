...
def _enum_descriptor(target: EnumConfig) -> AttributeTypeDescriptor:
    return AttributeTypeDescriptor(
...
def test_python_renderer_enum_option_names_and_defaults_share_safe_rendering() -> None:
    state_enum = make_enum(name="JobState")
    state_enum.enum_options = [
        EnumOption(
            enum_config_id=state_enum.id,
            value="value",
            position=0,
        ),
        EnumOption(
    ...
    job_cls = make_class(name="Job", is_base=True)
    state_attr = make_attribute(
        name="state",
        owner_key=job_cls.class_fqn,
        default_value='"value"',
        is_public=True,
        is_required=False,
        is_unique=False,
        is_virtual=False,
        type_descriptor=_enum_descriptor(state_enum),
    )
    job_cls.class_config_attribute_configs = [_acc(job_cls, state_attr, pos=0)]

    renderer = PythonRenderer(layout_strategy=_TestLayout(base_dir=Path("/tmp")))
    output = _render(renderer, meta_objects=[state_enum, job_cls], class_map={job_cls.id: job_cls})

    assert 'value_ = "value"' in output
    assert 'class_ = "class"' in output
    assert "state: JobState | None = Field(default=JobState.value_)" in output
    assert "JobState.value)" not in output


def test_python_renderer_enum_default_must_match_declared_option() -> None:
    state_enum = make_enum(name="JobState")
    state_enum.enum_options = [
        EnumOption(
            enum_config_id=state_enum.id,
    ...
    with pytest.raises(ValueError, match="Enum default does not match a declared option"):
        _render(renderer, meta_objects=[state_enum, job_cls], class_map={job_cls.id: job_cls})
