...
def test_service_protocol_pyproject_deps_follow_bootstrap_import_roots(
    ...
    module_path.write_text("class AwareEnvironmentServiceProtocol: ...\n", encoding="utf-8")
...
def test_orm_runtime_pyproject_declares_aware_orm_dependency(tmp_path: Path) -> None:
    render_root = tmp_path / "render"
    module_path = render_root / "content" / "content.py"
    module_path.parent.mkdir(parents=True)
    module_path.write_text("from aware_orm.models.orm_model import ORMModel\n", encoding="utf-8")
    ...
    _ = strategy.build_package(
        [module_path],
        ObjectConfigGraphPackageSpec(
            name="aware-content-ontology",
            package_name="aware-content-ontology",
            import_root="aware_content_ontology",
            package_root=output_root,
    ...
    pyproject = (output_root / "pyproject.toml").read_text(encoding="utf-8")
    assert '"aware-orm"' in pyproject


def test_api_policy_pyproject_does_not_declare_aware_orm_dependency(
    tmp_path: Path,
) -> None:
    render_root = tmp_path / "render"
    module_path = render_root / "models.py"
    module_path.parent.mkdir(parents=True)
    module_path.write_text("class ContentResponse: ...\n", encoding="utf-8")

    output_root = tmp_path / "package"
    strategy = PythonPackageStrategy(render_root)
    strategy.set_policy(PythonPackagePolicy.api_default())
    _ = strategy.build_package(
        [module_path],
        ObjectConfigGraphPackageSpec(
            name="aware-content-service-dto",
            package_name="aware-content-service-dto",
            import_root="aware_content_service_dto",
            package_root=output_root,
    ...
    pyproject = (output_root / "pyproject.toml").read_text(encoding="utf-8")
    assert "aware-orm" not in pyproject
...
def test_package_strategy_strips_rendered_import_root_prefix(
    ...
    render_root = tmp_path / "render"
    rendered_module = render_root / "aware_storage_ontology_dto" / "blob" / "storage_blob.py"
    rendered_module.parent.mkdir(parents=True)
    rendered_module.write_text("class StorageBlob: ...\n", encoding="utf-8")

    output_root = tmp_path / "package"
    nested_stale = output_root / "aware_storage_ontology_dto" / "aware_storage_ontology_dto" / "stale.py"
    ...
    expected_module = output_root / "aware_storage_ontology_dto" / "blob" / "storage_blob.py"
    duplicate_module = (
        output_root / "aware_storage_ontology_dto" / "aware_storage_ontology_dto" / "blob" / "storage_blob.py"
    )
