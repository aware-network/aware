...
class NodeWorkspace:
    ...
    def __init__(self, *, spec_path: str | Path, repo_root: str | Path | None = None):
        ...
        if not resolved_spec_path.exists():
            raise FileNotFoundError(f"aware.node.toml not found: {resolved_spec_path}")
    ...
    def build_snapshot(self) -> NodeWorkspaceSnapshot:
        spec = load_aware_node_toml_spec(toml_path=self._spec_path)
        ...
        if not sources_root.exists():
            raise FileNotFoundError(
                f"Node sources_dir does not exist: {sources_root} (from {self._spec_path})"
            )
        if not sources_root.is_dir():
            raise NotADirectoryError(
                f"Node sources_dir must be a directory: {sources_root}"
            )
