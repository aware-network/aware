...
async def test_ocg_genesis_executes_functioncalls_and_hydrates_head(
    ...
    try:
        ...
        assert execution_receipt["applied_invocation_count"] == 8
        ...
        declaration_receipt = _receipt_for_semantic_key(
            execution_receipt=execution_receipt,
            semantic_key=(
                f"ocg:{spec.fqn_prefix}"
                f"/projection_declaration:{spec.projection_name}"
            ),
            function_name="create_object_projection_graph_declaration",
        )
        assert declaration_receipt["commit_id"] is not None
        assert declaration_receipt["target_projection_name"] == "ObjectConfigGraph"
        binding_receipt = _receipt_for_semantic_key(
            execution_receipt=execution_receipt,
            semantic_key=(
                f"ocg:{spec.fqn_prefix}"
                f"/projection_declaration:{spec.projection_name}"
                f"/binding:{spec.class_fqn}"
            ),
            function_name="create_binding",
        )
        assert binding_receipt["commit_id"] is not None
        assert binding_receipt["target_projection_name"] == "ObjectConfigGraph"
        ...
        oig, _ = await materializer.get(
            ...
            commit_id=UUID(str(binding_receipt["commit_id"])),
        )
        ...
        assertions.expect_instance(UUID(spec.object_projection_graph_declaration_id))
        assertions.expect_instance(UUID(spec.object_projection_graph_binding_id))
        ...
        assertions.expect_primitive(
            instance_id=UUID(spec.object_projection_graph_declaration_id),
            field_name="projection_name",
            expected=spec.projection_name,
        )
        assertions.expect_primitive(
            instance_id=UUID(spec.object_projection_graph_binding_id),
            field_name="namespace",
            expected=spec.class_namespace,
        )
        assertions.expect_primitive(
            instance_id=UUID(spec.object_projection_graph_binding_id),
...
def _genesis_spec() -> MetaOcgGenesisSpec:
    ...
    object_projection_graph_binding_id = stable_object_projection_graph_binding_id(
    ...
    return MetaOcgGenesisSpec(
        ...
        object_projection_graph_binding_id=str(object_projection_graph_binding_id),
