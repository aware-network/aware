...
async def test_export_oig_commit_payload_ref_carries_verified_file_locator(
    ...
    assert metadata["body_contract"] == "aware.oig_commit_body.v1"
    assert isinstance(metadata["payload_size_bytes"], int)
    assert len(str(metadata["payload_sha256"])) == 64
    assert isinstance(metadata["body_size_bytes"], int)
    assert len(str(metadata["body_sha256"])) == 64
...
async def test_import_oig_commit_payload_ref_installs_verified_commit(
    ...
    installed = await target_store.get_commit_envelope(
        branch_id=branch_id,
        projection_hash=projection_hash,
        commit_id=commit.commit.id,
    )
    assert installed is not None
    assert installed.object_instance_graph_commit_id == commit.id
    ...
    assert installed_body.object_instance_graph_commit_id == commit.id
