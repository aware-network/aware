...
from aware_meta.handlers.impl.projection import (
    object_projection_graph_declaration as object_projection_graph_declaration_handler,
)
from aware_meta.handlers.impl.projection import (
...
@pytest.mark.asyncio
async def test_object_config_graph_create_projection_declaration_appends_once() -> None:
    graph = _make_ocg(uuid4(), fqn_prefix="aware_meta_test")

    created = (
        await object_config_graph_handler.create_object_projection_graph_declaration(
            object_config_graph=graph,
            key="aware_meta_test:Runtime",
    ...
    expected_id = stable_object_projection_graph_declaration_id(
    ...
    created_again = (
        await object_config_graph_handler.create_object_projection_graph_declaration(
            ...
            description="Runtime projection.",
            is_branchable=True,
            object_projection_graph_declaration_id=expected_id,
        )
    )

    assert created_again is created
    assert len(graph.object_projection_graph_declarations) == 1


@pytest.mark.asyncio
async def test_object_config_graph_create_projection_declaration_blocks_identity_drift() -> (
    None
):
    graph = _make_ocg(uuid4(), fqn_prefix="aware_meta_test")

    with pytest.raises(RuntimeError, match="does not match deterministic stable-id"):
        await object_config_graph_handler.create_object_projection_graph_declaration(
            object_config_graph=graph,
            key="aware_meta_test:Runtime",
            projection_name="Runtime",
...
@pytest.mark.asyncio
async def test_object_projection_graph_declaration_create_binding_appends_once() -> (
    None
):
    graph = _make_ocg(uuid4(), fqn_prefix="aware_meta_test")
    declaration = (
    ...
    created = await object_projection_graph_declaration_handler.create_binding(
        object_projection_graph_declaration=declaration,
    ...
    expected_id = stable_object_projection_graph_binding_id(
        object_projection_graph_declaration_id=declaration.id,
        fqn_prefix="aware_meta_test",
        namespace="home",
        class_name="Room",
    )
    ...
    assert created.id == expected_id
    created_again = await object_projection_graph_declaration_handler.create_binding(
        object_projection_graph_declaration=declaration,
        fqn_prefix="aware_meta_test",
        namespace="home",
        class_name="Room",
        ...
        target_projection_name="Runtime",
        side="source",
        object_projection_graph_binding_id=expected_id,
    )

    assert created_again is created
    assert len(declaration.object_projection_graph_bindings) == 1


@pytest.mark.asyncio
async def test_object_projection_graph_declaration_create_binding_blocks_identity_drift() -> (
    None
):
    graph = _make_ocg(uuid4(), fqn_prefix="aware_meta_test")
    declaration = (
    ...
    with pytest.raises(RuntimeError, match="does not match deterministic stable-id"):
        await object_projection_graph_declaration_handler.create_binding(
            object_projection_graph_declaration=declaration,
            fqn_prefix="aware_meta_test",
            namespace="home",
            class_name="Room",
            object_projection_graph_binding_id=uuid4(),
        )
