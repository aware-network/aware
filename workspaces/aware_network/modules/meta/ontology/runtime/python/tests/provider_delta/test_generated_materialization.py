...
from python_grammar.renderer_delta_orm_class import (
    PYTHON_ORM_CLASS_CLASS_ANCHOR_KEY as META_PYTHON_ORM_CLASS_CLASS_ANCHOR_KEY,
    PYTHON_ORM_CLASS_DESCRIPTION_ANCHOR_KEY as META_PYTHON_ORM_CLASS_DESCRIPTION_ANCHOR_KEY,
    PYTHON_ORM_CLASS_RENDERER_KEY as META_PYTHON_ORM_CLASS_RENDERER_KEY,
)
...
from python_grammar.renderer_delta_orm_runtime import (
    PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_ANCHOR_KEY as META_PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_ANCHOR_KEY,
    PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_RENDERER_KEY as META_PYTHON_ORM_ATTRIBUTE_DEFAULT_VALUE_RENDERER_KEY,
    PYTHON_ORM_ATTRIBUTE_FIELD_ANCHOR_KEY as META_PYTHON_ORM_ATTRIBUTE_FIELD_ANCHOR_KEY,
    PYTHON_ORM_ATTRIBUTE_FIELD_EVIDENCE_ONLY_DIAGNOSTIC as META_PYTHON_ORM_ATTRIBUTE_FIELD_EVIDENCE_ONLY_DIAGNOSTIC,
    PYTHON_ORM_ATTRIBUTE_FIELD_RENDERER_KEY as META_PYTHON_ORM_ATTRIBUTE_FIELD_RENDERER_KEY,
    PYTHON_ORM_ATTRIBUTE_FIELD_SPAN_MISSING_DIAGNOSTIC as META_PYTHON_ORM_ATTRIBUTE_FIELD_SPAN_MISSING_DIAGNOSTIC,
    PYTHON_ORM_ATTRIBUTE_TYPE_ANCHOR_KEY as META_PYTHON_ORM_ATTRIBUTE_TYPE_ANCHOR_KEY,
    PYTHON_ORM_ATTRIBUTE_TYPE_BASELINE_HASH_MISSING_DIAGNOSTIC as META_PYTHON_ORM_ATTRIBUTE_TYPE_BASELINE_HASH_MISSING_DIAGNOSTIC,
    PYTHON_ORM_ATTRIBUTE_TYPE_EVIDENCE_ONLY_DIAGNOSTIC as META_PYTHON_ORM_ATTRIBUTE_TYPE_EVIDENCE_ONLY_DIAGNOSTIC,
    PYTHON_ORM_ATTRIBUTE_TYPE_TARGET_RELATIVE_PATH_MISSING_DIAGNOSTIC as META_PYTHON_ORM_ATTRIBUTE_TYPE_TARGET_RELATIVE_PATH_MISSING_DIAGNOSTIC,
    PYTHON_ORM_ATTRIBUTE_TYPE_UNSUPPORTED_REASON as META_PYTHON_ORM_ATTRIBUTE_TYPE_UNSUPPORTED_REASON,
...
async def test_meta_python_orm_attribute_create_normalizes_workspace_relative_generated_path(
    ...
    current["generated_materialization"] = _python_orm_runtime_targets(
        "content/content_layout.py"
    )
...
async def test_meta_python_orm_class_create_delta_resolves_from_authored_source_context(
    ...
    assert resolved.package_delta is not None
    assert (
        resolved.package_delta.package_root
        == (package_root / "python" / "orm_runtime").as_posix()
    )
...
def test_meta_provider_delta_generated_materialization_stage_exposes_evidence(
    ...
    assert stage["generated_materialization_artifact_path_count"] == 0
    ...
    assert manifest["package_delta_entry_count"] == 0
    assert manifest["non_package_delta_entry_count"] == 1
    assert manifest["artifact_path_count"] == 0
...
def test_meta_provider_delta_generated_materialization_stage_exposes_package_delta_manifest(
    tmp_path,
    monkeypatch,
) -> None:
    def _fake_feature_results(
        operation: MetaProviderDeltaTypedOperation,
        context: MetaProviderDeltaGeneratedMaterializationContext,
    ) -> tuple[MetaProviderDeltaGeneratedMaterializationFeatureResult, ...]:
        runtime_target = CodeGeneratedMaterializationTargetRef(
            ...
            output_key="runtime-model",
        )
        migration_target = CodeGeneratedMaterializationTargetRef(
            target_language=context.target_language,
            package_name=context.package_name,
            renderer_profile="orm_runtime",
            materialization_source="semantic_contract",
            artifact_family="ocg_language_materialization",
            output_key="schema-migration",
        )
        result = CodeGeneratedMaterializationDeltaResult(
            ...
            entries=[
                CodeGeneratedMaterializationDeltaEntry(
                    ...
                    renderer_operations=[
                        CodeGeneratedRendererDeltaOperation(
                            ...
                            renderer_key="test.runtime_model",
                            renderer_profile="orm_runtime",
                        )
                    ],
                ),
                CodeGeneratedMaterializationDeltaEntry(
                    ...
                    artifact_family="ocg_language_materialization",
                    artifact_key="schema-migration",
                    package_delta=CodePackageDelta(
                        package_name=context.package_name,
                        paths=[
                            CodePackageDeltaPath(
                                relative_path="migrations/add_device.sql",
                                kind=CodePackageDeltaKind.create,
                                content_text="alter table add device\n",
                                path_role=CodePackagePathRole.generated_code,
                                metadata={
                    ...
                    renderer_operations=[
                        CodeGeneratedRendererDeltaOperation(
                            kind=CodeGeneratedRendererDeltaOperationKind.replace_anchor,
                            renderer_key="test.schema_migration",
                            renderer_profile="orm_runtime",
                        )
                    ],
                ),
        ...
        return (
            MetaProviderDeltaGeneratedMaterializationFeatureResult(
                feature_key="test.generated_materialization",
                ...
                status="generated_materialization_projected",
                reason="test_package_delta_ready",
                result=result,
            ),
        )

    monkeypatch.setattr(
        "aware_meta.materialization.deltas.generated_materialization."
        "generated_materialization_feature_results_from_typed_operation",
        _fake_feature_results,
    )
    ...
    assert stage["entry_count"] == 2
    assert stage["generated_materialization_artifact_path_count"] == 2
    assert stage["generated_materialization_artifact_family_counts"] == {
        "ocg_language_materialization": 2,
    }
    assert stage["generated_materialization_artifact_role_counts"] == {
        "runtime_model": 1,
        "schema_migration": 1,
    ...
    assert stage["generated_materialization_path_kind_counts"] == {
        "create": 1,
        "update": 1,
    }
    ...
    assert {path["relative_path"] for path in artifact_paths} == {
        "generated/device.model",
        "migrations/add_device.sql",
    }
    assert {path["artifact_role"] for path in artifact_paths} == {
        "runtime_model",
        "schema_migration",
    }


def test_meta_provider_delta_generated_materialization_stage_preserves_python_path_with_explicit_language(
    tmp_path,
    monkeypatch,
) -> None:
    _register_python_plugin(monkeypatch)
...
def test_meta_provider_delta_generated_materialization_stage_uses_previous_target_evidence(
    tmp_path,
    monkeypatch,
) -> None:
    _register_python_plugin(monkeypatch)
    ...
    stage = provider_delta_generated_materialization_stage(
        package_payload={
            ...
            "previous_materialization_evidence": {
                "generated_materialization_targets": (
                    {
                        "artifact_family": "ocg_language_materialization",
                        ...
                        "source_code_package_id": "home-source-package",
                        "provider_payload": {
                            "language_output_descriptor": True,
                            "target_language": "plugin",
                        },
                    },
                ),
...
def test_meta_provider_delta_generated_materialization_stage_blocks_unknown_explicit_language(
    tmp_path,
) -> None:
    stage = provider_delta_generated_materialization_stage(
        package_payload={"package_name": "home-ontology"},
        ...
        code_package_delta={
            ...
            "paths": (
                {
                    "relative_path": "home/tv_channel.rb",
                    "language": "aware",
                },
            ),
        },
    ...
    assert stage["status"] == "generated_materialization_blocked"
    assert stage["reason"] == "meta_generated_materialization_expected_output_missing"
    assert stage["renderer_operation_count"] == 0
    assert stage["blocked_feature_result_count"] == 1
    assert stage["diagnostics"] == (
        "target_language_not_supported:ruby",
        "meta_generated_materialization_target_language_unknown",
    )
    feature_results = cast(tuple[dict[str, object], ...], stage["feature_results"])
...
def test_meta_provider_delta_generated_materialization_stage_uses_operation_target_language(
    tmp_path,
    monkeypatch,
) -> None:
    _register_python_plugin(monkeypatch)
    ...
    operation = _class_create_operation()
...
def test_meta_python_orm_function_create_emits_code_ready_section_delta_evidence(
    tmp_path,
) -> None:
    generated_package_root = tmp_path / "structure" / "ontology" / "python"
    generated_source_root = generated_package_root / "aware_home_ontology" / "home"
...
@pytest.mark.asyncio
async def test_meta_python_orm_function_invocation_delta_lowers_aware_body_source(
    tmp_path,
) -> None:
    generated_package_root = tmp_path / "structure" / "ontology" / "python"
    generated_source_root = generated_package_root / "aware_home_ontology" / "home"
    ...
    operation = _function_invocation_typed_operation(include_body_evidence=False)
    operation = replace(
        operation,
        current={
            **dict(operation.current),
            "generated_materialization": _python_orm_runtime_targets(
                ...
                body_kind="aware_function_body",
                baseline_function_description="Rename the channel.",
...
async def test_meta_provider_delta_generated_materialization_stage_uses_stage_sources_root(
    ...
    assert result.entries[0].target.relative_path == (
        "aware_content_ontology/content/content_layout.py"
    )
    assert result.entries[0].relative_path == result.entries[0].target.relative_path
...
def test_meta_python_orm_attribute_type_emits_code_ready_section_delta_evidence(
    tmp_path,
) -> None:
    generated_source_root = tmp_path / "aware_home_ontology" / "home"
    generated_source_root.mkdir(parents=True)
    ...
    assert evidence.result.available is True
    assert evidence.result.diagnostics == []
    assert (
        evidence.result.mode
        is CodeGeneratedMaterializationDeltaMode.section_delta_ready
    ...
    assert len(entry.renderer_operations) == 1
    assert entry.section_delta is not None
    assert entry.section_delta.package_root == tmp_path.as_posix()
    assert entry.section_delta.sources_root == "aware_home_ontology"
    assert len(entry.section_delta.entries) == 1
    section_entry = entry.section_delta.entries[0]
    assert section_entry.section_ref.relative_path == "home/tv_channel.py"
...
def test_meta_python_orm_default_value_emits_code_ready_section_delta_evidence(
    tmp_path,
) -> None:
    generated_source_root = tmp_path / "aware_home_ontology" / "home"
    generated_source_root.mkdir(parents=True)
