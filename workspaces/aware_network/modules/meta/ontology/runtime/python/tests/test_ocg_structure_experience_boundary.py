...
""".strip(),
    ...
    ns, domains = _ns(fqn_prefix="boundary", namespace="default", code_ids=[code.id])
...
def test_ocg_builder_allows_oneof_identity_keyword_discriminator_variant(
    tmp_path: Path,
) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)
    code = _build_code(
        tmp_path,
        "oneof_keyword_variant.aware",
        """
class Envelope {
    kind String key
    class_id UUID?
...
ann default.Envelope oneof identity class_id enum_id discriminator kind class class_id enum enum_id
""".strip(),
    )
    ns, _domains = _ns(fqn_prefix="boundary", namespace="default", code_ids=[code.id])

    result = build_object_config_graph_from_code(
        name="boundary",
        description="boundary",
        fqn_prefix="boundary",
        file_codes=[("oneof_keyword_variant.aware", code)],
        namespace_by_code_id=ns,
    )

    assert result.graph.object_config_graph_annotations
