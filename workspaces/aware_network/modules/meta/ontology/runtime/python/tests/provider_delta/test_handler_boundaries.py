...
from aware_meta.handlers.impl.class_ import (
    class_config_relationship as relationship_handler,
)
...
from aware_meta.handlers.impl.class_.class_config_relationship import (
    create_association as _create_relationship_association_impl,
    create_attribute as _create_relationship_attribute_impl,
...
class _EmptyHandlerSession:
    def imap_get(self, _model_type: object, _object_id: UUID) -> object | None:
        return None
...
@pytest.mark.asyncio
async def test_class_config_relationship_create_association_mutates_only_relationship(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        relationship_handler,
        "current_handler_session",
        lambda: _EmptyHandlerSession(),
    )
    relationship_id = _test_uuid("handler-create-relationship-association")
    association_class_id = _test_uuid("handler-create-association-class")
    relationship = ClassConfigRelationship(
        id=relationship_id,
        class_config_id=_test_uuid("handler-create-association-source-class"),
        target_class_config_id=_test_uuid("handler-create-association-target"),
        relationship_key="room_devices",
        relationship_type=ClassConfigRelationshipType.one_to_many,
        forward_required=False,
    ...
    association = await _create_relationship_association_impl(
        relationship,
        class_config_id=association_class_id,
        forward_loading_strategy=ClassConfigRelationshipSideLoadingStrategy.eager,
        reverse_loading_strategy=ClassConfigRelationshipSideLoadingStrategy.lazy,
    )
    repeated = await _create_relationship_association_impl(
        relationship,
        class_config_id=association_class_id,
        forward_loading_strategy=ClassConfigRelationshipSideLoadingStrategy.eager,
        reverse_loading_strategy=ClassConfigRelationshipSideLoadingStrategy.lazy,
    ...
    assert repeated is association
    assert relationship.class_config_relationship_association_edge is association
    assert association.class_config_relationship_id == relationship_id
    assert association.class_config_id == association_class_id
    assert (
        association.forward_loading_strategy
        is ClassConfigRelationshipSideLoadingStrategy.eager
    )
    assert (
        association.reverse_loading_strategy
        is ClassConfigRelationshipSideLoadingStrategy.lazy
    )
    assert relationship.relationship_key == "room_devices"
    assert relationship.class_config_relationship_attributes == []


@pytest.mark.asyncio
async def test_class_config_relationship_create_attribute_mutates_only_relationship(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        relationship_handler,
    ...
    relationship_id = _test_uuid("handler-create-relationship-attribute")
    attribute_config_id = _test_uuid("handler-create-relationship-attribute-config")
    relationship = ClassConfigRelationship(
        id=relationship_id,
        class_config_id=_test_uuid("handler-create-attribute-source-class"),
        target_class_config_id=_test_uuid("handler-create-attribute-target"),
        relationship_key="room_devices",
        relationship_type=ClassConfigRelationshipType.one_to_many,
        forward_required=False,
        class_config_relationship_attributes=[],
    ...
    assert repeated is edge
    assert relationship.class_config_relationship_attributes == [edge]
    assert edge.class_config_relationship_id == relationship_id
    assert edge.attribute_config_id == attribute_config_id
    assert edge.direction is ClassConfigRelationshipDirection.forward
    assert edge.role is ClassConfigRelationshipAttributeRole.reference
    assert relationship.relationship_key == "room_devices"
    assert relationship.class_config_relationship_association_edge is None
