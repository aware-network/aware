...
def test_meta_provider_delta_runtime_package_index_patch_noop_is_empty(
    ...
    head_refs = {
        ...
        "semantic_root_object_instance_graph_commit_id": (
            baseline_ref.semantic_root_object_instance_graph_commit_id
        ),
    }
...
def test_meta_provider_delta_runtime_package_index_patch_persists_class_parent_update(
    tmp_path: Path,
) -> None:
    manifest_path = write_meta_delta_fixture(tmp_path)
    package_entry = MetaRuntimePackageIndexEntry(
    ...
    baseline_node_id = provider_delta_uuid("index-patch-parent-child-node")
    class_config_id = provider_delta_uuid("index-patch-parent-child-class-config")
    parent_class_config_id = provider_delta_uuid(
        "index-patch-parent-parent-class-config"
    )
    semantic_package_commit_id = provider_delta_uuid(
        "index-patch-parent-semantic-package-commit"
    )
    semantic_package_oig_commit_id = provider_delta_uuid(
        "index-patch-parent-semantic-package-oig-commit"
    )
    semantic_root_oig_commit_id = provider_delta_uuid(
        "index-patch-parent-root-oig-commit"
    )
    source_commit_id = provider_delta_uuid("index-patch-parent-source-commit")
    semantic_package_id = provider_delta_uuid("index-patch-parent-semantic-package")
    semantic_branch_id = provider_delta_uuid("index-patch-parent-semantic-branch")
    head_refs = {
        "head_ref_status": "head_refs_available",
        "source_object_instance_graph_commit_id": str(source_commit_id),
        "semantic_branch_id": str(semantic_branch_id),
        "semantic_projection_name": "ObjectConfigGraphPackage",
        "semantic_package_id": str(semantic_package_id),
        "semantic_package_commit_id": str(semantic_package_commit_id),
        "semantic_object_instance_graph_commit_id": str(semantic_package_oig_commit_id),
    ...
    request = SimpleNamespace(
        package=base_request.package,
        baseline_ref=base_request.baseline_ref,
        current_delta_fingerprint="sha256:index-patch-parent",
    )
    ...
    receipt = _provider_delta_runtime_package_index_patch_receipt(
            ...
            "typed_operations": (
                {
                    ...
                    "current": {
                        ...
                        "object_kind": "class",
                        "graph_semantic_key": "ocg:aware_demo",
                        "node_key": "aware_demo.default.home.Child",
                        "entity_id": str(class_config_id),
                        "entity_name": "Child",
                        "class_config_id": str(class_config_id),
                        "class_fqn": "aware_demo.default.home.Child",
                        ...
                        "payload": {
                            ...
                            "parent_class_semantic_key": parent_semantic_key,
                            "source_refs": ("home/model.aware",),
                            "semantic_fingerprint": "sha256:class-parent",
                        },
                    },
                },
            ),
        ...
        provider_delta_oig_commit_receipt={
            "status": "execute_flag_commit_applied",
            "reason": "meta_ocg_provider_delta_oig_commit_applied",
            "commit_id": str(semantic_package_commit_id),
            "object_instance_graph_commit_id": str(semantic_package_oig_commit_id),
        },
        current_delta_fingerprint="sha256:index-patch-parent",
    )
    ...
    assert receipt["status"] == "runtime_package_index_patch_applied"
    assert receipt["did_persist"] is True
    assert receipt["semantic_object_upsert_count"] == 1
    assert receipt["semantic_object_delete_count"] == 0
    assert receipt["semantic_object_upsert_keys"] == (class_semantic_key,)
    payload = json.loads(
        meta_runtime_package_projection_index_path(
            aware_root=tmp_path,
        ).read_text(encoding="utf-8")
    ...
    class_entry = semantic_objects[class_semantic_key]
    class_payload = cast(dict[str, object], class_entry["payload"])
    current_payload = cast(dict[str, object], class_payload["current_payload"])
    current = cast(dict[str, object], class_payload["current"])
    assert class_entry["object_kind"] == "class"
    assert class_entry["object_id"] == str(baseline_node_id)
    assert class_entry["entity_id"] == str(class_config_id)
    assert class_entry["node_key"] == "aware_demo.default.home.Child"
    assert class_entry["source_refs"] == ["home/model.aware"]
    assert class_entry["semantic_package_object_instance_graph_commit_id"] == (
        str(semantic_package_commit_id)
    )
    assert class_entry["semantic_root_object_instance_graph_commit_id"] == (
        str(semantic_root_oig_commit_id)
    )
    assert class_entry["source_object_instance_graph_commit_id"] == (
        str(source_commit_id)
    )
    assert class_entry["runtime_delta_fingerprint"] == "sha256:index-patch-parent"
    assert class_payload["provider_operation_type"] == "meta_ocg.class.parent.update"
    assert class_payload["baseline_object_id"] == str(baseline_node_id)
    assert class_payload["semantic_fingerprint"] == "sha256:class-parent"
    assert current["parent_class_id"] == str(parent_class_config_id)
    assert current["parent_class_fqn"] == "aware_demo.default.home.Parent"
    assert current["parent_class_semantic_key"] == parent_semantic_key
    assert current_payload["parent_class_id"] == str(parent_class_config_id)
    assert current_payload["parent_class_fqn"] == "aware_demo.default.home.Parent"
    assert current_payload["parent_class_semantic_key"] == parent_semantic_key
