...
def test_ocg_genesis_typed_operation_plan_names_package_root_and_members() -> None:
    ...
    assert plan["typed_operation_count"] == 9
...
def test_ocg_genesis_preflight_is_fully_executable_via_functioncalls() -> None:
    ...
    assert preflight["typed_operation_count"] == 9
    assert preflight["ontology_invocation_intent_count"] == 10
    assert preflight["executable_operation_count"] == 9
    ...
    assert ontology_plan["operation_handler_result_count"] == 9
    assert handler_reason_counts == {
        ...
        "meta_opg_projection_binding_create_function_call_ready": 1,
        "meta_opg_projection_declaration_create_function_call_ready": 1,
    ...
    assert capability_status_counts == {
        EXECUTABLE_VIA_ONTOLOGY_FUNCTION: 9,
    }
...
def test_ocg_genesis_preflight_orders_root_and_member_intents() -> None:
    ...
    assert [
        ...
        (
            ...
            "create_binding",
            str(provider_delta_uuid("ocg-genesis-opg-declaration")),
            "ObjectConfigGraph",
            None,
            True,
        ),
...
def _genesis_spec() -> MetaOcgGenesisSpec:
    return MetaOcgGenesisSpec(
        ...
        object_projection_graph_declaration_id=str(
            provider_delta_uuid("ocg-genesis-opg-declaration")
        ),
        object_projection_graph_binding_id=str(
            provider_delta_uuid("ocg-genesis-opg-binding")
        ),
