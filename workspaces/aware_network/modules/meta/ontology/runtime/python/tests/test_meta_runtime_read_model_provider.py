...
def test_meta_runtime_read_model_provider_caches_workspace_projection_context(
    ...
    def _build_context(
        *,
        ...
        assert composition_context_id is not None
    ...
    assert first.index.composition_context_id == composition_context_id
...
def test_meta_api_activation_read_model_sidecar_avoids_context_hydration(
    ...
    calls = 0
    seen_body_requirements: list[str | None] = []
    source_fingerprint = {
        "schema": "aware.meta.runtime.read_model.source_fingerprint.v1",
        "status": "ok",
        "packages": [
    ...
    def _build_context(
        *,
        ...
        runtime_context_graph_body_requirement: str | None = None,
    ) -> object:
        nonlocal calls
        _ = (
            repo_root,
        ...
        assert strict_package_graph_cache is True
        assert load_source_graph_payloads is False
        seen_body_requirements.append(runtime_context_graph_body_requirement)
        calls += 1
        return build_meta_graph_runtime_context(runtime_graphs=(_runtime_graph(),))
    ...
    assert first.cache_status == "miss"
    assert first.projection_hash_for_name("Workspace") == "sha256:test:Workspace"
    assert seen_body_requirements == ["index_only"]
