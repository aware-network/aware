...
async def test_oigi_history_projection_index_hit_avoids_head_materialization() -> None:
    ...
    hit = await _oigi_history_projection_head_index_hit(
        store=_Store(),  # pyright: ignore[reportArgumentType]
        oigi_head={
...
async def test_oigi_history_uses_identity_sidecar_for_class_instance_identities() -> (
    ...
    assert (
        ...
        "ensure_branch_lane" in phases
    )
    assert (
        "runtime.invoke_function.required_commit_reactions.oigi_history."
        "scan_existing_history" in phases
    )
    assert (
        "runtime.invoke_function.required_commit_reactions.oigi_history."
        "ensure_history_commit_wrapper" in phases
    )
    assert (
        "runtime.invoke_function.required_commit_reactions.oigi_history."
        "read_identity_sidecar" in phases
    )
    assert (
        "runtime.invoke_function.required_commit_reactions.oigi_history."
        "project_identity_sidecar" in phases
    )
    assert (
        "runtime.invoke_function.required_commit_reactions.oigi_history."
        "advance_history_lane_head" in phases
    )
