...
async def test_fs_commit_store_resolves_typed_oig_commit_id_from_ref_index(
    ...
    lineage = [
        item
        async for item in store.iter_lineage_forward_records(
            branch_id=branch_id,
            projection_hash=opg.projection_hash,
    ...
    assert [item.commit_id for item in lineage] == [commit.commit.id]
...
async def test_fs_store_session_read_cache_reuses_hot_lane_json(
    ...
    commit_first = await store.get_commit_record(
        branch_id=branch_id,
        projection_hash=opg.projection_hash,
        commit_id=commit.commit.id,
    )
    commit_second = await store.get_commit_record(
        branch_id=branch_id,
        projection_hash=opg.projection_hash,
        commit_id=commit.commit.id,
    )
    ...
    assert commit_first.commit_id == commit_second.commit_id
