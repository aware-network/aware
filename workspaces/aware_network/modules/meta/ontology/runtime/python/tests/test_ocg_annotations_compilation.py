...
def test_canonical_ocg_compiles_ann_annotations(tmp_path: Path) -> None:
    ...
    assert la.namespace == "main_domain.default"
...
def test_overlay_class_attribute_enum_option_compiles(tmp_path: Path) -> None:
    ...
    ns, domains = _ns(
        fqn_prefix="ann_pkg", namespace="main_domain.default", code_ids=[code.id]
    )
    ...
    assert class_over.namespace == "main_domain.default"
...
def test_override_fk_compiles_nullable_and_name(tmp_path: Path) -> None:
    ...
    assert ov.namespace == "main_domain.default"
...
""".strip(),
    ...
    assert dep_root.namespace == "dep_domain.dep_schema"
    assert dep_root.class_name == "Base"


def test_namespace_name_ambiguity_raises(tmp_path: Path) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)

    # Two code files define the same class name under different namespaces.
    # Annotation uses a partial namespace.Name identifier, which is ambiguous and must error.
    c1 = _build_code(
        tmp_path,
        "a.aware",
    ...
    ns_a, domains_a = _ns(fqn_prefix="pkg", namespace="dom_a", code_ids=[c1.id])
    ns_b, domains_b = _ns(fqn_prefix="pkg", namespace="dom_b", code_ids=[c2.id])
    ns = {**ns_a, **ns_b}
