...
def test_function_config_semantic_operation_resolution_is_feature_registered(
    ...
    function_registration = next(
        ...
        if registration.handler_key == "function_config.semantic_operation_resolution"
    )
...
def test_attribute_config_semantic_operation_resolution_is_feature_registered(
    ...
    attribute_registration = next(
        registration
        for registration in registrations
        if registration.handler_key == "attribute_config.semantic_operation_resolution"
    )
...
def test_enum_option_semantic_operation_resolution_is_feature_registered(
    ...
    enum_option_registration = next(
        registration
        for registration in registrations
        if registration.handler_key == "enum_option.semantic_operation_resolution"
    )
...
def test_exported_meta_ocg_semantic_operations_are_feature_registered(
    ...
    exported_operation_names = tuple(
        ...
        if name.startswith("META_OBJECT_CONFIG_GRAPH_") and name.endswith("_OPERATION")
    )
...
def test_attribute_create_from_source_delta_targets_class_config_receiver(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    ...
    class_fqn = "aware_home.home.TvChannel"
    ...
    class_config_id = str(
        stable_class_config_id(
            object_config_graph_node_id=UUID(object_config_graph_node_id),
            class_fqn=class_fqn,
        )
    )
    ...
    [resolution] = resolve_meta_semantic_operation_function_call_plan_previews(
        typed_operations=(
            {
                "operation_key": (
                    ...
                    "TvChannel.channel_number:create"
                ),
                "operation_family": "create",
                "semantic_operation_type": (
                    META_OBJECT_CONFIG_GRAPH_ATTRIBUTE_CREATE_OPERATION
                ),
                "semantic_key": "meta.attribute:TvChannel.channel_number",
                "semantic_subject_type": "aware_meta.AttributeConfig",
                "field_path": "definition",
                "event_key": (
                    "meta.attribute:TvChannel.channel_number:definition:upsert"
                ),
                "source": "aware_code.semantic_source_meaning",
                "source_refs": ("structure/aware/home/tv_channel.aware",),
        ...
        current_semantic_object_ids={
            "meta.class:TvChannel": object_config_graph_node_id,
        },
    )

    assert resolution.status == "function_call_plan_blocked"
    assert resolution.metadata["execution_ready"] is True
    assert resolution.metadata["class_fqn"] == class_fqn
    assert resolution.metadata["receiver_object_id"] == class_config_id
    assert resolution.metadata["semantic_apply_receiver_object_id"] == class_config_id
    assert resolution.metadata["semantic_source_object_id"] == (
        object_config_graph_node_id
    )
    assert resolution.metadata["object_config_graph_node_id"] == (
        object_config_graph_node_id
    )
    typed_plan = _mapping(resolution.metadata["provider_delta_typed_operation_plan"])
    ...
    assert current["owner_object_id"] == class_config_id
    assert current["object_config_graph_node_id"] == object_config_graph_node_id
    assert current["semantic_source_object_id"] == object_config_graph_node_id
    assert current["semantic_apply_receiver_object_id"] == class_config_id
    attribute_target = _mapping(
        _mapping(current["generated_materialization"])["targets"],
    )["orm_runtime"]
    attribute_target = _mapping(attribute_target)
    assert attribute_target["target_language"] == "python"
    assert attribute_target["relative_path"] == "home/tv_channel.py"
    assert payload["class_config_id"] == class_config_id
    payload_target = _mapping(
        _mapping(payload["generated_materialization"])["targets"],
    )["orm_runtime"]
    payload_target = _mapping(payload_target)
    assert payload_target["target_language"] == "python"
    assert payload_target["relative_path"] == "home/tv_channel.py"
    anchors = _sequence(typed_plan["semantic_object_anchors"])
    class_anchor = _mapping(anchors[-1])
    anchor_current = _mapping(class_anchor["current"])
    assert anchor_current["object_id"] == class_config_id
    assert anchor_current["class_config_id"] == class_config_id
    assert anchor_current["object_config_graph_node_id"] == object_config_graph_node_id
    assert anchor_current["semantic_source_object_id"] == object_config_graph_node_id
    ...
    assert intents[0]["target_object_id"] == class_config_id
    assert intents[0]["function_name"] == "create_primitive_attribute_config"


def test_collection_attribute_create_serializes_default_for_function_call(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    ...
    graph_id = UUID("00000000-0000-0000-0000-000000000602")
    ...
    [resolution] = resolve_meta_semantic_operation_function_call_plan_previews(
        typed_operations=(
            {
                "operation_key": (
                    "aware_meta.object_config_graph.attribute:"
                    "StorageBucket.allowed_mime_types:create"
                ),
                "operation_family": "create",
                "semantic_operation_type": (
                    META_OBJECT_CONFIG_GRAPH_ATTRIBUTE_CREATE_OPERATION
                ...
                "semantic_key": (
                    "meta.attribute:StorageBucket.allowed_mime_types"
                ),
                "semantic_subject_type": "aware_meta.AttributeConfig",
                "field_path": "definition",
                "event_key": (
                ...
                "source": "aware_code.semantic_source_meaning",
                "source_refs": ("structure/aware/bucket/storage_bucket.aware",),
        ...
        current_semantic_object_ids={
            "meta.class:StorageBucket": object_config_graph_node_id,
        },
    )
    ...
    assert intents[0]["target_object_id"] == class_config_id
    assert intents[0]["function_name"] == "create_collection_primitive_attribute_config"
    assert kwargs["default_value"] == "[]"
    assert kwargs["collection_kind"] == "list"
...
def test_function_create_preserves_list_default_for_generated_materialization(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    ...
    [resolution] = resolve_meta_semantic_operation_function_call_plan_previews(
        typed_operations=(
            {
                "operation_key": (
                    ...
                    "StorageBucket.build:create"
                ),
                "operation_family": "create",
                "semantic_operation_type": (
                    META_OBJECT_CONFIG_GRAPH_FUNCTION_CREATE_OPERATION
                ),
                "semantic_key": "meta.function:StorageBucket.build",
                "semantic_subject_type": "aware_meta.FunctionConfig",
                "field_path": "name",
        ...
        current_semantic_object_ids={
            "meta.class:StorageBucket": "storage-bucket-class-id",
        },
    )
    ...
    current = _mapping(typed_operation["current"])
    signature = _mapping(current["function_signature"])
    inputs = _sequence(signature["inputs"])
    input_payload = _mapping(inputs[0])
    assert input_payload["name"] == "allowed_mime_types"
    assert input_payload["default_value"] == []
...
def test_resolves_class_description_update_to_provider_delta_boundary(
    ...
    assert current["class_fqn"] == "aware_demo.default.home.TvChannel"
    ...
    orm_target = _mapping(targets["orm_runtime"])
    assert orm_target["renderer_profile"] == "orm_runtime"
    assert orm_target["relative_path"] == "home/tv_channel.py"
    generated_intent = _mapping(resolution.metadata["generated_materialization_intent"])
    assert generated_intent["intent_kind"] == (
...
def test_resolves_class_structural_create_to_provider_delta_boundary(
    ...
    assert generated_intent["status"] == "generated_materialization_intent_ready"
    assert (
        generated_intent["policy_key"]
        == "aware_meta.generated_materialization.class.create"
    )
    ontology_plan = build_provider_delta_ontology_execution_plan(
        request=SimpleNamespace(),
        provider_delta_typed_operation_plan=typed_plan,
...
def test_resolves_enum_description_update_to_provider_delta_boundary(
    ...
    _assert_neutral_generated_materialization_intent(
        generated_intent,
...
def test_resolves_enum_option_create_to_provider_delta_boundary(
    ...
    _assert_neutral_generated_materialization_intent(
        generated_intent,
        policy_key="aware_meta.generated_materialization.enum.option_line",
        materialization_target="enum_option_line",
    )
    assert resolution.metadata["generated_materialization_intent_blockers"] == ()
    ontology_plan = build_provider_delta_ontology_execution_plan(
        request=SimpleNamespace(),
...
def test_resolves_enum_structural_create_to_provider_delta_boundary(
    ...
    assert (
        generated_intent["policy_key"]
        == "aware_meta.generated_materialization.enum.create"
    )
    assert generated_intent["materialization_target"] == "enum_class"
    ontology_plan = build_provider_delta_ontology_execution_plan(
        request=SimpleNamespace(),
        provider_delta_typed_operation_plan=typed_plan,
    )
...
def test_resolves_enum_structural_delete_to_provider_delta_boundary(
    ...
    assert intent["policy_key"] == "aware_meta.generated_materialization.enum.delete"
    typed_plan = _mapping(resolution.metadata["provider_delta_typed_operation_plan"])
...
def test_function_signature_add_input_serializes_list_default_for_function_call(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    function_config_id = "00000000-0000-0000-0000-000000000121"
    ...
    [resolution] = resolve_meta_semantic_operation_function_call_plan_previews(
        ...
        current_semantic_object_ids={
            "meta.function:StorageBucket.build": function_config_id,
        },
    )

    assert resolution.status == "function_call_plan_blocked"
    typed_operation_plan = _mapping(
        resolution.metadata["provider_delta_typed_operation_plan"],
    )
    typed_operation = _mapping(_sequence(typed_operation_plan["typed_operations"])[0])
    current = _mapping(typed_operation["current"])
    signature = _mapping(current["function_signature"])
    input_payload = _mapping(_sequence(signature["inputs"])[0])
    assert input_payload["default_value"] == []
    ...
    assert ontology_plan["status"] == "ontology_execution_plan_ready"
    intents = cast(Sequence[Mapping[str, object]], ontology_plan["invocation_intents"])
    add_input_intent = next(
        intent
        for intent in intents
        if intent["function_name"] == "add_primitive_attribute_config"
    )
    kwargs = _mapping(add_input_intent["kwargs"])
    assert kwargs["name"] == "allowed_mime_types"
    assert kwargs["default_value"] == "[]"
...
def test_function_membership_constructor_resolution_requests_provider_delta_executor(
    ...
    assert resolution.metadata["receiver_semantic_key"] == ("meta.class:ContentLayout")
    assert resolution.metadata["receiver_object_id"] == class_config_id
...
def test_function_delete_resolution_requests_provider_delta_executor(
    ...
    assert current["generated_materialization"] == _python_orm_runtime_targets(
        relative_path="aware_content_ontology/content/content_layout.py",
    ...
    assert (
        generated_intent["provider_delta_typed_operation_plan"] == typed_operation_plan
    )
...
def test_function_impl_empty_body_without_identity_is_noop(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    ...
    assert (
        resolution.reason
        == "meta_ocg_function_impl_empty_body_without_identity_noop"
    )
    assert resolution.metadata["semantic_apply_boundary"] == "not_applicable"
    assert resolution.metadata["function_impl_body_empty"] is True
...
def test_relationship_create_resolution_requests_provider_delta_executor(
                    ...
                    "generated_materialization": {
                        "targets": {
                            "orm_runtime": {
                                ...
                                "renderer_profile": "orm_runtime",
                                "materialization_source": "ontology_orm_models",
                                "relative_path": "content/content_layout.py",
                            },
                        },
                    },
                },
    ...
    relationship_target = _mapping(
        _mapping(current["generated_materialization"])["targets"],
    )["orm_runtime"]
    relationship_target = _mapping(relationship_target)
    assert relationship_target["target_language"] == "python"
    assert relationship_target["relative_path"] == "content/content_layout.py"
    generated_intent = _mapping(resolution.metadata["generated_materialization_intent"])
    assert generated_intent["intent_kind"] == (
...
def test_relationship_create_resolution_honors_payload_class_config_ids(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    source_class_id = "00000000-0000-0000-0000-000000000311"
    target_class_id = "00000000-0000-0000-0000-000000000312"
    relationship_id = "00000000-0000-0000-0000-000000000313"
        ...
        typed_operations=(
            {
                ...
                "after_payload": {
                    "class_name": "ContentLayout",
                    "source_class_fqn": "content.ContentLayout",
                    "target_class_fqn": "content.ContentPlacement",
                    "relationship_key": "placements",
                    "relationship_type": "one_to_many",
                    "source_class_config_id": source_class_id,
...
def test_function_impl_body_resolution_normalizes_provider_delta_payload(
    ...
    targets = generated_materialization["targets"]
    assert isinstance(targets, Mapping)
    orm_runtime = targets["orm_runtime"]
    assert isinstance(orm_runtime, Mapping)
    assert "baseline_body_text" not in orm_runtime
    assert "body_text" not in orm_runtime
    for key, value in ORM_RUNTIME_TARGET_PROFILE.target_metadata().items():
        assert orm_runtime[key] == value
    assert orm_runtime["baseline_body_kind"] == "not_implemented"
    assert orm_runtime["body_kind"] == "aware_function_body"
    assert orm_runtime["baseline_function_description"] == (
        "Rename the channel display label for humans and assistants."
    )
    assert orm_runtime["baseline_aware_body_text"] == (
        "{\n"
        '    """\n'
        "    Rename the channel display label for humans and assistants.\n"
        '    """\n'
    ...
    assert orm_runtime["aware_body_text"] == ("{\n    set name = display_name\n}")
...
def _python_orm_runtime_targets(
    *,
    ...
    return {"targets": {ORM_RUNTIME_TARGET_PROFILE.target_key: target}}


def _assert_neutral_generated_materialization_intent(
    ...
    *,
    ...
    assert intent["policy_key"] == policy_key
    assert intent["materialization_target"] == materialization_target
