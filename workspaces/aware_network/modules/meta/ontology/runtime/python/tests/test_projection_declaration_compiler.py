...
def test_projection_declarations_compile_into_ocg_declarations_and_seed_observables(
    ...
    assert {v.observable_key for v in observables} == {"welcome", "home"}
    welcome = next(v for v in observables if v.observable_key == "welcome")
    home = next(v for v in observables if v.observable_key == "home")
    assert welcome.kind == ObjectProjectionGraphObservableKind.construct
    assert home.kind == ObjectProjectionGraphObservableKind.instance
    ...
    assert any(
        (v.get("observable_key") or "").strip() == "welcome" for v in dumped_views
    )

    assert actor_focus_opg_id in opgis_by_opg_id


def test_projection_observable_legacy_grouping_fails_during_parse(
    tmp_path: Path,
) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)
    ...
    with pytest.raises(ValueError, match="nested observables"):
        _build_code(
            tmp_path,
            "projection_legacy_grouping.aware",
            (
                "class Identity {\n"
                "}\n"
                "\n"
...
def test_projection_observable_without_explicit_kind_defaults_to_instance(
    tmp_path: Path,
) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)
    ...
    code = _build_code(
        tmp_path,
        "projection_implicit_kind.aware",
        (
            "class Identity {\n"
            "}\n"
            "\n"
    ...
    res = build_object_config_graph_from_code(
        ...
        description="proj_graph",
        fqn_prefix="proj_pkg",
        file_codes=[("projection_implicit_kind.aware", code)],
        namespace_by_code_id=ns,
    )
    ...
    assert [v.kind for v in observables] == [
        ObjectProjectionGraphObservableKind.instance,
