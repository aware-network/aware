...
def test_semantic_scope_closure_resolves_local_import_and_external_symbols() -> None:
    ...
    evidence = build_meta_ocg_semantic_scope_closure(
        ...
        namespace_by_code_id={
            ...
            api_code_id: NamespacePath(
                package="aware_demo",
                namespace="default.api.endpoint",
            ),
        },
    ...
    assert evidence.closure_hash.startswith("sha256:")
    assert [closure_ref.ref_key for closure_ref in evidence.closure_refs] == [
        "meta_ocg_scope:local:aware_demo",
        f"meta_ocg_scope:external:{external_graph.id}",
    ]
    assert [closure_ref.symbol_count for closure_ref in evidence.closure_refs] == [
        2,
        1,
    ]
    assert evidence.closure_refs[1].graph_id == str(external_graph.id)
    assert evidence.closure_refs[1].graph_hash == external_graph.hash
...
def test_semantic_scope_closure_refs_are_stable_for_same_inputs() -> None:
    code_id = provider_delta_uuid("semantic-scope-stable-ref-code")
    room_class = _class_config(
        key="semantic-scope-stable-ref-room",
        class_fqn="aware_demo.default.home.Room",
        name="Room",
    )
    external_graph = _external_graph()

    first = build_meta_ocg_semantic_scope_closure(
        package_fqn_prefix="aware_demo",
        namespace_by_code_id={
    ...
    second = build_meta_ocg_semantic_scope_closure(
        package_fqn_prefix="aware_demo",
        namespace_by_code_id={
            code_id: NamespacePath(
                package="aware_demo",
    ...
    assert first.evidence_payload()["closure_refs"] == (
        second.evidence_payload()["closure_refs"]
    )
    assert first.closure_refs[0].closure_hash == second.closure_refs[0].closure_hash
    assert first.closure_refs[1].closure_hash == second.closure_refs[1].closure_hash


def test_semantic_scope_external_closure_ref_changes_when_symbols_change() -> None:
    code_id = provider_delta_uuid("semantic-scope-external-drift-code")
    external_graph = _external_graph()
    changed_external_graph = _external_graph_with_extra_symbol()
    ...
    second = build_meta_ocg_semantic_scope_closure(
        ...
        external_graphs=(changed_external_graph,),
    )

    first_external = first.closure_refs[1]
    second_external = second.closure_refs[1]
    assert first_external.ref_key == second_external.ref_key
    assert first_external.symbol_count == 1
    assert second_external.symbol_count == 2
    assert first_external.closure_hash != second_external.closure_hash
    assert first.closure_hash != second.closure_hash
...
def test_semantic_scope_closure_payload_is_stable() -> None:
    ...
    assert isinstance(payload["closure_hash"], str)
    assert payload["closure_hash"].startswith("sha256:")
    assert payload["closure_refs"] == (
        {
            "ref_key": "meta_ocg_scope:local:aware_demo",
            "origin": "local",
            "package_fqn_prefix": "aware_demo",
            "closure_hash": evidence.closure_refs[0].closure_hash,
            "symbol_count": 1,
            "class_symbol_count": 1,
            "enum_symbol_count": 0,
            "graph_id": None,
            "graph_hash": None,
        },
...
def _external_graph_with_extra_symbol() -> ObjectConfigGraph:
    graph = _external_graph()
    camera_class = _class_config(
        key="semantic-scope-external-camera",
        class_fqn="aware_external.default.sensor.Camera",
        name="Camera",
    )
    graph.object_config_graph_nodes.append(
        _ocg_node(
            object_config_graph_id=graph.id,
            node_key=camera_class.class_fqn,
            class_config=camera_class,
        )
    )
    return graph
