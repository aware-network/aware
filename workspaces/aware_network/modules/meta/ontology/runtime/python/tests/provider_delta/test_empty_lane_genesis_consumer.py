...
async def test_provider_delta_empty_lane_routes_ocg_genesis(
    ...
    assert typed_plan["typed_operation_count"] == 9
    ...
    assert dirty_diff["dirty_entry_count"] == 9
    assert head_move_plan["status"] == "head_move_plan_ready"
    assert head_move_plan["planned_operation_count"] == 9
    assert ontology_plan["status"] == "ontology_execution_plan_ready"
    assert ontology_plan["invocation_intent_count"] == 10
...
async def test_provider_delta_empty_lane_executes_ocg_genesis_functioncalls(
    ...
    assert runtime_package_index_patch["semantic_object_upsert_count"] == 9
    ...
    assert invocation_execution["applied_invocation_count"] == 10
    ...
    assert mutation_proof["typed_operation_count"] == 9
    assert mutation_proof["ontology_invocation_intent_count"] == 10
    assert mutation_proof["ontology_invocation_receipt_count"] == 10
    assert mutation_proof["ontology_applied_invocation_count"] == 10
    assert mutation_proof["mutation_entry_count"] == 10
    assert mutation_proof["satisfied_mutation_entry_count"] == 10
    ...
    genesis_spec = _mapping(genesis_preflight["spec"])
    projection_name = str(genesis_spec["projection_name"])
    class_fqn = str(genesis_spec["class_fqn"])
    declaration_semantic_key = (
        f"ocg:aware_demo/projection_declaration:{projection_name}"
    )
    binding_semantic_key = f"{declaration_semantic_key}/binding:{class_fqn}"
    raw_upsert_keys = runtime_package_index_patch["semantic_object_upsert_keys"]
    assert isinstance(raw_upsert_keys, Sequence)
    assert not isinstance(raw_upsert_keys, str)
    upsert_keys = tuple(str(key) for key in raw_upsert_keys)
    assert declaration_semantic_key in upsert_keys
    assert binding_semantic_key in upsert_keys

    package_index = load_meta_runtime_package_projection_index(aware_root=aware_root)
    assert package_index is not None
    declaration_entry = package_index.semantic_objects_by_key[declaration_semantic_key]
    binding_entry = package_index.semantic_objects_by_key[binding_semantic_key]
    assert declaration_entry.object_kind == "object_projection_graph_declaration"
    assert binding_entry.object_kind == "object_projection_graph_binding"
    assert str(declaration_entry.object_id) == str(
        genesis_spec["object_projection_graph_declaration_id"]
    )
    assert str(binding_entry.object_id) == str(
        genesis_spec["object_projection_graph_binding_id"]
    )
