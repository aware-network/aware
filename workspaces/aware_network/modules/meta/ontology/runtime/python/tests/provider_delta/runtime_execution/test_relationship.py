...
@pytest.mark.asyncio
async def test_meta_provider_delta_executes_relationship_derived_edge_intents_through_runtime(
    tmp_path: Path,
) -> None:
    ctx = build_provider_delta_runtime_execution_context(
        workspace_root=tmp_path,
        key="provider-delta-relationship-derived-edges",
    )
    relationship_semantic_key = (
        "ocg:aware_demo/node:aware_demo.default.home.Room:room_devices:"
        "one_to_many:aware_demo.default.home.Device"
    ...
    relationship_config_id = provider_delta_uuid(
        "provider-delta-relationship-derived-parent"
    )
    association_class_config_id = provider_delta_uuid(
        "provider-delta-relationship-derived-association-class"
    )
    association_id = provider_delta_uuid(
        "provider-delta-relationship-derived-association"
    )
    attribute_config_id = provider_delta_uuid(
        "provider-delta-relationship-derived-attribute"
    )
    relationship_attribute_id = provider_delta_uuid(
        "provider-delta-relationship-derived-relationship-attribute"
    )
    typed_operation_plan: dict[str, object] = {
        "status": "typed_operation_plan_ready",
        "reason": "meta_ocg_provider_delta_typed_operation_plan_ready",
    ...
    assert len(ctx.runtime.requests) == 2
    association_request = ctx.runtime.requests[0]
    attribute_request = ctx.runtime.requests[1]
    assert association_request.call_target is MetaGraphCallTarget.instance
    assert association_request.function_id == provider_delta_uuid(
        "ClassConfigRelationship.create_association.function"
    )
    assert association_request.target_object_id == relationship_config_id
    assert association_request.expected_head_commit_id == (
        ctx.baseline_root_domain_commit_id
    )
    assert association_request.domain_projection_hash == ctx.root_projection_hash
    assert association_request.domain_object_instance_graph_id == (
        ctx.baseline_root_oig_id
    )
    assert association_request.domain_object_instance_graph_identity_id == (
        ctx.baseline_root_oigi_id
    )
    assert association_request.kwargs == {
        "class_config_id": str(association_class_config_id),
        "forward_loading_strategy": "eager",
        "reverse_loading_strategy": "lazy",
    }
    assert attribute_request.call_target is MetaGraphCallTarget.instance
    assert attribute_request.function_id == provider_delta_uuid(
        "ClassConfigRelationship.create_attribute.function"
    )
    assert attribute_request.target_object_id == relationship_config_id
    assert (
        attribute_request.expected_head_commit_id == ctx.runtime.receipts[0].commit_id
    )
    assert attribute_request.domain_projection_hash == ctx.root_projection_hash
    assert attribute_request.domain_object_instance_graph_id == (
        ctx.baseline_root_oig_id
    )
    assert attribute_request.domain_object_instance_graph_identity_id == (
        ctx.baseline_root_oigi_id
    )
    assert attribute_request.kwargs == {
        "attribute_config_id": str(attribute_config_id),
        "direction": "forward",
        "role": "reference",
    }
