...
CODE_ONE = """
class Example {
    fn make construct(name String) -> Example {
        \"\"\"Create Example.\"\"\"
    }
}
""".strip()
...
CODE_TWO = """
class Example {
    fn make construct(name String, count Int) -> Example {
        \"\"\"Create Example.\"\"\"
    }
}
""".strip()
...
def test_ocg_hash_changes_when_function_signature_changes(tmp_path: Path) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)

    code_a = _build_code(tmp_path, "a.aware", CODE_ONE)
    ns_a, domains_a = _ns(
    ...
    code_b = _build_code(tmp_path, "b.aware", CODE_TWO)
    ns_b, domains_b = _ns(
