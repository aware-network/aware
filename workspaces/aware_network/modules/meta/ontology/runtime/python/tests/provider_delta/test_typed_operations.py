...
from aware_meta.attribute.config.deltas.semantic_operation_resolution import (
...
def test_meta_attribute_create_source_meaning_merges_generated_target_path() -> None:
    class_semantic_key = "meta.class:Room"
    class_config_id = provider_delta_uuid("attribute-create-target-class-config")
    class_node_id = provider_delta_uuid("attribute-create-target-class-node")
    attribute_config_id = provider_delta_uuid("attribute-create-target-attribute")

    resolution = resolve_attribute_config_semantic_operation(
        operation={
            "operation_key": "aware_meta.object_config_graph.attribute:Room.nickname:create",
            "operation_family": "create",
            "semantic_operation_type": (
                META_OBJECT_CONFIG_GRAPH_ATTRIBUTE_CREATE_OPERATION
            ),
            "semantic_key": "meta.attribute:Room.nickname",
        ...
        operation_group=(),
        current_objects={class_semantic_key: str(class_config_id)},
        current_object_identities={
            class_semantic_key: {
                ...
                "semantic_source_object_id": str(class_node_id),
                "semantic_apply_receiver_object_id": str(class_config_id),
    ...
    orm_runtime = cast(dict[str, object], targets["orm_runtime"])
    assert orm_runtime["target_descriptor_source"] == (
        "aware_meta.target_language_profile_selection"
    )
    assert orm_runtime["relative_path"] == "home/room.py"
    assert current["owner_object_id"] == str(class_config_id)
    assert current["object_config_graph_node_id"] == str(class_node_id)
    assert current["attribute_config_id"] == str(attribute_config_id)
