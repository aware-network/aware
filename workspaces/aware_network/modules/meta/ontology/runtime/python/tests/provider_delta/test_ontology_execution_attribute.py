...
def test_meta_provider_delta_attribute_update_accepts_attribute_config_id() -> None:
    attribute_semantic_key = (
        "ocg:aware_demo/node:aware_demo.default.home.Room/attribute:name"
    )
    attribute_config_id = provider_delta_uuid("capability-attribute-config-id")
    typed_operation_plan = {
        "status": "typed_operation_plan_ready",
        "reason": "meta_ocg_provider_delta_typed_operation_plan_ready",
        "typed_operation_count": 1,
    ...
    assert intent["expected_result_object_id"] == str(attribute_config_id)
    kwargs = cast(dict[str, object], intent["kwargs"])
    assert kwargs["primitive_base_type"] == "integer"
...
def test_meta_provider_delta_collection_attribute_create_ontology_execution_plan_ready() -> (
    None
):
    ...
    assert intent["function_name"] == "create_collection_primitive_attribute_config"
    assert intent["target_object_id"] == str(class_config_id)
    assert intent["expected_result_object_id"] == str(attribute_config_id)
    kwargs = cast(dict[str, object], intent["kwargs"])
    assert kwargs["name"] == "remote_control"
