...
def test_meta_ocg_delta_coverage_matrix_marks_projected_code_segments() -> None:
    ...
    assert {
        ...
        ("class.update.parent", "class", "parent_class"),
...
def test_meta_ocg_delta_coverage_matrix_locks_workspace_delta_first_modes() -> None:
    assert workspace_delta_first_mode_counts() == {
        WORKSPACE_DELTA_FIRST_MODE_PUBLIC_SEGMENT_READY: 13,
        WORKSPACE_DELTA_FIRST_MODE_PUBLIC_GENERATED_APPLY_READY: 13,
        WORKSPACE_DELTA_FIRST_MODE_PUBLIC_GRAPH_ONLY_READY: 12,
        WORKSPACE_DELTA_FIRST_MODE_SEMANTIC_CONTRACT_ONLY: 0,
        WORKSPACE_DELTA_FIRST_MODE_EXPLICIT_FALLBACK_REQUIRED: 2,
    }
    assert {
        entry.case_key
    ...
    assert len(workspace_delta_first_ready_entries()) == 38
    assert all(
        entry.workspace_delta_first_ready
        for entry in workspace_delta_first_ready_entries()
    )
...
def test_meta_ocg_delta_coverage_matrix_payload_is_stable() -> None:
    ...
    assert payload["source_projection_ready_count"] == 13
    ...
    assert policy_counts == {
        ...
        SOURCE_PROJECTION_POLICY_RENDER_ALL_REQUIRED: 27,
        SOURCE_PROJECTION_POLICY_SEGMENT_READY: 13,
    }
...
def test_meta_ocg_delta_product_readiness_payload_is_stable() -> None:
    ...
    assert payload["ready_operation_count"] == 13
    assert payload["render_all_required_operation_count"] == 27
    ...
    assert payload["workspace_delta_first_ready_operation_count"] == 38
    assert payload["workspace_delta_first_mode_counts"] == {
        WORKSPACE_DELTA_FIRST_MODE_PUBLIC_SEGMENT_READY: 13,
        WORKSPACE_DELTA_FIRST_MODE_PUBLIC_GENERATED_APPLY_READY: 13,
        WORKSPACE_DELTA_FIRST_MODE_PUBLIC_GRAPH_ONLY_READY: 12,
    ...
    assert all(
        "public_lifecycle_status" in entry and "public_lifecycle_refs" in entry
        for entry in (*ready_operations, *render_all_required)
    )
    function_delete = next(
        entry for entry in render_all_required if entry["case_key"] == "function.delete"
    )
    ...
    function_impl_create = next(
        entry
        for entry in ready_operations
        if entry["case_key"] == "function_impl.create"
    )
    ...
    assert {entry["case_key"] for entry in workspace_contract_only} == set()
    assert all(
        not entry["workspace_delta_first_ready"] for entry in workspace_contract_only
    )
    assert {entry["case_key"] for entry in workspace_explicit_fallback} == {
        "object_projection_graph_binding.create",
        "object_projection_graph_declaration.create",
    }
