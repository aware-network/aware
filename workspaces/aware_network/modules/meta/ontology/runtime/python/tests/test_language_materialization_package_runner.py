...
def test_dart_packaged_language_dependency_contract_is_declared_strict(
    tmp_path: Path,
) -> None:
    workspace_root = tmp_path / "workspace"
    package_root = (
        workspace_root / "modules" / "demo" / "structure" / "ontology" / "dart"
    )
    helpers_dir = (
        workspace_root / "libs" / "model_helpers" / "dart" / "aware_model_helpers"
    )
    aware_api_dir = workspace_root / "modules" / "api" / "libs" / "api" / "dart"
    for directory in (package_root, helpers_dir, aware_api_dir):
    ...
    dependencies, optional_dependencies, metadata = (
        language_service_module._language_package_dependency_contract(  # noqa: SLF001
            request=request,
    ...
    assert metadata["package_dependencies_mode"] == "strict"
    assert metadata["repo_root"] == workspace_root.as_posix()
    assert "freezed_annotation: ^3.0.0" in dependencies
    assert "json_annotation: ^4.9.0" in dependencies
    assert "uuid: ^4.5.1" in dependencies
    assert (
        f"aware_model_helpers:\n  path: {expected_helpers_rel}" in dependencies
    )
    assert f"aware_api:\n  path: {expected_api_rel}" in dependencies
...
def test_dart_ontology_dto_dependency_contract_omits_aware_api(
    tmp_path: Path,
) -> None:
    workspace_root = tmp_path / "workspace"
    package_root = (
        workspace_root / "modules" / "demo" / "structure" / "ontology_dto" / "dart"
    )
    helpers_dir = (
        workspace_root / "libs" / "model_helpers" / "dart" / "aware_model_helpers"
    )
    for directory in (package_root, helpers_dir):
        directory.mkdir(parents=True, exist_ok=True)
    (workspace_root / "aware.workspace.toml").write_text(
        "aware = 1\n", encoding="utf-8"
    )
    ...
    assert metadata["package_dependencies_mode"] == "strict"
    assert any(
        dependency.startswith("aware_model_helpers:") for dependency in dependencies
    )
    assert not any(dependency.startswith("aware_api:") for dependency in dependencies)
...
def test_legacy_packaged_language_deleted_file_refs_cover_python_layout_migration() -> (
    ...
    legacy_deleted_refs = (
        language_service_module._legacy_packaged_language_deleted_file_refs
    )
    refs = legacy_deleted_refs(  # noqa: SLF001
        request=request,
        generated_files=(
