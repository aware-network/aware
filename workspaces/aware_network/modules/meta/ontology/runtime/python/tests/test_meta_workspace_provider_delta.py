...
async def test_meta_provider_delta_adapter_dry_run_contract(
    ...
    assert readiness["operation_count"] == 40
    assert readiness["ready_operation_count"] == 13
    assert readiness["render_all_required_operation_count"] == 27
    assert readiness["blocked_operation_count"] == 0
    assert readiness["workspace_delta_first_ready_operation_count"] == 38
    semantic_contract_only_operations = readiness[
        "workspace_semantic_contract_only_operations"
    ]
    assert isinstance(semantic_contract_only_operations, tuple)
    assert semantic_contract_only_operations == ()
    explicit_fallback_operations = readiness[
        "workspace_explicit_fallback_required_operations"
    ]
    assert isinstance(explicit_fallback_operations, tuple)
    assert {operation["case_key"] for operation in explicit_fallback_operations} == {
...
async def test_meta_provider_delta_workspace_forwards_declared_context(
    ...
    manifest_path = _write_meta_delta_fixture(tmp_path)
    request = _provider_delta_request(manifest_path=manifest_path).model_copy(
        update={
            "context": {
...
async def test_meta_provider_delta_executes_attribute_update_from_aware_delta(
    ...
    attribute_semantic_key = "ocg:aware_demo/node:aware_demo.home.Room/attribute:name"
