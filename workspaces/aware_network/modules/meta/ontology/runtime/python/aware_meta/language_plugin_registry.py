...
class MetaLanguagePluginRegistry:
    ...
    _plugins: dict[object, MetaLanguagePlugin] = {}
    _supported_languages: set[object] = set()
    _file_filter_overrides: dict[object, FilterConfig] = {}
    _structural_filter_overrides: dict[
        object, Callable[[str, str | None], StructuralFilterDecision]
    ] = {}
    ...
    def register(cls, plugin: MetaLanguagePlugin) -> None:
        ...
        if language_key not in cls._supported_languages:
            cls._plugins[language_key] = plugin
    ...
    def get_renderer(
        ...
        return plugin.get_renderer(
            output_directory, layout_strategy, overwrite, kind=kind, profile=profile
        )
    ...
    def get_default_renderers(
        ...
        """
        plugin = cls.get(language)
        return plugin.get_default_renderers(
            output_directory, layout_strategy, overwrite, profile=profile
        )
    ...
    def get_file_filter_config(cls, language: CodeLanguage):
        ...
        override = cls._file_filter_overrides.get(
            _language_key(language)
        ) or cls._file_filter_overrides.get(language)
        if override:
    ...
    def has_language(cls, language: CodeLanguage) -> bool:
        ...
        """
        return (
            _language_key(language) in cls._supported_languages
    ...
    ) -> type[ObjectConfigGraphRenderLayoutStrategy] | None:
        """
