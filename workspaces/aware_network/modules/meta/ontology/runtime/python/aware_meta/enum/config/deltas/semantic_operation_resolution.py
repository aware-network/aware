...
from aware_meta.semantic_operation_resolution import (
    ...
    _enum_name_from_enum_semantic_key,
    ...
    _set_missing_text,
    _stable_enum_config_id_for_create,
    _stable_object_config_graph_node_id_for_enum_create,
    _string_value,
    _tuple_values,
)


def resolve_enum_config_semantic_operation(
    *,
    ...
    if operation_type == META_OBJECT_CONFIG_GRAPH_ENUM_CREATE_OPERATION:
        return _resolve_enum_create(
            operation=_enum_create_operation_with_sibling_payload(
                operation=operation,
    ...
    if operation_type == META_OBJECT_CONFIG_GRAPH_ENUM_DELETE_OPERATION:
        return _resolve_enum_delete(
            operation=operation,
            current_objects=current_objects,
            current_object_identities=current_object_identities,
        )
    if operation_type == META_OBJECT_CONFIG_GRAPH_ENUM_DESCRIPTION_UPDATE_OPERATION:
        return _resolve_enum_description_update(
            operation=operation,
            current_objects=current_objects,
            current_object_identities=current_object_identities,
        )
    return _blocked_resolution(
        operation=operation,
        reason="meta_ocg_enum_operation_type_not_supported",
        blockers=(f"unsupported_operation_type:{operation_type or 'unknown'}",),
    )


def _resolve_enum_create(
    *,
    operation: Mapping[str, object],
    current_objects: Mapping[str, str],
    current_object_identities: Mapping[str, Mapping[str, object]],
    ...
    if operation_family not in {"create", "upsert"}:
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_create_requires_create_family",
            blockers=(
    ...
    if subject_type not in {
        "EnumConfig",
        "aware_meta.EnumConfig",
        "ObjectConfigGraphNode",
        "aware_meta.ObjectConfigGraphNode",
    }:
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_create_subject_not_supported",
            blockers=(
    ...
    enum_name = _first_text(
        ...
        _enum_name_from_enum_semantic_key(semantic_key),
    )
    raw_enum_fqn = _first_text(
        after_payload.get("enum_fqn"),
        after_payload.get("node_key"),
        _enum_fqn_from_enum_semantic_key(semantic_key),
        enum_name,
    )
    ...
    enum_config_id = _stable_enum_config_id_for_create(
    ...
    provider_delta_typed_operation_metadata = (
        _provider_delta_typed_operation_metadata_for_enum_create(
            operation=operation,
            graph_semantic_key=graph_semantic_key,
            graph_object_id=graph_source_object_id,
            object_config_graph_node_id=object_config_graph_node_id,
    ...
    metadata: dict[str, object] = {
        ...
        "result_object_id": enum_config_id,
        "object_config_graph_node_id": object_config_graph_node_id,
        **provider_delta_typed_operation_metadata,
    }
    if (
    ...
    return _blocked_resolution(
        operation=operation,
        reason=(
            "meta_ocg_enum_create_requires_provider_delta_"
            "ontology_operation_executor"
        ),
        blockers=(
            *blockers,
            "semantic_plan_single_function_call_preview_not_supported",
            "provider_delta_enum_create_operation_executor_required",
        ),
        metadata=metadata,
    )


def _enum_create_operation_with_sibling_payload(
    *,
    operation: Mapping[str, object],
    operation_group: tuple[Mapping[str, object], ...],
) -> Mapping[str, object]:
    ...
    description = _enum_create_description_from_siblings(
        enum_semantic_key=semantic_key,
        operation_group=operation_group,
    )
    if description is not None:
        _set_missing_text(enriched_after_payload, "description", description)
        _set_missing_text(enriched_after_payload, "enum_description", description)

    if not _tuple_values(enriched_after_payload.get("values")):
        values = _enum_create_values_from_siblings(
            enum_name=enum_name,
            enum_semantic_key=semantic_key,
            operation_group=operation_group,
        )
        if values:
            enriched_after_payload["values"] = values

    if enriched_after_payload == after_payload:
        return operation
...
def _enum_create_description_from_siblings(
    *,
    enum_semantic_key: str,
    operation_group: tuple[Mapping[str, object], ...],
) -> str | None:
    for candidate in operation_group:
        if _semantic_operation_type(candidate) != (
            META_OBJECT_CONFIG_GRAPH_ENUM_DESCRIPTION_UPDATE_OPERATION
        ):
            continue
        if _semantic_key(candidate) != enum_semantic_key:
            continue
        after_payload = _mapping_value(candidate.get("after_payload"))
        description = _first_text(
            after_payload.get("description"),
            after_payload.get("enum_description"),
        )
        if description is not None:
            return description
    return None
...
def _enum_create_values_from_siblings(
    *,
    enum_name: str,
    enum_semantic_key: str,
    operation_group: tuple[Mapping[str, object], ...],
) -> tuple[str, ...]:
    values_by_position: list[tuple[int, int, str]] = []
    seen: set[str] = set()
    for index, candidate in enumerate(operation_group):
        if _semantic_operation_type(candidate) != (
            META_OBJECT_CONFIG_GRAPH_ENUM_OPTION_CREATE_OPERATION
        ):
            continue
        option_enum_key = _enum_semantic_key_from_enum_option_semantic_key(
            _semantic_key(candidate),
        )
        option_enum_name = _enum_name_from_enum_option_semantic_key(
            _semantic_key(candidate),
        )
        if option_enum_key != enum_semantic_key and option_enum_name != enum_name:
            continue
        after_payload = _mapping_value(candidate.get("after_payload"))
        value = _first_text(
            after_payload.get("value"),
            after_payload.get("option_value"),
            after_payload.get("enum_option_value"),
            after_payload.get("name"),
            _enum_option_value_from_semantic_key(_semantic_key(candidate)),
        )
        if value is None or value in seen:
            continue
        position = _int_value(after_payload.get("position"))
        values_by_position.append(
            (position if position is not None else index, index, value)
        )
        seen.add(value)
    return tuple(value for _, _, value in sorted(values_by_position))


def _provider_delta_typed_operation_metadata_for_enum_create(
    *,
    operation: Mapping[str, object],
    ...
    if (
        ...
        and enum_config_id is not None
    ):
        typed_operation_plan = _enum_create_typed_operation_plan_with_identity(
    ...
    generated_materialization_metadata = (
        _enum_create_generated_materialization_intent_metadata(
            typed_operation_plan=typed_operation_plan,
    ...
    metadata: dict[str, object] = {
        ...
        "provider_delta_typed_operation_reason": (
            "enum_create_provider_delta_operation_ready"
            if ready
            else "enum_create_provider_delta_operation_blocked"
        ),
    ...
    if not ready:
        metadata["provider_delta_typed_operation_blockers"] = (
            "enum_create_provider_delta_typed_operation_unavailable",
        )
    return metadata


def _enum_create_generated_materialization_intent_metadata(
    *,
    ...
    blockers: tuple[str, ...] = (
        ("enum_create_generated_materialization_typed_plan_unavailable",)
        if not ready
        else ()
    )
    status = (
    ...
    reason = (
        "enum_create_generated_materialization_typed_plan_unavailable"
        if not ready
        else "enum_create_generated_materialization_intent_ready"
    )
    intent: dict[str, object] = {
        "intent_kind": "meta_enum_create_generated_materialization_intent",
        "contract_version": (
            META_ENUM_CREATE_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION
        ),
        "status": status,
    ...
    if ready:
        intent.update(
            generated_materialization_intent_target_metadata(
                policy_key="aware_meta.generated_materialization.enum.create",
                materialization_target="enum_class",
                target_language=(
                    _generated_materialization_target_language_from_typed_operation(
                        typed_operation
                    )
                ),
            )
...
def _enum_create_typed_operation_plan_with_identity(
    *,
    typed_operation_plan: Mapping[str, object],
    ...
    enriched_plan["typed_operations"] = tuple(
        (
            _enum_create_typed_operation_with_identity(
...
def _enum_create_typed_operation_with_identity(
    *,
    ...
    for target in (current, payload):
        ...
        target["enum_config_id"] = enum_config_id
        target["entity_id"] = enum_config_id
        if enum_name is not None:
            _set_missing_text(target, "name", enum_name)
            _set_missing_text(target, "entity_name", enum_name)
        if enum_fqn is not None:
            target["enum_fqn"] = enum_fqn
            target["node_key"] = enum_fqn
    if payload:
...
def _provider_delta_typed_operation_metadata_for_enum_delete(
    *,
    operation: Mapping[str, object],
    graph_semantic_key: str | None,
    graph_object_id: str | None,
    ...
    if (
        ...
        typed_operation_plan = _enum_delete_typed_operation_plan_with_identity(
            typed_operation_plan=typed_operation_plan,
            graph_semantic_key=graph_semantic_key,
            graph_object_id=graph_object_id,
            object_config_graph_node_id=object_config_graph_node_id,
    ...
    ready = status == "typed_operation_plan_ready" and typed_operation is not None
    generated_materialization_metadata = (
        _enum_delete_generated_materialization_intent_metadata(
            typed_operation_plan=typed_operation_plan,
            typed_operation=typed_operation,
            ready=ready,
        )
    ...
    metadata: dict[str, object] = {
        ...
        "provider_delta_typed_operation_reason": (
            "enum_delete_provider_delta_operation_ready"
            if ready
            else "enum_delete_provider_delta_operation_blocked"
        ),
        "provider_delta_typed_operation_plan": typed_operation_plan,
        **generated_materialization_metadata,
    }
    ...
    if not ready:
        metadata["provider_delta_typed_operation_blockers"] = (
            "enum_delete_provider_delta_typed_operation_unavailable",
        )
    return metadata


def _enum_delete_generated_materialization_intent_metadata(
    *,
    typed_operation_plan: Mapping[str, object],
    typed_operation: Mapping[str, object] | None,
    ready: bool,
    ...
    blockers: tuple[str, ...] = (
        ("enum_delete_generated_materialization_typed_plan_unavailable",)
        if not ready
        else ()
    )
    status = (
    ...
    reason = (
        "enum_delete_generated_materialization_typed_plan_unavailable"
        if not ready
        else "enum_delete_generated_materialization_intent_ready"
    )
    intent: dict[str, object] = {
        "intent_kind": "meta_enum_delete_generated_materialization_intent",
        "contract_version": (
            META_ENUM_DELETE_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION
        ),
        "status": status,
    ...
    if ready:
        intent.update(
            generated_materialization_intent_target_metadata(
                policy_key="aware_meta.generated_materialization.enum.delete",
                materialization_target="enum_class",
                target_language=(
                    _generated_materialization_target_language_from_typed_operation(
                        typed_operation
...
def _enum_delete_typed_operation_plan_with_identity(
    *,
    typed_operation_plan: Mapping[str, object],
    graph_semantic_key: str,
    graph_object_id: str,
    ...
    enriched_plan["typed_operations"] = tuple(
        (
            _enum_delete_typed_operation_with_identity(
                typed_operation=item,
                graph_semantic_key=graph_semantic_key,
                graph_object_id=graph_object_id,
                object_config_graph_node_id=object_config_graph_node_id,
...
def _enum_delete_typed_operation_with_identity(
    *,
    typed_operation: Mapping[str, object],
    graph_semantic_key: str,
    graph_object_id: str,
...
def _resolve_enum_delete(
    *,
    ...
    if operation_family != "delete":
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_delete_requires_delete_family",
            blockers=(
                "unsupported_operation_family:" f"{operation_family or 'unknown'}",
            ),
        )
    ...
    if subject_type not in {
        ...
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_delete_subject_not_supported",
            blockers=(
                "unsupported_semantic_subject_type:" f"{subject_type or 'unknown'}",
            ),
        )
    ...
    provider_delta_typed_operation_metadata = (
        _provider_delta_typed_operation_metadata_for_enum_delete(
            operation=operation,
            graph_semantic_key=graph_semantic_key,
            graph_object_id=graph_source_object_id,
            object_config_graph_node_id=object_config_graph_node_id,
    ...
    return _blocked_resolution(
        operation=operation,
        reason=(
            "meta_ocg_enum_delete_requires_provider_delta_"
            "ontology_operation_executor"
        ),
        blockers=(
            *blockers,
            "semantic_plan_single_function_call_preview_not_supported",
            "provider_delta_enum_delete_operation_executor_required",
        ),
        metadata=metadata,
    )


def _resolve_enum_description_update(
    *,
    operation: Mapping[str, object],
    current_objects: Mapping[str, str],
    current_object_identities: Mapping[str, Mapping[str, object]],
    ...
    if _string_value(operation.get("operation_family")) not in {"update", "upsert"}:
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_description_update_requires_update_family",
            blockers=(
    ...
    if _string_value(operation.get("semantic_subject_type")) not in {
        ...
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_description_update_subject_not_supported",
            blockers=(
    ...
    if _optional_text(operation.get("field_path")) not in {
        ...
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_enum_description_update_field_not_supported",
            blockers=(
    ...
    receiver_semantic_key = (
        f"meta.enum:{enum_name}" if enum_name is not None else semantic_key
    )
    receiver_object_id = _enum_config_id_for_structural_enum(
        operation=operation,
        current_objects=current_objects,
        current_object_identities=current_object_identities,
        enum_name=enum_name,
        enum_fqn=enum_fqn,
        enum_semantic_key=receiver_semantic_key,
    )
    provider_delta_typed_operation_metadata = (
        _provider_delta_typed_operation_metadata_for_enum_description_update(
            operation=operation,
            enum_config_id=receiver_object_id,
            enum_name=enum_name,
            enum_fqn=enum_fqn,
        )
    )
    ...
    return _blocked_resolution(
        ...
        blockers=(
            *blockers,
            "semantic_plan_single_function_call_preview_not_supported",
            "provider_delta_enum_update_operation_executor_required",
        ),
        metadata=metadata,
    )


def _provider_delta_typed_operation_metadata_for_enum_description_update(
    *,
    operation: Mapping[str, object],
    enum_config_id: str | None,
    enum_name: str | None,
    ...
    if enum_config_id is not None:
        typed_operation_plan = _enum_update_typed_operation_plan_with_identity(
            typed_operation_plan=typed_operation_plan,
            enum_config_id=enum_config_id,
            enum_name=enum_name,
            enum_fqn=enum_fqn,
    ...
    metadata: dict[str, object] = {
        ...
        "provider_delta_typed_operation_reason": (
            "enum_description_provider_delta_operation_ready"
            if ready
            else "enum_description_provider_delta_operation_blocked"
        ),
        "provider_delta_typed_operation_plan": typed_operation_plan,
    }
    if typed_operation is not None:
    ...
    if not ready:
        metadata["provider_delta_typed_operation_blockers"] = (
            "enum_description_provider_delta_typed_operation_unavailable",
        )
    metadata.update(
        _enum_description_generated_materialization_intent_metadata(
            typed_operation_plan=typed_operation_plan,
            typed_operation=typed_operation,
            ready=ready,
        )
...
def _enum_description_generated_materialization_intent_metadata(
    *,
    typed_operation_plan: Mapping[str, object],
    typed_operation: Mapping[str, object] | None,
    ready: bool,
    ...
    blockers: tuple[str, ...] = (
        ("enum_description_generated_materialization_typed_plan_unavailable",)
        if not ready
        else ()
    )
    status = (
    ...
    reason = (
        "enum_description_generated_materialization_typed_plan_unavailable"
        if not ready
        else "enum_description_generated_materialization_intent_ready"
    )
    intent: dict[str, object] = {
        "intent_kind": "meta_enum_description_generated_materialization_intent",
        "contract_version": (
            META_ENUM_DESCRIPTION_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION
        ),
        "status": status,
    ...
    if ready:
        intent.update(
            generated_materialization_intent_target_metadata(
                policy_key="aware_meta.generated_materialization.enum.description",
                materialization_target="enum_docstring",
                target_language=(
                    _generated_materialization_target_language_from_typed_operation(
                        typed_operation
                    )
...
def _enum_update_typed_operation_plan_with_identity(
    *,
    typed_operation_plan: Mapping[str, object],
    enum_config_id: str,
    enum_name: str | None,
    ...
    enriched_plan["typed_operations"] = tuple(
        (
            _enum_update_typed_operation_with_identity(
                typed_operation=item,
                enum_config_id=enum_config_id,
                enum_name=enum_name,
                enum_fqn=enum_fqn,
...
def _enum_update_typed_operation_with_identity(
    *,
    typed_operation: Mapping[str, object],
    enum_config_id: str,
    enum_name: str | None,
    ...
    baseline_object = _mapping_value(baseline.get("object"))
    for target in (current, payload):
        target.setdefault("entity_id", enum_config_id)
        target.setdefault("enum_config_id", enum_config_id)
        if enum_name is not None:
            target.setdefault("name", enum_name)
            target.setdefault("entity_name", enum_name)
        if enum_fqn is not None:
            target.setdefault("enum_fqn", enum_fqn)
    if enum_name is not None:
        baseline_object.setdefault("name", enum_name)
        baseline_object.setdefault("entity_name", enum_name)
    if enum_fqn is not None:
        baseline_object.setdefault("enum_fqn", enum_fqn)
    if payload:
        current["payload"] = payload
    if baseline_object:
        baseline["object"] = baseline_object
...
def _generated_materialization_target_language_from_typed_operation(
    typed_operation: Mapping[str, object] | None,
) -> str | None:
    ...
    for payload in (current, current_payload, baseline_object, baseline):
        generated = _mapping_value(payload.get("generated_materialization"))
        targets = _mapping_value(generated.get("targets"))
        for target_payload in targets.values():
            target = _mapping_value(target_payload)
            target_language = _optional_text(target.get("target_language"))
            if target_language is not None:
                return target_language
    return None


def _resolve_enum_unsupported_fallback(
    *,
    operation: Mapping[str, object],
    reason: str,
    blockers: tuple[str, ...],
    ...
    execution_preconditions = (
        ("ontology_delete_function_required",)
        if "enum_delete_ontology_function_missing" in blockers
        else ("explicit_fallback_required",)
    )
    metadata: dict[str, object] = {
        "source": "aware_meta.semantic_operation_resolution",
        "semantic_apply_boundary": "provider_delta_ontology_operation_executor",
        ...
        "generated_materialization_intent": {
            "intent_kind": "meta_enum_structural_fallback_required",
            "contract_version": (
                META_ENUM_DESCRIPTION_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION
            ),
