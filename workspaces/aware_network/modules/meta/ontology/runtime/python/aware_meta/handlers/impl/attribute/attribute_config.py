...
async def create_collection_primitive(
    ...
    """
    Create deterministic primitive-collection AttributeConfig and descriptor chain.
    """
    ...
    descriptor = await AttributeTypeDescriptor.create_collection_primitive(
    ...
    if existing is not None:
        ...
        if (
            existing.type_descriptor_id is not None
            and existing.type_descriptor_id != descriptor.id
        ):
            raise RuntimeError(
                "AttributeConfig descriptor mismatch for existing attribute config: "
                f"attribute_config_id={attribute_config_id}"
