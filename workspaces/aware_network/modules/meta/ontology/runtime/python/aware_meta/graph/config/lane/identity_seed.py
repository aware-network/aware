...
def _identity_seed_commit_id(
    *, branch_id: UUID, projection_hash: str, graph_hash_post: str
) -> UUID:
    if not (projection_hash or "").strip():
        raise GraphIdentitySeedError(
            "Identity seed commit id requires a non-empty projection_hash"
        )
    if not (graph_hash_post or "").strip():
        raise GraphIdentitySeedError(
            "Identity seed commit id requires a non-empty graph_hash_post"
        )
    return uuid5(
        NAMESPACE_URL,
...
async def ensure_graph_identity_seeded_lane(
    *,
    ...
    if not lane_empty:
        ...
        return GraphIdentitySeedPlan(
            ...
            graph_hash_post=(
                ...
                if head is not None
                and "graph_hash_post" in head
                and head["graph_hash_post"] is not None
                else None
            ),
        )
    ...
    if commit_id is None:
        raise GraphIdentitySeedError(
            "Identity seed plan missing deterministic commit_id"
        )
    ...
    if existing_envelope is not None:
        if (
            existing_envelope.graph_hash_pre != commit.graph_hash_pre
            or existing_envelope.graph_hash_post != commit.graph_hash_post
        ):
            raise GraphIdentitySeedError(
        ...
        parents = existing_envelope.parent_commit_ids
        if parents:
            raise GraphIdentitySeedError(
    ...
    try:
        ...
        raise GraphIdentitySeedError(
            f"Failed to build identity seed commit: {exc}"
        ) from exc
    except (ValueError, OigCommitValidationError) as exc:
        raise GraphIdentitySeedError(
            f"Failed to emit identity seed commit: {exc}"
        ) from exc
...
def _build_identity_seed_plan_and_commit(
    *,
    ...
    if not objects_by_id:
        raise GraphIdentitySeedError(
            "Identity seed requires at least one ORM object (empty root_instance)"
        )
