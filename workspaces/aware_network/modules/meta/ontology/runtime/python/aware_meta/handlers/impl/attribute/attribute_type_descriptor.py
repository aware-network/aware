...
async def create_collection_primitive(
    ...
    """
    Create deterministic collection descriptor whose element is one primitive descriptor.

    Contract:
    - The collection descriptor is keyed by collection kind and its element descriptor link.
    - V0 intentionally supports primitive elements only; enum/class/nested collections get their own
      typed rails.
    """

    # --- AWARE: LOGIC START create_collection_primitive
    collection_kind = _normalize_collection_kind(collection_kind)
    primitive_base_type = _normalize_primitive_base_type(primitive_base_type)
    child_descriptor = await create_primitive(primitive_base_type=primitive_base_type)
    child_descriptor_id = child_descriptor.id
    if child_descriptor_id is None:
        raise RuntimeError(
            "AttributeTypeDescriptor.create_collection_primitive requires child descriptor id"
        )

    child_links_fingerprint = (
        f"{AttributeTypeDescriptorRole.element.value}:0:{child_descriptor_id}"
    )
    attribute_type_descriptor_id = stable_attribute_type_descriptor_id(
        kind=AttributeTypeDescriptorKind.collection.value,
        collection_kind=collection_kind.value,
        entity_id=None,
    ...
    existing = session.imap_get(AttributeTypeDescriptor, attribute_type_descriptor_id)
    if existing is not None:
        if existing.kind != AttributeTypeDescriptorKind.collection:
            raise RuntimeError(
                "AttributeTypeDescriptor.create_collection_primitive kind mismatch "
                "for existing descriptor: "
                f"attribute_type_descriptor_id={attribute_type_descriptor_id}"
            )
        if existing.collection_kind != collection_kind:
            raise RuntimeError(
                "AttributeTypeDescriptor.create_collection_primitive collection kind "
                "mismatch for existing descriptor: "
                f"attribute_type_descriptor_id={attribute_type_descriptor_id}"
            )
        for existing_link in existing.child_links:
            if existing_link.id == link_id:
                if existing_link.child is None:
                    existing_link.child = child_descriptor
                    existing_link.child_id = child_descriptor_id
                return existing
        existing.child_links.append(
    ...
    # --- AWARE: LOGIC END create_collection_primitive
...
def _normalize_collection_kind(value: object) -> AttributeCollectionType:
    if isinstance(value, AttributeCollectionType):
        collection_kind = value
    else:
        raw_value = str(value or "").strip()
        try:
            collection_kind = AttributeCollectionType(raw_value)
        except ValueError:
            collection_kind = AttributeCollectionType[raw_value.lower()]
    if collection_kind == AttributeCollectionType.single:
        raise RuntimeError(
            "AttributeTypeDescriptor.create_collection_primitive requires a collection kind"
        )
    return collection_kind


def _normalize_primitive_base_type(value: object) -> CodePrimitiveBaseType:
    if isinstance(value, CodePrimitiveBaseType):
        return value
    raw_value = str(value or "").strip()
    try:
        return CodePrimitiveBaseType(raw_value)
    except ValueError:
        return CodePrimitiveBaseType[raw_value.lower()]
