...
OCG_GENESIS_CONTRACT_VERSION = "aware.meta.ocg.package-genesis-typed-operation.v0"
...
class MetaOcgGenesisSpec:
    ...
    @property
    def object_projection_graph_declaration_semantic_key(self) -> str:
        return (
            f"{self.graph_semantic_key}/projection_declaration:"
            f"{self.resolved_projection_name}"
        )

    @property
    def object_projection_graph_binding_semantic_key(self) -> str:
        return (
            f"{self.object_projection_graph_declaration_semantic_key}"
            f"/binding:{self.class_fqn}"
        )

    @property
    def object_projection_graph_node_semantic_key(self) -> str:
        return f"{self.object_projection_graph_semantic_key}" f"/node:{self.class_fqn}"
...
def ocg_genesis_typed_operation_plan(
    *,
    ...
    operations = (
        ...
        object_projection_graph_declaration_create_typed_operation(
            semantic_key=spec.object_projection_graph_declaration_semantic_key,
            graph_semantic_key=spec.graph_semantic_key,
            object_config_graph_id=spec.object_config_graph_id,
            object_projection_graph_declaration_id=(
                spec.object_projection_graph_declaration_id
            ),
            key=f"{spec.fqn_prefix}:{spec.resolved_projection_name}",
            projection_name=spec.resolved_projection_name,
            label=spec.resolved_projection_name,
            description=spec.projection_description,
            is_branchable=False,
            source_refs=(spec.source_ref,),
        ),
        object_projection_graph_binding_create_typed_operation(
            semantic_key=spec.object_projection_graph_binding_semantic_key,
            object_projection_graph_declaration_semantic_key=(
                spec.object_projection_graph_declaration_semantic_key
            ),
            object_projection_graph_declaration_id=(
                spec.object_projection_graph_declaration_id
            ),
            object_projection_graph_binding_id=spec.object_projection_graph_binding_id,
            fqn_prefix=spec.fqn_prefix,
            namespace=spec.class_namespace,
            class_name=spec.class_name,
            attribute_name=None,
            target_projection_name=spec.resolved_projection_name,
            side="root",
            source_refs=(spec.source_ref,),
        ),
        object_projection_graph_create_typed_operation(
...
def ocg_genesis_preflight(
    *,
    ...
    execution_allowed = functioncall_capability_matrix.get("execution_allowed") is True
    blockers = tuple(
        dict.fromkeys(
            (
    ...
    return {
        ...
        "contract_version": OCG_GENESIS_CONTRACT_VERSION,
        "status": (
            "ocg_genesis_executable" if execution_allowed else "ocg_genesis_blocked"
        ),
        "reason": (
...
def ocg_genesis_preflight_from_provider_delta_request(
    *,
    ...
    base_payload = {
        ...
        "contract_version": (OCG_GENESIS_PROVIDER_DELTA_CONSUMER_CONTRACT_VERSION),
    ...
    return {
        **base_payload,
        "status": (
            "ocg_genesis_consumer_ready" if execution_allowed else "ocg_genesis_blocked"
        ),
        "reason": (
        ...
        "functioncall_capability_matrix": (preflight["functioncall_capability_matrix"]),
...
def ocg_genesis_semantic_dirty_diff_from_preflight(
    *,
    ...
    return {
        ...
        "status": (
            "semantic_dirty_diff_ready" if ready else "semantic_dirty_diff_blocked"
        ),
        "reason": (
        ...
        "baseline_index_compare_status": (
            "baseline_index_compared" if ready else "baseline_index_compare_blocked"
        ),
...
def _ocg_genesis_scope_closure_blockers(
    *,
    ...
    closure_prefix = _optional_text(semantic_scope_closure.get("package_fqn_prefix"))
    if closure_prefix != spec.fqn_prefix:
        blockers.append(
            "semantic_scope_closure_package_mismatch:" f"{closure_prefix or 'unknown'}"
        )
    ...
    if not _scope_closure_contains_class_fqn(
        ...
        blockers.append("semantic_scope_closure_missing_class_fqn:" f"{spec.class_fqn}")
...
def _ocg_genesis_spec_from_provider_delta_analysis(
    *,
    ...
    class_fqn = _optional_text(class_config.class_fqn) or (
        f"{graph.fqn_prefix}.{class_name}"
    )
    class_namespace = _class_namespace_from_fqn(
    ...
    return {
        "spec": MetaOcgGenesisSpec(
            ...
            object_projection_graph_declaration_id=(
                _genesis_opg_declaration_id(
                    object_config_graph_id=str(graph.id),
                    projection_name=projection_name,
                )
            ),
            object_projection_graph_binding_id=_genesis_opg_binding_id(
                object_config_graph_id=str(graph.id),
                projection_name=projection_name,
                fqn_prefix=graph.fqn_prefix,
...
def _genesis_opg_declaration_id(
    *,
    object_config_graph_id: str,
    projection_name: str,
) -> str:
...
def _genesis_opg_binding_id(
    *,
    ...
    declaration_id = stable_object_projection_graph_declaration_id(
        object_config_graph_id=UUID(object_config_graph_id),
        projection_name=projection_name,
    )
    return str(
...
def _class_namespace_from_fqn(*, fqn_prefix: str, class_fqn: str) -> str:
    prefix = f"{fqn_prefix}."
    if class_fqn.startswith(prefix):
        suffix = class_fqn.removeprefix(prefix)
    else:
        suffix = class_fqn
...
def _ocg_genesis_spec_payload(
    *,
    ...
    return {
        ...
        "class_name": spec.class_name,
        "class_fqn": spec.class_fqn,
        "class_namespace": spec.class_namespace,
        "attribute_name": spec.attribute_name,
        "primitive_base_type": spec.primitive_base_type,
        "language": spec.language,
...
def _semantic_dirty_entry_from_genesis_operation(
    *,
    ...
    ontology_subject_kind = (
        _optional_text(operation.get("ontology_subject_kind")) or "unknown"
    )
    ...
    *,
    ...
    if not portal:
        ...
        if target_projection != OCG_GENESIS_OBJECT_PROJECTION_GRAPH_PROJECTION_NAME:
            blockers.append(
                "projection_portal_target_mismatch:" f"{target_projection or 'unknown'}"
            )
        if portal_status != "created_in_plan":
            blockers.append(
                "projection_portal_not_created_in_plan:" f"{portal_status or 'unknown'}"
            )
...
def _projection_portal_consumer_payload(
    *,
    ...
    return {
        ...
        "contract_version": (OCG_GENESIS_PROJECTION_PORTAL_CONSUMER_CONTRACT_VERSION),
        "status": status,
        "reason": reason,
        "required_policy_key": (OCG_GENESIS_OBJECT_PROJECTION_GRAPH_PORTAL_POLICY_KEY),
        "required_target_projection": (
            OCG_GENESIS_OBJECT_PROJECTION_GRAPH_PROJECTION_NAME
        ),
        "projection_portal_resolution_status": _optional_text(resolution.get("status")),
        "projection_portal_resolution_reason": _optional_text(resolution.get("reason")),
        "portal_status": _optional_text(portal.get("status")),
        "portal_reason": _optional_text(portal.get("reason")),
        "portal_target_projection": _optional_text(portal.get("target_projection")),
        "portal_target_lane_status": _optional_text(portal.get("target_lane_status")),
        "portal": dict(portal),
