...
_AWARE_STRUCTURE_SOURCE_TOP_LEVEL_RULES = frozenset(
    {
        "ann_def",
        "binding_def",
        "class_def",
...
def _validate_aware_structure_source_surface(
    *,
    file_codes: list[tuple[str, Code]],
) -> None:
    for rel_path, code in file_codes:
        content_part_text = code.content_part_text
        source_text = content_part_text.inline_text if content_part_text else None
        if source_text is None:
            continue

        language_plugin = CodeLanguagePluginRegistry.get_typed(CodeLanguage.aware)
        code_tree = language_plugin.tree_sitter_adapter.parse_content(source_text)
        if not code_tree:
            raise ValueError(
                f"Structure package boundary violation: {rel_path!r} "
                "is not parseable Aware source"
            )

        root_node = code_tree.root.node
        for child in getattr(root_node, "named_children", ()):
            rule_name = getattr(child, "type", "")
            if rule_name in _AWARE_STRUCTURE_SOURCE_TOP_LEVEL_RULES:
                continue
            if rule_name == "ERROR" and _is_annotation_argument_parse_gap(
            ...
            raise ValueError(
                f"Structure package boundary violation: {rel_path!r} "
                f"declares unsupported top-level Aware rule {rule_name!r}"
            )


def _is_annotation_argument_parse_gap(*, source_text: str, node: object) -> bool:
    start_point = getattr(node, "start_point", None)
    end_point = getattr(node, "end_point", None)
    if start_point is None or end_point is None:
        return False
    start_row = _tree_point_row(start_point)
    end_row = _tree_point_row(end_point)
    ...
    return line.startswith("ann ") and " discriminator " in f" {line} "


def _tree_point_row(point: object) -> int:
    row = getattr(point, "row", None)
    if row is not None:
        return int(row)
    if isinstance(point, Sequence) and point:
        return int(str(point[0]))
    return -1
...
def build_object_config_graph_from_code(
    ...
    if language == CodeLanguage.aware and package_kind == AwarePackageKind.ontology:
        _validate_aware_structure_source_surface(file_codes=file_codes)
