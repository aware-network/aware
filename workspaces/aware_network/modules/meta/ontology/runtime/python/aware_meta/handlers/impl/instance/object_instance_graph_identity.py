...
async def upsert_history_from_lane_head(
    ...
    expected_lane_id = stable_lane_id(
        branch_id=domain_branch_id, lane_hash=domain_projection_hash
    )
    ...
    if all(
        b.id != oigb.id
        for b in object_instance_graph_identity.object_instance_graph_branches
    ):
    ...
    while to_visit:
        ...
        if cid not in existing_commit_ids:
            ...
            if all(
                c.id != oig_commit.id
                for c in object_instance_graph_identity.object_instance_graph_commits
            ):
        ...
        for root_change in payload.object_instance_graph_changes:
            for class_change in root_change.class_instance_changes:
                ...
                if class_instance_id in existing_class_instance_ids:
                    continue
                created = (
                    await object_instance_graph_identity.create_class_instance_identity(
...
async def create_class_instance_relationship_identity(
    ...
    for (
        existing
    ) in object_instance_graph_identity.class_instance_relationship_identities:
    ...
    if all(
        ...
        object_instance_graph_identity.class_instance_relationship_identities.append(
            created
        )
    return created
...
async def create_via_object_projection_graph_identity(
    ...
    if existing is not None:
        if (
            existing.object_projection_graph_identity_id
