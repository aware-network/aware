...
async def create_collection_primitive_via_class_config(
    ...
    """
    Create deterministic ClassConfigAttributeConfig link for a primitive collection.
    """

    # --- AWARE: LOGIC START create_collection_primitive_via_class_config
    if position < 0:
        raise RuntimeError("ClassConfigAttributeConfig position must be >= 0")
    normalized_owner_key = (owner_key or "").strip().casefold()
    normalized_name = (name or "").strip().casefold()
    ...
    attribute_config = await attribute_config_handler.create_collection_primitive(
        owner_key=normalized_owner_key,
        name=normalized_name,
    ...
    if existing is not None:
        ...
        if int(existing.position) != int(position):
            raise RuntimeError(
                "ClassConfigAttributeConfig existing position mismatch: "
                f"class_config_attribute_config_id={edge_id}"
            )
        if bool(existing.is_identity_key) != bool(is_identity_key):
            raise RuntimeError(
    ...
    # --- AWARE: LOGIC END create_collection_primitive_via_class_config
