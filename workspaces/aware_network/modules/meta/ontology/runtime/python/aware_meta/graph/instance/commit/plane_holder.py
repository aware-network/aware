...
async def get_or_create_plane_oigb(
    ...
    if cached:
        existing = await ObjectInstanceGraphBranch.by_id(cached)
        if existing is not None:
            return existing

    # Try to find existing holder for this branch+opg
    try:
        candidate = await ObjectInstanceGraphBranch.one(branch_id=underlying_branch_id)
        if candidate and candidate.object_instance_graph_id:
            oig = await ObjectInstanceGraph.by_id(candidate.object_instance_graph_id)
