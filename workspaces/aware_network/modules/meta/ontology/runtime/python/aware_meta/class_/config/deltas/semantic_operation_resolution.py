...
def resolve_class_config_semantic_operation(
    *,
    ...
    if operation_type == META_OBJECT_CONFIG_GRAPH_CLASS_PARENT_UPDATE_OPERATION:
        return _resolve_class_parent_update(
            operation=operation,
            current_objects=current_objects,
        )
...
def _resolve_class_description_update(
    *,
    ...
    class_identity = _class_identity_for_update(
        operation=operation,
        current_object_identities=current_object_identities,
        class_name=class_name,
        receiver_semantic_key=receiver_semantic_key,
...
def _provider_delta_typed_operation_metadata_for_class_description_update(
    *,
    ...
    class_identity: Mapping[str, object],
) -> dict[str, object]:
    ...
    operation = _operation_with_class_identity(
        operation=operation,
        class_identity=class_identity,
    )
...
def _class_identity_for_update(
    *,
    operation: Mapping[str, object],
    current_object_identities: Mapping[str, Mapping[str, object]],
    class_name: str | None,
    ...
    direct = current_object_identities.get(receiver_semantic_key) or (
        current_object_identities.get(_semantic_key(operation))
    )
...
def _operation_with_class_identity(
    *,
    ...
    for payload_key in ("before_payload", "after_payload"):
        payload = _mapping_value(enriched.get(payload_key))
        _enrich_payload_with_class_identity(
            payload=payload,
            class_identity=class_identity,
        )
        enriched[payload_key] = payload
    semantic_source_object_id = _first_text(
        class_identity.get("semantic_source_object_id"),
        class_identity.get("class_config_id"),
        class_identity.get("entity_id"),
        class_identity.get("object_id"),
    )
...
def _enrich_payload_with_class_identity(
    *,
    ...
    for field_name in (
        ...
        if _is_missing(payload.get(field_name)) and field_name in class_identity:
            payload[field_name] = class_identity[field_name]
    identity_generated = _mapping_value(class_identity.get("generated_materialization"))
    payload_generated = _mapping_value(payload.get("generated_materialization"))
...
def _merged_generated_materialization(
    *,
    ...
    baseline_targets = _mapping_value(baseline.get("targets"))
    payload_targets = _mapping_value(payload.get("targets"))
...
def _resolve_class_parent_update(
    *,
    operation: Mapping[str, object],
    current_objects: Mapping[str, str],
) -> MetaSemanticOperationResolution:
    if _string_value(operation.get("operation_family")) not in {"update", "upsert"}:
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_class_parent_update_requires_update_family",
            blockers=(
                "unsupported_operation_family:"
                f"{_string_value(operation.get('operation_family')) or 'unknown'}",
            ),
    ...
    if _string_value(operation.get("semantic_subject_type")) not in {
        ...
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_class_parent_update_subject_not_supported",
            blockers=(
                "unsupported_semantic_subject_type:"
                f"{_string_value(operation.get('semantic_subject_type')) or 'unknown'}",
            ),
    ...
    if _optional_text(operation.get("field_path")) not in {
        ...
        return _blocked_resolution(
            operation=operation,
            reason="meta_ocg_class_parent_update_field_not_supported",
            blockers=(
                "unsupported_field_path:"
                f"{_string_value(operation.get('field_path')) or 'unknown'}",
            ),
    ...
    parent_class_config_id = _first_text(
        after_payload.get("parent_class_id"),
        after_payload.get("parent_class_config_id"),
    )
    receiver_semantic_key = (
        f"meta.class:{class_name}" if class_name is not None else semantic_key
    )
    ...
    parent_reference_required = _class_parent_reference_required(after_payload)
    blockers = tuple(
        blocker
        for blocker, blocked in (
            ("missing_class_name", class_name is None),
            (
                "missing_parent_update_payload",
                not _class_parent_update_payload_declared(after_payload),
            ),
            (
                "missing_parent_class_config_id",
                parent_reference_required and parent_class_config_id is None,
            ),
        )
        if blocked
    )
    provider_delta_typed_operation_metadata = (
        _provider_delta_typed_operation_metadata_for_class_parent_update(
            operation=operation,
            class_config_id=receiver_object_id,
        )
    )
    metadata: dict[str, object] = {
        ...
        "requires_baseline_object_identity": True,
        "requires_parent_class_identity": parent_reference_required,
        "execution_ready": (
            not blockers and receiver_object_id is not None
        ),
        "execution_preconditions": ("provider_delta_ontology_operation_executor",),
        "preview_only": True,
        "receiver_semantic_key": receiver_semantic_key,
        "receiver_object_id": receiver_object_id,
        "parent_class_config_id": parent_class_config_id,
        **provider_delta_typed_operation_metadata,
    }
    if class_name is not None:
        metadata["class_name"] = class_name
    parent_class_fqn = _optional_text(after_payload.get("parent_class_fqn"))
    if parent_class_fqn is not None:
        metadata["parent_class_fqn"] = parent_class_fqn
    parent_class_semantic_key = _optional_text(
        after_payload.get("parent_class_semantic_key")
    )
    if parent_class_semantic_key is not None:
        metadata["parent_class_semantic_key"] = parent_class_semantic_key
    return _blocked_resolution(
        operation=operation,
        reason=(
            "meta_ocg_class_parent_update_requires_provider_delta_"
            "ontology_operation_executor"
        ),
        blockers=(
            *blockers,
            "semantic_plan_single_function_call_preview_not_supported",
            "provider_delta_class_parent_update_operation_executor_required",
        ),
        metadata=metadata,
    )


def _class_parent_update_payload_declared(payload: Mapping[str, object]) -> bool:
    return any(
        key in payload
        for key in (
            "parent_class_id",
            "parent_class_config_id",
            "parent_class_fqn",
...
def _class_parent_reference_required(payload: Mapping[str, object]) -> bool:
    return any(
        _optional_text(payload.get(key)) is not None
        for key in ("parent_class_fqn", "parent_class_semantic_key")
    )


def _provider_delta_typed_operation_metadata_for_class_parent_update(
    *,
    operation: Mapping[str, object],
    class_config_id: str | None,
) -> dict[str, object]:
    ...
    metadata: dict[str, object] = {
        ...
        "provider_delta_typed_operation_reason": (
            "class_parent_update_provider_delta_operation_ready"
            if ready
            else "class_parent_update_provider_delta_operation_blocked"
        ),
        "provider_delta_typed_operation_plan": typed_operation_plan,
    }
    if typed_operation is not None:
    ...
    if not ready:
        metadata["provider_delta_typed_operation_blockers"] = (
            "class_parent_update_provider_delta_typed_operation_unavailable",
        )
    metadata.update(
        _class_parent_update_generated_materialization_intent_metadata(
            typed_operation_plan=typed_operation_plan,
            typed_operation=typed_operation,
            ready=ready,
        )
...
def _class_parent_update_generated_materialization_intent_metadata(
    *,
    ...
    blockers: tuple[str, ...] = (
        ()
        if ready
        else ("class_parent_update_generated_materialization_typed_plan_unavailable",)
    )
    status = (
    ...
    reason = (
        "class_parent_update_generated_materialization_intent_ready"
        if ready
        else "class_parent_update_generated_materialization_intent_blocked"
    )
    intent: dict[str, object] = {
        "intent_kind": "meta_class_parent_update_generated_materialization_intent",
        "contract_version": (
            META_CLASS_PARENT_UPDATE_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION
        ),
        "status": status,
    ...
    if ready:
        intent.update(
            generated_materialization_intent_target_metadata(
                policy_key="aware_meta.generated_materialization.class.parent.update",
...
def _class_delete_generated_materialization_intent_metadata(
    *,
    ...
    if ready:
        intent.update(
            generated_materialization_intent_target_metadata(
                policy_key="aware_meta.generated_materialization.class.delete",
                materialization_target="class",
            )
        )
    metadata: dict[str, object] = {
