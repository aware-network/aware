...
def _resolve_relationship_create(
    *,
    ...
    source_semantic_key = f"meta.class:{class_name}" if class_name is not None else None
    target_class_name = (
    ...
    signature = _mapping_value(after_payload.get("relationship_signature"))
    ...
    *,
    ...
    if ready:
        intent.update(
            generated_materialization_intent_target_metadata(
                policy_key=(
                    f"aware_meta.generated_materialization.relationship.{operation_kind}"
                ),
...
def _relationship_structural_typed_operation_with_identity(
    *,
    ...
    if generated_materialization_payload:
        ...
        payload["generated_materialization"] = dict(generated_materialization_payload)
    if payload:
        current["payload"] = payload
    if baseline_object:
        baseline["object"] = baseline_object
