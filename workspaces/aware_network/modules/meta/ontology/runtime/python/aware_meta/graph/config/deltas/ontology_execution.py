...
from aware_meta.materialization.semantic_function_call_resolution import (
    ...
    META_OCG_IDENTITY_CREATE_FUNCTION_REF,
)


HANDLER_KEY = "object_config_graph.function_calls"
IDENTITY_HANDLER_KEY = "object_config_graph_identity.function_calls"
GRAPH_IDENTITY_CREATE_INVOCATION_ORDER = 5
GRAPH_BUILD_INVOCATION_ORDER = 10
OBJECT_CONFIG_GRAPH_IDENTITY_PROJECTION_NAME = "ObjectConfigGraphIdentity"
...
def plan_object_config_graph_operation(
    ...
    del context
    if operation.ontology_subject_kind == "object_config_graph_identity":
        if operation.operation_family == "create":
            return _plan_graph_identity_create_operation(operation=operation)
        return blocked_handler_result(
            operation=operation,
            handler_key=IDENTITY_HANDLER_KEY,
            reason="meta_ocg_graph_identity_delta_requires_create_operation",
            blockers=(
                f"unsupported_operation_family:{operation.operation_family}",
            ),
        )
...
def _plan_graph_identity_create_operation(
    *,
    operation: OntologyTypedOperation,
) -> OntologyOperationHandlerResult:
    identity_id = _graph_identity_id(operation=operation)
    key = _graph_identity_key(operation=operation)
    missing = tuple(
        field_name
    ...
    if missing:
        return blocked_handler_result(
            operation=operation,
            handler_key=IDENTITY_HANDLER_KEY,
            reason="meta_ocg_graph_identity_create_requires_identity",
            blockers=tuple(
                f"missing_graph_identity_create_{field}" for field in missing
            ),
        )
    ...
    return OntologyOperationHandlerResult(
        ...
        reason="meta_ocg_graph_identity_create_function_call_ready",
        invocation_intents=(
            OntologyInvocationIntent(
                intent_key=f"{operation.operation_key}:create",
                operation_key=operation.operation_key,
                semantic_key=operation.semantic_key,
                invocation_order=GRAPH_IDENTITY_CREATE_INVOCATION_ORDER,
                invocation_mode="constructor",
                owner_class_name="ObjectConfigGraphIdentity",
                function_name="create",
                function_ref=META_OCG_IDENTITY_CREATE_FUNCTION_REF,
                target_object_id=None,
                receiver_semantic_key=None,
                result_semantic_key=operation.semantic_key,
                expected_result_object_id=identity_id,
                result_projection_name=OBJECT_CONFIG_GRAPH_IDENTITY_PROJECTION_NAME,
                kwargs=_graph_identity_create_arguments(
                    operation=operation,
                    key=key,
                ),
                reason="meta_ocg_graph_identity_create_function_call_ready",
...
def _graph_identity_create_arguments(
    *,
    operation: OntologyTypedOperation,
    key: str,
) -> dict[str, object]:
    ...
    return {
        "key": key,
        "label": _first_text(current.get("label"), payload.get("label")),
    }
...
def _graph_identity_id(*, operation: OntologyTypedOperation) -> str | None:
    current = operation.current
    payload = mapping_value(current.get("payload"))
    return _first_text(
        current.get("entity_id"),
...
def _graph_identity_key(*, operation: OntologyTypedOperation) -> str | None:
    current = operation.current
    payload = mapping_value(current.get("payload"))
    return _first_text(current.get("key"), payload.get("key"))
...
__all__ = [
    ...
    "GRAPH_IDENTITY_CREATE_INVOCATION_ORDER",
    "HANDLER_KEY",
    "IDENTITY_HANDLER_KEY",
    "OBJECT_CONFIG_GRAPH_IDENTITY_PROJECTION_NAME",
