...
from aware_meta.materialization.semantic_function_call_resolution import (
    ...
    META_CLASS_CONFIG_CREATE_COLLECTION_PRIMITIVE_ATTRIBUTE_FUNCTION_REF,
    ...
    META_FUNCTION_CONFIG_ADD_COLLECTION_PRIMITIVE_ATTRIBUTE_FUNCTION_REF,
...
def _attribute_create_binding(
    *,
    ...
    if descriptor_kind == "collection":
        collection_arguments = _collection_primitive_arguments(descriptor=descriptor)
        if collection_arguments["status"] != "collection_primitive_arguments_ready":
            return _blocked_binding(
                reason=string_value(collection_arguments.get("reason")),
                blockers=_tuple_text(collection_arguments.get("blockers")),
                descriptor_kind=descriptor_kind,
            )
        return _ready_binding(
            receiver_entity_kind=receiver_entity_kind,
            descriptor_kind="collection_primitive",
            arguments={
                **common_arguments,
                **mapping_value(collection_arguments.get("arguments")),
            },
        )
    return _blocked_binding(
...
def _attribute_common_arguments(
    *,
    ...
    arguments: dict[str, object] = {
        ...
        "default_value": _default_value_argument(signature.get("default_value")),
...
def _collection_primitive_arguments(
    *,
    descriptor: Mapping[str, object],
) -> dict[str, object]:
    collection_kind = _first_text(
        descriptor.get("collection_kind"),
        "list",
    )
    if collection_kind == "single":
        return {
            "status": "collection_primitive_arguments_blocked",
            "reason": "meta_ocg_attribute_collection_create_requires_collection_kind",
            "blockers": ("attribute_collection_kind_not_collection",),
            "arguments": {},
        }

    primitive_base_type = _collection_primitive_base_type(descriptor=descriptor)
    if primitive_base_type is None:
        return {
            "status": "collection_primitive_arguments_blocked",
            "reason": (
                "meta_ocg_attribute_collection_create_requires_primitive_element"
            ),
            "blockers": ("attribute_collection_primitive_element_missing",),
            "arguments": {},
        }

    return {
        "status": "collection_primitive_arguments_ready",
        "reason": "meta_ocg_attribute_collection_create_primitive_element_ready",
        "blockers": (),
        "arguments": {
...
def _collection_primitive_base_type(
    *,
    descriptor: Mapping[str, object],
) -> str | None:
    direct_value = _first_text(
        descriptor.get("element_primitive_base_type"),
        descriptor.get("primitive_base_type"),
        mapping_value(descriptor.get("element_descriptor")).get(
            "primitive_base_type"
        ),
        mapping_value(descriptor.get("target")).get("element_primitive_base_type"),
        mapping_value(descriptor.get("target")).get("primitive_base_type"),
    )
    if direct_value is not None:
        return direct_value

    primitive_child_values: list[str] = []
    for link in tuple_mappings(descriptor.get("child_links")):
        role = _first_text(link.get("role"), "element")
        if role != "element":
            continue
        child_descriptor = mapping_value(
            link.get("child_descriptor") or link.get("child")
        )
        if _descriptor_kind(descriptor=child_descriptor) != "primitive":
            continue
        child_base_type = _first_text(
            child_descriptor.get("primitive_base_type"),
            mapping_value(child_descriptor.get("target")).get(
                "primitive_base_type"
            ),
        )
        if child_base_type is not None:
            primitive_child_values.append(child_base_type)
    if len(primitive_child_values) == 1:
        return primitive_child_values[0]
    return None


def _default_value_argument(value: object) -> str | None:
    if value is None:
        return None
    ...
    *,
    ...
    if receiver_entity_kind == "class_config":
        return {
            ...
            "collection_primitive": "create_collection_primitive_attribute_config",
        }.get(descriptor_kind)
    if receiver_entity_kind == "function_config":
        return {
            ...
            "collection_primitive": "add_collection_primitive_attribute_config",
        }.get(descriptor_kind)
    return None
