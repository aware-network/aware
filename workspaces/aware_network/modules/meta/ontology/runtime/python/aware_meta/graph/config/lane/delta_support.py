...
class _MsgpackPackFn(Protocol):
    def __call__(
        self, o: object, **kwargs: object
    ) -> bytes | bytearray | memoryview: ...


class _MsgpackUnpackFn(Protocol):
    def __call__(
        self, packed: bytes | bytearray | memoryview, **kwargs: object
    ) -> object: ...
...
def load_pre_oig_from_snapshot_fast_path(
    *,
    ...
    pickle_enabled = bool_env_default_true(
        "AWARE_OCG_DELTA_SNAPSHOT_PICKLE_SIDECAR_ENABLED"
    )
    maybe_metric(
        timings,
    ...
    if index_path.exists():
        try:
            with maybe_timed(
                timings, "ocg_delta.materialize_oig_pre_snapshot_read_index"
            ):
    ...
    def _maybe_write_pickle_sidecar(*, oig: ObjectInstanceGraph) -> bool:
        ...
        try:
            ...
            with maybe_timed(
                timings, "ocg_delta.materialize_oig_pre_snapshot_write_pickle"
            ):
                _ = pickle_path.write_bytes(
                    pickle.dumps(oig, protocol=pickle.HIGHEST_PROTOCOL)
                )
            return True
        except Exception:
            return False
    ...
    if msgpack_path.exists() and unpackb is not None:
        try:
            ...
            maybe_metric(
                timings, "ocg_delta_materialize_pre_snapshot_loader", "msgpack"
            )
            maybe_metric(
                timings,
    ...
    try:
        ...
        with maybe_timed(
            timings, "ocg_delta.materialize_oig_pre_snapshot_validate_json"
        ):
            with disable_autobind(), _disable_wrapping_context():
...
def load_post_oig_diff_index_sidecar(
    *,
    ...
    if not enabled:
        maybe_metric(
            timings, "ocg_delta_oig_diff_index_post_sidecar_loader", "disabled"
        )
        return None
...
def _ocg_delta_progress_enabled() -> bool:
    return _bool_env_default_false(
        "AWARE_OCG_DELTA_LOG_PROGRESS"
    ) or _bool_env_default_false("AWARE_ENV_COMPOSED_RAIL_LOG_SUBSTEPS")
...
def expand_delta_scoped_candidate_ids(
    *,
    ...
    def _scan_relationships(
        graph: ObjectInstanceGraph, *, current: set[UUID]
    ) -> set[UUID]:
        to_add: set[UUID] = set()
    ...
    for depth in range(max_depth):
        ...
        if len(expanded) + len(to_add) > max_ids:
            ...
            added_post += sum(
                1 for candidate_id in chosen if candidate_id in to_add_post
            )
            depth_used = depth + 1
            break
    ...
    *,
    ...
    try:
        ...
        return OigFingerprintDiffScope(
            candidate_ids=set(), changed_relationship_tuples=set()
        )
    ...
    maybe_metric(
        timings, "ocg_delta_oig_diff_index_instance_count_pre", len(before_by_id)
    )
    maybe_metric(
        timings, "ocg_delta_oig_diff_index_instance_count_post", len(after_by_id)
    )
    ...
    for candidate_id in sorted(before_ids & after_ids, key=str):
        if before_fingerprints.get(candidate_id) != after_fingerprints.get(
            candidate_id
        ):
    ...
    maybe_metric(
        timings, "ocg_delta_oig_scoped_instance_ids_changed", changed_instances
    )
...
def _resolve_oig_scoped_relationship_coverage_limit() -> float:
    raw = (
        os.getenv("AWARE_OCG_DELTA_OIG_SCOPED_RELATIONSHIP_COVERAGE_MAX") or ""
    ).strip()
    try:
...
def _summarize_attribute_value(
    value: AttributeValue | None, *, max_len: int = 160
) -> str:
    if value is None:
        return "null"
    ...
    for change in changes:
        ...
        for ci_change in change.class_instance_changes or []:
            ...
            item: dict[str, object] = {
                ...
                "class_config_id": (
                    str(class_config_id) if class_config_id is not None else None
                ),
            ...
            for attr_change in (ci_change.attribute_changes or [])[:max_attrs]:
                ...
                attribute_config_id = (
                    ...
                    if post_attr is not None
                        ...
                        pre_attr.attribute_config_id if pre_attr is not None else None
                    )
                )
        ...
        for rel_change in change.class_instance_relationship_changes or []:
            ...
            source_ci = _index_class_instance_by_id(
                pre_index, source_id
            ) or _index_class_instance_by_id(
                post_index,
                source_id,
            )
            target_ci = _index_class_instance_by_id(
                pre_index, target_id
            ) or _index_class_instance_by_id(
                post_index,
                target_id,
            ...
            source_class_config_id = (
                source_ci.class_config_id if source_ci is not None else None
            )
            target_class_config_id = (
                target_ci.class_config_id if target_ci is not None else None
            )
            out.append(
                {
                    "kind": "relationship",
