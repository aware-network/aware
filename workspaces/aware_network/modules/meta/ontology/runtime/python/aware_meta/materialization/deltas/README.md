...
## Generated Materialization Evidence

`renderer_completeness.py` owns Meta/Code evidence contracts for generated
materialization output:

- path/content and final-state comparison operates on public Code DTOs
  (`CodeGeneratedMaterializationDeltaResult` and `CodePackageDelta`) without
  importing language renderers;
- the generated-materialization evidence manifest summarizes package-delta
  artifact paths, path roles, path kinds, delta forms, artifact family/role,
  target language, renderer profile, and materialization source from DTO
  evidence only;
- provider-delta generated-materialization stages expose the manifest and
  summary counts directly so Workspace and consumers can inspect artifact
  evidence without parsing raw renderer result entries;
- language plugins own renderer selection, source-artifact bytes, and
  migration-artifact bytes. Meta records the evidence envelope and readiness
  counts, but does not interpret renderer-specific artifact semantics.
