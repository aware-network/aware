...
def resolve_attribute_config_semantic_operation(
    *,
    ...
    if operation_type == META_OBJECT_CONFIG_GRAPH_ATTRIBUTE_MEMBERSHIP_UPDATE_OPERATION:
...
def _resolve_attribute_create(
    *,
    ...
    owner_identity = (
        current_object_identities.get(owner_semantic_key)
        if owner_semantic_key is not None
        else None
    ) or {}
    owner_source_object_id = _attribute_owner_source_object_id(
        operation=operation,
        payload=after_payload,
        owner_identity=owner_identity,
        owner_object_id=owner_object_id,
    )
    execution_receiver_object_id = _attribute_owner_execution_receiver_object_id(
        operation=operation,
        payload=after_payload,
        owner_identity=owner_identity,
        owner_object_id=owner_object_id,
        owner_source_object_id=owner_source_object_id,
        class_fqn=class_fqn,
    )
    attribute_config_id = _first_text(
        after_payload.get("attribute_config_id"),
    ...
    blockers = tuple(
        ...
        for blocker, value in (
            ...
            ("missing_receiver_object_id", execution_receiver_object_id),
            ("missing_attribute_config_id", attribute_config_id),
        )
        if value is None
    )
    provider_delta_typed_operation_metadata = (
        _provider_delta_typed_operation_metadata_for_attribute_structural(
            ...
            owner_object_id=execution_receiver_object_id,
            owner_source_object_id=owner_source_object_id,
            executable_object_id=execution_receiver_object_id,
            owner_key=class_fqn,
            class_name=class_name,
            attribute_name=attribute_name,
            attribute_config_id=attribute_config_id,
            owner_generated_materialization=_mapping_value(
    ...
    if owner_source_object_id is not None:
        metadata["semantic_source_object_id"] = owner_source_object_id
        if owner_source_object_id != execution_receiver_object_id:
            metadata["object_config_graph_node_id"] = owner_source_object_id
    if (
        execution_receiver_object_id is not None
        and execution_receiver_object_id != owner_object_id
    ):
        metadata["semantic_apply_receiver_object_id"] = execution_receiver_object_id
...
def _attribute_identity_for_membership_update(
    *,
    ...
    direct_identity = current_object_identities.get(semantic_key)
...
def _attribute_membership_signature_for_payload(
    *,
    ...
    signature = dict(_mapping_value(identity.get("attribute_membership_signature")))
...
def _attribute_owner_source_object_id(
    *,
    operation: Mapping[str, object],
    payload: Mapping[str, object],
    ...
    return _first_text(
        ...
        owner_identity.get("semantic_source_object_id"),
        owner_identity.get("object_config_graph_node_id"),
        owner_identity.get("source_object_id"),
        owner_identity.get("object_id")
        if _semantic_source_meaning_operation(operation=operation)
        else None,
        owner_object_id,
    )


def _attribute_owner_execution_receiver_object_id(
    *,
    operation: Mapping[str, object],
    payload: Mapping[str, object],
    owner_identity: Mapping[str, object],
    owner_object_id: str | None,
    owner_source_object_id: str | None,
    class_fqn: str | None,
) -> str | None:
    explicit_receiver = _first_text(
        payload.get("semantic_apply_receiver_object_id"),
        payload.get("executable_object_id"),
        owner_identity.get("semantic_apply_receiver_object_id"),
        owner_identity.get("executable_object_id"),
    )
    ...
    class_config_id = _first_text(
        ...
        owner_identity.get("class_config_id"),
    )
    if (
        class_config_id is not None
        and class_config_id != owner_source_object_id
    ):
        return class_config_id

    if _semantic_source_meaning_operation(operation=operation):
        derived_class_config_id = _stable_class_config_id_for_create(
            object_config_graph_node_id=owner_source_object_id,
            class_fqn=class_fqn,
        )
        if derived_class_config_id is not None:
            return derived_class_config_id

    return class_config_id or owner_object_id


def _semantic_source_meaning_operation(*, operation: Mapping[str, object]) -> bool:
    metadata = _mapping_value(operation.get("metadata"))
    return _string_value(operation.get("source")) == (
        "aware_code.semantic_source_meaning"
    ) or _string_value(metadata.get("contract_version")) == (
...
def _provider_delta_typed_operation_metadata_for_attribute_structural(
    *,
    ...
    attribute_name: str | None,
    attribute_config_id: str | None,
    owner_generated_materialization: Mapping[str, object],
) -> dict[str, object]:
    from aware_meta.materialization.deltas.source_projection import (  # noqa: WPS433,E501
        typed_operation_plan_from_semantic_source_meaning,
    )
    ...
    generated_materialization = _generated_materialization_payload(
        operation=operation,
        fallback_generated_materialization=owner_generated_materialization,
    )
    if (
        ...
        typed_operation_plan = _attribute_structural_typed_operation_plan_with_identity(
            ...
            owner_object_id=owner_object_id,
            owner_source_object_id=owner_source_object_id,
            executable_object_id=executable_object_id,
            owner_key=owner_key,
            class_name=class_name,
            attribute_name=attribute_name,
            attribute_config_id=attribute_config_id,
...
def _attribute_structural_typed_operation_plan_with_identity(
    *,
    ...
    receiver_object_id = executable_object_id or owner_object_id
    source_object_id = owner_source_object_id or owner_object_id
    ...
    enriched_plan["typed_operations"] = tuple(
        (
            _attribute_structural_typed_operation_with_identity(
                ...
                owner_semantic_key=owner_semantic_key,
                owner_object_id=receiver_object_id,
                owner_source_object_id=source_object_id,
                executable_object_id=receiver_object_id,
                owner_key=owner_key,
                class_name=class_name,
                attribute_name=attribute_name,
                attribute_config_id=attribute_config_id,
        ...
        *semantic_object_anchors,
        {
            ...
            "baseline": {
                ...
                "entity_id": source_object_id,
            },
            "current": {
                "entity_id": receiver_object_id,
                ...
                "object_config_graph_node_id": source_object_id,
                "semantic_source_object_id": source_object_id,
                "semantic_apply_receiver_object_id": receiver_object_id,
                "payload": {
                    "entity_id": receiver_object_id,
                    "object_id": receiver_object_id,
                    "object_kind": "class",
...
def _attribute_structural_typed_operation_with_identity(
    *,
    ...
    generated_materialization_payload = dict(generated_materialization)
    for target in (current, current_payload, baseline_object, baseline_payload):
        ...
        target["owner_object_id"] = owner_object_id
        target["class_config_id"] = owner_object_id
        target["owner_key"] = owner_key
        target["class_fqn"] = owner_key
        if class_name is not None:
            target["class_name"] = class_name
        target["attribute_name"] = attribute_name
        target["entity_id"] = attribute_config_id
        target["attribute_config_id"] = attribute_config_id
        if owner_source_object_id is not None:
            target["semantic_source_object_id"] = owner_source_object_id
            if owner_source_object_id != owner_object_id:
                target["object_config_graph_node_id"] = owner_source_object_id
        if (
            executable_object_id is not None
            and executable_object_id != owner_source_object_id
        ):
            target["semantic_apply_receiver_object_id"] = executable_object_id
            target["executable_object_id"] = executable_object_id
    if generated_materialization_payload:
        current["generated_materialization"] = dict(generated_materialization_payload)
        current_payload["generated_materialization"] = dict(
            generated_materialization_payload
        )
    if current_payload:
...
def _generated_materialization_payload(
    *,
    operation: Mapping[str, object],
    fallback_generated_materialization: Mapping[str, object] | None = None,
) -> dict[str, object]:
    generated_materialization: dict[str, object] = {}
    for source in (
        _mapping_value(operation.get("after_payload")),
        _mapping_value(operation.get("before_payload")),
        _mapping_value(operation.get("payload")),
        operation,
    ):
        generated = _mapping_value(source.get("generated_materialization"))
        if generated:
            generated_materialization = _merged_generated_materialization(
                current=generated_materialization,
                incoming=generated,
            )
    if fallback_generated_materialization:
        generated_materialization = _merged_generated_materialization(
            current=generated_materialization,
            incoming=fallback_generated_materialization,
        )
    ...
    *,
    ...
    for key, value in incoming.items():
        if key != "targets":
            ...
            continue
        merged["targets"] = _merged_generated_materialization_targets(
            current=_mapping_value(merged.get("targets")),
            incoming=_mapping_value(value),
        )
    return merged


def _merged_generated_materialization_targets(
    *,
    current: Mapping[str, object],
    incoming: Mapping[str, object],
) -> dict[str, object]:
    merged: dict[str, object] = {
        key: dict(_mapping_value(value)) for key, value in current.items()
    }
    for descriptor_key, incoming_value in incoming.items():
        incoming_target = _mapping_value(incoming_value)
        current_target: dict[str, object] = dict(
            _mapping_value(merged.get(str(descriptor_key)))
        )
        for key, value in incoming_target.items():
            current_target.setdefault(key, value)
        merged[str(descriptor_key)] = current_target
    return merged
