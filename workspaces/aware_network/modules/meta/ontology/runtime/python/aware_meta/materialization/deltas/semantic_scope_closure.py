...
@dataclass(frozen=True, slots=True)
class MetaOcgSemanticScopeClosureRefEvidence:
    ref_key: str
    origin: str
    ...
    def evidence_payload(self) -> dict[str, object]:
        return {
            ...
            "class_symbol_count": self.class_symbol_count,
            "enum_symbol_count": self.enum_symbol_count,
...
class MetaOcgSemanticScopeClosureEvidence:
    ...
    closure_refs: tuple[MetaOcgSemanticScopeClosureRefEvidence, ...]
    ...
    def evidence_payload(self) -> dict[str, object]:
        return {
            ...
            "closure_refs": tuple(
                closure_ref.evidence_payload() for closure_ref in self.closure_refs
            ),
...
def build_meta_ocg_semantic_scope_closure(
    *,
    ...
    external_graphs_tuple = tuple(external_graphs)
    blockers: list[str] = []
    ...
    for code_id, namespace in namespaces.items():
        if normalized_prefix and not _namespace_matches_prefix(
            ...
            blockers.append(
                "namespace_package_mismatch:" f"{code_id}:{namespace.prefix()}"
            )
    ...
    for graph in external_graphs_tuple:
    ...
    closure_refs = _build_closure_refs(
        package_fqn_prefix=normalized_prefix,
        symbols=symbols,
        external_graphs=external_graphs_tuple,
    )
    closure_hash = _stable_hash(
        {
            ...
            "imports": tuple(
                sorted(
                    ...
                    for code_id, aliases in imports.items()
                )
            ),
            "closure_refs": tuple(
                closure_ref.evidence_payload() for closure_ref in closure_refs
            ),
        }
    )
...
def meta_ocg_class_fqn_scope_closure_gate(
    *,
    ...
    if payload and closure_status != SCOPE_STATUS_READY:
        blockers.append(
            "semantic_scope_closure_not_ready:" f"{closure_status or 'unknown'}"
        )
    ...
    return {
        ...
        "status": (SCOPE_GATE_STATUS_READY if ready else SCOPE_GATE_STATUS_BLOCKED),
        "ready": ready,
        "consumed": True,
        ...
        "semantic_scope_closure_hash": _optional_text(payload.get("closure_hash")),
        "semantic_scope_closure_ref_keys": _closure_ref_keys(payload),
...
def _build_closure_refs(
    *,
    package_fqn_prefix: str,
    symbols: tuple[MetaOcgSemanticScopeSymbolEvidence, ...],
    external_graphs: tuple[ObjectConfigGraph, ...],
) -> tuple[MetaOcgSemanticScopeClosureRefEvidence, ...]:
    local_symbols = tuple(
        symbol for symbol in symbols if symbol.origin == SYMBOL_ORIGIN_LOCAL
    )
    refs: list[MetaOcgSemanticScopeClosureRefEvidence] = [
        _closure_ref(
            origin=SYMBOL_ORIGIN_LOCAL,
            package_fqn_prefix=package_fqn_prefix,
            ref_key=f"meta_ocg_scope:local:{package_fqn_prefix}",
    ...
    for graph in sorted(
        external_graphs,
        key=lambda graph: (
            str(graph.fqn_prefix),
            str(graph.id),
            str(graph.hash),
        ),
    ):
        graph_symbols = _external_graph_symbols_for_ref(
            graph=graph,
            symbols=symbols,
        )
        ...
        refs.append(
            _closure_ref(
                origin=SYMBOL_ORIGIN_EXTERNAL,
                package_fqn_prefix=str(graph.fqn_prefix),
                ref_key=f"meta_ocg_scope:external:{graph.id}",
...
def _external_graph_symbols_for_ref(
    *,
    graph: ObjectConfigGraph,
    symbols: tuple[MetaOcgSemanticScopeSymbolEvidence, ...],
) -> tuple[MetaOcgSemanticScopeSymbolEvidence, ...]:
    graph_fqns: set[str] = set()
    for node in graph.object_config_graph_nodes:
        if (
            node.type == ObjectConfigGraphNodeType.class_
            and node.class_config is not None
        ):
            graph_fqns.add(node.class_config.class_fqn)
        elif (
            node.type == ObjectConfigGraphNodeType.enum and node.enum_config is not None
        ):
            graph_fqns.add(node.enum_config.enum_fqn)
    return tuple(
        symbol
        for symbol in symbols
        if symbol.origin == SYMBOL_ORIGIN_EXTERNAL and symbol.fqn in graph_fqns
    )
...
def _closure_ref(
    *,
    ...
    graph_id: str | None = None,
    graph_hash: str | None = None,
) -> MetaOcgSemanticScopeClosureRefEvidence:
    class_symbol_count = sum(
        1 for symbol in symbols if symbol.symbol_kind == SYMBOL_KIND_CLASS
    )
    enum_symbol_count = sum(
        1 for symbol in symbols if symbol.symbol_kind == SYMBOL_KIND_ENUM
    )
    closure_hash = _stable_hash(
        {
            "origin": origin,
            ...
            "symbols": tuple(symbol.evidence_payload() for symbol in symbols),
        }
    )
    return MetaOcgSemanticScopeClosureRefEvidence(
        ref_key=ref_key,
        origin=origin,
        package_fqn_prefix=package_fqn_prefix,
        closure_hash=closure_hash,
        symbol_count=len(symbols),
        class_symbol_count=class_symbol_count,
        enum_symbol_count=enum_symbol_count,
        graph_id=graph_id,
        graph_hash=graph_hash,
    )
...
def _closure_ref_keys(semantic_scope_closure: Mapping[str, object]) -> tuple[str, ...]:
    ref_keys: list[str] = []
    for closure_ref in _sequence(semantic_scope_closure.get("closure_refs")):
        ref_payload = _mapping_value(closure_ref)
        ref_key = _optional_text(ref_payload.get("ref_key"))
        if ref_key is not None:
            ref_keys.append(ref_key)
    return tuple(ref_keys)


def meta_ocg_scope_closure_committed_ref_fields(
    *,
    ...
    for gate in gates:
        if closure_hash is None:
            closure_hash = _optional_text(gate.get("semantic_scope_closure_hash"))
        ref_keys.extend(_tuple_text(gate.get("semantic_scope_closure_ref_keys")))
    return {
        "semantic_scope_closure_hash": closure_hash,
        "semantic_scope_closure_ref_keys": tuple(dict.fromkeys(ref_keys)),
    }
