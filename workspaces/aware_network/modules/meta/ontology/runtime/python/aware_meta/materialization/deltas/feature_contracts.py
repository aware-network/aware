...
META_PROVIDER_DELTA_GENERATED_MATERIALIZATION_FEATURE_RESULT_CONTRACT_VERSION = "v0"
...
class MetaProviderDeltaSourceProjectionContext:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            ...
            optional_text(self.target_language_plugin_id),
        )
    ...
    def evidence_payload(self) -> dict[str, object]:
        return {
            ...
            "grammar_anchor_bindings": tuple(
                item.model_dump(mode="json") for item in self.grammar_anchor_bindings
            ),
            "grammar_anchor_sources": tuple(
                item.model_dump(mode="json") for item in self.grammar_anchor_sources
            ),
            "grammar_anchor_replacements": tuple(
...
class MetaProviderDeltaGeneratedMaterializationExpectation:
    ...
    def from_feature_result(
        ...
        if feature_result.projected:
            return cls(
                ...
                reason=("meta_generated_materialization_required_output_fulfilled"),
    ...
    def fulfilled(self) -> bool:
        return self.fulfillment == GENERATED_MATERIALIZATION_FULFILLMENT_FULFILLED

    @property
    def missing(self) -> bool:
        return (
            self.expectation == GENERATED_MATERIALIZATION_EXPECTATION_REQUIRED
            and self.fulfillment == GENERATED_MATERIALIZATION_FULFILLMENT_MISSING
        )

    @property
    def unsupported(self) -> bool:
        return self.expectation == GENERATED_MATERIALIZATION_EXPECTATION_UNSUPPORTED

    @property
    def deferred(self) -> bool:
        return self.expectation == GENERATED_MATERIALIZATION_EXPECTATION_DEFERRED

    @property
    def not_required(self) -> bool:
        return self.expectation == GENERATED_MATERIALIZATION_EXPECTATION_NOT_REQUIRED
    ...
    *,
    ...
    return tuple(
        sorted(
            ...
            | {event_ref for entry in result.entries for event_ref in entry.event_refs}
            | {
                event_ref
