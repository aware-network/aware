...
class FunctionImplBodyGeneratedMaterializationIntent:
    ...
    def evidence_payload(self) -> dict[str, object]:
        payload: dict[str, object] = {
            "intent_kind": ("meta_function_impl_body_generated_materialization_intent"),
            "contract_version": (
                FUNCTION_IMPL_BODY_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION
            ),
            "status": self.status,
...
def normalize_function_impl_body_source_meaning_provider_delta_operation(
    *,
    ...
    semantic_reference_object_ids = (
        current_semantic_source_object_ids or current_semantic_object_ids or {}
    )
...
def build_function_impl_body_generated_materialization_intent(
    *,
    ...
    source_refs = _tuple_text(operation.get("source_refs"))
    baseline_aware_body_text = _optional_text(before_payload.get("body_text"))
    baseline_function_description = _first_text(
    ...
    target_profile = generated_materialization_target_profile_from_payload(
    ...
    blockers = tuple(
        ...
        for blocker, value in (
            ...
            ("missing_function_impl_body_source", aware_body_text),
            ("missing_generated_materialization_target_profile", target_profile),
        )
        if value in {None, (), ""}
    )
    if blockers:
    ...
    invocation_semantic_key = f"{function_semantic_key}/invocation:function_impl_body"
    invocation_operation = function_invocation_create_typed_operation(
        ...
        target_function_config_id=(f"{semantic_key}:generated_materialization_body"),
    ...
    current = {
        **dict(invocation_operation.current),
        ...
        "generated_materialization": _generated_materialization_targets(
            baseline_aware_body_text=baseline_aware_body_text,
            baseline_function_description=baseline_function_description,
            aware_body_text=aware_body_text,
    ...
    typed_operation_plan = {
        ...
        "operation_contract_version": typed_operation.get("contract_version"),
        "status": "typed_operation_plan_ready",
        "reason": ("meta_function_impl_body_generated_materialization_intent_ready"),
        "source": "aware_meta.semantic_operation_resolution",
        "provider_key": "aware_meta",
        "typed_operation_count": 1,
        "typed_operations": (typed_operation,),
        ...
        instructions.append(
            {
                ...
                "set": {
                    ...
                    "value_source": {
                        ...
                        "source_function_config_attribute_config_id": (source_input_id),
...
def _generated_materialization_targets(
    *,
    baseline_aware_body_text: str | None,
    baseline_function_description: str | None,
    ...
    if baseline_aware_body_text is not None:
        target["baseline_aware_body_text"] = baseline_aware_body_text
    if baseline_function_description is not None:
        target["baseline_function_description"] = baseline_function_description
    if aware_body_text is not None:
        target["aware_body_text"] = aware_body_text
    return {"targets": {target_profile.target_key: target}}
    ...
    for raw_instruction in _tuple_values(signature.get("instructions")):
        ...
        if set_payload:
            ...
            changed |= _attach_nested_identity(
                ...
                receiver_object_id=_optional_text(
                    semantic_object_ids.get(
                        "meta.function_impl_instruction_set_receiver:" f"{key_prefix}"
                    )
                ),
            )
            value_source = _mapping_value(set_payload.get("value_source"))
...
def _function_config_identity_from_semantic_object_ids(
    *,
    ...
    for semantic_key, raw_object_id in sorted(semantic_object_ids.items()):
        if semantic_key.endswith(function_suffix) and class_marker in semantic_key:
            object_id = _optional_text(raw_object_id)
            if object_id is not None:
                return semantic_key, object_id
    ocg_node_suffix = f".{class_name}.{function_name}"
    for semantic_key, raw_object_id in sorted(semantic_object_ids.items()):
        if "/node:" not in semantic_key or not semantic_key.endswith(ocg_node_suffix):
            continue
        object_id = _optional_text(raw_object_id)
        if object_id is not None:
            return semantic_key, object_id
