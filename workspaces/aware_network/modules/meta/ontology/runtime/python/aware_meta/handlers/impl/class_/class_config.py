...
async def update_parent_class(
    class_config: ClassConfig,
    parent_class_config_id: UUID | None = None,
) -> None:
    """
    Update this ClassConfig inheritance parent.

    Contract:
    - Mutates only this ClassConfig parent reference.
    - `class_fqn` and `name` remain immutable identity.
    - Semantic planners must provide closure evidence for any new parent.
    """

    # --- AWARE: LOGIC START update_parent_class
    class_config_id = class_config.id
    if class_config_id is None:
        raise RuntimeError("ClassConfig.update_parent_class requires class_config.id")

    normalized_parent_id = (
        UUID(str(parent_class_config_id))
        if parent_class_config_id is not None
        else None
    )
    if normalized_parent_id == class_config_id:
        raise RuntimeError("ClassConfig.update_parent_class rejects self inheritance")

    class_config.parent_class_id = normalized_parent_id
    class_config.parent_class = (
        current_handler_session().imap_get(ClassConfig, normalized_parent_id)
        if normalized_parent_id is not None
        else None
    )
    return None
    # --- AWARE: LOGIC END update_parent_class
...
async def create_primitive_attribute_config(
    ...
    if owner_class_config_id is None:
        raise RuntimeError(
            "ClassConfig.create_primitive_attribute_config requires class_config.id"
        )
    class_owner_key = get_class_config_fqn(class_config)
    if not class_owner_key:
        raise RuntimeError(
            "ClassConfig.create_primitive_attribute_config requires class_config.class_fqn"
        )
    if position < 0:
        raise RuntimeError(
            "ClassConfig.create_primitive_attribute_config requires position >= 0"
        )
...
async def create_enum_attribute_config(
    ...
    if owner_class_config_id is None:
        raise RuntimeError(
            "ClassConfig.create_enum_attribute_config requires class_config.id"
        )
    class_owner_key = get_class_config_fqn(class_config)
    if not class_owner_key:
        raise RuntimeError(
            "ClassConfig.create_enum_attribute_config requires class_config.class_fqn"
        )
    if position < 0:
        raise RuntimeError(
            "ClassConfig.create_enum_attribute_config requires position >= 0"
        )
    ...
    if attribute_config is None:
        raise RuntimeError(
            "ClassConfig.create_enum_attribute_config expected edge.attribute_config after creation"
        )
    for existing_edge in class_config.class_config_attribute_configs:
        if existing_edge.id == edge.id:
            if existing_edge.attribute_config is None:
...
async def create_class_attribute_config(
    ...
    if owner_class_config_id is None:
        raise RuntimeError(
            "ClassConfig.create_class_attribute_config requires class_config.id"
        )
    class_owner_key = get_class_config_fqn(class_config)
    if not class_owner_key:
        raise RuntimeError(
            "ClassConfig.create_class_attribute_config requires class_config.class_fqn"
        )
    if position < 0:
        raise RuntimeError(
            "ClassConfig.create_class_attribute_config requires position >= 0"
        )
    ...
    if attribute_config is None:
        raise RuntimeError(
            "ClassConfig.create_class_attribute_config expected edge.attribute_config after creation"
        )
    for existing_edge in class_config.class_config_attribute_configs:
        if existing_edge.id == edge.id:
            if existing_edge.attribute_config is None:
...
async def remove_attribute_config(
    ...
    if not normalized_name:
        raise RuntimeError(
            "ClassConfig.remove_attribute_config requires non-empty name"
        )

    normalized_attribute_config_id = (
        UUID(str(attribute_config_id)) if attribute_config_id is not None else None
    )
    ...
    for edge in class_config.class_config_attribute_configs:
        ...
        edge_matches_id = (
            normalized_attribute_config_id is not None
            and edge_attribute_config_id == normalized_attribute_config_id
        )
        edge_matches_name = False
        if normalized_attribute_config_id is None:
            edge_matches_name = (
                edge.attribute_config.name or ""
...
async def create_function_config(
    ...
    if owner_class_config_id is None:
        raise RuntimeError(
            "ClassConfig.create_function_config requires class_config.id"
        )
    class_owner_key = get_class_config_fqn(class_config)
    if not class_owner_key:
        raise RuntimeError(
            "ClassConfig.create_function_config requires class_config.class_fqn"
        )
    if position < 0:
    ...
    if function_config is None:
        raise RuntimeError(
            "ClassConfig.create_function_config expected edge.function_config after creation"
        )
...
async def create_relationship(
    ...
    if not normalized_relationship_key:
        raise RuntimeError(
            "ClassConfig.create_relationship requires non-empty relationship_key"
        )
...
async def remove_relationship_config(
    class_config: ClassConfig,
    relationship_key: str,
    relationship_config_id: UUID | None = None,
) -> None:
    """
    ...
    if not normalized_relationship_key:
        raise RuntimeError(
            "ClassConfig.remove_relationship_config requires non-empty relationship_key"
        )
    normalized_relationship_config_id = (
        UUID(str(relationship_config_id))
    ...
    for relationship in class_config.class_config_relationships:
        relationship_matches_id = (
            normalized_relationship_config_id is not None
            and relationship.id == normalized_relationship_config_id
        )
        relationship_matches_key = False
        if normalized_relationship_config_id is None:
            relationship_matches_key = (
                relationship.relationship_key or ""
            ).strip() == normalized_relationship_key
...
async def create_via_object_config_graph_node(
    ...
    if not normalized_name:
        raise RuntimeError(
            "ClassConfig.create_via_object_config_graph_node requires non-empty name"
        )
    if not normalized_class_fqn:
        raise RuntimeError(
            "ClassConfig.create_via_object_config_graph_node requires non-empty class_fqn"
        )
    ...
    if existing is not None:
        if (
            (get_class_config_fqn(existing) or normalized_class_fqn)
            != normalized_class_fqn
