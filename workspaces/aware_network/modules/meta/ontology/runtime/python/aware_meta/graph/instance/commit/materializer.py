...
_OIG_MATERIALIZER_TRACE_CATEGORY = "meta.oig_materializer"
...
class OIGMaterializer:
        ...
        *,
        ...
        try:
            ...
            with commit_perf_span(
                phase="oig_materializer.get.index_build",
                category=_OIG_MATERIALIZER_TRACE_CATEGORY,
            ...
            _maybe_metric(
                "oig_materializer_index_build_ms", _elapsed_ms(started=index_started)
            )
            ...
            with commit_perf_span(
                phase="oig_materializer.get.head_read",
                category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                metadata=trace_metadata,
            ):
                head = await self.commits.head(
                    branch_id=branch_id, projection_hash=opg.projection_hash
                )
            _maybe_metric(
                "oig_materializer_head_read_ms", _elapsed_ms(started=head_started)
            )
            head_cid: UUID | None = (
                UUID(str(head["commit_id"])) if head and head.get("commit_id") else None
            )
            ...
            _maybe_metric(
                "oig_materializer_head_commit_id", str(head_cid) if head_cid else None
            )

            target_commit_id = commit_id
            if target_commit_id is not None:
                with commit_perf_span(
                    phase="oig_materializer.get.resolve_target_commit",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata=trace_metadata,
                ):
                    resolved_target_commit_id = await self.commits.domain_commit_id_for_object_instance_graph_commit_id(
                        branch_id=branch_id,
                        projection_hash=opg.projection_hash,
            ...
            with commit_perf_span(
                phase="oig_materializer.get.snapshot_lookup",
                category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                metadata={
                    **trace_metadata,
                    "target_commit_id": (
                        str(target_commit_id) if target_commit_id is not None else None
                    ),
                },
            ):
            ...
            preloaded_replay_commits: (
                tuple[ObjectInstanceGraphCommitBodyRecord, ...] | None
            ) = None
            if base:
                base_cid, graph, indexes = base
                hydration_started = time.perf_counter()
                with commit_perf_span(
                    phase="oig_materializer.get.snapshot_descriptor_hydration",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "base_commit_id": str(base_cid),
                    },
                ):
                    hydrate_object_instance_graph_value_type_descriptors(
                        graph=graph,
                ...
                _maybe_metric(
                    "oig_materializer_snapshot_descriptor_hydration_ms",
                    _elapsed_ms(started=hydration_started),
                )
            else:
                if head_cid is None:
                    raise RuntimeError(
                ...
                with commit_perf_span(
                    phase="oig_materializer.get.bootstrap_lineage_load",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "target_commit_id": str(target),
                    },
                ):
                ...
                bootstrap_commit = (
                    preloaded_replay_commits[0] if preloaded_replay_commits else None
                )
                ...
                if oig_id is None:
                    oig_id = bootstrap_commit.envelope.object_instance_graph_id
                if oig_id != bootstrap_commit.envelope.object_instance_graph_id:
                    raise RuntimeError(
                        "ObjectInstanceGraph materializer bootstrap commit targets unexpected OIG id: "
                        + f"expected={oig_id} bootstrap={bootstrap_commit.envelope.object_instance_graph_id}"
                    )
                graph = build_rooted_object_instance_graph_base(
                    key=bootstrap_commit.envelope.object_instance_graph_key,
                    name=bootstrap_commit.envelope.object_instance_graph_name,
                    description=bootstrap_commit.envelope.object_instance_graph_description
                    or "",
                    object_config_graph=ocg,
                    object_projection_graph=opg,
                    root_source_object_id=bootstrap_commit.envelope.root_source_object_id,
                    root_class_config_id=bootstrap_commit.envelope.root_class_config_id,
                    oig_id=oig_id,
                )
                bootstrap_hash_started = time.perf_counter()
                with commit_perf_span(
                    phase="oig_materializer.get.bootstrap_hash",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "bootstrap_commit_id": str(bootstrap_commit.envelope.commit_id),
                    },
                ):
                    bootstrap_hash_state = compute_oig_lane_hash_state(
                        graph=graph,
                        schema_attribute_configs_by_id=attr_cfgs,
                        expected_hash=bootstrap_commit.envelope.graph_hash_pre or "",
                    )
                _maybe_metric(
                    "oig_materializer_bootstrap_hash_ms",
                    _elapsed_ms(started=bootstrap_hash_started),
                )
                expected_bootstrap_hash = bootstrap_commit.envelope.graph_hash_pre or ""
                if expected_bootstrap_hash and not bootstrap_hash_state.matches(
                    ...
                    raise RuntimeError(
                        ...
                        + f"object_instance_graph_id={oig_id} commit_id={bootstrap_commit.envelope.commit_id}"
                    )
                graph.hash = bootstrap_hash_state.matched_hash_or_default(
                    expected_bootstrap_hash
                )
                indexes = {}
                base_cid = None
            ...
            if base_cid is not None and base_cid == target:
                ...
                with commit_perf_span(
                    phase="oig_materializer.get.exact_snapshot_return",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "base_commit_id": str(base_cid),
            ...
            if preloaded_replay_commits is None:
                ...
                with commit_perf_span(
                    phase="oig_materializer.get.replay_lineage_load",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "target_commit_id": str(target),
                        "base_commit_id": str(base_cid) if base_cid else None,
                    },
                ):
            ...
            _maybe_metric(
                "oig_materializer_replay_loaded_commit_count", len(replay_commits)
            )
            ...
            for c in replay_commits:
                ...
                envelope = c.envelope
                if (
                    envelope.projection_hash
                    and envelope.projection_hash != opg.projection_hash
                ):
                    raise ValueError(
                        f"Commit {envelope.commit_id} projection_hash mismatch"
                    )
                if (
                    envelope.object_instance_graph_identity_id
                    != c.body.object_instance_graph_identity_id
                ):
                    raise ValueError(f"Commit {envelope.commit_id} body OIGI mismatch")
                if envelope.object_instance_graph_id != c.body.object_instance_graph_id:
                    raise ValueError(f"Commit {envelope.commit_id} body OIG mismatch")

                validation_started = time.perf_counter()
                with commit_perf_span(
                    phase="oig_materializer.get.replay_validate_body",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                    ...
                    changes = object_instance_graph_changes_from_body(c.body)
                    for change in changes:
                        if (
                            ...
                            raise ValueError(
                                "Invalid OIG commit body OIGI target "
                                + f"(commit_id={envelope.commit_id})"
                            )
                        if change.object_instance_graph_id != graph.id:
                            raise ValueError(
                                "Invalid OIG commit body OIG target "
                                + f"(commit_id={envelope.commit_id})"
                            )
                ...
                with commit_perf_span(
                    phase="oig_materializer.get.replay_pre_hash",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "commit_id": str(envelope.commit_id),
                    ...
                    pre_hash_state = compute_oig_lane_hash_state(
                        graph=graph,
                        schema_attribute_configs_by_id=attr_cfgs,
                        expected_hash=envelope.graph_hash_pre or "",
                    )
                pre_hash_ms += _elapsed_ms(started=pre_hash_started)
                expected_pre = envelope.graph_hash_pre or ""
                if expected_pre and not pre_hash_state.matches(expected_pre):
                    raise ValueError(
                        ...
                        + f"expected={expected_pre} commit={envelope.commit_id}"
                    )
                ...
                if changes:
                    ...
                    with commit_perf_span(
                        phase="oig_materializer.get.replay_apply_changes",
                        category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                        metadata={
                            **trace_metadata,
                            "commit_id": str(envelope.commit_id),
                ...
                with commit_perf_span(
                    phase="oig_materializer.get.replay_post_hash",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "commit_id": str(envelope.commit_id),
                    ...
                    post_hash_state = compute_oig_lane_hash_state(
                        graph=graph,
                        schema_attribute_configs_by_id=attr_cfgs,
                        expected_hash=envelope.graph_hash_post or "",
                    )
                post_hash_ms += _elapsed_ms(started=post_hash_started)
                graph.hash = post_hash_state.matched_hash_or_default(
                    envelope.graph_hash_post or ""
                )
                expected_post = envelope.graph_hash_post
                if not post_hash_state.matches(expected_post):
                    change_tree = summarize_oig_change_tree(changes=changes)
            ...
            with commit_perf_span(
                phase="oig_materializer.get.snapshot_index_build",
                category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                metadata=trace_metadata,
            ):
            ...
            try:
                with commit_perf_span(
                    phase="oig_materializer.get.snapshot_write",
                    category=_OIG_MATERIALIZER_TRACE_CATEGORY,
                    metadata={
                        **trace_metadata,
                        "target_commit_id": str(target),
            ...
            _maybe_metric(
                "oig_materializer_total_ms", _elapsed_ms(started=total_started)
            )
    ...
    async def _load_lineage_forward(
        ...
        *,
        ...
        return tuple(
            [
                record
                async for record in self.commits.iter_lineage_forward_records(
                    branch_id=branch_id,
                    projection_hash=projection_hash,
                    head_commit_id=head_commit_id,
                    stop_at_commit_id=stop_at_commit_id,
                )
            ]
        )
    ...
    def indexes_from_graph(self, graph: ObjectInstanceGraph) -> dict[str, Any]:
        return self._indexes_from_graph(graph)
...
def _trace_metadata(
    *,
    ...
    return {
        ...
        "projection_hash": projection_hash,
        "requested_commit_id": str(commit_id) if commit_id is not None else None,
    }
