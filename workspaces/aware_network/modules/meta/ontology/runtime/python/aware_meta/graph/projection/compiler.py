...
def _resolve_class_namespace_from_type_ref(
    scope: FqnScope, type_ref: str
) -> tuple[_ResolvedNamespace, str]:
...
def _stable_opg_declaration_id(
    *,
    ...
    return ocg_stable_uuid(
        f"opg_declaration:{object_config_graph_id}:{projection_name}"
    )
...
def compile_object_config_graph_projections(
    ...
    *,
    ...
    projections = sorted(
        [
            p
            for p in (code_section_projections or [])
            if (p.projection_name or "").strip()
        ],
        key=lambda p: (
    ...
    for proj in projections:
        ...
        if not root_type_ref:
            raise ValueError(
                f"Projection {projection_name!r} must declare a root type (e.g. `root schema.Type`)"
            )
        ...
        for idx, view in enumerate(proj.projection_views or []):
            ...
            kind = (view.kind or "instance").strip().lower()
            if not observable_key:
                continue
            if observable_key in seen_observable_keys:
                raise ValueError(
                    f"Projection {projection_name!r} defines observable {observable_key!r} multiple times"
                )
        ...
        ns, canonical_class_name = _resolve_class_namespace_from_type_ref(
            scope, root_type_ref
        )
        ...
        if decl_id not in compiled_by_id:
            compiled_by_id[decl_id] = ObjectProjectionGraphDeclaration(
                ...
                object_projection_graph_bindings=[
                    bindings_by_id[k] for k in sorted(bindings_by_id, key=str)
                ],
            )
