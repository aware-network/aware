...
def build_object_instance_graph_commit_record_from_shallow_changes(
    *,
    root_metadata: ObjectInstanceGraphCommitRootMetadata,
    changes: list[ObjectInstanceGraphChange],
    branch_id: UUID,
    ...
    envelope = ObjectInstanceGraphCommitEnvelope(
        ...
        parent_commit_ids=(() if parent_commit_id is None else (parent_commit_id,)),
        ...
        graph_hash_post=graph_hash_post,
        projection_hash=projection_hash,
        source_language=_enum_text(source_language),
        graph_hash_source=graph_hash_source,
        body_contract=str(body.payload["c"]),
        body_media_type=OIG_COMMIT_BODY_MEDIA_TYPE,
        body_ref=f"{resolved_commit_id}.body.json",
...
def build_object_instance_graph_commit_record_from_body_draft(
    *,
    root_metadata: ObjectInstanceGraphCommitRootMetadata,
    ...
    if not body_draft.roots:
        raise OigCommitBuildError("Commit body draft must include at least one root")
...
def build_object_instance_graph_seed_commit_record(
    *,
    root_metadata: ObjectInstanceGraphCommitRootMetadata,
    branch_id: UUID,
    object_instance_graph_identity_id: UUID,
    ...
    if graph_hash_pre != graph_hash_post:
        raise OigCommitBuildError(
            "Seed commit record requires graph_hash_pre == graph_hash_post: "
