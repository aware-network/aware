...
async def build(
    ...
    if (
        object_config_graph_id is not None
        and object_config_graph_id != expected_graph_id
    ):
        raise RuntimeError(
    ...
    if object_config_graph_identity_id is not None:
        identity_key = f"{normalized_fqn_prefix}:{language_value}"
        expected_identity_id = stable_object_config_graph_identity_id(
            key=identity_key,
        )
        if object_config_graph_identity_id != expected_identity_id:
            raise RuntimeError(
                "ObjectConfigGraph.build object_config_graph_identity_id does not "
                "match deterministic stable-id for (fqn_prefix, language): "
                f"provided={object_config_graph_identity_id} "
                f"expected={expected_identity_id} "
                f"fqn_prefix={normalized_fqn_prefix!r} language={language_value!r}"
            )
    ...
    if existing is not None:
        if (
            ...
            or existing.object_config_graph_identity_id
            != object_config_graph_identity_id
        ):
            raise RuntimeError(
...
async def create_object_projection_graph(
    ...
    if object_config_graph_id is None:
        raise RuntimeError(
            "ObjectConfigGraph.create_object_projection_graph requires graph id"
        )
...
async def create_object_projection_graph_declaration(
    object_config_graph: ObjectConfigGraph,
    key: str,
    projection_name: str,
    label: str | None = None,
    ...
    object_projection_graph_declaration_id: UUID | None = None,
) -> ObjectProjectionGraphDeclaration:
    """
    Create deterministic ObjectProjectionGraphDeclaration under this ObjectConfigGraph.
    """

    # --- AWARE: LOGIC START create_object_projection_graph_declaration
    object_config_graph_id = object_config_graph.id
    if object_config_graph_id is None:
        raise RuntimeError(
    ...
    normalized_projection_name = (projection_name or "").strip()
    ...
    if not normalized_projection_name:
        raise RuntimeError(
            "ObjectConfigGraph.create_object_projection_graph_declaration "
            "requires non-empty projection_name"
        )

    expected_declaration_id = stable_object_projection_graph_declaration_id(
        object_config_graph_id=object_config_graph_id,
        projection_name=normalized_projection_name,
    )
    if (
        object_projection_graph_declaration_id is not None
        and object_projection_graph_declaration_id != expected_declaration_id
    ):
        raise RuntimeError(
            "ObjectConfigGraph.create_object_projection_graph_declaration "
            "object_projection_graph_declaration_id does not match deterministic "
            "stable-id for (graph, projection_name): "
            f"provided={object_projection_graph_declaration_id} "
            f"expected={expected_declaration_id}"
        )

    if object_config_graph.object_projection_graph_declarations is None:
        object_config_graph.object_projection_graph_declarations = []
    for existing in object_config_graph.object_projection_graph_declarations:
        if existing.id != expected_declaration_id:
            continue
        if (
            existing.object_config_graph_id != object_config_graph_id
            or existing.key != normalized_key
            or existing.projection_name != normalized_projection_name
            or (existing.label or None) != (label or None)
            or (existing.description or None) != (description or None)
            or existing.is_branchable != is_branchable
        ):
            raise RuntimeError(
                "ObjectConfigGraph.create_object_projection_graph_declaration "
                "payload mismatch for existing ObjectProjectionGraphDeclaration: "
                f"object_projection_graph_declaration_id={expected_declaration_id}"
            )
        return existing

    created = ObjectProjectionGraphDeclaration(
        id=expected_declaration_id,
        object_config_graph_id=object_config_graph_id,
        key=normalized_key,
        projection_name=normalized_projection_name,
        label=label,
    ...
    object_config_graph.object_projection_graph_declarations.append(created)
    return created
    # --- AWARE: LOGIC END create_object_projection_graph_declaration
...
async def create_object_config_graph_binding(
    ...
    if object_config_graph_id is None:
        raise RuntimeError(
            "ObjectConfigGraph.create_object_config_graph_binding requires graph id"
        )
