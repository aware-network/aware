...
def provider_delta_generated_materialization_stage(
    *,
    ...
    context = MetaProviderDeltaGeneratedMaterializationContext(
        ...
        target_language=_generated_materialization_target_language(
            package_payload=package_payload,
            code_package_delta=code_package_delta,
            typed_operations=typed_operations,
        ),
        target_language_plugin_id=(
            _generated_materialization_target_language_plugin_id(
                package_payload=package_payload,
                code_package_delta=code_package_delta,
                typed_operations=typed_operations,
            )
    ...
    evidence_manifest = _generated_materialization_evidence_manifest(
    ...
    return {
        ...
        "generated_materialization_evidence_manifest": (
            evidence_manifest.evidence_payload()
        ),
        "generated_materialization_artifact_path_count": (
            evidence_manifest.artifact_path_count
        ),
        "generated_materialization_artifact_family_counts": (
            evidence_manifest.artifact_family_counts
        ),
        "generated_materialization_artifact_role_counts": (
            evidence_manifest.artifact_role_counts
        ),
        "generated_materialization_delta_form_counts": (
            evidence_manifest.delta_form_counts
        ),
        "generated_materialization_path_kind_counts": (
            evidence_manifest.path_kind_counts
        ),
        "generated_materialization_path_role_counts": (
            evidence_manifest.path_role_counts
        ),
...
def _provider_delta_generated_materialization_blocked_stage(
    *,
    ...
    evidence_manifest = generated_materialization_evidence_manifest_from_results(())
    return {
        "stage_kind": "meta_ocg_provider_delta_generated_materialization",
        "contract_version": (
            META_PROVIDER_DELTA_GENERATED_MATERIALIZATION_CONTRACT_VERSION
        ...
        "generated_materialization_artifact_path_count": 0,
        "generated_materialization_artifact_family_counts": {},
        "generated_materialization_artifact_role_counts": {},
        "generated_materialization_delta_form_counts": {},
        "generated_materialization_path_kind_counts": {},
...
def _generated_materialization_evidence_manifest(
    *,
    feature_results: tuple[MetaProviderDeltaGeneratedMaterializationFeatureResult, ...],
) -> GeneratedMaterializationEvidenceManifest:
    return generated_materialization_evidence_manifest_from_results(
        item.result for item in feature_results if item.result is not None
    )
...
def _generated_materialization_sources_root(
    *,
    ...
    raw_sources_root = (
        optional_text(code_package_delta_payload.get("stage_sources_root"))
        or _object_text_attribute(code_package_delta, "sources_root")
...
def _generated_materialization_target_language(
    *,
    package_payload: Mapping[str, object],
    code_package_delta: object | None,
    ...
    raw_language = (
        optional_text(_object_attribute(code_package_delta, "target_language"))
        or optional_text(mapping_value(code_package_delta).get("target_language"))
        or optional_text(package_payload.get("target_language"))
    )
    if raw_language is not None:
        return raw_language

    for target in _generated_materialization_target_descriptors(
        package_payload=package_payload,
        typed_operations=typed_operations,
    ):
        provider_payload = mapping_value(target.get("provider_payload"))
        language = optional_text(
            target.get("target_language") or provider_payload.get("target_language")
        )
...
def _generated_materialization_target_language_plugin_id(
    *,
    package_payload: Mapping[str, object],
    code_package_delta: object | None,
    typed_operations: tuple[MetaProviderDeltaTypedOperation, ...] = (),
    ...
    raw_language = (
        optional_text(_object_attribute(code_package_delta, "target_language_plugin_id"))
        or optional_text(mapping_value(code_package_delta).get("target_language_plugin_id"))
        or optional_text(package_payload.get("target_language_plugin_id"))
        or optional_text(
            mapping_value(_object_attribute(code_package_delta, "metadata")).get(
                "target_language_plugin_id"
            )
        )
        or optional_text(
            mapping_value(
                _object_attribute(
                    _object_attribute(
                        _object_attribute(code_package_delta, "production"),
                        "producer",
                    ),
                    "provider_payload",
                )
            ).get("target_language_plugin_id")
        )
    )
    if raw_language is not None:
        return raw_language
    ...
    for target in _generated_materialization_target_descriptors(
        ...
        provider_payload = mapping_value(target.get("provider_payload"))
        plugin_id = optional_text(
            target.get("target_language_plugin_id")
            or provider_payload.get("target_language_plugin_id")
        )
...
def _generated_materialization_target_descriptors(
    *,
    package_payload: Mapping[str, object],
    typed_operations: tuple[MetaProviderDeltaTypedOperation, ...] = (),
) -> tuple[Mapping[str, object], ...]:
    previous_materialization_evidence = mapping_value(
        package_payload.get("previous_materialization_evidence")
    )
    descriptors: list[Mapping[str, object]] = []
    for value in (
        package_payload.get("generated_materialization_targets"),
        previous_materialization_evidence.get("generated_materialization_targets"),
    ):
        descriptors.extend(_mapping_items(value))
    for operation in typed_operations:
        descriptors.extend(
            _operation_generated_materialization_target_descriptors(
                operation=operation,
            )
        )
...
def _operation_generated_materialization_target_descriptors(
    *,
    operation: MetaProviderDeltaTypedOperation,
) -> tuple[Mapping[str, object], ...]:
    descriptors: list[Mapping[str, object]] = []
    for payload in _generated_materialization_operation_payloads(operation=operation):
        generated = mapping_value(payload.get("generated_materialization"))
        if not generated:
            continue
        for target_payload in mapping_value(generated.get("targets")).values():
...
def _generated_materialization_operation_payloads(
    *,
    operation: MetaProviderDeltaTypedOperation,
    ...
    for payload in (
        ...
        if payload and any(existing == payload for existing in payloads):
            continue
...
def _object_attribute(value: object, attribute_name: str) -> object | None:
    if value is None:
        return None
