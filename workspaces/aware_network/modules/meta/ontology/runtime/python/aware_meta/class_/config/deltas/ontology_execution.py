...
CLASS_CONFIG_UPDATE_PARENT_CLASS_FUNCTION_REF = (
...
def _plan_class_update_operation(
    *,
    ...
    if (
        _provider_operation_type(operation=operation)
        == CLASS_CONFIG_PARENT_UPDATE_PROVIDER_OPERATION_TYPE
    ):
        return _plan_class_parent_update_operation(
            operation=operation,
            class_config_id=class_config_id,
        )
...
def _plan_class_parent_update_operation(
    *,
    operation: OntologyTypedOperation,
    class_config_id: str,
) -> OntologyOperationHandlerResult:
    blockers = _class_parent_update_blockers(operation=operation)
    if blockers:
        return blocked_handler_result(
            operation=operation,
            handler_key=HANDLER_KEY,
            reason="meta_ocg_class_parent_update_requires_parent_identity",
            blockers=blockers,
        )
    return OntologyOperationHandlerResult(
        operation_key=operation.operation_key,
        ...
        handler_key=HANDLER_KEY,
        status="ontology_operation_handler_ready",
        reason="meta_ocg_class_parent_update_function_call_ready",
        invocation_intents=(
            OntologyInvocationIntent(
                intent_key=f"{operation.operation_key}:update_parent_class",
                operation_key=operation.operation_key,
                semantic_key=operation.semantic_key,
                invocation_order=CLASS_UPDATE_INVOCATION_ORDER,
                invocation_mode="instance",
                ...
                function_ref=CLASS_CONFIG_UPDATE_PARENT_CLASS_FUNCTION_REF,
                target_object_id=class_config_id,
                receiver_semantic_key=operation.semantic_key,
                result_semantic_key=operation.semantic_key,
                expected_result_object_id=class_config_id,
                kwargs=_class_parent_update_arguments(operation=operation),
                reason="meta_ocg_class_parent_update_function_call_ready",
            ),
        ),
    )
...
def _class_parent_update_arguments(
    *,
    operation: OntologyTypedOperation,
) -> dict[str, object]:
    return {
        "parent_class_config_id": _class_parent_id(operation=operation),
    }


def _class_parent_update_blockers(
    *,
    operation: OntologyTypedOperation,
) -> tuple[str, ...]:
    if _class_parent_id(operation=operation) is not None:
        return ()
    if _class_parent_reference_required(operation=operation):
        return ("missing_parent_class_config_id",)
    return ()


def _class_parent_reference_required(
    *,
    operation: OntologyTypedOperation,
) -> bool:
    payload = mapping_value(operation.current.get("payload"))
    return any(
        ...
        for value in (
            operation.current.get("parent_class_fqn"),
            operation.current.get("parent_class_semantic_key"),
            operation.current.get("parent_class_config_id"),
    ...
    *,
    ...
    for field_name in ("class_fqn", "name"):
        ...
        if (
            ...
            blockers.append(f"class_identity_change_requires_replacement:{field_name}")
...
def _class_parent_id(*, operation: OntologyTypedOperation) -> str | None:
    payload = mapping_value(operation.current.get("payload"))
    return _first_text(
        operation.current.get("parent_class_id"),
        operation.current.get("parent_class_config_id"),
...
def _class_signature(*, operation: OntologyTypedOperation) -> dict[str, object]:
    ...
    return mapping_value(
        operation.current.get("class_signature") or payload.get("class_signature")
    )
