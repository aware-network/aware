...
def function_config_create_typed_operation(
    *,
    ...
    position: int = 0,
    generated_materialization: Mapping[str, object] | None = None,
    ...
    if inputs:
        function_signature["inputs"] = tuple(dict(item) for item in inputs)
    if outputs:
        function_signature["outputs"] = tuple(dict(item) for item in outputs)
    ...
    if generated_materialization:
        target_payload = dict(generated_materialization)
        current["generated_materialization"] = target_payload
        payload["generated_materialization"] = target_payload
...
def _owner_class_scope_closure_fields(
    *,
    ...
    return {
        ...
        **meta_ocg_scope_closure_committed_ref_fields(gates=(gate,)),
