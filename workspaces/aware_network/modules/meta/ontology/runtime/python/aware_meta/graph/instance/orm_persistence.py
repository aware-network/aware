...
def _build_enum_option_value_by_id(index: MetaOrmProjectionIndex) -> dict[UUID, str]:
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to read runtime OCG for enum mapping: {exc}"
        ) from exc
...
async def stage_domain_persistence(
    *,
    ...
    try:
        plan = ProjectionRuntime.require_plan(
            dialect="postgres", projection_hash=proj_hash
        )
    except KeyError:
        if backend == "fs":
    ...
    *,
    ...
    for ci_change in _iter_class_instance_changes(changes):
        ...
        for cd in ch.change_deltas:
            if (
                cd.kind != ChangeDeltaKind.scalar_set
                or cd.property != "class_config_id"
            ):
                continue
            raw = _delta_value(cd)
            if raw is None:
    ...
    fk_specs_by_owner_cc = _build_fk_specs_by_owner_cc_id(
        only_class_config_ids=relevant_cc_ids
    )
    fk_specs_by_relationship_id = _build_fk_specs_by_relationship_id(
        only_class_config_ids=relevant_cc_ids
    )

    async def _dependencies_for(
        instance_id: UUID, *, population: set[UUID]
    ) -> set[UUID]:
        cc_id = class_config_by_instance_id.get(instance_id)
        if cc_id is None:
            raise RuntimeError(f"Missing class_config_id for instance {instance_id}")
        ...
        if orm_class is None:
            raise RuntimeError(
                f"ORM class not found for ClassConfig {cc_id} (instance_id={instance_id})"
            )
        orm_model = await orm_class.by_id(instance_id, cache_valid=True)
        ...
        for spec in fk_specs_by_owner_cc.get(cc_id, ()):
            ...
            if ref_id in population:
                ...
                if ref_cc_id is None:
                    raise RuntimeError(
                        f"Missing class_config_id for FK target {ref_id} (from {instance_id})"
                    )
    ...
    async def _deferred_optional_fks_for(
        instance_id: UUID, *, population: set[UUID]
    ) -> dict[str, UUID]:
        cc_id = class_config_by_instance_id.get(instance_id)
        if cc_id is None:
        ...
        fk_field_names = {
            spec.field_name for spec in fk_specs_by_owner_cc.get(cc_id, ())
        }
        for spec in fk_specs_by_owner_cc.get(cc_id, ()):
            if spec.is_required:
                continue
    ...
    def _toposort(nodes: set[UUID], deps_by_node: dict[UUID, set[UUID]]) -> list[UUID]:
        ...
        heap: list[tuple[tuple[str, str], UUID]] = [
            (_stable_key(n), n) for n, d in in_degree.items() if d == 0
        ]
        heapq.heapify(heap)
        out: list[UUID] = []
        ...
        if len(out) != len(nodes):
            remaining = sorted(
                (n for n, d in in_degree.items() if d > 0), key=_stable_key
            )
            details = {
                str(n): sorted((str(d) for d in deps_by_node.get(n, set())), key=str)
                for n in remaining
            }
            raise RuntimeError(
                f"FK dependency cycle detected in persistence plan: {details}"
            )
        return out
    ...
    *, only_class_config_ids: set[UUID] | None = None
    ...
    for rel in relationships_by_id.values():
        ...
        for rel_attr in rel.class_config_relationship_attributes:
            ...
            if (
                only_class_config_ids is not None
                and expected_owner_cc_id not in only_class_config_ids
            ):
                continue
...
def _build_fk_specs_by_relationship_id(
    *, only_class_config_ids: set[UUID] | None = None
    ...
    for rel in relationships_by_id.values():
        ...
        if (
            only_class_config_ids is not None
            and owner_cc_id not in only_class_config_ids
        ):
            continue
