    ...
    *,
    ...
    for class_config_id, class_config in sorted(
        ...
        if class_config_id not in membership_by_class_id:
            diagnostics.append(
                MetaSemanticDiagnostic(
                    severity=severity,
                    code=("aware_meta.completeness." "projection_membership_missing"),
                    message=(
        ...
        if class_config_id not in reachable_by_class_id:
            diagnostics.append(
                MetaSemanticDiagnostic(
                    severity=severity,
                    code=("aware_meta.completeness." "projection_node_unreachable"),
                    message=(
    ...
    for class_config_id, class_config in sorted(
        ...
        diagnostics.append(
            MetaSemanticDiagnostic(
                severity=severity,
                code=("aware_meta.completeness." "constructor_projection_unreachable"),
                message=(
...
def _projection_reachability_by_class_id(
    ...
    for graph in graphs:
        relationships = _relationships_by_id(
            (graph, *tuple(g for g in graphs if g is not graph))
        )
        for opg in graph.object_projection_graphs or []:
            for class_config_id in _reachable_classes_for_opg(
                ...
                reachable.setdefault(class_config_id, set()).add(
                    opg.name or str(opg.id)
                )
    return reachable
    ...
    for opg in object_config_graph.object_projection_graphs or []:
        ...
        for parent_id, child_id, relationship in sorted(
            ...
            if parent_class is None or child_class is None:
                continue
            if _is_association_target_reference(relationship):
                continue
            ...
            diagnostics.append(
                MetaSemanticDiagnostic(
                    severity=severity,
                    code=("aware_meta.completeness." "constructor_path_missing"),
                    message=(
...
def _is_association_target_reference(
    relationship: ClassConfigRelationship,
) -> bool:
    identity_rail = relationship.identity_rail
    reified_role = relationship.reified_role
    is_reference_rail = (
        identity_rail is ClassConfigRelationshipIdentityRail.reference
        or identity_rail == ClassConfigRelationshipIdentityRail.reference.value
    )
    is_association_to_target = (
        reified_role is ClassConfigRelationshipReifiedRole.association_to_target
        or reified_role
        == ClassConfigRelationshipReifiedRole.association_to_target.value
    )
    return is_reference_rail and is_association_to_target
...
def _constructor_owner_by_function_id(
    ...
    for class_config in _concrete_class_configs_by_id(graphs).values():
        for function_link in class_config.class_config_function_configs or []:
            ...
            if not function_link.is_constructor or function_config_id is None:
                continue
