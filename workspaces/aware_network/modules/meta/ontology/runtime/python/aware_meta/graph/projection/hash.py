...
def calculate_hash(
    *,
    ...
    for g in graphs:
        ...
        for n in g.object_config_graph_nodes:
            ...
            if prev_id is not None and prev_id != n.class_config.id:
                raise ValueError(
                    f"OPG hash: duplicate class FQN {fqn!r} maps to {prev_id} and {n.class_config.id}"
                )
    ...
    def _index_relationship(rel: ClassConfigRelationship) -> None:
        ...
        if src_fqn is None:
            raise ValueError(
                f"OPG hash: missing source class_fqn for relationship {rel.id}"
            )
        ...
        if prev is not None and prev != key:
            raise ValueError(
                f"OPG hash: relationship_id={rel.id} maps to multiple keys: {prev!r} and {key!r}"
            )
    ...
    for g in graphs:
        for n in g.object_config_graph_nodes:
            if (
                ...
                or n.class_config_relationship is None
            ):
                continue
    ...
    if relationships:
        ...
        for rel in relationships:
            ...
            target_class_fqn = class_id_to_fqn.get(
                cfg_rel.target_class_config_id
            ) or str(cfg_rel.target_class_config_id)

            relationships_entries.append(
                {
                    "relationship_key": rel_id_to_key.get(
...
def canon_node(*, node: ObjectProjectionGraphNode, class_id_to_fqn: dict[UUID, str]):
    ...
    #     sub_opg = await ObjectProjectionGraph.by_id(rule.subgraph_opg_id)
    #     if sub_opg is None:
    ...
    #         sub = sub_opg.projection_hash or await sub_opg.calculate_hash()
    return {
        "class_fqn": class_id_to_fqn.get(node.class_config_id)
        or str(node.class_config_id),
...
def canon_edge(*, edge: ObjectProjectionGraphEdge, rel_id_to_key: dict[UUID, str]):
    return {
        ...
        "attribute_role": (
            edge.attribute_role.value if edge.attribute_role is not None else None
        ),
    }


def _declaring_attr_name(
    *, rel: ClassConfigRelationship, attr_name_by_id: dict[UUID, tuple[UUID, str]]
) -> str:
    ...
    if ref_attr_id is None:
        ...
        raise ValueError(
            f"OPG hash: relationship {rel.id} missing FORWARD+REFERENCE attribute binding"
        )
    owner_and_name = attr_name_by_id.get(ref_attr_id)
    if owner_and_name is None:
        raise ValueError(
            f"OPG hash: relationship {rel.id} reference attribute_config_id={ref_attr_id} not found"
        )
