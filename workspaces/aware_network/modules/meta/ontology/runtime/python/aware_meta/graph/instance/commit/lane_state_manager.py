...
class OIGLaneStateManager:
    ...
    def _lane_branch_id(self, branch: ObjectInstanceGraphBranch) -> UUID:
        ...
        if (
            getattr(branch, "branch", None) is not None
            and getattr(branch.branch, "id", None) is not None
        ):
    ...
    def _indexes_from_graph(
        self, graph: ObjectInstanceGraph
    ...
    async def ensure_loaded(
        ...
        *,
        ...
        async with self._lock_for(key):
            oig_lane = self._resolve_or_create_lane(
                branch=branch, projection_hash=projection_hash
            )

            head_meta = await self._commits.head(
                branch_id=lane_branch_id, projection_hash=projection_hash
            )
            head_id = (
                UUID(str(head_meta["commit_id"]))
                if head_meta and head_meta.get("commit_id")
                else None
            )
            head_oig_id = (
                head_meta.get("object_instance_graph_id") if head_meta else None
            )
            ...
            inst_map = {
                UUID(k): UUID(v) for k, v in (idx.get("instance_map") or {}).items()
            }
            cc_map = {
                UUID(k): UUID(v) for k, v in (idx.get("classcfg_map") or {}).items()
            }
            ...
            try:
                commit_count = len(
                    list(
                        (
                            self._commits._commits_dir(lane_branch_id, projection_hash)
                        ).glob("*.json")
                    )
                )
            except Exception:
    ...
    def resolve_object_instance_id(
        self, *, branch_id: UUID, projection_hash: str, instance_id: UUID
    ) -> UUID:
        idx = self._instance_index.get((branch_id, projection_hash), {})
        if instance_id not in idx:
            raise KeyError(
                f"instance_id {instance_id} not present in lane {(branch_id, projection_hash)}"
            )
    ...
    def resolve_class_config_id(
        ...
        if instance_id not in idx:
            raise KeyError(
                f"class_config_id for {instance_id} not present in lane {(branch_id, projection_hash)}"
            )
        return idx[instance_id]
        ...
        *,
        ...
        async with self._lock_for(key):
            ...
            if not head_meta or head_meta.get("commit_id") != str(commit.commit.id):
                raise ValueError(
                    "on_commit_appended: HEAD not updated to appended commit"
                )
            ...
            if commit.commit.commit_parents:
                if len(commit.commit.commit_parents) > 1:
                    raise ValueError(
                        "on_commit_appended: non-linear commit has >1 parent"
                    )
            ...
            if prev is not None:
                ...
                if prev.head_commit_id == parent_commit_id and pre_hash_state.matches(
                    ...
                    try:
                        ...
                        raise ValueError(
                            f"on_commit_appended: invalid commit payload: {e}"
                        ) from e
                    ...
                    prev.oig.hash = post_hash_state.matched_hash_or_default(
                        commit.graph_hash_post or ""
                    )
                    ...
                    self._instance_index[key] = {
                        UUID(k): UUID(v)
                        for k, v in (idx.get("instance_map") or {}).items()
                    }
                    self._classcfg_index[key] = {
                        UUID(k): UUID(v)
                        for k, v in (idx.get("classcfg_map") or {}).items()
                    }
                    ...
                    if self._snapshot_every and (
                        commit_count % self._snapshot_every == 0
                    ):
                        try:
            ...
            if prev is not None:
                ...
                try:
                    commit_count = len(
                        list(
                            (
                                self._commits._commits_dir(
                                    lane_branch_id, projection_hash
                                )
                            ).glob("*.json")
                        )
                    )
...
class _LazyOIGLaneStateManager:
    def __init__(self) -> None:
        self._manager: OIGLaneStateManager | None = None

    def _get(self) -> OIGLaneStateManager:
        if self._manager is None:
            self._manager = OIGLaneStateManager()
...
oig_lane_state_manager = cast(OIGLaneStateManager, _LazyOIGLaneStateManager())
