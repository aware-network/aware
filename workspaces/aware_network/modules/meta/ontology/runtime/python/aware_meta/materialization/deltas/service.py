    ...
    def record(self, stage_name: str) -> Iterator[None]:
        ...
        try:
            with commit_perf_span(
                phase=f"provider_delta.{stage_name}",
...
async def materialize_delta(request: object) -> dict[str, object]:
    ...
    with stage_timings.record("generated_materialization"):
        provider_delta_generated_materialization = (
            provider_delta_generated_materialization_stage(
                package_payload=_generated_materialization_package_payload(
                    package_payload=package_payload,
                    request=request,
                ),
                manifest_path=manifest_path,
...
def _generated_materialization_package_payload(
    *,
    package_payload: Mapping[str, object],
    request: object,
) -> dict[str, object]:
    payload = {str(key): item for key, item in package_payload.items()}
    previous_materialization_evidence = _model_payload(
        getattr(request, "previous_materialization_evidence", None)
    )
    if previous_materialization_evidence:
