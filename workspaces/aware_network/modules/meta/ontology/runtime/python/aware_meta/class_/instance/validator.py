...
def validate_class_instance(
    *,
    ...
    for link in class_config.class_config_attribute_configs:
        ...
        expected_order[attr_cfg.id] = _AttrOrderKey(
            position=link.position, name=attr_cfg.name
        )
    ...
    for attr in class_instance.attributes:
        ...
        if attr_id in seen_attr_ids:
            raise ClassInstanceValidationError(
                f"Duplicate AttributeConfig on instance: {attr_id}"
            )
        ...
        if key is None:
            raise ClassInstanceValidationError(
                f"AttributeConfig not found on ClassConfig: {attr_id}"
            )
        order_keys.append(key)

    if order_keys != sorted(order_keys, key=lambda k: (k.position, k.name)):
        raise ClassInstanceValidationError(
            "ClassInstance.attributes are not in deterministic order"
        )

    missing_required = required_attribute_ids - seen_attr_ids
    if missing_required:
        missing_names = [
            allowed_attribute_configs[i].name for i in sorted(missing_required, key=str)
        ]
        raise ClassInstanceValidationError(
            f"Missing required attributes: {missing_names}"
        )
...
def _validate_attribute(
    *,
    ...
    if root is None:
        raise ClassInstanceValidationError(
            f"Attribute missing value_root for {attr_cfg.name}"
        )

    if (
        root.type_descriptor_id is not None
        and root.type_descriptor_id != attr_cfg.type_descriptor.id
    ):
        raise ClassInstanceValidationError(
    ...
    try:
        validate_attribute_value_tree_against_descriptor(
            root,
            expected=attr_cfg.type_descriptor,
        )
    except AttributeValueTreeValidationError as exc:
        raise ClassInstanceValidationError(
            f"Invalid AttributeValue tree for {attr_cfg.name}: {exc}"
        ) from exc
