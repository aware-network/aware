...
class ObjectProjectionGraphPortalIndex(BaseModel):
    ...
    portals_by_source_projection_hash: dict[str, list[ObjectProjectionGraphPortal]] = (
        Field(default_factory=dict)
    )
    portals_by_source_projection_hash_and_relationship_id: dict[
        str, dict[UUID, list[ObjectProjectionGraphPortal]]
    ] = Field(default_factory=dict)

    reference_attribute_config_id_by_relationship_id: dict[UUID, UUID] = Field(
        default_factory=dict
    )
    reference_field_name_by_relationship_id: dict[UUID, str] = Field(
        default_factory=dict
    )
    foreign_key_attribute_config_id_by_relationship_id: dict[UUID, UUID] = Field(
        default_factory=dict
    )
    foreign_key_field_name_by_relationship_id: dict[UUID, str] = Field(
        default_factory=dict
    )
...
def build_portal_closure_context(
    ...
    *,
    ...
    include_installed_binding_metadata: bool = False,
    ...
    for g in graphs:
        for node in g.object_config_graph_nodes:
            ...
            if (
                ...
                relationships_by_id[node.class_config_relationship.id] = (
        ...
        for rel in g.object_config_graph_relationships:
            ...
            for class_config_relationship in rel.class_config_relationships:
                relationships_by_id[class_config_relationship.id] = (
                    class_config_relationship
                )

    if include_installed_binding_metadata:
    ...
    *,
    ...
    if closure_context is None:
        closure_context = build_portal_closure_context(
            ...
            include_installed_binding_metadata=include_installed_binding_metadata,
        )
    ...
    for rel in relationships_by_id.values():
        ...
        if ref_attr_id is None:
            raise ValueError(
                f"Portal index: relationship {rel.id} missing FORWARD+REFERENCE attribute binding"
            )
    ...
    for source_opg in ocg.object_projection_graphs:
        for opg_rel in source_opg.object_projection_graph_relationships:
            ...
            ref_attr_id = reference_attribute_config_id_by_relationship_id.get(
                cfg_rel.id
            )
    ...
    portals_by_source_hash_and_rel: dict[
        str, dict[UUID, list[ObjectProjectionGraphPortal]]
    ] = {}
    for portal in portals:
        portals_by_source_hash.setdefault(portal.source_projection_hash, []).append(
            portal
        )
        portals_by_source_hash_and_rel.setdefault(
            portal.source_projection_hash, {}
        ).setdefault(portal.class_config_relationship_id, []).append(portal)
    ...
    *,
    ...
    if existing is None:
        ...
        existing_function_names = {
            ...
            and (link.function_config.name or "").strip()
        }
