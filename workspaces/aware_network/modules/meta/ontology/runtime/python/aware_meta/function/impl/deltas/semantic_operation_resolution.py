...
def _resolve_function_impl_body_update(
    *,
    ...
    current_objects: Mapping[str, str],
    current_object_identities: Mapping[str, Mapping[str, object]],
    generated_materialization_target_profile: object | None,
) -> MetaSemanticOperationResolution:
    ...
    if (
        normalization.reason == "function_impl_body_requires_function_impl_object_id"
        and _function_impl_body_text_is_empty(after_payload.get("body_text"))
    ):
        return MetaSemanticOperationResolution(
            operation_key=_string_value(operation.get("operation_key")) or "",
            semantic_operation_type=_string_value(
                operation.get("semantic_operation_type")
            )
            or "",
            semantic_key=semantic_key,
            status="unsupported_operation",
            reason="meta_ocg_function_impl_empty_body_without_identity_noop",
            metadata={
                "source": (
                    "aware_meta.function.impl.deltas."
                    "semantic_operation_resolution"
                ),
                "semantic_apply_boundary": "not_applicable",
                "function_impl_body_empty": True,
...
def _function_impl_body_text_is_empty(value: object) -> bool:
    body_text = _optional_text(value)
