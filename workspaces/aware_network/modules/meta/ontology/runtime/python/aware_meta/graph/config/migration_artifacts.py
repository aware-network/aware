...
class MetaOcgMigrationArtifactBundle:
    ...
    def lane_index(self) -> MetaOcgMigrationArtifact:
        ...
        raise MetaOcgMigrationArtifactError(
            "OCG migration bundle is missing lane_index artifact"
        )
    ...
    *,
        ...
        commit_index_entries.append(
            {
                ...
                "parent_commit_id": (
                    None if parent_commit_id is None else str(parent_commit_id)
                ),
    ...
    receipt: JsonObject = {
        ...
        "object_config_graph_package_id": (
            ...
            else str(object_config_graph_package_id)
        ),
    ...
    *,
    ...
    while current_commit_id is not None:
        ...
        if record is None:
            ...
            if commit is None:
                raise MetaOcgMigrationArtifactError(
                    "OCG migration lane HEAD chain references missing commit "
                    f"{current_commit_id}"
                )
        else:
            commit = _commit_from_record(record=record)
    ...
    for index, lane_commit in enumerate(lane_commits):
        expected_parent = (
            None if index == 0 else lane_commits[index - 1].commit.commit.id
        )
    ...
    *,
    ...
    return ObjectInstanceGraphCommit(
        ...
        object_instance_graph_changes=list(
            object_instance_graph_changes_from_body(record.body)
        ),
    ...
    *,
    ...
    if hint is not None:
        ...
        delta = build_ocg_delta_from_oig_commit(
            commit=commit, schema_graph=schema_graph
        )
...
def _artifact_relative_path(artifact: MetaOcgMigrationArtifact) -> Path:
    ...
    if not parts:
        raise MetaOcgMigrationArtifactError(
            "OCG migration artifact key cannot be empty"
        )
...
def _normalize_dialects(dialects: Iterable[str]) -> tuple[str, ...]:
    clean = tuple(
        dict.fromkeys(_require_text(dialect, "dialect") for dialect in dialects)
    )
    if not clean:
        raise MetaOcgMigrationArtifactError(
            "At least one migration dialect is required"
        )
    return clean
