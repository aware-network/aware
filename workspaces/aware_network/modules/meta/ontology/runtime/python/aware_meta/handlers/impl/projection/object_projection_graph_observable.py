...
async def create_via_object_projection_graph_identity(
    ...
    if not observable_key_norm:
        raise ValueError(
            "ObjectProjectionGraphIdentity.create_observable requires observable_key"
        )

    kind_norm = (
        str(kind) if kind is not None else "instance"
    ).strip().lower() or "instance"
    if kind_norm not in {"construct", "instance"}:
        raise ValueError(
    ...
    parent = session.imap_get(
        ObjectProjectionGraphIdentity, object_projection_graph_identity_id
    )
    if parent is None:
        raise RuntimeError(
    ...
    if existing is not None:
        ...
        existing_observable_key = (
            getattr(existing, "observable_key", None)
            or getattr(existing, "view_key", None)
            or ""
        ).strip()
        ...
        existing_kind = (existing.kind or "instance").strip().lower() or "instance"
        if existing_kind != kind_norm:
            raise RuntimeError(
