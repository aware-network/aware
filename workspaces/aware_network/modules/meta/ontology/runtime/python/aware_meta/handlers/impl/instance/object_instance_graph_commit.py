...
async def create_via_object_instance_graph_identity(
    ...
    parent_identity = session.imap_get(
    ...
    if str(payload.object_instance_graph_identity_id) != str(
    ...
    if payload.commit.id != commit_id:
        raise RuntimeError(
            "Domain commit payload mismatch: "
            + f"commit_id={commit_id} payload_commit_id={payload.commit.id}"
        )
    ...
    parent_ids = [
        stable_commit_id(lane_id=lane_id, key=str(parent.parent_commit_id))
    ...
    for parent_id in parent_ids:
        if any(
            existing.parent_commit_id == parent_id for existing in commit.commit_parents
        ):
            continue
