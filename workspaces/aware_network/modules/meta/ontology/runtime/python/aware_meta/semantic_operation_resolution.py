...
META_OBJECT_CONFIG_GRAPH_PACKAGE_CREATE_OPERATION = (
    "aware_meta.object_config_graph_package.create"
)
META_OBJECT_CONFIG_GRAPH_PACKAGE_ATTACH_GRAPH_OPERATION = (
    "aware_meta.object_config_graph_package.graph.attach"
)
META_OBJECT_CONFIG_GRAPH_IDENTITY_CREATE_OPERATION = (
    "aware_meta.object_config_graph_identity.create"
)
META_OBJECT_CONFIG_GRAPH_CREATE_OPERATION = "aware_meta.object_config_graph.create"
...
META_FUNCTION_CREATE_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION = (
    "aware.meta.function.create.generated_materialization_intent.v0"
)
...
META_CLASS_PARENT_UPDATE_GENERATED_MATERIALIZATION_INTENT_CONTRACT_VERSION = (
    "aware.meta.class.parent_update.generated_materialization_intent.v0"
)
...
def _class_fqn_from_package_context(
    *,
    ...
    if _operation_source_ref_has_declared_sources_root(operation=operation):
        return ".".join((fqn_prefix, source_namespace, resolved_name))
    return ".".join((fqn_prefix, "default", source_namespace, resolved_name))


def _operation_source_ref_has_declared_sources_root(
    *,
    operation: Mapping[str, object],
) -> bool:
    for source_ref_value in _tuple_values(operation.get("source_refs")):
        ...
        for index, part in enumerate(parts):
            if part == "structure" and parts[index + 1 : index + 2] == ("aware",):
                return True
    return False
