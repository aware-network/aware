...
def test_meta_generated_class_instance_create_attribute_delegates_to_target() -> None:
    generated_handler_source = (
        _ONTOLOGY_ROOT
        / "runtime/python/aware_meta/handlers/_generated/meta_handlers.py"
    ).read_text(encoding="utf-8")
    impl_source = (
        _ONTOLOGY_ROOT
        / "runtime/python/aware_meta/handlers/impl/class_/class_instance.py"
    ).read_text(encoding="utf-8")

    assert "owner_key=source_object_id" not in generated_handler_source
    assert "result = await _impl(class_instance=target, **call_kwargs)" in (
        generated_handler_source
    )
    assert "owner_key=class_instance.source_object_id" in impl_source
