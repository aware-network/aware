...
class ObjectConfigGraph(ORMModel):
    ...
    async def create_object_projection_graph_declaration(
        ...
        Contract:
        - Parent `object_config_graph_id` is propagated by constructor lowering.
        - `object_projection_graph_declaration_id` is provider-delta identity evidence.
        - Child identity is declaration-scoped and must be stable across compiler/provider rails.
        """
        ...
        result = await invoke_instance(
            orm_model=self, function_name="create_object_projection_graph_declaration", payload=payload
        )
        value = result.get("value") if isinstance(result, dict) and "value" in result else result
        from aware_meta_ontology.graph.projection.object_projection_graph_declaration import (
            ObjectProjectionGraphDeclaration,
...
class ObjectConfigGraphCreateObjectProjectionGraphDeclarationInput(BaseModel):
    key: str
    projection_name: str
    label: str | None = Field(default=None)
    description: str | None = Field(default=None)
    is_branchable: bool = Field(default=False)
    object_projection_graph_declaration_id: UUID | None = Field(default=None)


class ObjectConfigGraphCreateObjectProjectionGraphDeclarationOutput(BaseModel):
...
FUNCTIONS = {
    "ObjectConfigGraph": {
        ...
        "create_object_projection_graph_declaration": {
            "canonical": {
                "name": "create_object_projection_graph_declaration",
                "description": "Create deterministic ObjectProjectionGraphDeclaration under this ObjectConfigGraph.\n\nContract:\n- Parent `object_config_graph_id` is propagated by constructor lowering.\n- `object_projection_graph_declaration_id` is provider-delta identity evidence.\n- Child identity is declaration-scoped and must be stable across compiler/provider rails.",
                "is_constructor": False,
            },
            "input": ObjectConfigGraphCreateObjectProjectionGraphDeclarationInput,
            "output": ObjectConfigGraphCreateObjectProjectionGraphDeclarationOutput,
        },
...
__all__ = [
    ...
    "ObjectConfigGraphCreateObjectProjectionGraphDeclarationInput",
    "ObjectConfigGraphCreateObjectProjectionGraphDeclarationOutput",
