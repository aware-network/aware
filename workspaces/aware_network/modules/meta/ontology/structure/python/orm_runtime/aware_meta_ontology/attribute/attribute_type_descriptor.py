...
class AttributeTypeDescriptor(ORMModel):
    ...
    async def create_collection_primitive(
        ...
        payload = {"collection_kind": collection_kind, "primitive_base_type": primitive_base_type}
        result = await invoke_constructor(orm_class=cls, function_name="create_collection_primitive", payload=payload)
        value = result.get("value") if isinstance(result, dict) and "value" in result else result
        if isinstance(value, AttributeTypeDescriptor):
            return value
        return AttributeTypeDescriptor.validate_invocation_value(value)
...
class AttributeTypeDescriptorCreateCollectionPrimitiveInput(BaseModel):
...
class AttributeTypeDescriptorCreateCollectionPrimitiveOutput(BaseModel):
    value: AttributeTypeDescriptor
...
FUNCTIONS = {
    "AttributeTypeDescriptor": {
        ...
        "create_collection_primitive": {
            "canonical": {
                "name": "create_collection_primitive",
                "description": "Create deterministic collection descriptor whose element is one primitive descriptor.\n\nContract:\n- The collection descriptor is keyed by collection kind and its element descriptor link.\n- V0 intentionally supports primitive elements only; enum/class/nested collections get their own typed rails.",
                "is_constructor": True,
            },
            "input": AttributeTypeDescriptorCreateCollectionPrimitiveInput,
            "output": AttributeTypeDescriptorCreateCollectionPrimitiveOutput,
        },
        "create_child_link": {
            "canonical": {
...
__all__ = [
    ...
    "AttributeTypeDescriptorCreateCollectionPrimitiveInput",
    "AttributeTypeDescriptorCreateCollectionPrimitiveOutput",
