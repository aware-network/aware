...
class MetaGraphViewportStateV1(BaseModel):
    # Attributes
    scale: float = Field(default=1.0)
    offset_x: float = Field(default=0.0)
    offset_y: float = Field(default=0.0)
    focused_identity: str | None = Field(default=None)


class MetaGraphCanvasViewStateV1(BaseModel):
    # Attributes
    ...
    viewport_state: MetaGraphViewportStateV1
    summary: str | None = Field(default=None)
    empty_message: str = Field(default="No Meta graph snapshot available")
...
class MetaGraphSelectIdentityViewActionRequest(BaseModel):
    # Attributes
    actor_id: UUID | None = Field(default=None)
    domain_branch_id: UUID
    domain_projection_hash: str
    ...
    viewport_state: MetaGraphViewportStateV1 | None = Field(default=None)
    metadata: JsonObject = Field(default_factory=JsonObject)


class MetaGraphSelectIdentityViewActionResponse(BaseModel):
    # Attributes
    ...
    object_instance_graph_commit_id: UUID | None = Field(default=None)
    selected_identity: str | None = Field(default=None)
    graph_canvas_view_state: MetaGraphCanvasViewStateV1 | None = Field(default=None)
    provenance: JsonObject = Field(default_factory=JsonObject)
    error: str | None = Field(default=None)
