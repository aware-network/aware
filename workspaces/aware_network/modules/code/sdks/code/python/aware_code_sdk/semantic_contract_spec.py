...
def normalize_code_semantic_contract(
    ...
    return CodeSemanticContract(
        ...
        grammar_rule_declarations=[
            CodeSemanticGrammarRuleDescriptor(
                semantic_owner=_required_attr(item, "semantic_owner"),
                rule_name=_required_attr(item, "rule_name"),
                language=_str_attr(item, "language", default="aware"),
                grammar_backend=_str_attr(
                    item,
                    "grammar_backend",
                    default="tree_sitter.aware",
                ),
                top_level=_bool_attr(item, "top_level", default=False),
                section_type=_optional_str_attr(item, "section_type"),
                fields=[
                    CodeSemanticGrammarRuleFieldDescriptor(
                        field_path=_required_attr(field, "field_path"),
                        field_role=_optional_str_attr(field, "field_role"),
                        value_kind=_optional_str_attr(field, "value_kind"),
                        required=_bool_attr(field, "required", default=False),
                        child_rule_refs=_list_attr(field, "child_rule_refs"),
                        token_literals=_list_attr(field, "token_literals"),
                        provider_payload=_json_object_or_none(
                            _attr(field, "provider_payload"),
                        ),
                    )
                    for field in _items_attr(item, "fields")
                ],
                child_rule_refs=_list_attr(item, "child_rule_refs"),
                literal_tokens=_list_attr(item, "literal_tokens"),
                source_anchor_fields=_list_attr(item, "source_anchor_fields"),
                generation_status=_str_attr(
                    item,
                    "generation_status",
                    default="declaration_only",
                ),
                priority=_int_attr(item, "priority", default=100),
                provider_payload=_json_object_or_none(
                    _attr(item, "provider_payload"),
                ),
            )
            for item in _items_attr(semantic_contract, "grammar_rule_declarations")
        ],
        package_roles=[
            CodeSemanticPackageRoleDescriptor(
