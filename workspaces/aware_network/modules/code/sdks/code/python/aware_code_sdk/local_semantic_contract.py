...
_LAYOUT_CONTROL_METADATA_KEYS = frozenset(
    {
...
class CodeSdkPackageLayoutBinding:
    ...
    manifest: CodePackageManifestContract | None = None
    semantic_binding: CodeSemanticPackageBindingContract | None = None
    config_binding: CodePackageConfigBindingContract | None = None
...
class CodeSdkSemanticContractCatalog:
    ...
    def validate_layout_contract(
        ...
        for key in sorted(_LAYOUT_CONTROL_METADATA_KEYS):
            if key in metadata:
                diagnostics.append(
                    "layout metadata must not carry control-plane key: "
                    f"{key!r}."
                )
        return tuple(diagnostics)
        ...
        *,
        ...
        for manifest_path in manifest_paths:
            ...
            if layout_contract is None:
                diagnostics.append(
                    "Code SDK package layout catalog has no declared layout "
                    "contract for manifest path: "
        ...
        *,
        ...
        for layout in self._layouts_by_key.values():
            layout_manifest_path = (
                layout.manifest.manifest_relative_path
                if layout.manifest is not None
                else layout.manifest_relative_path
            )
            if _optional_key(layout_manifest_path) == manifest_path:
                return layout
        return None
...
class LocalCodeSdkSemanticContractCapabilityClient:
    ...
    async def describe(
        ...
        if request.include_layout and layout_contract is None:
            return DescribeCodeSemanticContractResponse(
                request_id=request.request_id,
                success=False,
                error=(
                    "Code SDK package layout catalog has no declared layout "
                    "contract for requested semantic package."
                ),
                semantic_contract=contract,
    ...
    async def describe(
        ...
        if layout_contract is None:
            return DescribeCodePackageLayoutResponse(
                ...
                error=(
                    "Code SDK package layout catalog has no declared layout "
                    "contract for requested package."
                ),
...
class LocalCodeSdkSourceOwnershipCapabilityClient:
    async def classify(
        self,
        request: ClassifyCodeSourceOwnershipRequest,
    ) -> ClassifyCodeSourceOwnershipResponse:
        return _classify_source_ownership_response(request)


@dataclass(frozen=True, slots=True)
class LocalCodeSdkSourceOwnershipProvider:
    def classify_source_ownership(
        self,
        request: ClassifyCodeSourceOwnershipRequest | None = None,
        *,
        workspace_root: str | None = None,
        package_bindings: Iterable[CodeSourceOwnershipPackageBinding] = (),
        observed_paths: Iterable[CodeSourceOwnershipObservedPath] = (),
        strict: bool = True,
    ) -> ClassifyCodeSourceOwnershipResponse:
        ...
        return _classify_source_ownership_response(resolved_request)
...
class LocalCodeSdkGrammarProfileCapabilityClient:
    async def resolve(
        ...
        return ResolveCodeGrammarProfileResponse(
            ...
            lane_count=sum(
                len(contract.syntax_lanes) for contract in request.semantic_contracts
            ),
            rule_count=0,
        )
...
class LocalCodeSdkCodeApiClient:
    def __init__(self, *, catalog: CodeSdkSemanticContractCatalog) -> None:
        self.grammar_anchor_binding = LocalCodeSdkGrammarAnchorBindingCapabilityClient()
        ...
        self.package_layout = LocalCodeSdkPackageLayoutCapabilityClient(catalog=catalog)
        self.source_ownership = LocalCodeSdkSourceOwnershipCapabilityClient()


class LocalCodeSdkApiClient:
    def __init__(
        self, *, catalog: CodeSdkSemanticContractCatalog | None = None
    ) -> None:
        self.catalog = catalog or CodeSdkSemanticContractCatalog()
        self.code = LocalCodeSdkCodeApiClient(catalog=self.catalog)

    def source_ownership_provider(self) -> LocalCodeSdkSourceOwnershipProvider:
        return LocalCodeSdkSourceOwnershipProvider()
...
def _classify_source_ownership_response(
    request: ClassifyCodeSourceOwnershipRequest,
) -> ClassifyCodeSourceOwnershipResponse:
    ownership_request = request.ownership_request
    normalized_bindings, diagnostics = _normalize_source_ownership_bindings(
        bindings=tuple(ownership_request.package_bindings),
        strict=ownership_request.strict,
    )
    matches = [
        _source_ownership_match(
    ...
    return ClassifyCodeSourceOwnershipResponse(
        ...
        ownership_result=CodeSourceOwnershipResult(
            ...
            source_owned_path_count=sum(
                ...
                is CodeSourceOwnershipClassification.source_owned
            ),
...
@dataclass(frozen=True, slots=True)
class _SourceOwnershipCandidate:
...
def _normalize_source_ownership_bindings(
    *,
    bindings: tuple[CodeSourceOwnershipPackageBinding, ...],
    strict: bool,
) -> tuple[tuple[CodeSourceOwnershipPackageBinding, ...], list[str]]:
    normalized: list[CodeSourceOwnershipPackageBinding] = []
    diagnostics: list[str] = []
    for index, binding in enumerate(bindings):
        package_root = _normalize_relative_path_or_none(binding.package_root)
        sources_root = (
            _normalize_relative_path_or_none(binding.sources_root) or package_root
        )
        ...
        normalized.append(
            binding.model_copy(
                update={
                    ...
                    "manifest_relative_path": _normalize_relative_path_or_none(
                        binding.manifest_relative_path
                    ),
                    "generated_roots": sorted(
                        _normalize_relative_path_or_none(root) or "."
                    ...
                    "owned_file_paths": sorted(
                        ...
                        for normalized_path in (
                            _normalize_relative_path_or_none(path),
                        )
...
def _source_ownership_match(
    *,
    bindings: tuple[CodeSourceOwnershipPackageBinding, ...],
    observed_path: CodeSourceOwnershipObservedPath,
) -> CodeSourceOwnershipPathMatch:
    path = _normalize_relative_path_or_none(observed_path.path)
    if path is None:
    ...
    best: _SourceOwnershipCandidate | None = None
    for index, binding in enumerate(bindings):
        candidate = _source_ownership_candidate(
            binding=binding,
            binding_index=index,
            path=path,
        )
...
def _source_ownership_candidate(
    *,
    binding: CodeSourceOwnershipPackageBinding,
    binding_index: int,
    path: str,
) -> _SourceOwnershipCandidate | None:
    owned_paths = frozenset(binding.owned_file_paths)
    package_relative_from_owned_path = (
        _source_ownership_package_relative_path(binding=binding, path=path)
        if path in owned_paths
        else None
    )
    if package_relative_from_owned_path is not None:
        return _SourceOwnershipCandidate(
            binding=binding,
    ...
    if binding.package_root == "." and owned_paths:
        return None
    ...
    if package_relative_from_root is None:
        return None
    sources_root = binding.sources_root or binding.package_root
    ...
    if package_relative_from_sources is not None and not _is_generated_fallout_path(
        ...
        return _SourceOwnershipCandidate(
            ...
            score=(3, _path_depth(sources_root)),
        )
    if sources_root != binding.package_root or _is_generated_fallout_path(
        path=path,
        relative_path=package_relative_from_root,
        binding=binding,
    ):
...
def _source_ownership_package_relative_path(
    *,
    binding: CodeSourceOwnershipPackageBinding,
    path: str,
) -> str | None:
...
def _relative_path_under_root(*, path: str, root: str | None) -> str | None:
    normalized_path = _normalize_relative_path_or_none(path)
    normalized_root = _normalize_relative_path_or_none(root) or "."
...
def _normalize_relative_path_or_none(value: str | None) -> str | None:
    if value is None:
        return None
...
def _path_depth(path: str | None) -> int:
    return len(
        [part for part in Path(path or ".").parts if part not in {"", "."}]
    )


def _layout_manifest_contract(
    *,
    manifest_kind: str | None,
    manifest_relative_path: str | None,
    language: CodeLanguage | str | None,
) -> CodePackageManifestContract | None:
    if manifest_kind is None or manifest_relative_path is None or language is None:
        return None
    resolved_manifest_kind = manifest_kind.strip()
    resolved_manifest_path = manifest_relative_path.strip()
    if not resolved_manifest_kind or not resolved_manifest_path:
        return None
    try:
        resolved_language = (
            language if isinstance(language, CodeLanguage) else CodeLanguage(language)
        )
    except ValueError:
        return None
    return CodePackageManifestContract(
        manifest_kind=resolved_manifest_kind,
        manifest_relative_path=resolved_manifest_path,
...
def _default_path_roles(
    *,
    ...
    source_prefix = _package_relative_path(
        package_root=package_root, value=sources_root
    )
    authored_patterns = (
        [
...
def _layout_keys(layout: CodePackageLayoutContract) -> tuple[str, ...]:
    ...
    return tuple(
        ...
        for key in (
            ...
            _optional_key(layout.package_fqn),
            _optional_key(manifest_relative_path),
...
def _compact_metadata(values: Mapping[str, object | None]) -> dict[str, object]:
...
__all__ = [
    ...
    "LocalCodeSdkSourceOwnershipCapabilityClient",
    "LocalCodeSdkSourceOwnershipProvider",
    "PackageLayoutInput",
