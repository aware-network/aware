...
def source_ownership_package_binding_from_layout_contract(
    ...
    manifest = layout_contract.manifest
    language = manifest.language.value if manifest is not None else None
    manifest_kind = manifest.manifest_kind if manifest is not None else None
    manifest_relative_path = (
        manifest.manifest_relative_path
        if manifest is not None
        else layout_contract.manifest_relative_path
    )
    return CodeSourceOwnershipPackageBinding(
        ...
        owned_file_paths=list(layout_contract.owned_file_paths),
