...
class ServiceBackedCodeSdkSemanticOwnershipProvider:
    ...
    def semantic_provider_ownerships_for_manifest_kind(
        ...
        *,
        ...
        return tuple(
            _semantic_provider_ownership_from_mapping(item) for item in raw_items
        )
...
@dataclass(frozen=True, slots=True)
class LocalCatalogCodeSdkSemanticOwnershipProvider:
    """Semantic ownership provider over local CodeSemanticContract DTOs."""

    semantic_contracts: tuple[object, ...] = ()
    ...
    def semantic_provider_ownerships_for_manifest_kind(
        ...
        *,
        ...
        for contract in self.semantic_contracts:
            provider_key = _required_attr_str(contract, "provider_key")
            semantic_contract_module = _metadata_string(
                getattr(contract, "metadata", None),
                "semantic_contract_module",
            )
            package_roles = tuple(getattr(contract, "package_roles", ()) or ())
            for descriptor in tuple(getattr(contract, "manifest_resolution", ()) or ()):
                descriptor_manifest_kind = _optional_attr_str(
                    descriptor,
                    "manifest_kind",
                )
                if descriptor_manifest_kind != manifest_kind_value:
                    continue
                package_role_name = _optional_attr_str(
                    descriptor,
                    "package_role",
                ) or _optional_attr_str(descriptor, "semantic_owner")
                package_role = _package_role_by_name(
                    package_roles=package_roles,
                    role=package_role_name,
                )
                if package_role is None:
                    continue
                ownership = _semantic_provider_ownership_from_contract_role(
                    provider_key=provider_key,
                    semantic_contract_module=semantic_contract_module,
                    package_role=package_role,
                    manifest_kind=manifest_kind_value,
                    code_package_surface=_optional_attr_str(
                        descriptor,
        ...
        return tuple(
            sorted(
                ownerships.values(),
                key=lambda item: (
                    item.owned_manifest_kind_count,
                    item.provider_key,
                    item.role,
                    item.module,
                ),
...
def _semantic_provider_ownership_key(
    ownership: CodeSdkSemanticProviderOwnership,
) -> tuple[str, str, str, str]:
    return (
...
def _semantic_provider_ownership_from_contract_role(
    *,
    provider_key: str,
    ...
    code_package_surface: str | None,
    code_package_surface_by_package_kind: Mapping[str, str] | None,
) -> CodeSdkSemanticProviderOwnership | None:
    if _optional_attr_str(package_role, "contract") != "aware.semantic_provider":
        return None
    owned_manifest_kinds = tuple(
        item.strip()
        for item in tuple(getattr(package_role, "owns_manifest_kinds", ()) or ())
        if isinstance(item, str) and item.strip()
    )
    if manifest_kind not in owned_manifest_kinds:
        return None
    return CodeSdkSemanticProviderOwnership(
        provider_key=provider_key,
        role=_required_attr_str(package_role, "role"),
        contract="aware.semantic_provider",
        module=semantic_contract_module,
        code_package_surface=code_package_surface
        or _optional_attr_str(package_role, "package_kind"),
        code_package_surface_by_package_kind=code_package_surface_by_package_kind,
        owned_manifest_kind_count=len(owned_manifest_kinds),
    )


def _package_role_by_name(
    *,
    package_roles: tuple[object, ...],
    role: str | None,
) -> object | None:
    if role is None:
        return None
    for package_role in package_roles:
        if _optional_attr_str(package_role, "role") == role:
            return package_role
    return None


def _required_attr_str(value: object, name: str) -> str:
    text = _optional_attr_str(value, name)
    if text is None:
        raise ValueError(f"semantic ownership object requires {name!r}.")
    return text


def _optional_attr_str(value: object, name: str) -> str | None:
    raw = getattr(value, name, None)
    return raw if isinstance(raw, str) and raw.strip() else None


def _attr_mapping(value: object, name: str) -> Mapping[str, object]:
    raw = getattr(value, name, None)
    return {"value": raw} if isinstance(raw, Mapping) else {}
...
__all__ = [
    ...
    "CodeSdkSemanticProviderOwnership",
    "LocalCatalogCodeSdkSemanticOwnershipProvider",
