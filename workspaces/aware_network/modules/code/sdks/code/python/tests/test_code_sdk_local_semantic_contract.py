...
@pytest.mark.asyncio
async def test_local_code_sdk_package_layout_describe_requires_declared_layout() -> None:
    api_client = build_local_code_sdk_api_client()
...
@pytest.mark.asyncio
async def test_local_code_sdk_layout_validation_rejects_control_metadata() -> None:
    api_client = build_local_code_sdk_api_client()

    response = await api_client.code.package_layout.validate(
        ValidateCodePackageLayoutRequest(
            layout_contract=CodePackageLayoutContract(
                package_root="modules/demo",
                manifest_relative_path="modules/demo/pyproject.toml",
                metadata=JsonObject(
                    {
    ...
    assert response.diagnostics == [
        "layout metadata must not carry control-plane key: 'fqn_prefix'.",
        "layout metadata must not carry control-plane key: 'language'.",
        "layout metadata must not carry control-plane key: 'manifest_kind'.",
        "layout metadata must not carry control-plane key: 'package_fqn'.",
    ]
