"""CodePackage semantic contract execution resolver."""
...
_PACKAGE_SEMANTIC_CONTRACT_CAPABILITY_FUNCTIONS: dict[str, str] = {
...
class ResolvedPackageSemanticContractProvider:
    capability: str
    provider_key: str
    semantic_owner: str
    contract: str
...
def package_semantic_contract_callable_name(*, capability: str) -> str | None:
    return _PACKAGE_SEMANTIC_CONTRACT_CAPABILITY_FUNCTIONS.get(capability)
...
def package_semantic_contract_provider_descriptors(
    *,
    ...
    if (
        ...
        or package_semantic_contract_callable_name(capability=capability) is None
    ):
        return ()
...
def resolve_package_semantic_contract_providers(
    *,
...
) -> tuple[ResolvedPackageSemanticContractProvider, ...]:
    if (
        semantic_contract is None
        or package_semantic_contract_callable_name(capability=capability) is None
    ):
    ...
    if (
        not selected_provider_keys
        or semantic_contract.role not in selected_provider_keys
    ):
        return ()

    module_runtime_root = (
        workspace_root.resolve() / module_root_relative_path / "runtime"
    ).resolve()
    resolved: list[ResolvedPackageSemanticContractProvider] = []
    for binding in semantic_contract.bindings:
        if binding.capability != capability:
            continue
        module = _import_contract_binding_module(
        ...
        if not callable(provider):
            continue
        resolved.append(
            ResolvedPackageSemanticContractProvider(
                capability=capability,
                provider_key=semantic_contract.role,
                semantic_owner=semantic_contract.role,
...
def _import_contract_binding_module(
    *,
    module_name: str,
    module_runtime_root: Path,
) -> ModuleType | None:
