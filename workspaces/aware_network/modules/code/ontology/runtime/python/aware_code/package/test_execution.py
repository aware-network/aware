...
def _python_pytest_subprocess_env(
    *,
    ...
    pythonpath_entries = [
        ...
        *_aware_test_runner_pythonpath_entries(),
    ]
...
def _aware_test_runner_pythonpath_entries() -> tuple[str, ...]:
    if find_spec("aware_test_runner") is not None:
        return ()
    for parent in Path(__file__).resolve().parents:
        candidate = parent / "tools" / "test-runner"
        if (candidate / "aware_test_runner" / "core" / "collector.py").is_file():
