...
from aware_code.package.semantic_contract import (
...
from aware_code.language_service.workspace_contracts import (
...
class CapabilityExecutionScope:
    ...
    def allows(self, *, descriptor: CapabilityProviderDescriptor) -> bool:
        if (
            self.semantic_owners is not None
            and descriptor.semantic_owner not in self.semantic_owners
        ):
            return False
        if (
            self.provider_keys is not None
            and descriptor.provider_key not in self.provider_keys
        ):
            return False
        return True
...
class WorkspaceCapabilityConfiguration:
    diagnostics: WorkspaceCapabilitySelection = field(
        default_factory=WorkspaceCapabilitySelection
    )
    semantic_tokens: WorkspaceCapabilitySelection = field(
        default_factory=WorkspaceCapabilitySelection
    )
...
class CapabilityExecutionPlan:
    ...
    workspace_binding_provider_descriptors: tuple[
...
class CapabilityProviderExecutionContext:
    ...
    @property
    def workspace_semantic_scope_resolutions(
        self,
    ...
    def semantic_scope_resolution(
        self, *, scope_key: str
    ) -> SemanticScopeResolution | None:
    ...
    def nearest_manifest_path(self, *, filename: str | None = None) -> Path | None:
        ...
        if filename is not None and (
            manifest_path is None or manifest_path.name != filename
        ):
            return None
...
def build_capability_execution_plan(
    *,
    ...
    workspace_package_resolution: CodePackagePathResolution | None = None,
    workspace_semantic_scope_resolutions: (
        Iterable[SemanticScopeResolution] | None
    ) = None,
    workspace_binding_provider_descriptors: (
        Iterable[CapabilityProviderDescriptor] | None
    ) = None,
    available_provider_keys: Iterable[str] | None = None,
) -> CapabilityExecutionPlan:
    descriptor_tuple = tuple(descriptors)
    package_provider_keys = _normalize_scope_values(
        workspace_semantic_package_provider_keys
    )
    ...
    binding_provider_descriptor_tuple = tuple(
        workspace_binding_provider_descriptors or ()
    )
    available_provider_key_set = _normalize_scope_values(available_provider_keys)
    available_semantic_scope_keys = frozenset(
        resolution.scope_key for resolution in semantic_scope_resolution_tuple
    )
    ...
    effective_scope = CapabilityExecutionScope.from_iterables(
        semantic_owners=[
            descriptor.semantic_owner for descriptor in active_descriptors
        ],
...
def build_capability_execution_plan_from_workspace_owner_plan(
    *,
    ...
    workspace_owner_plan: CodeLanguageServiceCapabilityOwnerPlan,
    ...
    binding_provider_descriptors = (
        _workspace_module_package_binding_provider_descriptors(
            capability=capability,
    ...
    return build_capability_execution_plan(
        ...
        workspace_owner_scope=(
            CapabilityExecutionScope.from_iterables(
...
def _descriptor_activated_by_workspace_owner_scope(
    *,
    ...
    if descriptor.workspace_activation == "always":
        return True
    owner_scope = (
        workspace_owner_scope.semantic_owners
        if workspace_owner_scope is not None
        else None
    )
