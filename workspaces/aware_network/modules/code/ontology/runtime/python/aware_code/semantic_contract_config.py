...
def source_code_package_config_ref(
    *,
    ...
    resolved_surface = _required_code_package_surface(surface)
...
def source_code_package_config_descriptor(
    *,
    ...
    return code_package_config_descriptor_from_manifest_resolution_descriptor(
        ...
        declared_surface=resolved_surface,
    )
    ...
    *,
    ...
    if normalized_package_kind is not None and isinstance(surface_by_kind, Mapping):
        ...
        if by_kind is not None:
            return by_kind
    declared = _code_package_surface(declared_surface)
    if declared is not None:
        return declared
    manifest_kind = _required_text_attribute(descriptor, "manifest_kind")
    raise ValueError(
        "Semantic manifest-resolution descriptor must declare a CodePackage surface "
        "via code_package_surface, code_package_surface_by_package_kind, or a typed "
        f"caller-declared surface (manifest_kind={manifest_kind!r}, "
        f"package_kind={normalized_package_kind!r})"
    )


def _required_code_package_surface(value: str | None) -> str:
    resolved = _code_package_surface(value)
    if resolved is not None:
        return resolved
    raise ValueError("CodePackageConfig surface is required.")
