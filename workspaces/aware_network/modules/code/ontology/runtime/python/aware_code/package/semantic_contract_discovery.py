...
_WORKSPACE_TOML_REGISTRY_SOURCE = "workspace_toml"
_IGNORED_SEGMENTS = frozenset(
    {".aware", ".git", "__pycache__", "node_modules", ".venv", "venv"}
)


class _WorkspaceRegistryEntry(Protocol):
    resolved_path: Path


class _WorkspaceRegistry(Protocol):
    source: object
    entries: tuple[_WorkspaceRegistryEntry, ...]
    module_package_manifest_paths: tuple[Path, ...]


_WorkspaceRegistryLoader = Callable[..., _WorkspaceRegistry | None]
...
class SemanticContractCodePackageDiscovery(CodePackageDiscovery):
    ...
    def __init__(
        ...
        *,
        ...
        workspace_registry_loader: _WorkspaceRegistryLoader | None = None,
    ) -> None:
        self._require_workspace_membership = require_workspace_membership
        self._workspace_registry_loader = workspace_registry_loader
        self._cache: dict[tuple[str, str], _ResolvedSemanticContractPackage] = {}
        self._workspace_registry_by_root: dict[Path, _WorkspaceRegistry | None] = {}
    ...
    def _ensure_manifest_is_workspace_declared(self, *, manifest_path: Path) -> None:
        ...
        if (
            registry is None
            or _workspace_registry_source_value(registry)
            != _WORKSPACE_TOML_REGISTRY_SOURCE
        ):
            raise ValueError(
    ...
    def _manifest_is_declared_in_workspace_registry(
        ...
        *,
        manifest_path: Path,
        registry: _WorkspaceRegistry | None,
    ) -> bool:
        if (
            registry is None
            or _workspace_registry_source_value(registry)
...
def _workspace_registry_loader() -> _WorkspaceRegistryLoader:
    try:
        registry_module = import_module("aware_workspace.registry")
    except ModuleNotFoundError as exc:
        if exc.name != "aware_workspace":
            raise
        raise RuntimeError(
            "Workspace membership validation requires the `aware-workspace` "
            "package. Disable `require_workspace_membership` for isolated Code "
            "package discovery, or run through a project that declares "
            "`aware-workspace`."
        ) from exc
    return registry_module.load_aware_workspace_registry


def _workspace_registry_source_value(registry: _WorkspaceRegistry) -> str:
    ...
    *,
    ...
    for key in descriptor.copy_code_package_metadata_keys:
        ...
        if value is not None:
            metadata[key] = value
    metadata.update(
        _manifest_dependency_package_metadata(
            manifest_spec=manifest_spec,
...
def _manifest_dependency_package_metadata(
    *,
    manifest_spec: object,
    descriptor_metadata: Mapping[str, object],
) -> dict[str, object]:
    dependency_attribute_name = _optional_text(
        descriptor_metadata.get("dependency_attribute_name")
    )
    if dependency_attribute_name is None:
        return {}
    dependency_package_names = _dependency_package_names(
        getattr(manifest_spec, dependency_attribute_name, ()),
    )
    if not dependency_package_names:
        return {}
    return {
        "dependency_package_names": dependency_package_names,
        "package_dependencies": dependency_package_names,
    }


def _dependency_package_names(value: object) -> tuple[str, ...]:
    ...
    for item in values:
        if isinstance(item, str):
            ...
            package_name = str(getattr(item, "package_name", "") or "").strip()
