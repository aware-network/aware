...
class CompletionMixin(ServiceMixinBase, SegmentMixin):
    _snapshot: CodeLanguageServiceSnapshot | None
    ...
    def _program_call_target_completion_items(
        self, *, prefix: str
        ...
        def _add(
            label: str, *, kind: int | None = None, detail: str | None = None
        ) -> None:
            key = (label, kind)
            if key in seen:
                return
        ...
        res = resolve_owner_to_class(
            owner=base, classes_by_fqn=self._snapshot.fqn_resolver.classes_by_fqn
        )
    ...
    def completion(
        self, *, uri: str, position: Utf16Position, document_text: str
    ) -> CompletionResultDict:
        ...
        seg = self._find_completion_segment_at(
            uri=uri, byte_offset=offset, document_bytes=doc_bytes
        )
        if seg is None:
            ...
            try:
                ...
                if call_at is None and offset > 0:
                    call_at = find_program_call_target_at(
                        root=root, byte_offset=offset - 1
                    )
            except Exception:
                call_at = None
            if call_at is None:
        ...
        if seg.kind == "annotation_args":
            return {
                "isIncomplete": False,
                "items": self._annotation_args_completion_items(
                    uri=uri, byte_offset=offset, document_bytes=doc_bytes
                ),
            }
    ...
    def _default_value_completion_items(
        ...
        *,
        ...
        if (
            attr is None
            or not isinstance(attr.type_text, str)
            or not attr.type_text.strip()
        ):
            return []
        ...
        if prim is not None:
            ...
            if prim.base_type == CodePrimitiveBaseType.json:
                kind = None
                constraints = (
                    prim.constraints if isinstance(prim.constraints, dict) else None
                )
    ...
    def _annotation_args_completion_items(
        ...
        prev = (
            (args[arg_index - 1].text or "").strip().lower()
            if arg_index > 0 and args
            else ""
        )
        ...
        if verb == "identity":
            ...
            structural_token = (
                (args[1].text or "").strip().lower() if len(args) >= 2 else ""
            )
        ...
        if verb == "reference":
            ...
            if prev == "bind":
                if token_idx is not None:
                    ...
                    path_prefix = ""
                return self._annotation_path_completion_items(
                    uri=uri, prefix=path_prefix
                )
        ...
        if verb == "oneof":
            ...
            args_texts = [
                (tok.text or "").strip() for tok in args if (tok.text or "").strip()
            ]
            mode: str | None = None
            mode_offset = 0
            ...
            if effective_mode != "identity":
                selected_members = {
                    text.casefold() for text in args_texts[mode_offset:]
                }
            ...
            discriminator_positions = [
                idx
                for idx, text in enumerate(identity_tokens)
                if text.casefold() == "discriminator"
            ]
            discriminator_pos = (
                discriminator_positions[0] if discriminator_positions else None
            )
            ...
            member_names = [
                text for text in member_tokens if text.casefold() != "discriminator"
            ]
            ...
            discriminator_attr_name = (
                identity_tokens[discriminator_pos + 1]
                if len(identity_tokens) > discriminator_pos + 1
                else ""
            )
            if mapping_slot % 2 == 0:
                ...
                enum_options = (
                    getattr(enum_config, "enum_options", ())
                ...
                if not variant_values:
                    discriminator_attr = next(
                        (
                            attr
                            for attr in cls.code_section_attributes
                            if (attr.name or "").strip() == discriminator_attr_name
                        ),
                        None,
                    )
                    discriminator_type_ref = (
                        (discriminator_attr.type_text or "").strip()
                    ...
                    if discriminator_type_ref:
                        ...
                        if "." not in discriminator_type_ref and code_ns is not None:
                            enum_candidates.append(
                                f"{code_ns.prefix()}.{discriminator_type_ref}"
                            )
                        resolved_enum = None
                        for enum_candidate in enum_candidates:
                            resolved_enum = scope.try_resolve_enum_with_fqn(
                                enum_candidate
                            )
                            if resolved_enum is not None:
                                break
    ...
    def _annotation_path_completion_items(
        self, *, uri: str, prefix: str
    ) -> list[CompletionItemDict]:
        ...
        parts = prefix.split("::")
        if len(parts) <= 1:
            return self._completion_items_for_prefix(
                uri=uri, prefix=prefix, mode="annotation_path"
            )
    ...
    def _completion_items_for_prefix(
        ...
        *,
        ...
        prefix: str,
        mode: Literal[
            "type", "import_module", "import_alias", "annotation_path"
        ] = "type",
    ) -> list[CompletionItemDict]:
        if self._snapshot is None:
            return []
        ...
        raw = (prefix or "").strip()
        if "." not in raw:
            # Unqualified name completion: local namespace symbols + import aliases.
            ...
            try:
                ...
                for primitive_base in primitive_bases:
                    try:
                        label = plugin.primitive_codec.render(
                            CodePrimitiveType(base_type=primitive_base)
                        )
                    except Exception:
                        continue
                    if label is None:
        ...
        if mode in ("type", "annotation_path") and "." not in base:
            # Cross-package shorthand: `dep_pkg.<entry>.<Name>` omits the namespace group.
            # Offer entry completions when the prefix is a known package namespace.
            if base in package_prefixes:
                ...
                for entry in sorted(entries):
                    if partial and not entry.startswith(partial):
                        continue
                    _add(entry, kind=9, detail=f"{base}.{entry}")
            else:
                _add_namespace_entry_completions(
                    add=_add,
        ...
        # Cross-package shorthand leaf completion: `dep_pkg.entry.<Name>`
        if mode in ("type", "annotation_path") and "." in base:
            ...
            if len(base_parts) == 2 and base_parts[0] in package_prefixes:
                pkg_prefix, entry = base_parts

                # Only suggest names that are unambiguous across groups for this (pkg, entry).
                ...
                for fqn, cfg in classes_by_fqn.items():
                    ...
                    if len(parts) != 4:
                        continue
                    if parts[0] != pkg_prefix or parts[2] != entry:
                        continue
                    name = parts[3]
                    if partial and not name.startswith(partial):
                        continue
        ...
        # Post-process: refine kinds for leaf symbols when completing at the namespace entry boundary.
        if candidate_prefixes and all(
            len(p.split(".")) >= 3 for p in candidate_prefixes
        ):
            # When prefix is pkg.group.entry, candidates can be class/enum names.
...
def _add_namespace_entry_completions(
    *,
    ...
    """Add `entry.Name` completion candidates across namespace groups in the current package."""
    base = (entry or "").strip()
    if not base:
        return
...
def _candidate_completion_prefixes(
    *,
    ...
    if (
        expanded
        and pkg
        and not expanded.startswith(pkg + ".")
        and expanded.split(".", 1)[0] != pkg
    ):
