...
from aware_code.language.contracts import CodeNamespaceGroup, StructuralFilterDecision
...
class CodeLanguagePlugin(Generic[T_Node]):
    ...
    layout_plugin: CodeLanguagePluginLayout | None = (
        None  # Namespace layout extraction logic
    )
    ...
    test_discovery: CodeLanguageTestDiscovery | None = (
        None  # Package-scoped test discovery logic
    )
    ...
    injected_structural_filter: (
        Callable[[str, str | None], StructuralFilterDecision | bool] | None
    ) = None
    ...
    def extract_metadata(
        self, section_type: CodeSectionType, raw_comment: str
    ) -> CodeSectionMetadata:
        """
        ...
        for directory_path in self._iter_candidate_directories(file_tree):
            try:
                if self.module_discovery.is_module_root(directory_path, workspace_root):
                    ...
                    logger.debug(
                        f"Discovered {self.language.value} module: {module_name} at {directory_path}"
                    )

            except Exception as e:
                logger.warning(
                    f"Failed to process potential module at {directory_path}: {e}"
                )
                continue
    ...
    def discover_packages(
        ...
        for directory_path in self._iter_candidate_directories(file_tree):
            try:
                if self.package_discovery.is_package_root(
                    directory_path, workspace_root
                ):
                    ...
                    package_info = SemanticPackageRegistry.enrich_code_package(
                        package_info
                    )
                    packages.append(package_info)
                    logger.debug(
                        f"Discovered {self.language.value} package: {package_name} at {directory_path}"
                    )

            except Exception as e:
                logger.warning(
                    f"Failed to process potential package at {directory_path}: {e}"
                )
                continue
    ...
    def discover_tests(
        self, context: CodeTestDiscoveryContext
    ) -> CodeTestDiscoveryResult:
    ...
    def get_namespace_groups(
        ...
        **_kwargs: object,
        ...
        """
        Extract namespace groups using layout-based discovery.
        ...
        try:
            groups = self.layout_plugin.extract_namespace_groups(
                file_paths,
                self.language,
                enforce_namespace_layout,
            )
            logger.debug(
                f"Layout plugin extracted {len(groups)} namespace groups for {self.language.value}"
            )
            return groups
        except Exception as e:
