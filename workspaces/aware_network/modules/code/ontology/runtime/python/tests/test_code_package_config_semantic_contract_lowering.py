...
def test_manifest_resolution_descriptor_requires_declared_surface() -> None:
    manifest_resolution = ModuleSemanticManifestResolutionDescriptor(
        semantic_owner=_FAKE_OWNER,
        manifest_kind="aware_unsurfaced_toml",
        filename="aware.unsurfaced.toml",
        contract="aware.unsurfaced",
        loader_module="aware_unsurfaced.manifest.loader",
        loader_name="load_aware_unsurfaced_toml_spec",
    )

    with pytest.raises(ValueError, match="must declare a CodePackage surface"):
        code_package_config_ref_from_manifest_resolution_descriptor(
    ...
    ref = code_package_config_ref_from_manifest_resolution_descriptor(
        descriptor=manifest_resolution,
        declared_surface="structure",
    )
    assert ref.config_key == "source:aware_unsurfaced_toml:structure"
