...
class _ReceiptContext:
    ...
    trigger_candidate_class_instance_identity_ids: set[UUID]
    class_instance_identity_id_by_instance_id: dict[UUID, UUID]
...
class LaneMaterializedConditionEvaluator:
    ...
    async def evaluate_with_trace(
        ...
        *,
        ...
        try:
            ...
            scoped_identity_filter_ids = set(
                binding.scoped_class_instance_identity_ids or []
            )
            allowed_instance_ids = None
            if scoped_identity_filter_ids:
                scoped_identity_ids = (
                    ctx.trigger_candidate_class_instance_identity_ids
                    & scoped_identity_filter_ids
                )
                if not scoped_identity_ids:
                    _trace_entry(
                        entries,
                        kind="binding",
                        path=f"binding:{binding.id}",
                        result=False,
                        reason="scoped_trigger_candidate_missing",
                        metadata={
                            ...
                            "trigger_candidate_class_instance_identity_ids": sorted(
                                str(item)
                                for item in (
                                    ctx.trigger_candidate_class_instance_identity_ids
                                )
                            ),
                            "scoped_class_instance_identity_ids": sorted(
                                str(item) for item in scoped_identity_filter_ids
                            ),
                        },
                    )
                ...
                scoped_trigger_candidates = {
                    instance_id
                    for (
                        instance_id,
                        class_instance_identity_id,
                    ) in ctx.class_instance_identity_id_by_instance_id.items()
                    if class_instance_identity_id in scoped_identity_ids
                }
                scoped_trigger_candidates &= ctx.trigger_candidate_instance_ids
                if not scoped_trigger_candidates:
                    _trace_entry(
                        entries,
                        kind="binding",
                        path=f"binding:{binding.id}",
                        result=False,
                        reason="scoped_trigger_instance_missing",
                        metadata={
                            "condition_config_id": str(binding.condition_config_id),
                            "scoped_class_instance_identity_ids": sorted(
                                str(item) for item in scoped_identity_filter_ids
                            ),
                            "matching_class_instance_identity_ids": sorted(
                                str(item) for item in scoped_identity_ids
                            ),
                        },
                    )
                    return ConditionEvaluationTrace(
                ...
                allowed_instance_ids = scoped_trigger_candidates
            ...
            _trace_entry(
                ...
                metadata={
                    ...
                    "trigger_candidate_class_instance_identity_ids": sorted(
                        str(item)
                        for item in ctx.trigger_candidate_class_instance_identity_ids
                    ),
                    "scoped_class_instance_identity_ids": sorted(
                        str(item) for item in scoped_identity_filter_ids
                    ),
    ...
    async def _load_bindings(
        ...
        *,
        ...
        try:
            event_scope_class_id = self._resolve_projection_class_config_id(
                index=index,
                opg=event_opg,
                class_name="EventConfigConditionConfigScope",
            )
        except RuntimeError:
            event_scope_class_id = None
        ...
        async for branch_id, head in self._commits.iter_lane_heads_by_projection(
            ...
            parent_condition_by_scope_id: dict[UUID, UUID] = {}
            ...
            for rel in relationships:
                ...
                if source_ci.class_config_id == event_config_class_id:
                    ...
                    continue
                if (
                    event_scope_class_id is not None
                    and source_ci.class_config_id == event_condition_class_id
                    and target_ci.class_config_id == event_scope_class_id
                ):
                    parent_condition_by_scope_id[target_id] = source_id

            scoped_instance_ids_by_condition_id: dict[UUID, set[UUID]] = defaultdict(
                set
            )
            if event_scope_class_id is not None:
                for ci in oig.class_instances:
                    if ci.class_config_id != event_scope_class_id or ci.id is None:
                        continue
                    attrs = self._decode_class_instance_attributes(ci)
                    condition_binding_id = _as_uuid(
                        attrs.get("event_config_condition_config_id")
                        or attrs.get("event_config_condition_config")
                    )
                    if condition_binding_id is None:
                        condition_binding_id = parent_condition_by_scope_id.get(ci.id)
                    class_instance_identity_id = _as_uuid(
                        attrs.get("class_instance_identity_id")
                        or attrs.get("class_instance_identity")
                    )
                    if (
                        condition_binding_id is not None
                        and class_instance_identity_id is not None
                    ):
                        scoped_instance_ids_by_condition_id[condition_binding_id].add(
                            class_instance_identity_id
                        )
            ...
            for ci in oig.class_instances:
                ...
                candidate = EventConditionBindingResolution(
                    ...
                    is_required=_as_bool(attrs.get("is_required"), default=False),
                    scoped_class_instance_identity_ids=sorted(
                        scoped_instance_ids_by_condition_id.get(ci.id, set()),
                        key=str,
                    ),
    ...
    async def _build_receipt_context(
        ...
        *,
        ...
        trigger_candidate_class_instance_identity_ids: set[UUID] = set()
        class_instance_identity_id_by_instance_id: dict[UUID, UUID] = {}
        if receipt.class_instance_identity_id is not None:
            trigger_candidate_class_instance_identity_ids.add(
                receipt.class_instance_identity_id
            )
            if receipt.root_object_id is not None:
                class_instance_identity_id_by_instance_id[receipt.root_object_id] = (
                    receipt.class_instance_identity_id
                )
        return _ReceiptContext(
            ...
            trigger_candidate_class_instance_identity_ids=(
                trigger_candidate_class_instance_identity_ids
            ),
    ...
    def _candidate_instance_id_sets(
        ...
        *,
        ...
        if allowed_instance_ids is not None:
            ...
            scope_reason = "explicit_instance_scope"
        else:
