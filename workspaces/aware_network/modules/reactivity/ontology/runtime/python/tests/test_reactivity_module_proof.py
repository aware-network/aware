...
ACTION_CLASS_FQN = "aware_reactivity.action.Action"
...
ACTION_FEEDBACK_CLASS_FQN = "aware_reactivity.action.ActionFeedback"
ACTION_INTENT_CLASS_FQN = "aware_reactivity.action.ActionIntent"
CONDITION_CLASS_FQN = "aware_reactivity.condition.Condition"
CONDITION_CONFIG_CLASS_FQN = "aware_reactivity.condition.ConditionConfig"
CONDITION_CONFIG_ATTRIBUTE_CONFIG_CLASS_FQN = (
    "aware_reactivity.condition.ConditionConfigAttributeConfig"
)
CONDITION_CONFIG_CLASS_CONFIG_CLASS_FQN = (
    "aware_reactivity.condition.ConditionConfigClassConfig"
)
EVENT_CLASS_FQN = "aware_reactivity.event.Event"
...
@pytest.mark.asyncio
async def test_reactivity_scoped_binding_intersects_trigger_candidates() -> None:
    ...
    door_class_config_id = uuid4()
    front_door_instance_id = uuid4()
    front_door_identity_id = uuid4()
    back_door_instance_id = uuid4()
    back_door_identity_id = uuid4()
    graph_id = uuid4()
    state = _GraphState(
        class_instances_by_id={
            front_door_instance_id: ClassInstance(
                id=front_door_instance_id,
                object_instance_graph_id=graph_id,
                class_config_id=door_class_config_id,
                source_object_id=uuid4(),
            ),
            back_door_instance_id: ClassInstance(
                id=back_door_instance_id,
                object_instance_graph_id=graph_id,
                class_config_id=door_class_config_id,
                source_object_id=uuid4(),
            ),
        },
        class_instance_ids_by_class_config={
            door_class_config_id: {front_door_instance_id, back_door_instance_id}
        },
        attribute_by_id={},
        attribute_id_by_instance_and_config={},
        value_by_instance_and_config={},
        relationships_by_source_and_config={},
    )

    def evaluator_for_trigger(trigger_instance_id):
        evaluator = LaneMaterializedConditionEvaluator.__new__(
            LaneMaterializedConditionEvaluator
        )
        evaluator._init_error_logged = False
        evaluator._event_projection_hash = None
        evaluator._condition_projection_hash = None
        evaluator._bindings_by_id = {
            binding_id: EventConditionBindingResolution(
                id=binding_id,
                ...
                is_required=True,
                scoped_class_instance_identity_ids=[front_door_identity_id],
            )
        }
        evaluator._condition_policies_by_id = {
            condition_config_id: _ConditionPolicy(
                id=condition_config_id,
                is_enabled=True,
                logic_strategy="all",
                classes=(
                    _ClassPolicy(
        ...
        evaluator._descendants_by_class_config = {}

        async def ensure_loaded(*, force_refresh):
            _ = force_refresh

        async def build_context(*, receipt):
            _ = receipt
            identity_id = (
                front_door_identity_id
                if trigger_instance_id == front_door_instance_id
                else back_door_identity_id
            )
            return _ReceiptContext(
                pre=state,
                post=state,
                changes=_CommitChangeSet(
                    class_instance_ids={trigger_instance_id},
                    changed_attribute_ids=set(),
                    changed_instance_attr_configs=set(),
                    changed_relationship_keys=set(),
                ),
                trigger_candidate_instance_ids={trigger_instance_id},
                trigger_candidate_class_instance_identity_ids={identity_id},
                class_instance_identity_id_by_instance_id={
                    trigger_instance_id: identity_id
                },
            )

        evaluator._ensure_loaded = ensure_loaded
        evaluator._build_receipt_context = build_context
        return evaluator

    receipt = SimpleNamespace(projection_hash="projection:test")
    front_trace = await evaluator_for_trigger(
        front_door_instance_id
    ).evaluate_with_trace(
        receipt=receipt,
        event_config_condition_config_id=binding_id,
    )
    back_trace = await evaluator_for_trigger(back_door_instance_id).evaluate_with_trace(
        receipt=receipt,
        event_config_condition_config_id=binding_id,
    )

    assert front_trace.result is True
    assert back_trace.result is False
    assert back_trace.entries[-1].reason == "scoped_trigger_candidate_missing"


@pytest.mark.asyncio
async def test_reactivity_condition_event_and_action_projection_roots_registered(
    ...
    with IsolatedAwareRoot(
        ...
        action_opg = next(
            opg
            for opg in idx.ocg.object_projection_graphs
            if opg.name == "ActionConfig"
        )
        action_intent_opg = next(
            opg
            for opg in idx.ocg.object_projection_graphs
            if opg.name == "ActionIntent"
        )
        ...
        assert any(
            node.is_root for node in action_intent_opg.object_projection_graph_nodes
        )


@pytest.mark.asyncio
async def test_reactivity_event_config_portals_to_condition_and_action_registered(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    ...
    with IsolatedAwareRoot(
        ...
        event_portals = [
            ...
            if portal.source_projection_hash == event_opg.projection_hash
        ]
...
@pytest.mark.asyncio
async def test_reactivity_scope_projection_exposes_class_instance_identity_portal(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    ...
    with IsolatedAwareRoot(
        ...
        scope_portals = [
            portal
            for portal in idx.portal_index.portals
            if portal.source_projection_hash == scope_opg.projection_hash
        ]

        assert any(
            portal.reference_field_name == "class_instance_identity"
            and portal.target_projection_hash == oigi_opg.projection_hash
            for portal in scope_portals
        )


@pytest.mark.asyncio
async def test_reactivity_condition_config_create_module_proof(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    ...
    with IsolatedAwareRoot(
        ...
        result, assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_reactivity.condition.ConditionConfig",
        ...
        assertions.expect_primitive(
            instance_id=expected_condition_config_id, field_name="name", expected=name
        )
        assertions.expect_primitive(
...
@pytest.mark.asyncio
async def test_reactivity_condition_config_nested_policy_constructors_module_proof(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    from aware_reactivity_ontology.condition.condition_enums import (
    ...
    with IsolatedAwareRoot(
        ...
        assert runtime.context is not None
        idx = runtime.context.index
        condition_config_fqn = "aware_reactivity.condition.ConditionConfig"
        condition_class_config_fqn = (
            "aware_reactivity.condition.ConditionConfigClassConfig"
        )
        condition_attr_config_fqn = (
            "aware_reactivity.condition.ConditionConfigAttributeConfig"
        )

        condition_cfg_cc = next(
            cc
            for cc in idx.class_configs_by_id.values()
            if cc.name == "ConditionConfig"
        )
        condition_class_cfg_cc = next(
            cc
            for cc in idx.class_configs_by_id.values()
            if cc.name == "ConditionConfigClassConfig"
        )
        ...
        name_attr_cfg = next(
            ...
            and link.attribute_config.name == "name"
        )
        logic_attr_cfg = next(
            link.attribute_config
            for link in condition_cfg_cc.class_config_attribute_configs
            if link.attribute_config is not None
            and link.attribute_config.name == "logic_strategy"
        )
        ...
        if enum_config is None:
            enum_config = next(
                (
                    ...
                    and node.enum_config.id == enum_config_id
                ),
                None,
            )
...
@pytest.mark.asyncio
async def test_reactivity_event_config_add_condition_config_module_proof(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    ...
    with IsolatedAwareRoot(
        ...
        condition_config_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/condition-config"
        )
        ...
        result, assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_reactivity.event.EventConfig",
                    function_name="create",
        ...
        assertions.expect_primitive(
            instance_id=event_config_id, field_name="name", expected=event_name
        )
        assertions.expect_primitive(
        ...
        assertions.expect_primitive(
            instance_id=expected_binding_id, field_name="priority", expected=3
        )
        assertions.expect_primitive(
            instance_id=expected_binding_id, field_name="cache_result", expected=True
        )
        assertions.expect_primitive(
            instance_id=expected_binding_id,
...
@pytest.mark.asyncio
async def test_reactivity_event_config_add_action_config_module_proof(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    ...
    with IsolatedAwareRoot(
        ...
        api_capability_endpoint_id = uuid5(
            NAMESPACE_URL,
            "aware://tests/reactivity/event-config/action-endpoint",
        )
        ...
        _, action_assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_reactivity.action.ActionConfig",
                    function_name="create",
                    args=[action_name, action_description],
                    kwargs={
        ...
        action_assertions.expect_primitive(
            instance_id=action_config_id, field_name="name", expected=action_name
        )
        action_assertions.expect_primitive(
        ...
        _expect_uuid_primitive(
            ...
            expected=api_capability_endpoint_id,
        )
        ...
        assertions.expect_primitive(
            instance_id=expected_binding_id, field_name="priority", expected=2
        )
        assertions.expect_primitive(
            instance_id=expected_binding_id,
...
@pytest.mark.asyncio
async def test_reactivity_runtime_evidence_constructors_module_proof(
    tmp_path: Path, monkeypatch
) -> None:
    import aware_reactivity_ontology  # noqa: F401
    ...
    with IsolatedAwareRoot(
        ...
        lane = LaneIds(branch_id=uuid4())
        activation_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/activation"
        )
        trigger_commit_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/commit"
        )
        trigger_branch_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/branch"
        )
        trigger_opg = next(
            opg for opg in idx.ocg.object_projection_graphs if opg.name == "Condition"
        )
        ...
        condition_config_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/condition-config"
        )
        event_config_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/event-config"
        )
        ...
        action_config_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/action-config"
        )
        ...
        actor_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/actor"
        )
        target_actor_id = uuid5(
            NAMESPACE_URL, "aware://tests/reactivity/runtime-evidence/target-actor"
        )
        ...
        _, condition_assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_reactivity.condition.Condition",
                    function_name="create",
                    args=[
                        condition_config_id,
                        activation_id,
        ...
        event_result, event_assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_reactivity.event.Event",
                    function_name="create",
                    args=[
                        event_config_id,
                        activation_id,
        ...
        action_lane = LaneIds(branch_id=event_result.branch_id)
        action_result, action_assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_reactivity.action.Action",
        ...
        action_intent_lane = LaneIds(branch_id=action_result.branch_id)
        action_intent_result, action_intent_assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ...
                ProofCall(
                    target="instance",
                    class_fqn="aware_reactivity.action.ActionExecution",
        ...
        assert any(
            portal.reference_field_name == "actions"
            and portal.target_projection_hash == action_result.projection_hash
            for portal in event_portals
        )
        assert any(
...
@pytest.mark.asyncio
async def test_reactivity_evidence_writer_requires_receipt_object_instance_graph_id() -> (
    None
):
