...
@pytest.mark.asyncio
async def test_history_ocg_completeness_reports_warning_debt() -> None:
    response = await analyze_kernel_module_ocg_completeness("history")

    assert response.status == "succeeded"
    assert response.package_name == "history-ontology"
