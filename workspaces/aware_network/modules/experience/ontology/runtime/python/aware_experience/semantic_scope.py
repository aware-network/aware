...
EXPERIENCE_SEMANTIC_SCOPE_KEY = "aware_experience.semantic_scope"


@dataclass(frozen=True, slots=True)
class ExperienceSemanticScope:
    manifest_path: Path
...
def _experience_semantic_scope_payload(
    *,
    scope: ExperienceSemanticScope,
    workspace_root: Path,
) -> SemanticScopePayloadObject:
    payload: dict[str, SemanticScopePayloadValue] = {
        "experiencePackageName": scope.snapshot.spec.experience.package_name,
        "fqnPrefix": scope.snapshot.spec.experience.fqn_prefix,
        "environmentHandle": scope.snapshot.spec.build.environment_handle,
...
def load_experience_semantic_scope(
    *,
    manifest_path: Path,
    repo_root: Path | None = None,
) -> ExperienceSemanticScope:
    workspace = ExperienceWorkspace.from_toml(toml_path=manifest_path, repo_root=repo_root)
    snapshot = workspace.build_snapshot()
    return ExperienceSemanticScope(
...
def _experience_materialization_dependencies(
    *,
    scope: ExperienceSemanticScope,
    workspace_root: Path,
    ...
    return tuple(
        SemanticScopeMaterializationDependency(
            ...
            reason=(
                "Experience semantic materialization requires declared package "
                "dependencies before projection refs can resolve."
            ),
        )
...
class _ExperienceSemanticScopeProvider(SemanticScopeProvider):
    @property
    ...
    def scope_keys(self) -> tuple[str, ...]:
        return (EXPERIENCE_SEMANTIC_SCOPE_KEY,)
    ...
    def resolve(
        ...
        *,
        ...
        try:
            scope = load_experience_semantic_scope(
        ...
        return (
            SemanticScopeResolution(
                scope_key=EXPERIENCE_SEMANTIC_SCOPE_KEY,
                provider_key=self.provider_key,
                payload=_experience_semantic_scope_payload(
                    scope=scope,
                    workspace_root=workspace_root,
                ),
                materialization_dependencies=(
                    _experience_materialization_dependencies(
                        scope=scope,
                        workspace_root=workspace_root,
                    )
                ),
...
_PROVIDER = _ExperienceSemanticScopeProvider()
...
__all__ = [
    "EXPERIENCE_SEMANTIC_SCOPE_KEY",
    "ExperienceSemanticScope",
    "load_experience_semantic_scope",
