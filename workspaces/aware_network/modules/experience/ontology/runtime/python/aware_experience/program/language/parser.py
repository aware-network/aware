...
def _parse_json_literal(node: Node) -> dict[str, object] | list[object]:
    try:
        ...
        raise ProgramParseError(f"Invalid JSON literal in program: {exc}") from exc
...
def _parse_declaration_call_stmt(node: Node) -> ProgramCall:
    ...
    if node.type == "port_decl_stmt":
        ...
        if len(ref_parts) != 1:
            raise ProgramParseError("port declaration ref must use `<Experience>` form")
        ...
        for child in body_node.named_children:
            ...
            if child.type == "port_decl_node_stmt":
                ...
                args.append(ProgramCallArg(name=f"node_{node_name}", value=node_ref))
    ...
    if not target:
        raise ProgramParseError(
            f"Unsupported declaration statement node: {node.type!r}"
        )
