...
from aware_experience.manifest.spec import AwareExperienceTomlSpec


@dataclass(frozen=True, slots=True)
class ExperienceWorkspaceSnapshot:
    repo_root: Path
    package_root: Path
    spec_path: Path
...
class ExperienceWorkspace:
    def __init__(self, *, spec_path: str | Path, repo_root: str | Path | None = None):
        ...
        if not resolved_spec_path.exists():
            raise FileNotFoundError(f"aware.experience.toml not found: {resolved_spec_path}")
    ...
    @classmethod
    def from_toml(cls, *, toml_path: str | Path, repo_root: str | Path | None = None) -> ExperienceWorkspace:
    ...
    def build_snapshot(self) -> ExperienceWorkspaceSnapshot:
        spec = load_aware_experience_toml_spec(toml_path=self._spec_path)
        ...
        if not sources_root.exists():
            raise FileNotFoundError(f"Experience sources_dir does not exist: {sources_root} (from {self._spec_path})")
        if not sources_root.is_dir():
            raise NotADirectoryError(f"Experience sources_dir must be a directory: {sources_root}")
        ...
        return ExperienceWorkspaceSnapshot(
