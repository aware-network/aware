...
from aware_experience.program.language.ast import (
    ProgramCall,
    ProgramCallArg,
    ProgramDeclaration,
...
from aware_experience.program.language.parser import parse_program_declarations
from aware_experience.program.language.plan import (
    InvocationPlan,
...
_PROGRAM_AWARE_PRIMITIVE_BASE_TYPES: dict[str, CodePrimitiveBaseType] = {
...
_PROGRAM_PRIMITIVE_TYPE_DOC = ", ".join(
    sorted(
        {
            *(token.casefold() for token in _PROGRAM_AWARE_PRIMITIVE_BASE_TYPES.keys()),
...
_PROGRAM_CONTINUATION_ACTION_TARGET = "program.continuation_action"
_PROGRAM_CONTINUATION_ACTIVATION_TARGET = "program.continuation_activation_field"
_PROGRAM_CONTINUATION_OUTCOME_TARGET = "program.continuation_outcome_field"
_PROGRAM_CONTINUATION_RECEIPT_TARGET = "program.continuation_receipt_field"
_PROGRAM_CONTINUATION_TARGETS = {
    _PROGRAM_CONTINUATION_ACTION_TARGET,
    _PROGRAM_CONTINUATION_ACTIVATION_TARGET,
    _PROGRAM_CONTINUATION_OUTCOME_TARGET,
    _PROGRAM_CONTINUATION_RECEIPT_TARGET,
}
...
def _canonical_program_attribute_type_ref(
    ...
    *,
    ...
    primitive_base_type = _program_aware_primitive_base_type(raw)
    if primitive_base_type is not None:
        return primitive_base_type.value
...
def _program_aware_primitive_base_type(type_ref: str) -> CodePrimitiveBaseType | None:
    token = (type_ref or "").strip()
    if len(token) >= 2 and token[0] == token[-1] and token[0] in {"'", '"'}:
        token = token[1:-1].strip()
    if "(" in token and token.endswith(")"):
        token = token.split("(", 1)[0].strip()
    for aware_token, base_type in _PROGRAM_AWARE_PRIMITIVE_BASE_TYPES.items():
        if aware_token.casefold() == token.casefold():
            return base_type
    return None
...
def _required_continuation_key(
    kwargs: dict[str, PlanExpr], *, name: str, target: str
) -> str:
    value = kwargs.pop(name, None)
    if not isinstance(value, str):
        raise ProgramCompileError(f"{target} requires string `{name}`")
    key = value.strip()
    if not key or _PORT_KEY_RX.match(key) is None:
        raise ProgramCompileError(f"{target} `{name}` must be a valid identifier")
    return key


def _required_continuation_expr(
    kwargs: dict[str, PlanExpr], *, name: str, target: str
) -> PlanExpr:
    value = kwargs.pop(name, None)
    if value is None:
        raise ProgramCompileError(f"{target} requires `{name}`")
    return value


def _continuation_binding_options(
    kwargs: dict[str, PlanExpr], *, target: str
) -> tuple[bool, int | None]:
    required = kwargs.pop("required", True)
    if not isinstance(required, bool):
        raise ProgramCompileError(f"{target} `required` must be a boolean")
    position = kwargs.pop("position", None)
    if position is not None and (not isinstance(position, int) or position < 0):
        raise ProgramCompileError(
            f"{target} `position` must be a non-negative integer or null"
        )
    return required, position


def _reject_continuation_unknown_args(
    kwargs: Mapping[str, PlanExpr], *, target: str
) -> None:
    if kwargs:
        raise ProgramCompileError(
            f"{target} received unsupported args: {', '.join(sorted(kwargs))}"
        )


def _compile_continuation_action(call: PlanCall) -> PlanIntentActionConfig:
    kwargs = _call_kwargs(call)
    key = _required_continuation_key(
        kwargs, name="key", target=_PROGRAM_CONTINUATION_ACTION_TARGET
    )
    action_ref = _required_continuation_expr(
        kwargs,
        name="action_config_id",
        target=_PROGRAM_CONTINUATION_ACTION_TARGET,
    )
    event_ref = _required_continuation_expr(
        kwargs,
        name="event_config_id",
        target=_PROGRAM_CONTINUATION_ACTION_TARGET,
    )
    endpoint_ref = _required_continuation_expr(
        kwargs,
        name="api_capability_endpoint_id",
        target=_PROGRAM_CONTINUATION_ACTION_TARGET,
    )
    request_class_config_ref = _required_continuation_expr(
        kwargs,
        name="request_class_config_id",
        target=_PROGRAM_CONTINUATION_ACTION_TARGET,
    )
    response_class_config_ref = _required_continuation_expr(
        kwargs,
        name="response_class_config_id",
        target=_PROGRAM_CONTINUATION_ACTION_TARGET,
    )
    _reject_continuation_unknown_args(
        kwargs, target=_PROGRAM_CONTINUATION_ACTION_TARGET
    )
    return PlanIntentActionConfig(
        action_ref=action_ref,
        event_ref=event_ref,
        continuation_key=key,
        api_capability_endpoint_ref=endpoint_ref,
        request_class_config_ref=request_class_config_ref,
        response_class_config_ref=response_class_config_ref,
    )


def _compile_continuation_binding(call: PlanCall) -> PlanActionContinuationBinding:
    kwargs = _call_kwargs(call)
    target = call.target
    target_intent_key = _required_continuation_key(
        kwargs, name="target_intent", target=target
    )
    target_attribute = _required_continuation_expr(
        kwargs,
        name="target_request_attribute_config_id",
        target=target,
    )
    required, position = _continuation_binding_options(kwargs, target=target)
    if target == _PROGRAM_CONTINUATION_ACTIVATION_TARGET:
        source_input_key = _required_continuation_key(
            kwargs, name="source_input", target=target
        )
        source_attribute = _required_continuation_expr(
            kwargs, name="source_attribute_config_id", target=target
        )
        source_class = _required_continuation_expr(
            kwargs, name="source_class_config_id", target=target
        )
        _reject_continuation_unknown_args(kwargs, target=target)
        return PlanActionContinuationActivationFieldBinding(
            source_input_key=source_input_key,
            source_class_config_ref=source_class,
            source_attribute_config_ref=source_attribute,
            target_intent_key=target_intent_key,
            target_request_attribute_config_ref=target_attribute,
            required=required,
            position=position,
        )

    source_intent_key = _required_continuation_key(
        kwargs, name="source_intent", target=target
    )
    if target == _PROGRAM_CONTINUATION_OUTCOME_TARGET:
        source_attribute = _required_continuation_expr(
            kwargs,
            name="source_response_attribute_config_id",
            target=target,
        )
        _reject_continuation_unknown_args(kwargs, target=target)
        return PlanActionContinuationOutcomeFieldBinding(
            source_intent_key=source_intent_key,
            source_response_attribute_config_ref=source_attribute,
            target_intent_key=target_intent_key,
            target_request_attribute_config_ref=target_attribute,
            required=required,
            position=position,
    ...
    source_receipt_class = _required_continuation_expr(
        kwargs, name="source_receipt_class_config_id", target=target
    )
    source_receipt_attribute = _required_continuation_expr(
        kwargs, name="source_receipt_attribute_config_id", target=target
    )
    _reject_continuation_unknown_args(kwargs, target=target)
    return PlanActionContinuationReceiptFieldBinding(
        source_intent_key=source_intent_key,
        source_receipt_class_config_ref=source_receipt_class,
        source_receipt_attribute_config_ref=source_receipt_attribute,
        target_intent_key=target_intent_key,
        target_request_attribute_config_ref=target_attribute,
        required=required,
        position=position,
...
def _validate_action_continuation_plan(
    *,
    steps: list[PlanStep],
    bindings: list[PlanActionContinuationBinding],
) -> None:
    intents_by_key: dict[str, tuple[int, PlanIntentActionConfig]] = {}
    for sequence, step in enumerate(steps):
        if not isinstance(step, PlanIntentActionConfig):
            continue
        key = (step.continuation_key or "").strip()
        if not key:
            continue
        if key in intents_by_key:
            raise ProgramCompileError(
                f"Duplicate Program continuation action key: {key!r}"
            )
        if (
            step.api_capability_endpoint_ref is None
            or step.request_class_config_ref is None
            or step.response_class_config_ref is None
        ):
            raise ProgramCompileError(
                f"Program continuation action {key!r} is missing endpoint/ClassConfig identity"
            )
        intents_by_key[key] = (sequence, step)

    if not bindings:
        return
    if not intents_by_key:
        raise ProgramCompileError(
            "Program continuation bindings require program.continuation_action declarations"
        )

    claimed_targets: set[tuple[str, str]] = set()
    for binding in bindings:
        target = intents_by_key.get(binding.target_intent_key)
        if target is None:
            raise ProgramCompileError(
                "Program continuation binding references unknown target intent "
                + repr(binding.target_intent_key)
            )
        target_sequence, _target_step = target
        target_claim = (
            binding.target_intent_key,
            repr(binding.target_request_attribute_config_ref),
        )
        if target_claim in claimed_targets:
            raise ProgramCompileError(
                "Program continuation bindings ambiguously target the same request attribute "
                + f"(target_intent={binding.target_intent_key!r})"
            )
        claimed_targets.add(target_claim)

        if isinstance(binding, PlanActionContinuationActivationFieldBinding):
            continue
        source = intents_by_key.get(binding.source_intent_key)
        if source is None:
            raise ProgramCompileError(
                "Program continuation binding references unknown source intent "
                + repr(binding.source_intent_key)
            )
        source_sequence, _source_step = source
        if source_sequence >= target_sequence:
            raise ProgramCompileError(
                "Program continuation outcome source must precede its target "
                + f"(source={binding.source_intent_key!r} target={binding.target_intent_key!r})"
            )
    ...
    *,
    ...
    for stmt in program.statements:
        ...
        if isinstance(stmt, ProgramCall):
            ...
            if target in _PROGRAM_CONTINUATION_TARGETS:
                if stmt.actor is not None or stmt.object_expr is not None:
                    raise ProgramCompileError(
                        f"{target} is a Program declaration intrinsic and cannot have actor/object dispatch context"
                    )
                saw_non_declaration_stmt = True
                compiled_call = _compile_call(
                    stmt,
                    locals_,
                    type_id_registry=type_id_registry,
                )
                if target == _PROGRAM_CONTINUATION_ACTION_TARGET:
                    steps.append(_compile_continuation_action(compiled_call))
                else:
                    action_continuation_bindings.append(
                        _compile_continuation_binding(compiled_call)
                    )
                continue
            if target not in _PROGRAM_DECLARATION_TARGETS:
                saw_non_declaration_stmt = True
    ...
    _validate_action_continuation_plan(
        steps=steps,
        bindings=action_continuation_bindings,
    )
