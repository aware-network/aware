...
def decode_invocation_plan_artifact(payload: object) -> InvocationPlan:
    ...
    version = _coerce_int(
        data.get("version"), context="invocation_plan_artifact.version"
    )
...
def _encode_invocation_plan(plan: InvocationPlan) -> dict[str, object]:
    return {
        ...
        "action_continuation_bindings": [
            _encode_action_continuation_binding(binding)
            for binding in plan.action_continuation_bindings
        ],
    }
...
def _decode_invocation_plan(payload: object) -> InvocationPlan:
    ...
    raw_actors = _expect_list(
        data.get("actors") or [], context="invocation_plan.actors"
    )
    raw_ports = _expect_list(data.get("ports") or [], context="invocation_plan.ports")
    raw_continuation_bindings = _expect_list(
        data.get("action_continuation_bindings") or [],
        context="invocation_plan.action_continuation_bindings",
    )
    return InvocationPlan(
        ...
        action_continuation_bindings=tuple(
            _decode_action_continuation_binding(binding)
            for binding in raw_continuation_bindings
        ),
    )
...
def _encode_step(step: PlanStep) -> dict[str, object]:
    ...
    if isinstance(step, PlanIntentActionConfig):
        return {
            ...
            "continuation_key": step.continuation_key,
            "api_capability_endpoint_ref": _encode_expr(
                step.api_capability_endpoint_ref
            ),
            "request_class_config_ref": _encode_expr(step.request_class_config_ref),
            "response_class_config_ref": _encode_expr(step.response_class_config_ref),
        }
...
def _decode_step(payload: object) -> PlanStep:
    ...
    if step_type == "intent_action_config":
        return PlanIntentActionConfig(
            ...
            continuation_key=_coerce_optional_str(data.get("continuation_key")),
            api_capability_endpoint_ref=(
                _decode_expr(data.get("api_capability_endpoint_ref"))
                if data.get("api_capability_endpoint_ref") is not None
                else None
            ),
            request_class_config_ref=(
                _decode_expr(data.get("request_class_config_ref"))
                if data.get("request_class_config_ref") is not None
                else None
            ),
            response_class_config_ref=(
                _decode_expr(data.get("response_class_config_ref"))
                if data.get("response_class_config_ref") is not None
                else None
            ),
        )
    if step_type == "let":
...
def _encode_action_continuation_binding(
    binding: PlanActionContinuationBinding,
) -> dict[str, object]:
    common = {
        "target_intent_key": binding.target_intent_key,
        "target_request_attribute_config_ref": _encode_expr(
    ...
    if isinstance(binding, PlanActionContinuationActivationFieldBinding):
        return {
            "$binding": "activation_field",
            **common,
            "source_input_key": binding.source_input_key,
            "source_class_config_ref": _encode_expr(binding.source_class_config_ref),
            "source_attribute_config_ref": _encode_expr(
                binding.source_attribute_config_ref
            ),
        }
    if isinstance(binding, PlanActionContinuationOutcomeFieldBinding):
        return {
            "$binding": "outcome_field",
            **common,
            "source_intent_key": binding.source_intent_key,
            "source_response_attribute_config_ref": _encode_expr(
                binding.source_response_attribute_config_ref
            ),
        }
    return {
        "$binding": "receipt_field",
        **common,
        "source_intent_key": binding.source_intent_key,
        "source_receipt_class_config_ref": _encode_expr(
            binding.source_receipt_class_config_ref
        ),
        "source_receipt_attribute_config_ref": _encode_expr(
            binding.source_receipt_attribute_config_ref
        ),
    }


def _decode_action_continuation_binding(
    payload: object,
) -> PlanActionContinuationBinding:
    data = _expect_mapping(payload, context="action_continuation_binding")
    binding_type = str(data.get("$binding") or "").strip()
    target_intent_key = str(data.get("target_intent_key") or "").strip()
    if not target_intent_key:
        raise ValueError("action_continuation_binding.target_intent_key is required")
    target_attribute = _decode_expr(data.get("target_request_attribute_config_ref"))
    required = bool(data.get("required", True))
    raw_position = data.get("position")
    position = (
        _coerce_int(raw_position, context="action_continuation_binding.position")
        if raw_position is not None
        else None
    )
    if binding_type == "activation_field":
        source_input_key = str(data.get("source_input_key") or "").strip()
        if not source_input_key:
            raise ValueError("action_continuation_binding.source_input_key is required")
        return PlanActionContinuationActivationFieldBinding(
            source_input_key=source_input_key,
            source_class_config_ref=_decode_expr(data.get("source_class_config_ref")),
            source_attribute_config_ref=_decode_expr(
                data.get("source_attribute_config_ref")
            ),
    ...
    source_intent_key = str(data.get("source_intent_key") or "").strip()
    if not source_intent_key:
        raise ValueError("action_continuation_binding.source_intent_key is required")
    if binding_type == "outcome_field":
        return PlanActionContinuationOutcomeFieldBinding(
            source_intent_key=source_intent_key,
            source_response_attribute_config_ref=_decode_expr(
                data.get("source_response_attribute_config_ref")
            ),
            target_intent_key=target_intent_key,
            target_request_attribute_config_ref=target_attribute,
            required=required,
    ...
    if binding_type == "receipt_field":
        return PlanActionContinuationReceiptFieldBinding(
            source_intent_key=source_intent_key,
            source_receipt_class_config_ref=_decode_expr(
                data.get("source_receipt_class_config_ref")
            ),
            source_receipt_attribute_config_ref=_decode_expr(
                data.get("source_receipt_attribute_config_ref")
            ),
            target_intent_key=target_intent_key,
            target_request_attribute_config_ref=target_attribute,
            required=required,
    ...
    raise ValueError(f"Unsupported action continuation binding type: {binding_type!r}")
...
def _decode_port_projection_node_contract(
    ...
    raw_keys = _expect_list(
        data.get("keys") or [], context="plan_port_projection_node.keys"
    )
...
def _decode_expr(payload: object) -> PlanExpr | None:
    ...
    if isinstance(payload, dict):
        ...
        return out
    raise ValueError(
        f"Unsupported plan expression payload type: {type(payload).__name__}"
    )
