...
from aware_skill.manifest import (
    AwareSkillTomlSpec,
...
class SkillWorkspace:
    ...
    def __init__(self, *, spec_path: str | Path, repo_root: str | Path | None = None):
        ...
        if not resolved_spec_path.exists():
            raise FileNotFoundError(f"aware.skill.toml not found: {resolved_spec_path}")
    ...
    @classmethod
    def from_toml(cls, *, toml_path: str | Path, repo_root: str | Path | None = None) -> SkillWorkspace:
    ...
    def build_snapshot(self) -> SkillWorkspaceSnapshot:
        spec = load_aware_skill_toml_spec(toml_path=self._spec_path)
        ...
        if not sources_root.exists():
            raise FileNotFoundError(f"Skill sources_dir does not exist: {sources_root} (from {self._spec_path})")
        if not sources_root.is_dir():
            raise NotADirectoryError(f"Skill sources_dir must be a directory: {sources_root}")
