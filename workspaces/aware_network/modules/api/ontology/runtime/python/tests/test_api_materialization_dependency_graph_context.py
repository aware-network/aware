...
def test_resolve_api_package_spec_uses_runtime_graph_artifact_without_artifact_rebuild(
    ...
    snapshot = api_compile.compile_api_workspace(
        toml_path=toml_path,
        repo_root=tmp_path,
    ).snapshot
    dependency_source_digests = {
        package.package_name: api_runtime_resolution._compute_runtime_dependency_source_digest(
            package=package,
        )
        for package in api_runtime_resolution._resolve_api_dependency_packages(
            snapshot=snapshot,
