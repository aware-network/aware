...
from aware_api_runtime.packages.dependency_contract import (  # noqa: E402
...
def test_api_public_package_dependency_contract_resolves_python_dependencies() -> None:
    specs = api_public_package_python_dependency_specs(
        dependency_import_roots=("aware_home_api",)
    )
    ...
    assert [str(spec.dependency_kind.value) for spec in specs] == [
        "workspace_code_package",
        "workspace_code_package",
        "workspace_code_package",
        "workspace_code_package",
        "external",
    ]
    assert specs[-1].requirement == ">=2.13.4,<3.0.0"
    assert resolve_api_public_package_python_dependencies(
        dependency_import_roots=("aware_home_api",)
    ) == [
        "aware_home_api",
        "aware-api-client",
...
def test_api_public_package_dependency_contract_resolves_dart_dependencies() -> None:
    specs = api_public_package_dart_dependency_specs(
        path_dependencies=(
            ("aware_api", "../../modules/api/libs/api/dart"),
            (
                "aware_model_helpers",
                "../../libs/model_helpers/dart/aware_model_helpers",
            ),
        )
    )
    resolved = resolve_api_public_package_dart_dependencies(
        path_dependencies=(
            ("aware_api", "../../modules/api/libs/api/dart"),
            (
                "aware_model_helpers",
    ...
    assert list(resolved.dependencies) == [
        "freezed_annotation: ^3.0.0",
        "json_annotation: ^4.9.0",
        "uuid: ^4.5.1",
        "aware_api:\n  path: ../../modules/api/libs/api/dart",
        "aware_model_helpers:\n  path: ../../libs/model_helpers/dart/aware_model_helpers",
    ]
    assert resolved.optional_dependencies["dev"] == (
        "build_runner: ^2.4.11",
        "freezed: ^3.0.0",
...
def test_build_api_public_package_render_job_canonicalizes_language_enum_value(
    tmp_path: Path,
) -> None:
    ...
    render_job = build_api_public_package_render_job(
        handoff=handoff,
        target=ApiPublicPackageRenderTarget(
            target_language=ForeignCodeLanguage.python,
            source_aware_toml_path=toml_path,
            target_output_dir=root / "render" / "python",
            package_root=root / "sdk" / "python",
            package_name="aware_home_story_sdk",
    ...
    assert render_job.target.target_language is ForeignCodeLanguage.python
    assert render_job.materialization_config.target_language == CodeLanguage.python
    assert render_job.materialization_config.name == "api-public-package-python"
...
def test_build_api_public_package_render_job_for_dart_target(tmp_path: Path) -> None:
    ...
    assert package_spec.optional_dependencies == {
        "dev": [
            "build_runner: ^2.4.11",
            "freezed: ^3.0.0",
            "json_serializable: ^6.8.0",
