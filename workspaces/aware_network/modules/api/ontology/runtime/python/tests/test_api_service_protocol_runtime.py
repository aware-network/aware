...
def test_service_protocol_decode_includes_registered_nested_dto_class_configs() -> None:
    ...
    class_configs_by_id = _request_class_configs_by_id_for_decode(
        ...
        request_type_ref=_CODE_DELTA_REQUEST_CLASS_REF,
    )
...
def test_build_api_service_dispatch_plan_from_invocation_ir_uses_deferred_envelope(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    import aware_api_runtime.service_protocol.runtime as runtime_mod
    ...
    endpoint_binding = ApiServiceProtocolEndpointBinding(
        endpoint_ref="openai.door.open",
        request_type_ref="proof.ProofRequest",
        response_type_ref=None,
        stream_event_type_refs=(),
        execution_protocol_ref=None,
        build_execution=None,
    ...
    loaded_package = LoadedApiServiceProtocolPackage(
        ...
        public_package_import_root="proof",
        service_protocol_import_root="proof_protocol",
        endpoint_bindings={"openai.door.open": endpoint_binding},
        runtime_fulfillment_bindings={},
    )
    ir = resolve_api_invocation_ir(
        ...
        endpoint_ref="openai.door.open",
        request_payload={"label": "front"},
    )
    ir = replace(ir, request_class_config_id=request_class_config_id)
    branch_id = uuid4()
    monkeypatch.setattr(
        runtime_mod,
        "load_api_service_protocol_package",
        lambda **_: loaded_package,
    )
    monkeypatch.setattr(
        runtime_mod,
        "_resolve_request_model_class",
        lambda **_: ProofRequest,
    )

    plan = build_api_service_dispatch_plan_from_invocation_ir(
        index=cast(
            Any,
            SimpleNamespace(
                class_configs_by_id={},
    ...
    assert plan.endpoint_ref == "openai.door.open"
    assert plan.request_object == ProofRequest(label="front")
    assert plan.fulfillment_bindings == ()
    assert plan.build_execution is None
    assert plan.envelope.branch_id == branch_id
    assert plan.envelope.projection_hash == "sha256:api-call"
    assert plan.envelope.request_class_config_id == request_class_config_id
    assert plan.envelope.request_hash.startswith("sha256:")
    assert plan.envelope.commit_id == plan.envelope.head_commit_id
    assert plan.envelope.deferred_api_call is True
...
async def test_decode_committed_api_call_request_into_generated_service_protocol_package_request_model(
    ...
    with IsolatedMetaAwareRoot(
        ...
        assert (
            "api_invocation.materialize_api_call.resolve_request_contract_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call."
            "request_contract.verified_ir_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call."
            "request_contract.verified_ir.fulfillment_guard_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call."
            "request_contract.derived_endpoint_hydrate_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call."
            "request_contract.from_derived_endpoint_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call."
            "request_contract.endpoint.registered_pydantic_lookup_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call."
            "request_contract.pydantic.package_register_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call.resolve_target_lane_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call.create_api_call_s"
            in api_invocation_timings
        )
        assert (
            "api_invocation.materialize_api_call.invoke_create_handler_s"
            in api_invocation_timings
        )
