...
_CODE_DELTA_REQUEST_PACKAGE_PREFIX = "aware_code_service_dto"
_CODE_DELTA_REQUEST_CLASS_NAME = "NormalizeCodePackageDeltaRequest"
_CODE_DELTA_REQUEST_CLASS_REF = (
    "aware_code_service_dto.code.features.package_delta."
...
def test_api_call_request_materialization_closes_imported_dto_class_configs() -> None:
    ...
    request_class_config = _pydantic_class_config_by_name(
        package_prefix=_CODE_DELTA_REQUEST_PACKAGE_PREFIX,
        name=_CODE_DELTA_REQUEST_CLASS_NAME,
    )
    ...
    class_configs_by_id = call_mod._request_class_configs_by_id_for_materialization(
        ...
        request_class_ref=_CODE_DELTA_REQUEST_CLASS_REF,
    )
    ...
    request_model = build_inline_value_instance_from_mapping(
        ...
        values={
            "operation": "normalize_package_delta",
...
async def test_api_call_constructor_returns_generated_bootstrap_root_identity(
    ...
    with scoped_api_call_materialization_input(
        request_payload={
            "operation": "normalize_package_delta",
            "delta": {"package_name": "content-ontology", "paths": []},
        },
        request_class_config=request_class_config,
...
@pytest.mark.asyncio
async def test_api_call_record_stream_event_validates_endpoint_stream_contract(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import aware_api_runtime.handlers.impl.api.api_call as api_call_mod
    from aware_api_ontology.api.api_call import ApiCall
    ...
    stream_event_config_id = uuid4()
    created_event = ApiCallStreamEvent.model_construct(
        id=uuid4(),
        api_call_id=api_call_id,
        api_capability_endpoint_stream_event_config_id=stream_event_config_id,
    ...
    async def _create_via_api_call(**kwargs: object) -> ApiCallStreamEvent:
        assert kwargs["api_call_id"] == api_call_id
        assert kwargs["sequence"] == 1
        assert (
            kwargs["api_capability_endpoint_stream_event_config_id"]
            == stream_event_config_id
        )
        return created_event

    monkeypatch.setattr(
        api_call_mod.ApiCallStreamEvent,
        "create_via_api_call",
        _create_via_api_call,
    )
    ...
    with scoped_api_call_stream_event_materialization_input(
        ...
        result = await api_call_mod.record_stream_event(
            api_call=api_call,
            sequence=1,
            api_capability_endpoint_stream_event_config_id=stream_event_config_id,
            description="notice",
    ...
    assert api_call.stream_events == [created_event]


@pytest.mark.asyncio
async def test_api_call_record_stream_event_rejects_foreign_stream_config() -> None:
    import aware_api_runtime.handlers.impl.api.api_call as api_call_mod
    from aware_api_ontology.api.api_call import ApiCall
    from aware_api_ontology.api.api_capability_endpoint import ApiCapabilityEndpoint
    from aware_api_ontology.api.api_capability_endpoint_request_config import (
    ...
    request_stream_config = ApiCapabilityEndpointStreamConfig.model_construct(
        id=uuid4(),
        api_capability_endpoint_request_config_id=uuid4(),
        stream_mode=ApiCapabilityEndpointStreamMode.server,
        description=None,
    ...
    foreign_stream_event_config = (
        ApiCapabilityEndpointStreamEventConfig.model_construct(
            id=uuid4(),
    ...
    endpoint = ApiCapabilityEndpoint.model_construct(
        ...
        request_config=ApiCapabilityEndpointRequestConfig.model_construct(
            ...
            class_config_id=uuid4(),
            description=None,
            stream_config=request_stream_config,
        ),
    )
    ...
    with scoped_api_call_stream_event_materialization_input(
        ...
        api_call=api_call,
        api_capability_endpoint=endpoint,
        stream_event_config=foreign_stream_event_config,
    ):
        with pytest.raises(RuntimeError, match="outside this call's endpoint"):
            await api_call_mod.record_stream_event(
                api_call=api_call,
                sequence=1,
                api_capability_endpoint_stream_event_config_id=(
                    foreign_stream_event_config.id
                ),
            )


@pytest.mark.asyncio
async def test_api_call_stream_event_constructor_builds_typed_event_model(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import aware_api_runtime.handlers.impl.api.api_call_stream_event as stream_mod
    from aware_api_ontology.api.api_call import ApiCall
    from aware_api_ontology.api.api_capability_endpoint_stream_enums import (
        ApiCapabilityEndpointStreamEventKind,
    )
    ...
    from aware_api_runtime.invocation.materialization.pydantic_class_config_closure import (
    ...
    class_config = _pydantic_class_config_by_name(
        package_prefix=_CODE_DELTA_REQUEST_PACKAGE_PREFIX,
        name=_CODE_DELTA_REQUEST_CLASS_NAME,
    )
    stream_event_config = ApiCapabilityEndpointStreamEventConfig.model_construct(
    ...
    monkeypatch.setattr(stream_mod, "current_handler_session", lambda: _Session())
    ...
    with scoped_api_call_stream_event_materialization_input(
        ...
        event_class_config=class_config,
        event_class_configs_by_id=pydantic_class_configs_by_id_for_ref(
            base_class_configs_by_id={},
            root_class_config=class_config,
            class_ref=_CODE_DELTA_REQUEST_CLASS_REF,
        ),
        api_call=api_call,
        stream_event_config=stream_event_config,
    ):
        created = await stream_mod.create_via_api_call(
            api_call_id=api_call_id,
            sequence=sequence,
    ...
    assert created.id == stream_event_id
    assert created.api_call_id == api_call_id
    assert created.api_capability_endpoint_stream_event_config_id == (
        stream_event_config.id
    )
    assert created.event_model is not None
    assert created.event_model_id == stable_inline_value_instance_id(
        class_config_id=class_config.id,
        owner_key=stream_event_id,
    )
    assert created.event_model.class_config_id == class_config.id
    assert created.event_model.owner_key == stream_event_id
...
async def test_api_call_outcome_materialization_uses_api_call_hint_without_lane_hydration(
    ...
    with collect_api_invocation_trace_timings() as timings:
        result = await outcome_mod.materialize_api_call_outcome(
            runtime=_Runtime(),
            index=cast(Any, SimpleNamespace(class_configs_by_id={})),
            actor_id=None,
    ...
    assert api_call.outcome is outcome
    assert result.binding.api_call_outcome_id == outcome.id
    assert "api_call_outcome.hydrate_api_call_s" not in timings
    assert "api_call_outcome.resolve_runtime_lane_s" in timings
    assert "api_call_outcome.ensure_projection_readiness_s" in timings
    assert "api_call_outcome.resolve_payload_mode_s" in timings
    assert "api_call_outcome.response_class_config_closure_s" in timings
    assert "api_call_outcome.activate_create_outcome_s" in timings
    assert "api_call_outcome.build_result_s" in timings
...
def test_api_call_request_contract_uses_package_payload_lookup_before_package_iteration(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import aware_api_runtime.invocation.materialization.call as call_mod
    ...
    def _package_payload(
        *,
        ...
        assert package_prefix == "aware_identity_service_dto"
        assert class_config_id == str(request_class_config_id)
        return cast(dict[str, object], request_class_config.model_dump(mode="json"))

    def _iter_package_class_config_payloads(
        *,
        package_prefix: str,
    ) -> tuple[object, ...]:
        _ = package_prefix
        raise AssertionError(
            "direct package lookup must avoid package-wide ClassConfig iteration"
        )
    ...
    monkeypatch.setattr(
        call_mod,
        "iter_pydantic_package_class_config_payloads",
        _iter_package_class_config_payloads,
    )
...
def test_api_call_request_contract_prefers_package_snapshot_over_global_registry_conflict(
    ...
    monkeypatch.setattr(
        call_mod,
        "get_pydantic_package_class_config_payload",
        lambda *, package_prefix, class_config_id: None,
    )
    monkeypatch.setattr(
        call_mod,
        "iter_pydantic_package_class_config_payloads",
...
@pytest.mark.asyncio
async def test_materialize_api_call_stream_events_reads_committed_api_contract(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    repo_root = REPO_ROOT
    ...
    import aware_api_ontology  # noqa: F401
    import aware_api_runtime.invocation.materialization.call_stream_event as stream_mod
    ...
    with IsolatedMetaAwareRoot(
        ...
        assert runtime_context is not None
        runtime_index = runtime_context.index
        lane = ApiMetaRuntimeLane(branch_id=uuid4())
        ...
        api_snapshot = await commit_api_reference_snapshot(
            ...
            endpoint_request_class_config_ids={endpoint_ref: class_config_id},
            endpoint_stream_event_class_config_ids={
                endpoint_ref: {
                    "notice": class_config_id,
                    "delta": class_config_id,
                    "complete": class_config_id,
                },
            },
        ...
        call_result = await materialize_api_call(
            runtime=runtime,
            index=runtime_index,
            actor_id=lane.actor_id,
            source_lane=source_lane,
        ...
        stream_event_config_ids = api_snapshot.endpoint_stream_event_config_ids_by_ref[
            endpoint_ref
        ]

        event_results = []
        for sequence, kind in enumerate(("notice", "delta", "complete"), start=1):
            event_results.append(
                await materialize_api_call_stream_event(
                    runtime=runtime,
                    index=runtime_index,
                    actor_id=lane.actor_id,
                    target_lane=target_lane,
                    api_source_lane=source_lane,
                    api_call_id=call_result.binding.api_call_id,
                    api_call_hint=call_result.api_call,
                    sequence=sequence,
                    api_capability_endpoint_stream_event_config_id=(
                        stream_event_config_ids[kind]
                    ),
                    event_values={
                        request_attribute_name: request_attribute_value,
                    },
                    event_class_config=request_class_config,
                    description=f"{kind} event",
                )
            )

        assert [item.binding.sequence for item in event_results] == [1, 2, 3]
        assert [item.binding.event_class_config_id for item in event_results] == [
            class_config_id,
            class_config_id,
            class_config_id,
        ]
        assert event_results[-1].binding.commit_id is not None
        for event_result in event_results:
            event_model = event_result.api_call_stream_event.event_model
            assert event_model is not None
            event_attributes = resolve_inline_value_instance_attributes(
                inline_value_instance=event_model,
                class_config=request_class_config,
            )
            assert len(event_attributes) == 1
            assert event_attributes[0].attribute_config.name == request_attribute_name
            assert (
                decode_oig_attribute_value(
                    event_attributes[0].attribute.value_root,
                    class_configs_by_id=runtime_index.class_configs_by_id,
                )
                == request_attribute_value
            )

        hydrated_api_call = await stream_mod._hydrate_materialized_api_call(
            index=runtime_index,
            target_lane=target_lane,
            api_call_id=call_result.binding.api_call_id,
        )
        stream_events = sorted(
            hydrated_api_call.stream_events,
            key=lambda item: item.sequence,
        )
        assert [item.sequence for item in stream_events] == [1, 2, 3]
        assert [
            item.api_capability_endpoint_stream_event_config_id
            for item in stream_events
        ] == [
            stream_event_config_ids["notice"],
            stream_event_config_ids["delta"],
            stream_event_config_ids["complete"],
        ]
        for stream_event in stream_events:
            assert stream_event.id == stable_api_call_stream_event_id(
                api_call_id=call_result.binding.api_call_id,
                sequence=stream_event.sequence,
            )
            assert stream_event.event_model is not None
            assert stream_event.event_model.class_config_id == class_config_id
            assert stream_event.event_model.inline_value_instance_attributes
