...
def test_api_receipt_payload_summary_is_bounded(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AWARE_API_RECEIPT_PAYLOAD_SUMMARY_BUDGET", "3")

    summary = api_receipt_payload_summary(
        {
            "items": [
                {"value": 1},
                {"value": 2},
    ...
    assert summary["container_field_count"] == 1
    assert summary["nested_container_count"] == 3
    assert summary["nested_container_count_truncated"] == 1
...
class _RuntimeLane:
    def __init__(self) -> None:
        self._last_commit_id = uuid4()
        self._last_head_commit_id = uuid4()

    @property
    def last_commit_id(self) -> UUID:
        return self._last_commit_id

    @property
    def last_head_commit_id(self) -> UUID:
        return self._last_head_commit_id

    def activate(self, *, commit: bool, publish: bool) -> ContextManager[object]:
...
class _Runtime:
    ...
    def bind(
        ...
        *,
        ...
        _ = (projection, branch_id, actor_id)
        return self.lane
...
@pytest.mark.asyncio
async def test_compact_api_call_handler_uses_context_request_hash(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from aware_api_runtime.handlers.impl.api import api_call as handler_mod
    from aware_api_runtime.invocation.materialization.context import (
    ...
    call_key = uuid4()
    request_hash = "sha256:compact-request-hash"
    session = _FakeHandlerSession()

    def _fail_inline_value_hash(**_: object) -> str:
        raise AssertionError(
            "compact request context must not rehash the empty request anchor"
        )
    ...
    assert api_call.request_hash == request_hash
    assert api_call.request_model_id == stable_inline_value_instance_id(
        class_config_id=UUID(str(request_class_config.id)),
        owner_key=call_key,
    )
...
async def test_compact_api_call_outcome_receipt_skips_response_payload_graph(
    ...
    async def _create_outcome(
        ...
        *,
        ...
        assert materialization_input.response_class_config is None
    ...
    async def _fail_response_contract_resolution(**_: object) -> object:
        raise AssertionError(
            "compact outcome must not hydrate committed endpoint response contract"
        )
    ...
    monkeypatch.setattr(
        ...
        _fail_response_contract_resolution,
    )
...
async def test_deferred_api_call_outcome_creates_hinted_call_on_empty_lane(
    monkeypatch: pytest.MonkeyPatch,
    ...
    request_class_config = ClassConfig.model_construct(
        id=uuid4(),
        name="WorkspaceMaterializeRequest",
        class_fqn="aware_workspace_service_dto.WorkspaceMaterializeRequest",
        value_mode=ClassValueMode.inline_value,
    )
    call_key = uuid4()
    api_capability_endpoint_id = uuid4()
    ...
    api_call_hint = ApiCall.model_construct(
        id=api_call_id,
        api_capability_endpoint_id=api_capability_endpoint_id,
        request_model_id=request_model.id,
        request_model=request_model,
        call_key=call_key,
        request_hash="sha256:deferred-request",
        outcome=None,
    )
    async def _create_via_endpoint(
        *,
        api_capability_endpoint_id: UUID,
        call_key: UUID,
        request_class_config_id: UUID,
        ...
        raise AssertionError("deferred compact outcome must use hint fast path")

    async def _fail_build_outcome(
        *,
        api_call_id: UUID,
        status: ApiCallOutcomeStatus,
        error: str | None,
    ) -> ApiCallOutcome:
        _ = (api_call_id, status, error)
        raise AssertionError("deferred compact outcome must be materializer-owned")

    async def _fail_create_outcome(
        self: ApiCall,
        *,
        status: ApiCallOutcomeStatus,
        error: str | None,
        ...
        _ = (self, status, error)
        raise AssertionError("deferred empty-lane outcome must use constructor")

    runtime = _Runtime()
    monkeypatch.setattr(outcome_mod, "FSCommitStore", lambda: _HeadlessStore())
    monkeypatch.setattr(
        outcome_mod.ApiCall,
        "create_via_api_capability_endpoint",
        staticmethod(_create_via_endpoint),
    )
    monkeypatch.setattr(
        outcome_mod.ApiCallOutcome,
        "build_via_api_call",
        staticmethod(_fail_build_outcome),
    )
    monkeypatch.setattr(ApiCall, "create_outcome", _fail_create_outcome)
    ...
    with collect_api_invocation_trace_timings() as timings:
        result = await outcome_mod.materialize_api_call_outcome(
            ...
            api_call_deferred=True,
            commit=True,
            publish=False,
        )

    assert current_api_call_materialization_input() is None
    assert "api_call_outcome.fast_deferred_api_call_from_hint_s" in timings
    assert "api_call_outcome.create_deferred_api_call_s" not in timings
    assert "api_call_outcome.construct_deferred_compact_outcome_s" in timings
    assert "api_call_outcome.attach_deferred_compact_outcome_s" in timings
    assert "api_call_outcome.create_standard_outcome_s" not in timings
    assert result.api_call is not api_call_hint
    assert result.api_call.id == api_call_hint.id
    assert result.api_call.api_capability_endpoint_id == (
        api_call_hint.api_capability_endpoint_id
    )
    assert result.api_call.call_key == api_call_hint.call_key
    assert result.api_call.request_hash == api_call_hint.request_hash
    assert result.api_call.request_model is request_model
    assert result.api_call.request_model_id == request_model.id
    assert result.api_call_outcome.id == stable_api_call_outcome_id(
        api_call_id=api_call_id,
    )
    assert result.api_call_outcome.api_call_id == api_call_id
    assert result.api_call_outcome.status is ApiCallOutcomeStatus.succeeded
    assert result.api_call.outcome is result.api_call_outcome
    assert result.binding.commit_id == runtime.lane.last_commit_id
