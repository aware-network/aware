...
API_RENDER_INPUT_PRUNING_CONTRACT_VERSION = "aware.api.render-input-pruning.v1"
...
def api_delta_generated_code_package_deltas_from_patch_receipt(
    *,
    generated_artifact_file_patch: Mapping[str, object],
) -> tuple[CodePackageDelta, ...]:
    status = _optional_text(generated_artifact_file_patch.get("status"))
    if status not in {
        "generated_artifact_file_patch_applied",
        "generated_artifact_file_patch_noop",
    }:
        return ()
    workspace_root_text = _optional_text(
        generated_artifact_file_patch.get("workspace_root")
    )
    runtime_package_dir_text = _optional_text(
        generated_artifact_file_patch.get("runtime_package_dir")
    )
    if workspace_root_text is None or runtime_package_dir_text is None:
        return ()
    workspace_root = Path(workspace_root_text).resolve()
    runtime_package_dir = Path(runtime_package_dir_text).resolve()
    file_payloads = (
        *_tuple_mapping_payloads(generated_artifact_file_patch.get("changed_files")),
        *_tuple_mapping_payloads(generated_artifact_file_patch.get("deleted_files")),
    )
    deltas: list[CodePackageDelta] = []
    for target in _SUPPORTED_PATCH_TARGETS:
        delta = _api_delta_generated_code_package_delta_for_target(
            target=target,
    ...
    *,
    ...
    if blockers or plan_blockers or event_driver_blockers:
        return _api_client_service_protocol_patch_payload(
            ...
            executor_ready=not plan_blockers
            and not blockers
            and not event_driver_blockers,
            would_patch=False,
            did_patch=False,
    ...
    if (
        missing_target_receipt_blockers
        and not _generated_artifact_file_patch_covers_targets(
            generated_artifact_file_patch=generated_artifact_file_patch,
            patch_targets=patch_targets,
        )
    ):
...
def _render_api_client_service_protocol_patch(
    *,
    ...
    public_package_render_input_class_refs = (
        tuple(
            _tuple_text(
                render_input_pruning_plan.get("public_package_render_input_class_refs")
            )
        )
    ...
    if refresh_result.service_protocol_materialization is not None:
        ...
        runtime_package_dir = (
            refresh_result.public_package_materialization.runtime_package_dir
        )
...
def _materialization_event_artifact_driver_payload(
    *,
    ...
    candidates = tuple(
        candidate
        for event in events
        for candidate in _tuple_mapping_payloads(event.get("generated_path_candidates"))
    )
    ...
    if report_status != "api_materialization_event_report_ready":
        blockers.append(
            "materialization_event_report_not_ready:" f"{report_status or 'unknown'}"
        )
...
def _generated_path_candidate_file_scope(
    *,
    ...
    for candidate in _tuple_mapping_payloads(candidate_plan.get("candidates")):
        ...
        for section_ref in _tuple_mapping_payloads(
            candidate.get("render_section_refs")
        ):
            section_key = _optional_text(section_ref.get("section_key"))
            if section_key is None:
                continue
            target_candidate_render_section_refs.setdefault(target, {})[section_key] = (
                dict(section_ref)
            )

    missing_patch_targets = tuple(
        target for target in patch_targets if target not in target_candidate_file_counts
    )
    ...
    return tuple(
        ...
        for ref in sorted(
            (
                ref
                for ref in raw_refs
                if isinstance(ref, Mapping) and ref.get("section_key")
            ),
            key=lambda item: (
...
def _generated_path_candidate_file_scope_payload(
    *,
    ...
    candidate_render_section_refs = _sorted_render_section_refs(
        ref for refs in target_candidate_render_section_refs.values() for ref in refs
    )
    return {
        ...
        "target_candidate_class_refs": {
            target: tuple(refs) for target, refs in target_candidate_class_refs.items()
        },
...
def _renderer_candidate_paths_from_scope(
    *,
    ...
    for relpath in _tuple_text(
        generated_path_candidate_file_scope.get("candidate_runtime_package_relpaths")
    ):
...
def _generated_artifact_renderer_candidate_scope_payload(
    *,
    ...
    renderer_candidate_path_count = len(public_package_candidate_paths) + len(
...
def _render_input_pruning_plan_payload(
    *,
    ...
    service_protocol_full_input_required = service_protocol_target_requested and bool(
        service_protocol_candidate_paths
    )
    ...
    if service_protocol_full_input_required:
        service_protocol_input_strategy = "full_graph_required_global_protocol_renderer"
...
def _render_input_pruning_execution_payload(
    *,
    ...
    if dto_graph is not None:
        ...
        payload["actual_public_package_graph_ref"] = _optional_text(
            getattr(dto_graph, "hash", None)
...
def _file_level_patch_render_result(
    *,
    ...
    generated_artifact_file_patch = {
        ...
        "generated_artifact_renderer_candidate_scope": dict(renderer_candidate_scope),
        "generated_artifact_renderer_candidate_scope_status": _optional_text(
...
def _api_delta_generated_code_package_delta_for_target(
    *,
    target: str,
    workspace_root: Path,
    runtime_package_dir: Path,
    file_payloads: tuple[Mapping[str, object], ...],
) -> CodePackageDelta | None:
    root_segments = _PATCH_TARGET_ARTIFACT_ROOT_SEGMENTS.get(target)
    if root_segments is None:
        return None
    package_root = runtime_package_dir.joinpath(*root_segments).resolve()
    paths: list[CodePackageDeltaPath] = []
    for file_payload in file_payloads:
        if _optional_text(file_payload.get("target")) != target:
            continue
        delta_path = _api_delta_generated_code_package_delta_path(
    ...
    if not paths:
        return None
    sorted_paths = sorted(paths, key=lambda item: item.relative_path)
    package_name = _api_delta_generated_code_package_name(
        target=target,
        paths=tuple(sorted_paths),
    )
    package_root_text = _workspace_relative_or_absolute_path(
        path=package_root,
        workspace_root=workspace_root,
    )
...
def _api_delta_generated_code_package_delta_path(
    *,
    file_payload: Mapping[str, object],
    package_root: Path,
    workspace_root: Path,
) -> CodePackageDeltaPath | None:
    path_text = _optional_text(file_payload.get("path"))
    if path_text is None:
        return None
    path = Path(path_text)
    if not path.is_absolute():
        path = (workspace_root / path).resolve()
    relative_path = _api_delta_generated_code_package_relative_path(
        path=path,
        manifest_path_text=_optional_text(file_payload.get("manifest_path")),
        package_root=package_root,
        workspace_root=workspace_root,
    )
    if relative_path is None:
        return None
    change_kind = _api_delta_generated_code_package_delta_kind(
        change_kind=_optional_text(file_payload.get("change_kind")),
    )
    content_text = None
    size_bytes = None
    if change_kind != CodePackageDeltaKind.delete:
    ...
    return CodePackageDeltaPath(
        ...
        before_hash=_optional_text(file_payload.get("previous_digest")),
        after_hash=_optional_text(file_payload.get("current_digest")),
        size_bytes=size_bytes,
        language=CodeLanguage.python,
        is_structural=False,
        path_role=_api_delta_generated_code_package_path_role(
            relative_path=relative_path,
        ),
    )


def _api_delta_generated_code_package_relative_path(
    *,
    path: Path,
    ...
    for candidate in (
        ...
        if candidate is None:
            continue
        try:
            return PurePosixPath(candidate.relative_to(package_root).as_posix())
        except ValueError:
            continue
    return None
...
def _manifest_path_candidate(
    *,
    ...
    if manifest_path_text is None:
        return None
    path = Path(manifest_path_text)
    return path.resolve() if path.is_absolute() else (workspace_root / path).resolve()


def _api_delta_generated_code_package_delta_kind(
    *,
    change_kind: str | None,
) -> CodePackageDeltaKind:
    if change_kind == "delete":
        return CodePackageDeltaKind.delete
    if change_kind == "create":
        return CodePackageDeltaKind.create
    return CodePackageDeltaKind.update


def _api_delta_generated_code_package_name(
    *,
    target: str,
    paths: tuple[CodePackageDeltaPath, ...],
) -> str:
    for path in paths:
        relative_path = PurePosixPath(path.relative_path)
        if len(relative_path.parts) > 1 and relative_path.suffix == ".py":
            return relative_path.parts[0]
    return target.replace("_", "-")


def _api_delta_generated_code_package_path_role(
    *,
...
def _file_patch_file_payload(
    *,
    ...
    if render_section_refs:
        ...
        payload["service_protocol_render_section_ref_count"] = len(render_section_refs)
...
def _service_protocol_render_section_refs_for_file(
    *,
    ...
    for runtime_relpath, refs in refs_by_runtime_relpath.items():
        if not any(
            candidate.endswith(runtime_relpath) for candidate in path_candidates
        ):
            continue
        for ref in refs:
            section_key = _optional_text(ref.get("section_key"))
...
def _service_protocol_section_render_execution_payload(
    *,
    ...
    noop_section_operation_count = (
        len(section_operations) - changed_section_operation_count
    )
    ...
    if blocked:
        ...
        reason = (
            "api_service_protocol_section_render_execution_requires_resolved_sections"
        )
...
def _language_artifact_delta_apply_payload(
    *,
    ...
    fragment_guided = file_scope_source == "api_runtime_artifact_fragment_plan"
    blockers: list[str] = []
    if would_patch and not file_patch_ready:
        blockers.append(f"generated_artifact_file_patch_not_ready:{file_patch_status}")
    if (
        would_patch
        and event_driver_status != "materialization_event_artifact_driver_ready"
    ):
        blockers.append(
            "materialization_event_artifact_driver_not_ready:" f"{event_driver_status}"
        )
    if would_patch and file_scope_source not in {
        ...
        blockers.append(
            "language_artifact_delta_apply_scope_not_supported:" f"{file_scope_source}"
        )
    if would_patch and renderer_scope_status != (
        "generated_artifact_renderer_candidate_scope_applied"
    ):
        blockers.append(
            "renderer_candidate_scope_not_applied:" f"{renderer_scope_status}"
        )
    ...
    *,
    ...
    for event in _materialization_event_report_events(
        ...
        event_ref = {
            ...
            "ontology_subject_kind": _optional_text(event.get("ontology_subject_kind")),
    ...
    return {relpath: tuple(refs) for relpath, refs in sorted(refs_by_relpath.items())}
...
def _semantic_event_refs_for_file_payload(
    *,
    ...
    for runtime_relpath, event_refs in event_refs_by_relpath.items():
        if not (
            manifest_path.endswith(runtime_relpath) or path.endswith(runtime_relpath)
        ):
            continue
...
def _language_from_file_payload(*, file_payload: Mapping[str, object]) -> str | None:
    for field_name in ("manifest_path", "path"):
        ...
        for index, part in enumerate(parts):
            if part in {"public_package", "service_protocol"} and index + 1 < len(
                parts
            ):
...
def _materialization_event_report_events(
    *,
    ...
    return tuple(
        item for item in _tuple_evidence(raw_events) if isinstance(item, Mapping)
    )
