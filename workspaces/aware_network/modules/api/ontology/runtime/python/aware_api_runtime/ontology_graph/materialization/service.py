...
async def _build_api_plan_snapshot_objects(
    *,
    ...
    with _record_optional_phase(phase_timings_s, f"{prefix}.index_view_rows"):
        stream_policy_row_by_view: dict[str, Any] = {
            row.view_name: row for row in spec.plan.view_stream_policies
        }

    api_view_by_name: dict[str, ApiView] = {}
    with _record_optional_phase(phase_timings_s, f"{prefix}.create_views"):
        for row in spec.plan.views:
            view_prefix = f"{prefix}.view:{row.name}"
            with _record_optional_phase(
                phase_timings_s,
                f"{view_prefix}.resolve_observable",
            ):
            ...
            with _record_optional_phase(
                phase_timings_s,
                f"{view_prefix}.resolve_state_model",
            ):
                state_model_id = _resolve_class_config_id(
                    index=index,
                    accessible_graphs=accessible_graphs,
                    class_ref=row.state_model_ref,
                    class_config_id=row.state_model_id,
                )
            api_view = _remember_api_object(
                objects_by_id,
                ApiView(
            ...
            _append_unique(api.api_views, api_view)
            api_view_by_name[row.name] = api_view

            stream_policy_row = stream_policy_row_by_view.get(row.name)
            if stream_policy_row is not None:
                stream_mode = _resolve_api_view_stream_mode(
                    stream_policy_row.stream_mode
                )
                stream_policy = _remember_api_object(
                    objects_by_id,
                    ApiViewStreamPolicy(
                        id=stable_api_view_stream_policy_id(
                            api_view_id=api_view.id,
                            stream_mode=stream_mode.value,
                        ),
                        api_view_id=api_view.id,
                        stream_mode=stream_mode,
                        description=stream_policy_row.description,
                    ),
                )
                api_view.stream_policy = stream_policy

    with _record_optional_phase(
        phase_timings_s,
        f"{prefix}.create_view_capability_endpoints",
    ):
        for row in spec.plan.view_capability_endpoints:
            view_prefix = f"{prefix}.view:{row.view_name}"
            api_view = api_view_by_name.get(row.view_name)
            endpoint = endpoint_by_key.get((row.capability_name, row.endpoint_name))
            if api_view is None:
                raise RuntimeError(
                    "Invalid api ontology materialization plan: view endpoint binding references unknown view "
                    + f"(view={row.view_name!r}, action_key={row.action_key!r})"
                )
            if endpoint is None:
                raise RuntimeError(
                    "Invalid api ontology materialization plan: view endpoint binding references unknown endpoint "
                    + f"(view={row.view_name!r}, action_key={row.action_key!r}, endpoint_ref={row.endpoint_ref!r})"
                )
            capability_endpoint = _remember_api_object(
                objects_by_id,
                ApiViewCapabilityEndpoint(
                    id=stable_api_view_capability_endpoint_id(
                        api_view_id=api_view.id,
                        api_capability_endpoint_id=endpoint.id,
                    ),
                    api_view_id=api_view.id,
            ...
            _append_unique(api_view.capability_endpoints, capability_endpoint)
            logger.debug(
                "Created API view capability endpoint binding: %s action=%s endpoint=%s",
                view_prefix,
                row.action_key,
                row.endpoint_ref,
            )
    ...
    *,
    ...
    with _record_optional_phase(phase_timings_s, f"{prefix}.create_views"):
        for row in spec.plan.views:
            ...
            with _record_optional_phase(
                phase_timings_s,
                f"{view_prefix}.create_view",
            ):
                api_view = await api.create_view(
                    name=row.name,
                    object_projection_graph_observable_id=observable.id,
                    state_model_id=state_model_id,
                    view_ref=row.view_ref,
            ...
            if stream_policy_row is not None:
                with _record_optional_phase(
                    phase_timings_s,
                    f"{view_prefix}.set_stream_policy",
                ):
                    _ = await api_view.set_stream_policy(
                        stream_mode=_resolve_api_view_stream_mode(
                            stream_policy_row.stream_mode
                        ),
                        description=stream_policy_row.description,
    ...
    with _record_optional_phase(
        ...
        for row in spec.plan.view_capability_endpoints:
            ...
            with _record_optional_phase(
                phase_timings_s,
                f"{view_prefix}.bind_capability_endpoint:{row.action_key}",
            ):
                _ = await api_view.bind_capability_endpoint(
                    action_key=row.action_key,
                    api_capability_endpoint_id=endpoint.id,
                    endpoint_ref=row.endpoint_ref,
                    description=row.description,
...
def _resolve_api_view_stream_mode(stream_mode: str) -> ApiViewStreamMode:
    try:
        return ApiViewStreamMode((_normalize_token(stream_mode)))
    except ValueError as exc:
        raise RuntimeError(
            f"Invalid api ontology view stream mode {stream_mode!r}"
        ) from exc
...
def _build_step_details(*, spec: APIOntologyMaterializationSpec) -> dict[str, object]:
    ...
    return {
        ...
        "view_stream_policy_count": len(plan.view_stream_policies),
        "view_capability_endpoint_count": len(plan.view_capability_endpoints),
