...
from .dependency_contract import resolve_api_dto_package_python_dependencies
...
def _code_language_value(value: object) -> str:
    return str(getattr(value, "value", value) or "").strip()


def _is_code_language(value: object, expected: CodeLanguage) -> bool:
    return _code_language_value(value) == expected.value
...
def refresh_api_public_package_from_runtime_artifacts(
    *,
    ...
    use_external_api_dto_types = bool(
        ...
        and _is_code_language(public_package_target_language, CodeLanguage.python)
    )
    ...
    dto_graph_root_class_refs = (
        ...
        if _is_code_language(public_package_target_language, CodeLanguage.dart)
            ...
            materialization_config=MaterializationConfig(
                ...
                packages=[
                    ObjectConfigGraphPackageSpec(
                        ...
                        dependencies=resolve_api_dto_package_python_dependencies(
...
def _build_public_package_render_target(
    *,
    ...
    if _is_code_language(target_language, CodeLanguage.python):
    ...
    if _is_code_language(target_language, CodeLanguage.dart):
    ...
    raise ValueError(
        "Unsupported API public-package target language: "
        f"{_code_language_value(target_language)}"
    )
...
def _runtime_public_package_root(
    *,
    ...
    return (
        runtime_package_dir
        / "public_package"
        / _code_language_value(target_language)
        / "package"
    ).resolve()
...
def _runtime_package_spec(
    *,
    ...
    dependencies = _runtime_package_dependencies(
        package=package,
...
def _runtime_package_dependencies(
    *,
    package: ObjectConfigGraphPackageSpec,
    runtime_package_root: Path,
    metadata: Mapping[str, object],
) -> list[str]:
    path_dependencies = _metadata_path_dependencies(metadata)
    if not path_dependencies:
        return list(package.dependencies)

    repo_root = _metadata_repo_root(metadata)
    runtime_root = runtime_package_root.resolve()
    dependency_blocks: dict[str, str] = {}
    for name, path_text in path_dependencies.items():
        dependency_root = _resolve_metadata_path_dependency(
            path_text=path_text,
            repo_root=repo_root,
            package_root=package.package_root,
        )
        rel = os.path.relpath(dependency_root, start=runtime_root)
        dependency_blocks[name] = f"{name}:\n  path: {Path(rel).as_posix()}"

    dependencies: list[str] = []
    for dependency in package.dependencies:
        dependency_name = _dependency_block_name(dependency)
        if dependency_name in dependency_blocks:
            dependencies.append(dependency_blocks[dependency_name])
            continue
        dependencies.append(dependency)
    return dependencies


def _metadata_path_dependencies(metadata: Mapping[str, object]) -> dict[str, str]:
    raw_dependencies = metadata.get("path_dependencies")
    if not isinstance(raw_dependencies, Mapping):
        return {}
    dependencies: dict[str, str] = {}
    for raw_name, raw_path in raw_dependencies.items():
...
def _metadata_repo_root(metadata: Mapping[str, object]) -> Path | None:
    raw_repo_root = metadata.get("repo_root")
    repo_root_text = str(raw_repo_root or "").strip()
    if not repo_root_text:
        return None
    return Path(repo_root_text).expanduser().resolve()


def _resolve_metadata_path_dependency(
    *,
    path_text: str,
    repo_root: Path | None,
    ...
    if package_root is not None:
        return (Path(package_root).resolve() / path).resolve()
    return path.resolve()


def _dependency_block_name(dependency: str) -> str:
    head = str(dependency or "").strip().splitlines()[0] if dependency else ""
    return head.split(":", 1)[0].strip().replace("-", "_")
...
def _build_public_package_layout_strategy(
    *,
    ...
    if _is_code_language(render_job.target.target_language, CodeLanguage.python):
        return _ApiPublicPackagePythonLayout(
            base_dir=render_job.materialization_config.target_output_dir,
            import_root=render_job.materialization_config.import_root,
        )
    if _is_code_language(render_job.target.target_language, CodeLanguage.dart):
    ...
    raise ValueError(
        ...
        + f"{_code_language_value(render_job.target.target_language)}"
    )
...
def _compile_plan_input_graph_package_resolution(
    *,
    ...
    for indexed_package_name, aware_toml_path in sorted(
        ...
        try:
            spec = load_object_config_graph_package_manifest(toml_path=aware_toml_path)
        except Exception:
            continue
...
def _clean_api_runtime_render_output_for_full_render(
    *,
    ...
    safe_root = _api_runtime_scratch_root_for_output(
        aware_root=aware_root,
...
def _clean_api_python_package_output_for_full_render(
    *,
    ...
    _ = aware_root, materialization_config, candidate_paths
    # Published API DTO package roots are not scratch space. The Python package
    # strategy reconciles stale DTO modules from the successful render manifest.
    return


def _api_runtime_scratch_root_for_output(*, aware_root: Path, output_dir: Path) -> Path:
    declared_root = (aware_root / ".aware" / "api" / "runtime").resolve()
    if output_dir.is_relative_to(declared_root):
        return declared_root
    for parent in output_dir.parents:
        if (
            parent.name == "runtime"
            and parent.parent.name == "api"
            and parent.parent.parent.name == ".aware"
        ):
            return parent.resolve()
    return declared_root


def _validate_api_dto_rendered_entity_files(
    *,
    layout_strategy: ObjectConfigGraphRenderLayoutStrategy,
    ...
    if not _is_code_language(
        materialization_config.target_language, CodeLanguage.python
    ):
        return
    ...
    expected_paths = _entity_output_file_paths_for_graph(
        layout_strategy=layout_strategy,
        language_graph=language_graph,
        output_root=materialization_config.target_output_dir,
    )
    ...
    missing_paths = tuple(
        sorted(
            expected_paths.difference(rendered_path_set),
    ...
    package_name = materialization_config.source_package_name or (
        materialization_config.name
    )
    sample = ", ".join(
        (
            path.relative_to(
                materialization_config.target_output_dir.resolve()
            ).as_posix()
            if path.is_relative_to(materialization_config.target_output_dir.resolve())
            else path.as_posix()
        )
        for path in missing_paths[:5]
    )
    raise RuntimeError(
        "API DTO Python render did not produce declared entity files; refusing "
        "to build package output from an incomplete render manifest: "
        + f"package={package_name!r} missing_count={len(missing_paths)} "
        + f"sample=[{sample}]"
    )


def _entity_output_file_paths_for_graph(
    *,
    layout_strategy: ObjectConfigGraphRenderLayoutStrategy,
    language_graph: ObjectConfigGraph,
    output_root: Path,
...
def _absolute_output_path(*, output_root: Path, path: Path) -> Path:
    path = Path(path)
    if path.is_absolute():
        return path.resolve()
...
def _execute_materialization_package_post_steps(
    *,
    ...
    for package in package_results:
        ...
        if _is_code_language(materialization_config.target_language, CodeLanguage.dart):
...
def _expand_entity_candidate_paths_for_graph(
    *,
    ...
    if not candidate_paths or not _is_code_language(
        layout_strategy.language, CodeLanguage.python
    ):
...
def _python_language_external_import_overrides(
    *,
    ...
    if not _is_code_language(target_language, CodeLanguage.python):
        return {}
    ...
    for graph in sorted_graphs:
        try:
            ...
            package_names = tuple(
                package.package_name
                for package in accessible_dependency_packages
                if package.graph.id == graph.id
            )
            namespace_membership = build_namespace_membership_payload_from_ocg_identity(
                ocg=graph
            )
            raise RuntimeError(
                "API accessible dependency graph artifact emission failed for "
                "package-owned graph: "
                f"package_names={package_names!r} graph_id={graph.id!s} "
                f"name={graph.name!r} fqn_prefix={graph.fqn_prefix!r} "
                f"node_count={len(graph.object_config_graph_nodes)} "
                f"namespace_membership_count={len(namespace_membership)}"
            ) from exc
    payload = {
        "schema_version": 1,
