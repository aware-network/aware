...
_VALID_API_VIEW_STREAM_MODES = frozenset({"snapshot", "snapshot_delta", "live"})
    ...
    *,
    ...
    for relpath, source_text in source_texts.items():
        ...
        for node in tree.root_node.named_children:
            ...
            views_by_name: dict[str, APIViewOwnership] = {}
            views_by_ref: dict[str, APIViewOwnership] = {}
            ...
            for child in _iter_api_children(node=node):
                ...
                if child.type == "api_view_def":
                    view = _load_api_view_definition(
                        node=child,
                        api_name=api_name,
                        source_path=source_path,
                        source_rel=source_rel,
                    )
                    view_key = view.name.casefold()
                    if view_key in views_by_name:
                        raise ValueError(
                            f"API declaration {api_name!r} has duplicate view {view.name!r} "
                            + f"in {source_path}"
                        )
                    view_ref_key = (view.view_ref or "").casefold()
                    if view_ref_key in views_by_ref:
                        raise ValueError(
                            f"API declaration {api_name!r} has duplicate view_ref {view.view_ref!r} "
                            + f"in {source_path}"
                        )
                    views_by_name[view_key] = view
                    views_by_ref[view_ref_key] = view

            if not capabilities_by_name and not views_by_name:
                raise ValueError(
                    f"API declaration {api_name!r} must include at least one capability or view in {source_path}"
                )

            _validate_api_view_capability_endpoint_refs(
                api_name=api_name,
                source_path=source_path,
...
def _validate_api_view_capability_endpoint_refs(
    *,
    api_name: str,
    source_path: Path,
    views_by_name: dict[str, APIViewOwnership],
    ...
    endpoint_names_by_capability: dict[str, set[str]] = {
        capability_key: {
            endpoint.name.casefold().strip() for endpoint in capability.endpoints
        }
    ...
    for view in views_by_name.values():
        for binding in view.capability_endpoints:
            endpoint_parts = tuple(
                part.strip() for part in binding.endpoint_ref.split(".") if part.strip()
            )
            if len(endpoint_parts) != 3:
                raise ValueError(
                    f"API declaration {api_name!r} view {view.name!r} action "
                    + f"{binding.action_key!r} must reference endpoint as "
                    + f"{api_name}.<capability>.<endpoint>, got {binding.endpoint_ref!r} "
                    + f"in {source_path}"
                )
            ...
            if ref_api_name.casefold() != api_name.casefold():
                raise ValueError(
                    f"API declaration {api_name!r} view {view.name!r} action "
                    + f"{binding.action_key!r} references endpoint from another API "
                    + f"{binding.endpoint_ref!r} in {source_path}"
                )
            capability_key = capability_name.casefold()
            endpoint_key = endpoint_name.casefold()
            endpoint_names = endpoint_names_by_capability.get(capability_key)
            if endpoint_names is None:
                raise ValueError(
                    f"API declaration {api_name!r} view {view.name!r} action "
                    + f"{binding.action_key!r} references unknown capability "
                    + f"{capability_name!r} in {source_path}"
                )
            if endpoint_key not in endpoint_names:
                raise ValueError(
                    f"API declaration {api_name!r} view {view.name!r} action "
                    + f"{binding.action_key!r} references unknown endpoint "
                    + f"{binding.endpoint_ref!r} in {source_path}"
                )
...
def _load_api_view_definition(
    *,
    ...
    view_name = _symbol_key(_field_text(node, "view_name"))
    observable_ref = _qualified_text(node.child_by_field_name("observable"))
    state_model_ref = _qualified_text(node.child_by_field_name("state_model"))
    if not view_name:
        raise ValueError(
            f"API declaration {api_name!r} has view with empty name in {source_path}"
        )
    if not observable_ref:
        raise ValueError(
            f"API declaration {api_name!r} view {view_name!r} has empty observable reference in {source_path}"
        )
    if not state_model_ref:
        raise ValueError(
            f"API declaration {api_name!r} view {view_name!r} has empty state model reference in {source_path}"
        )

    stream_policy: APIViewStreamPolicyOwnership | None = None
    capability_endpoints: list[APIViewCapabilityEndpointOwnership] = []
    capability_endpoint_action_keys: set[str] = set()
    capability_endpoint_refs: set[str] = set()
    for child in _iter_api_view_children(node=node):
        if child.type == "api_view_stream_policy_def":
            if stream_policy is not None:
                raise ValueError(
                    f"API declaration {api_name!r} view {view_name!r} has multiple stream policies "
                    + f"in {source_path}"
                )
            stream_policy = _load_api_view_stream_policy_definition(
                node=child,
                api_name=api_name,
                view_name=view_name,
                source_path=source_path,
            ...
            continue
        if child.type == "api_view_capability_endpoint_def":
            binding = _load_api_view_capability_endpoint_definition(
                node=child,
                api_name=api_name,
                view_name=view_name,
                source_path=source_path,
            ...
            action_key = binding.action_key.casefold()
            if action_key in capability_endpoint_action_keys:
                raise ValueError(
                    f"API declaration {api_name!r} view {view_name!r} has duplicate endpoint binding "
                    + f"action_key {binding.action_key!r} in {source_path}"
                )
            endpoint_key = binding.endpoint_ref.casefold()
            if endpoint_key in capability_endpoint_refs:
                raise ValueError(
                    f"API declaration {api_name!r} view {view_name!r} has duplicate endpoint binding "
                    + f"endpoint_ref {binding.endpoint_ref!r} in {source_path}"
                )
            capability_endpoint_action_keys.add(action_key)
            capability_endpoint_refs.add(endpoint_key)
            capability_endpoints.append(binding)
            continue
    ...
    return APIViewOwnership(
        ...
        source_path=source_rel,
        view_ref=f"{api_name}.{view_name}",
        view_key=view_name,
        stream_policy=stream_policy,
        capability_endpoints=tuple(
            sorted(
                capability_endpoints,
                key=lambda item: (item.endpoint_ref, item.action_key.casefold()),
            )
        ),
        description=_extract_block_description(node.child_by_field_name("body")),
    )
...
def _load_api_view_stream_policy_definition(
    *,
    node: Node,
    api_name: str,
    view_name: str,
    ...
    if stream_mode not in _VALID_API_VIEW_STREAM_MODES:
        raise ValueError(
            f"API declaration {api_name!r} view {view_name!r} has invalid stream mode "
            + f"{stream_mode!r} in {source_path}"
        )
    return APIViewStreamPolicyOwnership(
...
def _load_api_view_capability_endpoint_definition(
    *,
    node: Node,
    api_name: str,
    view_name: str,
    ...
    action_key = _symbol_key(_field_text(node, "action_key"))
    endpoint_ref = _qualified_text(node.child_by_field_name("endpoint"))
    if not action_key:
        raise ValueError(
            f"API declaration {api_name!r} view {view_name!r} has endpoint binding with empty action_key "
            + f"in {source_path}"
        )
    if not endpoint_ref:
        raise ValueError(
            f"API declaration {api_name!r} view {view_name!r} action {action_key!r} has empty endpoint_ref "
            + f"in {source_path}"
        )
    capability_name, endpoint_name = _local_api_view_endpoint_ref_parts(
        api_name=api_name,
        view_name=view_name,
    ...
    return APIViewCapabilityEndpointOwnership(
        action_key=action_key,
...
def _local_api_view_endpoint_ref_parts(
    *,
    api_name: str,
    view_name: str,
    action_key: str,
    ...
    if len(endpoint_parts) != 2:
        raise ValueError(
            f"API declaration {api_name!r} view {view_name!r} action "
            + f"{action_key!r} must reference same-API endpoint as "
            + f"<capability>.<endpoint>, got {endpoint_ref!r} in {source_path}"
        )
...
def _iter_api_view_children(*, node: Node) -> tuple[Node, ...]:
    children: list[Node] = []
    body = node.child_by_field_name("body")
    if body is None:
        return ()
    for child in body.named_children:
        if child.type == "api_view_item":
            children.extend(child.named_children)
            continue
        if child.type == "api_view_stream_policy_def":
            children.append(child)
