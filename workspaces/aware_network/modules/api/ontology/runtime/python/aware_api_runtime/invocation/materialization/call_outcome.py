...
import inspect
import os
from typing import Awaitable, ContextManager, Mapping, Protocol, cast
    ...
    *,
    ...
    if hydrated_api_call is None:
        hydrated_api_call = await await_with_api_invocation_trace(
            ...
            phase="api_call_outcome.hydrate_api_call",
            fields=trace_fields,
        )
    hint_request_class_config = (
        _api_call_hint_request_class_config(api_call_hint=api_call_hint)
    ...
    runtime_lane = await await_with_api_invocation_trace(
        ...
        phase="api_call_outcome.resolve_runtime_lane",
        fields=trace_fields,
    )
    await await_with_api_invocation_trace(
        ...
        phase="api_call_outcome.ensure_projection_readiness",
        fields=trace_fields,
    )
    with api_invocation_trace_phase(
        "api_call_outcome.resolve_payload_mode",
        **trace_fields,
    ):
        ...
        if (
            not use_compact_receipt_payload
            and api_call_deferred
            and api_call_hint is not None
            and response_payload is not None
            and response_class_config is None
        ):
            use_compact_receipt_payload = True
            logger.info(
                "ApiCallOutcome materialization selected compact response payload "
                "receipt for deferred ApiCall without explicit response contract "
                f"api_call_id={api_call_id} status={status.value!r}"
            )
    resolved_response_class_config = response_class_config
    if (
        resolved_response_class_config is None
        and response_payload is not None
        and not use_compact_receipt_payload
    ):
        _, resolved_response_class_config = await await_with_api_invocation_trace(
            _resolve_committed_api_call_response_contract(
                index=index,
                target_lane=target_lane,
                api_source_lane=api_source_lane or target_lane,
                api_call_id=api_call_id,
            ),
            phase="api_call_outcome.resolve_response_contract",
            fields=trace_fields,
        )
    if use_compact_receipt_payload:
        with api_invocation_trace_phase(
            "api_call_outcome.response_payload_summary",
            **trace_fields,
        ):
    ...
    with api_invocation_trace_phase(
        "api_call_outcome.response_class_config_closure",
        compact_payload=use_compact_receipt_payload,
        **trace_fields,
    ):
    ...
    with api_invocation_trace_phase(
        "api_call_outcome.activate_create_outcome",
        **trace_fields,
    ):
        with runtime_lane.activate(commit=commit, publish=publish):
            api_call_for_outcome = hydrated_api_call
            created_deferred_api_call = False
            if (
                api_call_deferred
                and api_call_hint is not None
                and hint_request_class_config is not None
            ):
                if use_compact_receipt_payload:
                    with api_invocation_trace_phase(
                        "api_call_outcome.fast_deferred_api_call_from_hint",
                        **trace_fields,
                    ):
                        api_call_for_outcome = (
                            _build_deferred_compact_api_call_from_hint(
                                api_call_hint=api_call_hint,
                                request_class_config=hint_request_class_config,
                            )
                        )
                    created_deferred_api_call = True
                else:
                    with scoped_api_call_materialization_input(
                        request_payload={},
                        request_class_config=hint_request_class_config,
                        request_class_configs_by_id=None,
                        request_hash=api_call_hint.request_hash,
                    ):
                        with api_invocation_trace_phase(
                            "api_call_outcome.create_deferred_api_call",
                            **trace_fields,
                        ):
                            api_call_for_outcome = (
                                await ApiCall.create_via_api_capability_endpoint(
                                    api_capability_endpoint_id=(
                                        api_call_hint.api_capability_endpoint_id
                                    ),
                                    call_key=api_call_hint.call_key,
                                    request_class_config_id=cast(
                                        UUID,
                                        hint_request_class_config.id,
                                    ),
                                    description=api_call_hint.description,
                                )
                            )
                    created_deferred_api_call = True
                    if api_call_for_outcome.id != api_call_hint.id:
                        raise RuntimeError(
                            "Deferred ApiCallOutcome materialization created "
                            "mismatched ApiCall identity: "
                            f"expected_api_call_id={api_call_hint.id} "
                            f"created_api_call_id={api_call_for_outcome.id}"
                        )
            with scoped_api_call_outcome_materialization_input(
                ...
                api_call=api_call_for_outcome,
            ):
                if created_deferred_api_call:
                    if api_call_for_outcome.id is None:
                        raise RuntimeError(
                            "Deferred ApiCallOutcome materialization requires "
                            "created ApiCall.id"
                        )
                    with api_invocation_trace_phase(
                        "api_call_outcome.construct_deferred_compact_outcome",
                        **trace_fields,
                    ):
                        api_call_outcome = ApiCallOutcome(
                            id=stable_api_call_outcome_id(
                                api_call_id=api_call_for_outcome.id,
                            ),
                            api_call_id=api_call_for_outcome.id,
                    ...
                    with api_invocation_trace_phase(
                        "api_call_outcome.attach_deferred_compact_outcome",
                        **trace_fields,
                    ):
                        api_call_for_outcome.outcome = api_call_outcome
                else:
                    with api_invocation_trace_phase(
                        "api_call_outcome.create_standard_outcome",
                        **trace_fields,
                    ):
                        api_call_outcome = await api_call_for_outcome.create_outcome(
                            status=status,
                            error=error,
                        )

    with api_invocation_trace_phase(
        "api_call_outcome.build_result",
        **trace_fields,
    ):
...
def _api_call_hint_request_class_config(
    *,
    api_call_hint: ApiCall,
) -> ClassConfig | None:
    request_model = api_call_hint.request_model
    if request_model is None:
        return None
    request_class_config = request_model.class_config
    if isinstance(request_class_config, ClassConfig):
        return request_class_config
    request_class_config_id = request_model.class_config_id
    if request_class_config_id is None:
        return None
    return ClassConfig.model_construct(
        id=UUID(str(request_class_config_id)),
        value_mode=ClassValueMode.inline_value,
    )


def _build_deferred_compact_api_call_from_hint(
    *,
    api_call_hint: ApiCall,
    request_class_config: ClassConfig,
) -> ApiCall:
    """Build deferred compact ApiCall truth from the already validated hint."""

    if api_call_hint.id is None:
        raise RuntimeError("Deferred compact ApiCall hint requires ApiCall.id")
    if request_class_config.id is None:
        raise RuntimeError(
            "Deferred compact ApiCall hint requires request ClassConfig.id"
        )
    expected_api_call_id = stable_api_call_id(
        api_capability_endpoint_id=api_call_hint.api_capability_endpoint_id,
        call_key=api_call_hint.call_key,
    )
    if api_call_hint.id != expected_api_call_id:
        raise RuntimeError(
            "Deferred compact ApiCall hint is not stable for endpoint/call key: "
            f"expected_api_call_id={expected_api_call_id} "
            f"hint_api_call_id={api_call_hint.id}"
        )
    request_model = api_call_hint.request_model
    if request_model is None:
        raise RuntimeError(
            "Deferred compact ApiCall hint requires request_model truth"
        )
    request_model_id = api_call_hint.request_model_id or request_model.id
    if request_model_id is None:
        raise RuntimeError(
            "Deferred compact ApiCall hint requires request_model_id"
        )
    if request_model.id is not None and request_model.id != request_model_id:
        raise RuntimeError(
            "Deferred compact ApiCall hint has mismatched request_model id: "
            f"request_model_id={request_model_id} "
            f"request_model.id={request_model.id}"
        )
    if request_model.class_config_id != request_class_config.id:
        raise RuntimeError(
            "Deferred compact ApiCall hint has mismatched request ClassConfig: "
            f"expected_request_class_config_id={request_class_config.id} "
            f"got_request_class_config_id={request_model.class_config_id}"
        )
    request_hash = (api_call_hint.request_hash or "").strip()
    if not request_hash:
        raise RuntimeError("Deferred compact ApiCall hint requires request_hash")
    return ApiCall(
        id=api_call_hint.id,
        api_capability_endpoint_id=api_call_hint.api_capability_endpoint_id,
...
async def _resolve_committed_api_call_response_contract(
    *,
    ...
    response_class_config_hint = response_config.class_config
    if inspect.isawaitable(response_class_config_hint):
        response_class_config_hint = await response_class_config_hint
    response_class_config: ClassConfig | None = _class_config_hint_or_none(
        response_class_config_hint
    )
    ...
    if response_class_config is None:
        ...
        if orm_class is not None:
            response_class_config = _class_config_hint_or_none(
                orm_class.get_class_config()
            )
    registered_response_class_config = (
        _resolve_registered_pydantic_response_class_config(
            response_class_config_id=response_config.class_config_id,
...
def _class_config_hint_or_none(value: object) -> ClassConfig | None:
    return value if isinstance(value, ClassConfig) else None
