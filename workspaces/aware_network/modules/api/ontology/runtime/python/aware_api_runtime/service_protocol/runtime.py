...
from ..invocation import ApiInvocationIR, ResolvedApiInvocationEnvelope
from ..invocation.materialization.pydantic_class_config_closure import (
    pydantic_class_configs_by_id_for_ref,
)
from ..request_hash import compute_api_request_hash_from_mapping
...
def build_api_service_dispatch_plan_from_invocation_ir(
    *,
    index: MetaGraphRuntimeIndex,
    ir: ApiInvocationIR,
    runtime_package_dir: str | Path,
    ...
    endpoint_binding = loaded_package.endpoint_bindings.get(ir.endpoint_ref)
    if endpoint_binding is None:
        raise RuntimeError(
            "API service protocol package runtime could not resolve a compiled "
            "API dispatch binding for deferred endpoint_ref="
            f"{ir.endpoint_ref!r}."
        )
    request_class_config = _request_class_config_from_invocation_ir(
        index=index,
        ir=ir,
    ...
    envelope = _build_deferred_api_invocation_envelope(
        ir=ir,
        branch_id=branch_id,
...
def _request_class_config_from_invocation_ir(
    *,
    index: MetaGraphRuntimeIndex,
    ir: ApiInvocationIR,
    request_type_ref: str,
    ...
    if request_class_config_id is None:
        raise RuntimeError(
            "API service protocol deferred dispatch requires request_class_config_id "
    ...
    return ClassConfig.model_construct(
        ...
        name=_class_name_from_ref(request_type_ref),
        class_fqn=request_type_ref,
        value_mode=ClassValueMode.inline_value,
    )


def _build_deferred_api_invocation_envelope(
    *,
    ir: ApiInvocationIR,
    branch_id: UUID,
    projection_hash: str,
    ...
    request_model_id = stable_inline_value_instance_id(
        class_config_id=cast(UUID, request_class_config.id),
        owner_key=call_key,
    )
    ...
    return ResolvedApiInvocationEnvelope(
        ...
        request_class_config_id=cast(UUID, request_class_config.id),
