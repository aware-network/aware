...
def _code_language(value: object) -> CodeLanguage:
    language_value = _code_language_value(value)
    try:
        return CodeLanguage(language_value)
    except ValueError as exc:
        raise ValueError(f"Unknown CodeLanguage value: {language_value}") from exc
...
def build_api_public_package_render_job(
    *,
    ...
    target_language = _code_language(target.target_language)
    ...
    if target_language == CodeLanguage.python:
        dependencies = resolve_api_public_package_python_dependencies(
    ...
    elif target_language == CodeLanguage.dart:
        dart_dependencies = resolve_api_public_package_dart_dependencies(
            path_dependencies=(
                (
                    name.replace("-", "_"),
                    _path_dependency_package_value(
                        path=path,
        ...
        dependencies = list(dart_dependencies.dependencies)
        optional_dependencies = {
            group: list(group_dependencies)
            for group, group_dependencies in dart_dependencies.optional_dependencies.items()
        }
    ...
    materialization_config = MaterializationConfig(
        name=_build_materialization_name(target_language=target_language),
...
def build_api_service_protocol_render_job(
    *,
    ...
    materialization_config = MaterializationConfig(
        name=_build_service_protocol_materialization_name(
            target_language=target_language
        ),
        source_aware_toml_path=target.source_aware_toml_path,
        source_package_name=handoff.package_name,
...
def _build_materialization_name(*, target_language: CodeLanguage) -> str:
    return f"api-public-package-{target_language.value}"
...
def _build_service_protocol_materialization_name(
    *, target_language: CodeLanguage
    ...
    return f"api-service-protocol-{target_language.value}"
...
def _path_dependency_package_value(*, path, package_root) -> str:
    resolved_path = path.resolve()
    resolved_package_root = Path(package_root).resolve()
    rel = os.path.relpath(resolved_path, start=resolved_package_root)
