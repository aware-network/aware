...
def build_api_compile_plan(
    *,
    ...
    with _record_optional_phase(
        phase_timings_s,
        "build_api_compile_plan.bind_api_class_config_ids",
    ):
        api_ownership = bind_api_class_config_ids(
...
def _decode_api_ownership(*, item: object, index: int) -> APIOwnership:
    ...
    return APIOwnership(
        ...
        views=tuple(
            ...
            for view_index, view in enumerate(_optional_list(row.get("views")))
        ),
    )
...
def _decode_api_view(
    *,
    ...
    row = _required_mapping(
        item,
        field_name=f"api_ownership[{api_name}].views[{index}]",
    )
    view_name = _required_text(
        row.get("name"),
        field_name=f"api_ownership[{api_name}].views[{index}].name",
    )
    stream_policy = row.get("stream_policy")
    return APIViewOwnership(
        name=view_name,
        observable_ref=_required_text(
            row.get("observable_ref"),
            field_name=f"api_ownership[{api_name}].views[{view_name}].observable_ref",
        ),
        state_model_ref=_required_text(
            row.get("state_model_ref"),
            field_name=f"api_ownership[{api_name}].views[{view_name}].state_model_ref",
        ),
        state_model_id=_optional_uuid(row.get("state_model_id")),
        view_ref=_optional_text(row.get("view_ref")),
        view_key=_optional_text(row.get("view_key")),
        description=_optional_text(row.get("description")),
        source_path=_required_text(
            row.get("source_path"),
        ...
        stream_policy=(
            _decode_api_view_stream_policy(
                item=stream_policy,
                api_name=api_name,
                view_name=view_name,
            )
        ...
        capability_endpoints=tuple(
            _decode_api_view_capability_endpoint(
                item=capability_endpoint,
                api_name=api_name,
                view_name=view_name,
                index=capability_endpoint_index,
            )
            for capability_endpoint_index, capability_endpoint in enumerate(
                _optional_list(row.get("capability_endpoints"))
            )
        ),
    )


def _decode_api_view_capability_endpoint(
    *,
    item: object,
    api_name: str,
    view_name: str,
    ...
    row = _required_mapping(
        item,
        field_name=(
            f"api_ownership[{api_name}].views[{view_name}].capability_endpoints[{index}]"
        ),
    )
    return APIViewCapabilityEndpointOwnership(
        action_key=_required_text(
            row.get("action_key"),
            field_name=(
                f"api_ownership[{api_name}].views[{view_name}].capability_endpoints[{index}].action_key"
            ),
        ),
        endpoint_ref=_required_text(
            row.get("endpoint_ref"),
            field_name=(
                f"api_ownership[{api_name}].views[{view_name}].capability_endpoints[{index}].endpoint_ref"
            ),
        ),
        description=_optional_text(row.get("description")),
        source_path=_required_text(
            row.get("source_path"),
            field_name=(
                f"api_ownership[{api_name}].views[{view_name}].capability_endpoints[{index}].source_path"
            ),
        ),
    )


def _decode_api_view_stream_policy(
    *,
    item: object,
    api_name: str,
    view_name: str,
    ...
    row = _required_mapping(
        item,
        field_name=f"api_ownership[{api_name}].views[{view_name}].stream_policy",
    )
                ...
                "views": [
                    {
                        ...
                        "state_model_id": (
                            str(view.state_model_id)
                            if view.state_model_id is not None
                            else None
                        ),
                        ...
                        "stream_policy": (
                            {
                                "stream_mode": view.stream_policy.stream_mode,
                                "description": view.stream_policy.description,
                                "source_path": view.stream_policy.source_path,
                            }
...
def bind_api_endpoint_class_config_ids(
    *,
    ...
    return bind_api_class_config_ids(
        api_ownership=api_ownership,
        class_config_id_by_ref=class_config_id_by_ref,
    )


def bind_api_class_config_ids(
    *,
    api_ownership: tuple[APIOwnership, ...],
    class_config_id_by_ref: dict[str, UUID],
) -> tuple[APIOwnership, ...]:
    ...
    for api in api_ownership:
        ...
        for view in api.views:
            bound_views.append(
                replace(
                    view,
                    state_model_id=class_config_id_by_ref.get(
                        view.state_model_ref.strip()
                    ),
                )
            )
        bound_apis.append(
            replace(
                api,
                capabilities=tuple(bound_capabilities),
