...
@dataclass(frozen=True, slots=True)
class APIOntologyViewOperation:
    api_name: str
    name: str
    observable_ref: str
    state_model_ref: str
...
@dataclass(frozen=True, slots=True)
class APIOntologyViewStreamPolicyOperation:
    api_name: str
    view_name: str
    stream_mode: str
    description: str | None
...
@dataclass(frozen=True, slots=True)
class APIOntologyViewCapabilityEndpointOperation:
    api_name: str
    view_name: str
    action_key: str
    endpoint_ref: str
...
class APIOntologyPlan:
    ...
    views: tuple[APIOntologyViewOperation, ...]
    view_stream_policies: tuple[APIOntologyViewStreamPolicyOperation, ...]
    view_capability_endpoints: tuple[APIOntologyViewCapabilityEndpointOperation, ...]
...
def build_api_ontology_plans(
    *,
    ...
    for ownership in api_ownership:
        ...
        view_rows: list[APIOntologyViewOperation] = []
        view_stream_policy_rows: list[APIOntologyViewStreamPolicyOperation] = []
        view_capability_endpoint_rows: list[
            APIOntologyViewCapabilityEndpointOperation
        ] = []
        ...
        for view in ownership.views:
            view_name = _required_str_token(
                view.name,
                field_name=f"api_ownership[{api_name}].views[].name",
            )
            view_source_path = _required_str_token(
                view.source_path,
                field_name=f"api_ownership[{api_name}].views[{view_name}].source_path",
            )
            view_rows.append(
                APIOntologyViewOperation(
                    api_name=api_name,
                    name=view_name,
                    observable_ref=_required_str_token(
                        view.observable_ref,
                        field_name=f"api_ownership[{api_name}].views[{view_name}].observable_ref",
                    ),
                    state_model_ref=_required_str_token(
                        view.state_model_ref,
                        field_name=f"api_ownership[{api_name}].views[{view_name}].state_model_ref",
                    ),
                    state_model_id=view.state_model_id,
                    view_ref=_required_str_token(
                        view.view_ref or f"{api_name}.{view_name}",
                        field_name=f"api_ownership[{api_name}].views[{view_name}].view_ref",
                    ),
            ...
            if view.stream_policy is not None:
                view_stream_policy_rows.append(
                    APIOntologyViewStreamPolicyOperation(
                        api_name=api_name,
                        view_name=view_name,
                        stream_mode=_required_str_token(
                            view.stream_policy.stream_mode,
                            field_name=(
                                f"api_ownership[{api_name}].views[{view_name}].stream_policy.stream_mode"
                            ),
                        ),
                        description=view.stream_policy.description,
                        source_path=_required_str_token(
                            view.stream_policy.source_path,
                            field_name=(
                                f"api_ownership[{api_name}].views[{view_name}].stream_policy.source_path"
                            ),
                        ),
                    )
                )
            for binding in view.capability_endpoints:
                capability_name, endpoint_name = _api_view_endpoint_ref_parts(
                    api_name=api_name,
                    view_name=view_name,
                    action_key=binding.action_key,
                    endpoint_ref=binding.endpoint_ref,
                )
                view_capability_endpoint_rows.append(
                    APIOntologyViewCapabilityEndpointOperation(
                        api_name=api_name,
                        view_name=view_name,
                        action_key=_required_str_token(
                            binding.action_key,
                            field_name=(
                                f"api_ownership[{api_name}].views[{view_name}]"
                                + ".capability_endpoints[].action_key"
                            ),
                        ),
                        endpoint_ref=_required_str_token(
                            binding.endpoint_ref,
                            field_name=(
                                f"api_ownership[{api_name}].views[{view_name}]"
                                + ".capability_endpoints[].endpoint_ref"
                            ),
                        ),
                        capability_name=capability_name,
                        endpoint_name=endpoint_name,
                        ...
                        source_path=_required_str_token(
                            ...
                            field_name=(
                                f"api_ownership[{api_name}].views[{view_name}]"
                                + ".capability_endpoints[].source_path"
                            ),
                        ),
                    )
                )
        ...
        plans.append(
            APIOntologyPlan(
                ...
                view_stream_policies=tuple(
                    sorted(
                        view_stream_policy_rows,
                        key=lambda item: (
                            item.view_name,
                            item.stream_mode,
                            item.source_path,
                ...
                view_capability_endpoints=tuple(
                    sorted(
                        view_capability_endpoint_rows,
                        key=lambda item: (
                            item.view_name,
                            item.action_key,
                            item.endpoint_ref,
...
def _api_view_endpoint_ref_parts(
    *,
    api_name: str,
    view_name: str,
    action_key: str,
    ...
    endpoint_parts = tuple(
        part.strip() for part in endpoint_ref.split(".") if part.strip()
    )
    if len(endpoint_parts) != 3:
        raise ValueError(
            f"Invalid API view endpoint binding: api={api_name!r} view={view_name!r} "
            + f"action={action_key!r} must reference endpoint as "
            + f"{api_name}.<capability>.<endpoint>, got {endpoint_ref!r}"
        )
    ref_api_name, capability_name, endpoint_name = endpoint_parts
    if ref_api_name.casefold() != api_name.casefold():
        raise ValueError(
            f"Invalid API view endpoint binding: api={api_name!r} view={view_name!r} "
            + f"action={action_key!r} references another API endpoint {endpoint_ref!r}"
        )
    return capability_name, endpoint_name
    ...
    return [
        {
            ...
            "views": [
                {
                    ...
                    "state_model_id": (
                        str(row.state_model_id)
                        if row.state_model_id is not None
                        else None
                    ),
            ...
            "view_stream_policies": [
                ...
                for row in plan.view_stream_policies
            ],
            "view_capability_endpoints": [
                ...
                for row in plan.view_capability_endpoints
            ],
            "graphs": [
                {
                    "api_name": row.api_name,
...
def _decode_api_ontology_plan_row(
    *,
    ...
    views = tuple(
        ...
        for index, obj in enumerate(
            _expect_list(
                row.get("views", []), field_name=f"api_ontology[{row_index}].views"
            )
        )
    )
    view_stream_policies = tuple(
        _decode_view_stream_policy_row(
            row_index=row_index, api_name=api_name, item=obj, item_index=index
        )
        for index, obj in enumerate(
            _expect_list(
                row.get("view_stream_policies", []),
                field_name=f"api_ontology[{row_index}].view_stream_policies",
            )
        )
    )
    view_capability_endpoints = tuple(
        _decode_view_capability_endpoint_row(
            row_index=row_index, api_name=api_name, item=obj, item_index=index
        )
        for index, obj in enumerate(
            _expect_list(
                row.get("view_capability_endpoints", []),
                field_name=f"api_ontology[{row_index}].view_capability_endpoints",
            )
        )
    )
    graphs = tuple(
    ...
    return APIOntologyPlan(
        ...
        views=views,
        view_stream_policies=view_stream_policies,
        view_capability_endpoints=view_capability_endpoints,
        graphs=graphs,
...
def _decode_view_row(
    *,
    ...
    row = _expect_mapping(
        item, field_name=f"api_ontology[{row_index}].views[{item_index}]"
    )
    row_api_name = _required_str_token(
        row.get("api_name"),
        field_name=f"api_ontology[{row_index}].views[{item_index}].api_name",
    )
    if row_api_name != api_name:
        raise ValueError(
            "Invalid api compile plan: view.api_name must match api.name "
            + f"(api_ontology[{row_index}] api={api_name!r} view.api_name={row_api_name!r})"
        )
    view_name = _required_str_token(
        row.get("name"),
        field_name=f"api_ontology[{row_index}].views[{item_index}].name",
    )
    return APIOntologyViewOperation(
        api_name=row_api_name,
        name=view_name,
        observable_ref=_required_str_token(
            row.get("observable_ref"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].observable_ref",
        ),
        state_model_ref=_required_str_token(
            row.get("state_model_ref"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].state_model_ref",
        ),
        state_model_id=_optional_uuid_token(
            row.get("state_model_id"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].state_model_id",
        ),
        view_ref=_required_str_token(
            row.get("view_ref"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].view_ref",
        ),
        view_key=_optional_str_token(
            row.get("view_key"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].view_key",
        ),
        description=_optional_str_token(
            row.get("description"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].description",
        ),
        source_path=_required_str_token(
            row.get("source_path"),
            field_name=f"api_ontology[{row_index}].views[{item_index}].source_path",
        ),
    )


def _decode_view_stream_policy_row(
    *,
    row_index: int,
    api_name: str,
    item: object,
    ...
    row = _expect_mapping(
        item,
        field_name=f"api_ontology[{row_index}].view_stream_policies[{item_index}]",
    )
    row_api_name = _required_str_token(
        row.get("api_name"),
        field_name=f"api_ontology[{row_index}].view_stream_policies[{item_index}].api_name",
    )
    if row_api_name != api_name:
        raise ValueError(
            "Invalid api compile plan: view_stream_policy.api_name must match api.name "
            + f"(api_ontology[{row_index}] api={api_name!r} view_stream_policy.api_name={row_api_name!r})"
        )
    return APIOntologyViewStreamPolicyOperation(
        api_name=row_api_name,
        view_name=_required_str_token(
            row.get("view_name"),
            field_name=f"api_ontology[{row_index}].view_stream_policies[{item_index}].view_name",
        ),
        stream_mode=_required_str_token(
            row.get("stream_mode"),
            field_name=f"api_ontology[{row_index}].view_stream_policies[{item_index}].stream_mode",
        ),
        description=_optional_str_token(
            row.get("description"),
            field_name=f"api_ontology[{row_index}].view_stream_policies[{item_index}].description",
        ),
        source_path=_required_str_token(
            row.get("source_path"),
            field_name=f"api_ontology[{row_index}].view_stream_policies[{item_index}].source_path",
        ),
    )


def _decode_view_capability_endpoint_row(
    *,
    row_index: int,
    api_name: str,
    item: object,
    ...
    row = _expect_mapping(
        item,
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}]",
    )
    row_api_name = _required_str_token(
        row.get("api_name"),
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].api_name",
    )
    if row_api_name != api_name:
        raise ValueError(
            "Invalid api compile plan: view_capability_endpoint.api_name must match api.name "
            + f"(api_ontology[{row_index}] api={api_name!r} "
            + f"view_capability_endpoint.api_name={row_api_name!r})"
        )
    view_name = _required_str_token(
        row.get("view_name"),
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].view_name",
    )
    action_key = _required_str_token(
        row.get("action_key"),
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].action_key",
    )
    endpoint_ref = _required_str_token(
        row.get("endpoint_ref"),
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].endpoint_ref",
    )
    parsed_capability_name, parsed_endpoint_name = _api_view_endpoint_ref_parts(
        api_name=api_name,
        view_name=view_name,
    ...
    capability_name = _required_str_token(
        row.get("capability_name") or parsed_capability_name,
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].capability_name",
    )
    endpoint_name = _required_str_token(
        row.get("endpoint_name") or parsed_endpoint_name,
        field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].endpoint_name",
    )
    if capability_name.casefold() != parsed_capability_name.casefold():
        raise ValueError(
            "Invalid api compile plan: view_capability_endpoint.capability_name "
            + f"does not match endpoint_ref (endpoint_ref={endpoint_ref!r}, "
            + f"capability_name={capability_name!r})"
        )
    if endpoint_name.casefold() != parsed_endpoint_name.casefold():
        raise ValueError(
            "Invalid api compile plan: view_capability_endpoint.endpoint_name "
            + f"does not match endpoint_ref (endpoint_ref={endpoint_ref!r}, "
            + f"endpoint_name={endpoint_name!r})"
        )
    return APIOntologyViewCapabilityEndpointOperation(
        api_name=row_api_name,
        view_name=view_name,
        action_key=action_key,
        endpoint_ref=endpoint_ref,
        ...
        description=_optional_str_token(
            row.get("description"),
            field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].description",
        ),
        source_path=_required_str_token(
            row.get("source_path"),
            field_name=f"api_ontology[{row_index}].view_capability_endpoints[{item_index}].source_path",
        ),
    )
