...
_API_ENDPOINT_MODEL_CLASS_CACHE: dict[str, type[BaseModel]] = {}
...
@dataclass(frozen=True, slots=True)
class ApiEndpointInvocationTimingDiagnostics:
    """Timing receipt for one caller-side API endpoint invocation."""

    endpoint_ref: str
    discriminant: str
    api_invoker_timings_s: Mapping[str, float]
    transport_receipt: object | None = None
    ...
    def as_dict(self) -> dict[str, object]:
        payload: dict[str, object] = {
            ...
            "api_invoker_timings_s": dict(self.api_invoker_timings_s),
        }
        if self.transport_receipt is not None:
            payload["transport_receipt"] = self.transport_receipt
        return payload
...
class AwareApiEndpointInvoker:
    ...
    def __init__(self, transport: ApiEndpointTransport) -> None:
        ...
        self._last_invocation_diagnostics: (
            ApiEndpointInvocationTimingDiagnostics | None
        ) = None
    ...
    @property
    def last_invocation_diagnostics(
        self,
    ) -> ApiEndpointInvocationTimingDiagnostics | None:
        return self._last_invocation_diagnostics
    ...
    def prepare_api_endpoint_invocation(
        ...
        *,
        ...
        stream_event_class_refs = MappingProxyType(
            {
                ...
                for event in (
                    endpoint.endpoint.stream.events if endpoint.endpoint.stream else []
                )
            }
        )
        ...
        return PreparedApiEndpointInvocation(
            ...
            response_class_ref=(
                endpoint.endpoint.response.class_ref
    ...
    async def invoke_api_endpoint(
        ...
        *,
        ...
        phase_timings_s: dict[str, float] = {
            "api_invoker.prepare_s": _elapsed_seconds(phase_started_at),
        }
        phase_started_at = perf_counter()
        ...
        phase_timings_s["api_invoker.raw_invoke_s"] = _elapsed_seconds(phase_started_at)
        phase_started_at = perf_counter()
        status = _normalize_token(response.status)
        phase_timings_s["api_invoker.status_check_s"] = _elapsed_seconds(
            phase_started_at
        )
        if status == "pending":
            phase_timings_s["api_invoker.total_s"] = _elapsed_seconds(
                operation_started_at
            )
            self._record_last_invocation_diagnostics(
                endpoint_ref=prepared.endpoint.endpoint_ref,
                discriminant=prepared.endpoint.endpoint.discriminant,
                phase_timings_s=phase_timings_s,
                transport_receipt=_api_endpoint_transport_receipt(response=response),
            )
        ...
        phase_started_at = perf_counter()
        decoded = decode_api_endpoint_response_payload(
            prepared=prepared,
            response_payload=response.response_payload,
        )
        phase_timings_s["api_invoker.response_decode_s"] = _elapsed_seconds(
            phase_started_at
        )
        phase_timings_s["api_invoker.total_s"] = _elapsed_seconds(operation_started_at)
        self._record_last_invocation_diagnostics(
            endpoint_ref=prepared.endpoint.endpoint_ref,
            discriminant=prepared.endpoint.endpoint.discriminant,
            phase_timings_s=phase_timings_s,
    ...
    def _record_last_invocation_diagnostics(
        self,
        *,
        endpoint_ref: str,
        discriminant: str,
        ...
        self._last_invocation_diagnostics = ApiEndpointInvocationTimingDiagnostics(
            endpoint_ref=endpoint_ref,
            discriminant=discriminant,
            api_invoker_timings_s=MappingProxyType(
    ...
    async def invoke_api_endpoint_raw(
        ...
        *,
        ...
        invocation = ApiEndpointInvocation(
            ...
            request_payload=JsonObject(
                _normalize_api_endpoint_request_payload(request_payload)
            ),
        )
    ...
    async def open_api_endpoint_stream_raw(
        ...
        *,
        ...
        if not isinstance(stream, ApiEndpointStream):
            raise TypeError(
                "API endpoint stream transport returned an unexpected stream handle."
            )
        return stream
...
def decode_api_endpoint_response_payload(
    *,
    ...
    response_model_ref = (
        prepared.response_python_model_ref or prepared.response_class_ref
    )
...
def resolve_api_endpoint_model_class(class_ref: str) -> type[BaseModel]:
    cached = _API_ENDPOINT_MODEL_CLASS_CACHE.get(class_ref)
    ...
    if not module_name or not class_name:
        raise RuntimeError(
            "API endpoint response class_ref must be fully qualified: " f"{class_ref!r}"
        )
    ...
    _API_ENDPOINT_MODEL_CLASS_CACHE[class_ref] = model_cls
    return model_cls
...
def _generated_pydantic_package_model_namespace(
    class_ref: str,
...
def _resolve_api_invocation_endpoint(
    *,
    ...
    lookup_count = sum(value is not None for value in (endpoint_ref, discriminant)) + (
        ...
        or capability_name is not None
        or endpoint_name is not None
        else 0
    )
    if lookup_count != 1:
        raise ValueError(
...
def _validate_api_endpoint_client_contract(
...
def _normalize_json_object(payload: Mapping[str, Any]) -> dict[str, JsonValue]:
    ...
    for key, value in payload.items():
        if not isinstance(key, str):
            raise TypeError(
                "request_payload keys must be strings for invoke_api_endpoint."
            )
...
def _normalize_json_value(value: object) -> JsonValue:
    ...
    if isinstance(value_attr, bool | int | float | str):
        return value_attr
    raise TypeError(
        f"Unsupported JSON value for API endpoint payload: {type(value).__name__}"
    )
...
def _coerce_api_endpoint_response(response: object) -> ApiEndpointResponse:
    ...
    if isinstance(response, BaseModel):
        return ApiEndpointResponse(
            ...
            transport_receipt=getattr(response, "transport_receipt", None),
    ...
    if isinstance(response, Mapping):
        return ApiEndpointResponse(
            ...
            stream_lifecycle=_normalize_token(
                response.get("stream_lifecycle", "auto_close")
            ),
        )
    raise TypeError(
        f"Unsupported API endpoint response type: {type(response).__name__}"
    )
...
def _api_endpoint_transport_receipt(
    *,
    response: ApiEndpointResponse,
) -> object | None:
    if response.transport_receipt is not None:
        return response.transport_receipt
...
__all__ = [
    ...
    "ApiEndpointInvocationTimingDiagnostics",
