...
import aware_api.invoker as invoker_mod
...
class _RecordingTransport:
    def __init__(self, response: ApiEndpointResponse | None = None) -> None:
        self.response = response or ApiEndpointResponse(
...
async def test_invoker_delegates_endpoint_invocation_to_supplied_transport() -> None:
    ...
    diagnostics = invoker.last_invocation_diagnostics
    assert diagnostics is not None
    assert diagnostics.endpoint_ref == "home_devices.door.lock"
    assert "api_invoker.prepare_s" in diagnostics.api_invoker_timings_s
    assert "api_invoker.raw_invoke_s" in diagnostics.api_invoker_timings_s
    assert "api_invoker.response_decode_s" in diagnostics.api_invoker_timings_s
    assert "api_invoker.total_s" in diagnostics.api_invoker_timings_s
...
def test_invoker_resolves_authored_package_level_class_ref_from_package_bootstrap() -> (
    None
):
...
def test_invoker_caches_response_model_resolution_and_rebuild(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class CachedDoorResult(BaseModel):
        accepted: bool

    class_ref = "generated_home_api.models.cached_door_result.CachedDoorResult"
    _install_generated_model_module(
        monkeypatch,
        "generated_home_api.models.cached_door_result",
        CachedDoorResult=CachedDoorResult,
    )
    rebuilds: list[tuple[type[BaseModel], str]] = []
    ...
    def _record_rebuild(
        *,
        ...
        rebuilds.append((model_cls, class_ref))

    monkeypatch.setattr(
        invoker_mod,
        "_ensure_api_endpoint_model_rebuilt",
        _record_rebuild,
    )

    first = resolve_api_endpoint_model_class(class_ref)
    second = resolve_api_endpoint_model_class(class_ref)

    assert first is CachedDoorResult
    assert second is CachedDoorResult
    assert rebuilds == [(CachedDoorResult, class_ref)]
