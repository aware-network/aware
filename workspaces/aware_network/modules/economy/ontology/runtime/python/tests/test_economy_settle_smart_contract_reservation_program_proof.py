...
def _artifact_plan_name(artifact: dict[str, object]) -> str:
    ...
    if not isinstance(name, str):
        raise AssertionError(
            f"Invocation plan artifact missing plan.name: {artifact!r}"
        )
    return name
...
async def _apply_settle_smart_contract_reservation_sequence(
    *,
    runtime: MetaGraphRuntime,
    smart_contract_lane: LaneIds,
    payer_wallet_lane: LaneIds,
    receiver_wallet_lane: LaneIds,
    ...
    payer_expected_coin_balance: Decimal,
    payer_new_coin_balance: Decimal,
    receiver_finance_entity_id: UUID,
    receiver_wallet_id: UUID,
    receiver_wallet_public_id: UUID,
    receiver_expected_coin_balance: Decimal,
    receiver_new_coin_balance: Decimal,
    coin_id: UUID,
    escrow_id: UUID,
) -> None:
    calls = (
        ...
        await _invoke_instance(
            runtime=runtime,
            lane=payer_wallet_lane,
            projection_hash=wallet_projection_hash,
            object_id=SourceObjectId(payer_wallet_id),
        ...
        await _invoke_instance(
            runtime=runtime,
            lane=receiver_wallet_lane,
            projection_hash=wallet_projection_hash,
            object_id=SourceObjectId(receiver_wallet_id),
            class_fqn=ECONOMY_WALLET_CLASS_FQN,
            function_name="reconcile_coin_balance",
            args=[
                coin_id,
...
@pytest.mark.asyncio
async def test_meta_program_sequence_settle_smart_contract_reservation_idempotent(
    tmp_path: Path,
) -> None:
    repo_root = REPO_ROOT
    ...
    settle_smart_contract_reservation_invocation_plan_artifact = (
        _load_invocation_plan_artifact(
            repo_root=repo_root,
            program_rel_path=(
                "workspaces/aware_network/modules/economy/experiences/"
                "aware_economy/programs/settlement/"
                "settle_smart_contract_reservation_v2.aware"
            ),
            program_name="SettleSmartContractReservation_v2",
        )
    )
    ...
    assert (
        _artifact_plan_name(settle_smart_contract_reservation_invocation_plan_artifact)
        == "SettleSmartContractReservation_v2"
    )
    assert (
    ...
    with IsolatedAwareRoot(
        ...
        assert payer_lane.branch_id is not None
        assert receiver_lane.branch_id is not None
        assert provider_lane.branch_id is not None

        payer_identity_id = uuid4()
        receiver_identity_id = uuid4()
        payer_address, payer_public_key, payer_private_key_encrypted = (
            _custody_wallet_inputs(identity_id=payer_identity_id)
        )
        receiver_address, receiver_public_key, receiver_private_key_encrypted = (
            _custody_wallet_inputs(identity_id=receiver_identity_id)
        )
        ...
        payer_finance_entity_id = stable_finance_entity_id(
            identity_id=payer_identity_id
        )

        receiver_wallet_public_id = stable_wallet_public_id(
            public_key=receiver_public_key
        )
        _ = stable_wallet_private_id(
        ...
        receiver_finance_entity_id = stable_finance_entity_id(
        ...
        payer_start_balance = Decimal("10.0")
        receiver_start_balance = Decimal("2.0")
        for lane, wallet_id, balance in (
            (payer_lane, payer_wallet_id, payer_start_balance),
            (receiver_lane, receiver_wallet_id, receiver_start_balance),
        ):
            response = await _invoke_instance(
                runtime=runtime,
                lane=lane,
        ...
        pricing_policy_id = stable_pricing_policy_id(
            name="default-service-policy", version=1
        )
        await run_meta_runtime_proof(
            runtime=runtime,
            lane=provider_lane,
        ...
        assert (
            contract_setup_assertions.primitive(
                instance_id=permit_id, field_name="nonce"
            )
            == 0
        )
            ...
            calls=[
                ProofCall(
                    ...
                    args=[
                        ...
                        payer_finance_entity_id,
                        payer_wallet_public_id,
                        reservation_op_nonce,
                        "args:hash:1",
                        format(settlement_amount, "f"),
                        rate_snapshot_id,
        ...
        reserve_assertions.expect_instance(escrow_id)

        smart_contract_commit_before_invalid, _ = await _require_head_ids(
            branch_id=provider_lane.branch_id,
            projection_hash=smart_contract_projection_hash,
            label="smart_contract before invalid settlement sequence",
        )
        payer_wallet_commit_before_invalid, _ = await _require_head_ids(
            branch_id=payer_lane.branch_id,
            projection_hash=wallet_projection_hash,
            label="payer wallet before invalid settlement sequence",
        )
        receiver_wallet_commit_before_invalid, _ = await _require_head_ids(
            branch_id=receiver_lane.branch_id,
            projection_hash=wallet_projection_hash,
            label="receiver wallet before invalid settlement sequence",
        )
        await _assert_failed_no_commit(
            ...
            args=[
                ...
                reservation_id,
                payer_start_balance,
                payer_start_balance - settlement_amount,
                receiver_start_balance,
                receiver_start_balance + settlement_amount + Decimal("1.0"),
                coin_id,
            ],
            error_fragment="conservation mismatch",
        )
        await _assert_head_commit(
            branch_id=payer_lane.branch_id,
            projection_hash=wallet_projection_hash,
            expected_commit_id=payer_wallet_commit_before_invalid,
            label="payer wallet after invalid settlement sequence",
        )
        await _assert_head_commit(
            branch_id=receiver_lane.branch_id,
            projection_hash=wallet_projection_hash,
            expected_commit_id=receiver_wallet_commit_before_invalid,
            label="receiver wallet after invalid settlement sequence",
        )
        await _assert_head_commit(
            branch_id=provider_lane.branch_id,
            projection_hash=smart_contract_projection_hash,
            expected_commit_id=smart_contract_commit_before_invalid,
            label="smart_contract after invalid settlement sequence",
        )

        await _apply_settle_smart_contract_reservation_sequence(
            runtime=runtime,
            smart_contract_lane=provider_lane,
            payer_wallet_lane=payer_lane,
            receiver_wallet_lane=receiver_lane,
            smart_contract_projection_hash=smart_contract_projection_hash,
            ...
            payer_expected_coin_balance=payer_start_balance,
            payer_new_coin_balance=payer_start_balance - settlement_amount,
            receiver_finance_entity_id=receiver_finance_entity_id,
            receiver_wallet_id=receiver_wallet_id,
            receiver_wallet_public_id=receiver_wallet_public_id,
            receiver_expected_coin_balance=receiver_start_balance,
            receiver_new_coin_balance=receiver_start_balance + settlement_amount,
            coin_id=coin_id,
            escrow_id=escrow_id,
        )

        smart_contract_commit_after_first, smart_contract_oig_after_first = (
            await _require_head_ids(
                branch_id=provider_lane.branch_id,
                projection_hash=smart_contract_projection_hash,
                label="smart_contract after first settlement sequence",
            )
        )
        smart_contract_oig = await _materialize_lane_head(
            index=idx,
            branch_id=provider_lane.branch_id,
            projection_hash=smart_contract_projection_hash,
        )
        assertions_after_first = MetaOIGAssertions(oig=smart_contract_oig, index=idx)
        assertions_after_first.expect_instance(settlement_id)
        _expect_enum_value(
        ...
        assert (
            assertions_after_first.primitive(
                instance_id=reservation_id,
        ...
        transaction_branch_id = stable_portal_branch_id(
            object_instance_graph_id=smart_contract_oig_after_first,
        ...
        transaction_commit_after_first, _ = await _require_head_ids(
            ...
            projection_hash=transaction_projection_hash,
            label="transaction after first settlement sequence",
        )
        ...
        payer_wallet_commit_after_first, _ = await _require_head_ids(
            branch_id=payer_lane.branch_id,
            projection_hash=wallet_projection_hash,
            label="payer wallet after first settlement sequence",
        )
        payer_wallet_oig = await _materialize_lane_head(
            index=idx,
            branch_id=payer_lane.branch_id,
            projection_hash=wallet_projection_hash,
        )
        payer_wallet_assertions = MetaOIGAssertions(oig=payer_wallet_oig, index=idx)
        payer_wallet_balance_id = stable_wallet_balance_id(
            wallet_id=payer_wallet_id,
            coin_id=coin_id,
        )
        assert (
            payer_wallet_assertions.primitive(
                instance_id=payer_wallet_balance_id,
                field_name="balance",
            )
            == "0"
        )

        receiver_wallet_commit_after_first, _ = await _require_head_ids(
            branch_id=receiver_lane.branch_id,
            projection_hash=wallet_projection_hash,
            label="receiver wallet after first settlement sequence",
        )
        receiver_wallet_oig = await _materialize_lane_head(
            index=idx,
            branch_id=receiver_lane.branch_id,
            projection_hash=wallet_projection_hash,
        )
        receiver_wallet_assertions = MetaOIGAssertions(
            oig=receiver_wallet_oig,
            index=idx,
        )
        receiver_wallet_balance_id = stable_wallet_balance_id(
            wallet_id=receiver_wallet_id,
            coin_id=coin_id,
        )
        assert (
            receiver_wallet_assertions.primitive(
                instance_id=receiver_wallet_balance_id,
                field_name="balance",
            )
            == "12"
        )
        ...
        await _assert_head_commit(
            branch_id=provider_lane.branch_id,
            projection_hash=smart_contract_projection_hash,
            expected_commit_id=smart_contract_commit_after_first,
            label="smart_contract after idempotent settlement rerun",
        )
        await _assert_head_commit(
            branch_id=payer_lane.branch_id,
            projection_hash=wallet_projection_hash,
            expected_commit_id=payer_wallet_commit_after_first,
            label="payer wallet after idempotent settlement rerun",
        )
        await _assert_head_commit(
            branch_id=receiver_lane.branch_id,
            projection_hash=wallet_projection_hash,
            expected_commit_id=receiver_wallet_commit_after_first,
            label="receiver wallet after idempotent settlement rerun",
        )
        await _assert_head_commit(
            branch_id=transaction_branch_id,
            projection_hash=transaction_projection_hash,
            expected_commit_id=transaction_commit_after_first,
            label="transaction after idempotent settlement rerun",
        )
