...
from aware_content.handlers.impl.chain import content_chain as content_chain_impl
from aware_content_ontology.stable_ids import stable_content_chain_content_id
...
CONTENT_CLASS_FQN = "aware_content.content.Content"
CONTENT_PART_TEXT_CLASS_FQN = "aware_content.part.ContentPartText"
...
def _lane_ids(key: str) -> LaneIds:
    return LaneIds(
        branch_id=uuid5(NAMESPACE_URL, f"aware://tests/content/{key}/branch"),
        actor_id=uuid5(NAMESPACE_URL, f"aware://tests/content/{key}/actor"),
    )
...
def test_content_chain_content_identity_includes_content_id() -> None:
    content_chain_id = uuid5(NAMESPACE_URL, "aware://tests/content/chain")
    first_content_id = uuid5(NAMESPACE_URL, "aware://tests/content/chain/first")
    second_content_id = uuid5(NAMESPACE_URL, "aware://tests/content/chain/second")

    first_edge_id = stable_content_chain_content_id(
        content_chain_id=content_chain_id,
        content_id=first_content_id,
        position=0,
    )
    second_edge_id = stable_content_chain_content_id(
        content_chain_id=content_chain_id,
...
@pytest.mark.asyncio
async def test_content_chain_append_inline_text_uses_seed_as_content_key(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    content_id = uuid5(NAMESPACE_URL, "aware://tests/content/append-inline/content")
    edge_id = uuid5(NAMESPACE_URL, "aware://tests/content/append-inline/edge")
    captured: dict[str, object] = {}

    async def _create_content(**kwargs: object) -> object:
        captured["create_content"] = dict(kwargs)
        return SimpleNamespace(id=content_id)

    async def _append_content(**kwargs: object) -> UUID:
        captured["append_content"] = dict(kwargs)
        return edge_id

    monkeypatch.setattr(
        content_chain_impl.Content,
        "create_content",
        _create_content,
    )
    monkeypatch.setattr(content_chain_impl, "append_content", _append_content)

    content_chain = SimpleNamespace(
        id=uuid5(NAMESPACE_URL, "aware://tests/content/append-inline/chain"),
        content_chain_contents_edges=[],
    )

    result = await content_chain_impl.append_inline_text(
        content_chain=cast(Any, content_chain),
        seed_inline_text="Luis and Codex share one local conversation.",
    )

    assert result == edge_id
    assert captured["create_content"] == {
        "key": "Luis and Codex share one local conversation.",
        "title": None,
        "source": content_chain_impl.ContentSource.user,
        "token_count": None,
        "seed_inline_text": "Luis and Codex share one local conversation.",
    }
    assert captured["append_content"] == {
