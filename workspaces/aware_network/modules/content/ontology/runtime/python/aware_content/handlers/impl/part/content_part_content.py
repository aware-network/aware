...
async def create_content_part_content(
    content_id: UUID, content_part_id: UUID, position: int
) -> ContentPartContent:
    """
    ...
    content_part = ContentPart.by_id_cached(content_part_id)
    if content_part is None:
        raise ValueError(
            "ContentPart not available in write context "
            f"(content_part_id={content_part_id})"
        )
