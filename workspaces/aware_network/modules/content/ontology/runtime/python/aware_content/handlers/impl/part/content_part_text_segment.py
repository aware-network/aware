...
async def upsert(
    ...
    existing = ContentPartTextSegment.by_id_cached(segment_id)
    ...
    content_part_text = ContentPartText.by_id_cached(content_part_text_id)
    ...
    style = (
        ContentPartTextStyle.by_id_cached(style_id) if style_id is not None else None
    )
...
async def delete_segment(content_part_text_segment: ContentPartTextSegment) -> None:
    ...
    if style is None and style_id is not None:
        style = ContentPartTextStyle.by_id_cached(style_id)
