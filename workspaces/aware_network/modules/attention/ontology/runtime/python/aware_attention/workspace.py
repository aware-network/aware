...
from aware_attention.manifest.loader import load_aware_attention_toml_spec
from aware_attention.manifest.spec import AwareAttentionTomlSpec


@dataclass(frozen=True, slots=True)
class AttentionWorkspaceSnapshot:
    repo_root: Path
    package_root: Path
    spec_path: Path
    spec: AwareAttentionTomlSpec
...
class AttentionWorkspace:
    ...
    def __init__(self, *, spec_path: str | Path, repo_root: str | Path | None = None):
        ...
        if not resolved_spec_path.exists():
            raise FileNotFoundError(f"aware.attention.toml not found: {resolved_spec_path}")
    ...
    def build_snapshot(self) -> AttentionWorkspaceSnapshot:
        spec = load_aware_attention_toml_spec(toml_path=self._spec_path)
        ...
        if not sources_root.exists():
            raise FileNotFoundError(
                f"Attention sources_dir does not exist: {sources_root} (from {self._spec_path})"
            )
        if not sources_root.is_dir():
            raise NotADirectoryError(f"Attention sources_dir must be a directory: {sources_root}")

        anchor_path: Path | None = None
        if spec.build.anchor_path is not None:
            anchor_path = (self._package_root / spec.build.anchor_path).resolve()
            _assert_within(base=self._package_root, candidate=anchor_path, label="[build].anchor_path")
            if not anchor_path.exists():
                raise FileNotFoundError(f"Attention anchor_path does not exist: {anchor_path}")
            if not anchor_path.is_file():
                raise IsADirectoryError(f"Attention anchor_path must be a file: {anchor_path}")
        ...
        if anchor_path is not None:
            anchor_relpath = anchor_path.relative_to(self._package_root).as_posix()
            files_by_rel[anchor_relpath] = Path(anchor_relpath)
        ordered_source_files = tuple(files_by_rel[key] for key in sorted(files_by_rel))

        return AttentionWorkspaceSnapshot(
