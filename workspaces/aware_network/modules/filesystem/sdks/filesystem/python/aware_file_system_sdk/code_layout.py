...
from aware_code_sdk.dto import CodePackagePathRole
from aware_code_sdk.dto import CodeSemanticContract

from aware_file_system_sdk.base import FileSystemSdkError, normalize_relative_path
...
def _normalized_layout(layout_contract: CodePackageLayoutContract) -> _NormalizedLayout:
    ...
    manifest_contract = getattr(layout_contract, "manifest", None)
    manifest_path_value = (
        getattr(manifest_contract, "manifest_relative_path", None)
        if manifest_contract is not None
        else getattr(layout_contract, "manifest_relative_path", None)
    )
...
def _provider_key(
    *,
    ...
    if semantic_contract is not None:
        ...
        if provider_key is not None:
            return provider_key
    semantic_binding = getattr(layout_contract, "semantic_binding", None)
    return _optional_str(getattr(semantic_binding, "provider_key", None))
