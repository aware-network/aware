...
def build_aware_file_system_service_protocol_handler() -> object:
    return _AwareFileSystemServiceProtocolHandler()
...
class _FileSystemProtocolSupport:
    def __init__(self) -> None:
        self._rust_apply_service: RustWorkspaceApplyService | None = None

    def rust_apply_service(self) -> RustWorkspaceApplyService:
    ...
    def warm_rust_apply_service(self) -> JsonObject:
    ...
    def backend_receipt(
        ...
        *,
        ...
        timings: JsonObject = JsonObject()
        if duration_s is not None:
            timings["duration_s"] = duration_s
        receipt_metadata = JsonObject(metadata or {})
    ...
    def capabilities(
        ...
        *,
        ...
        service_capability = FileSystemBackendCapability(
            backend_kind=FileSystemBackendKind.service,
            backend_name="aware-file-system-service",
        ...
        capabilities = [service_capability, python_capability]
        ...
        selected = service_capability
        if requested_backend_kind is FileSystemBackendKind.python:
            selected = python_capability
        elif requested_backend_kind is FileSystemBackendKind.rust:
        ...
        elif requested_backend_kind is FileSystemBackendKind.service:
            selected = service_capability
...
class _FileSystemSnapshotCapabilityHandler:
    def __init__(self, *, support: _FileSystemProtocolSupport) -> None:
    ...
    async def scan(
        ...
        try:
            ...
            receipt = self._support.backend_receipt(
                ...
                metadata=JsonObject({"error": str(exc)}),
            )
            return ScanFileSystemSnapshotResponse(
                request_id=request.request_id,
                success=False,
...
class _FileSystemDeltaCapabilityHandler:
    def __init__(self, *, support: _FileSystemProtocolSupport) -> None:
        self._support = support
    ...
    async def apply(
        ...
        try:
            ...
            elif request.backend_kind is FileSystemBackendKind.rust:
...
class _FileSystemRootCapabilityHandler:
    def __init__(self, *, support: _FileSystemProtocolSupport) -> None:
        self._support = support
    ...
    async def verify(
        ...
        try:
            ...
            return VerifyFileSystemRootResponse(
                ...
                error=None if root_ok or not request.reject_path_escape else "path_escape_rejected",
                root_ok=root_ok,
                rejected_paths=rejected,
                backend_receipt=self._support.backend_receipt(
                    operation=request.operation,
                    metadata=JsonObject({"checked_path_count": len(request.relative_paths)}),
                ),
            )
        except Exception as exc:
            return VerifyFileSystemRootResponse(
...
class _FileSystemBackendCapabilityHandler:
    def __init__(self, *, support: _FileSystemProtocolSupport) -> None:
        self._support = support
...
class _FileSystemApiServiceProtocolHandler:
    def __init__(self, *, support: _FileSystemProtocolSupport) -> None:
        self.backend = _FileSystemBackendCapabilityHandler(support=support)
        self.delta = _FileSystemDeltaCapabilityHandler(support=support)
        self.root = _FileSystemRootCapabilityHandler(support=support)
        self.snapshot = _FileSystemSnapshotCapabilityHandler(support=support)


class _AwareFileSystemServiceProtocolHandler:
    def __init__(self) -> None:
        support = _FileSystemProtocolSupport()
        self._support = support
        self.filesystem = _FileSystemApiServiceProtocolHandler(support=support)

    def warm_rust_apply_backend(self) -> JsonObject:
...
def _scan_root_snapshot(
    *,
    ...
    started = perf_counter()
    scan_result, current_files = index.scan_relative_metadata(force_refresh=force_refresh)
    ...
    for relative_path in sorted(current_files):
        ...
        entries.append(
            FileSystemEntrySnapshot(
                ...
                metadata=_entry_metadata(root_path=root_path, relative_path=normalized, metadata=metadata),
                digest=digest,
            )
        )
    ...
    cache_hit_count = max(0, int(getattr(scan_result, "files_processed", 0)) - cache_miss_count)
...
def _collect_delta_set(
    *,
    ...
    for relative_path in sorted(selected_paths):
        ...
        elif before is not None and after is not None and _snapshot_entry_changed(before, after):
            entries.append(
                _delta_entry(
    ...
    totals = FileSystemDeltaTotals(
        create_count=sum(1 for entry in entries if entry.operation is FileSystemDeltaOperation.create),
        update_count=sum(1 for entry in entries if entry.operation is FileSystemDeltaOperation.update),
        delete_count=sum(1 for entry in entries if entry.operation is FileSystemDeltaOperation.delete),
        byte_count=sum(
            int(
                (
...
def _delta_entry(
    *,
    ...
    content_text = None
    if include_content and after is not None and operation is not FileSystemDeltaOperation.delete:
    ...
    *,
        ...
        if operation in {"create", "update"}:
            ...
            if target.exists():
                if not target.is_file():
                    raise ValueError(
                        f"FileSystem dry-run delete target is not a file: {entry.path.relative_path}"
                    )
                bytes_deleted += target.stat().st_size
            elif not policy.allow_delete_missing:
                raise ValueError(
                    f"FileSystem dry-run delete target is missing: {entry.path.relative_path}"
                )
        else:
    ...
    return FileSystemApplyReceipt(
        ...
        backend_receipt=support.backend_receipt(
            operation="apply_delta",
            duration_s=duration_s,
            metadata=JsonObject({"dry_run": True}),
        ),
    )
    ...
    *,
    ...
    if operation == "delete":
        if not target.exists() and not policy.allow_delete_missing:
            raise ValueError(f"FileSystem delete target is missing: {entry.path.relative_path}")
...
def _entry_content_bytes(entry: FileSystemDeltaEntry) -> bytes:
    if entry.content_ref is not None:
        raise ValueError(
            "FileSystem service v0 does not resolve content_ref payloads yet."
        )
...
def _verify_content_digest(
    ...
    if digest.algorithm is not FileSystemDigestAlgorithm.sha256:
        raise ValueError(f"Unsupported FileSystem digest algorithm: {digest.algorithm.value}")
...
def _validate_apply_policy(policy: FileSystemApplyPolicy) -> None:
    if not policy.reject_path_escape:
        raise ValueError("FileSystem service requires reject_path_escape=true.")
    if policy.store_artifacts:
        raise ValueError("FileSystem service v0 does not store artifacts yet.")
...
def _snapshot_entry_changed(
    ...
    return before.metadata.model_dump(mode="json") != after.metadata.model_dump(mode="json")
...
def _path_selected(
    ...
    *,
    ...
    if include_tokens and not any(_path_matches_selector(relative_path, token) for token in include_tokens):
        return False
    return not any(_path_matches_selector(relative_path, token) for token in exclude_tokens)
