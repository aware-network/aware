...
def create_blob_from_content(
    ...
    blob = StorageBlob(
        ...
        path_local=(
            str(blob_store._blob_path(sha))
            if isinstance(blob_store, LocalBlobStore)
            else None
        ),
    )
    logger.debug(
        f"🔧 Created new blob record {sha[:8]}... in bucket {bucket.name} ({len(content)} bytes)"
    )
    return blob
...
def lazy_text(blob: StorageBlob, blob_store: BlobStore) -> LazyBlobContent:
    ...
    if not blob.bucket:
        if blob.bucket_id:
            blob.bucket = StorageBucket.by_id_cached(blob.bucket_id)
            if not blob.bucket:
...
def get_content_as_text(
    blob: StorageBlob, blob_store: BlobStore, encoding: str = "utf-8"
) -> str:
    """
...
async def check_blob_deduplication(
    sha: str, bucket: StorageBucket
) -> StorageBlob | None:
    """
    ...
    """
    try:
        existing_blob = await StorageBlob.one(sha=sha, bucket_id=bucket.id)
        if existing_blob:
            logger.debug(
                f"✅ Reusing existing blob {sha[:8]}... in bucket {bucket.name}"
            )
...
def get_by_sha(sha: str, bucket: StorageBucket | None = None) -> StorageBlob | None:
    ...
    """
    logger.debug(
        "Synchronous SHA lookup is not backed by ORM reads; "
        f"use async StorageBlob.one(...) for blob {sha[:8]}..."
    )
    return None


def find_by_content(
    content: bytes | str, bucket: StorageBucket | None = None
) -> StorageBlob | None:
    """
