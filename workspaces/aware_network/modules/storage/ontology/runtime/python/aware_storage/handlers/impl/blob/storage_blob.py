...
async def create(sha: str, mime_type: str, size_bytes: int) -> StorageBlob:
    ...
    normalized_sha = (
        ""
        if ("" if sha is None else sha).strip() is None
        else ("" if sha is None else sha).strip()
    ).lower()
    ...
    _aware_stable_binding_6 = CONSTRUCTOR_STABLE_ID_BINDINGS_BY_CLASS_CONFIG_ID.get(
        "20ce9a91-cb0a-5517-b919-e9256e5c4fa8"
    )
    if _aware_stable_binding_6 is not None:
        ...
        _aware_missing_stable_keys_6 = [
            key
            for key in _aware_stable_key_names_6
            if key not in _aware_construct_values_6
        ]
        ...
        _aware_stable_values_6 = {
            key: _aware_construct_values_6[key] for key in _aware_stable_key_names_6
        }
        _aware_construct_id_6 = getattr(
            import_module("aware_storage_ontology.stable_ids"), _aware_stable_fn_6
        )(**_aware_stable_values_6)
        _aware_constructed_6 = StorageBlob.by_id_cached(_aware_construct_id_6)
        if _aware_constructed_6 is None:
            _aware_constructed_6 = StorageBlob(
                id=_aware_construct_id_6, **_aware_construct_values_6
            )
    else:
