...
async def _hydrate_bucket(blob: StorageBlob) -> StorageBucket | None:
    ...
    bucket = StorageBucket.by_id_cached(blob.bucket_id)
    if bucket is None:
        bucket = await StorageBucket.by_id(blob.bucket_id)
    blob.bucket = bucket
    return bucket
...
def _resolve_blob_store(
    *, blob: StorageBlob, bucket: StorageBucket | None
) -> BlobStore:
        ...
        try:
            ...
            if bucket.backend == StorageBackend.local and blob.path_local:
                store = LocalBlobStore(
                    _derive_local_root_from_path_local(blob.path_local)
                )
            else:
                raise
    ...
    raise ValueError(
        "Unable to resolve blob store: StorageBlob has no bucket and no path_local"
    )
...
async def download_blob_bytes(
    *,
    ...
    try:
        ...
        logger.debug(
            "download_blob_bytes: read complete sha=%s bytes=%d", blob.sha, len(data)
        )
        return data
...
async def download_file(
    ...
    if save_path is None:
        with tempfile.NamedTemporaryFile(
            mode="wb", prefix="download_", delete=False
        ) as temp_file:
