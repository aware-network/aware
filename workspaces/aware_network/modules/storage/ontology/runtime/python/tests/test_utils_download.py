...
@pytest.mark.asyncio
async def test_download_blob_bytes_reads_from_local_blob_store(
    ...
    async def _fake_by_id(
        cls, obj_id, cache_valid: bool = True, eager: bool = True
    ):  # noqa: ANN001
        return blob if obj_id == blob_id else None

    monkeypatch.setattr(StorageBlob, "by_id", classmethod(_fake_by_id))

    data = await storage_utils.download_blob_bytes(
        finance_entity_id=uuid4(), object_id=blob_id
    )
    assert data == content


@pytest.mark.asyncio
async def test_download_blob_bytes_calls_access_checker(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    storage_utils.clear_blob_store_cache()
    ...
    data = await storage_utils.download_blob_bytes(
        finance_entity_id=finance_entity_id, object_id=blob_id
    )
...
async def test_download_file_writes_bytes(
    ...
    saved = await storage_utils.download_file(
        finance_entity_id=uuid4(), object_id=blob_id, save_path=str(out_path)
    )
...
async def test_download_blob_bytes_raises_for_missing_blob(
    ...
    with pytest.raises(FileNotFoundError):
        await storage_utils.download_blob_bytes(
            finance_entity_id=uuid4(), object_id=uuid4()
        )
