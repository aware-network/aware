...
async def test_storage_module_proof_constructor_projected_to_storage_blob_opg(
    ...
    with IsolatedAwareRoot(
        ...
        lane = LaneIds(
            branch_id=uuid5(NAMESPACE_URL, "aware://tests/storage/branch"),
            actor_id=uuid5(NAMESPACE_URL, "aware://tests/storage/actor"),
        )
        result, assertions = await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    target="constructor",
                    class_fqn="aware_storage.blob.StorageBlob",
                    function_name="create",
                    args=[sha, " ", 12],
