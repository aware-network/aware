...
async def test_storage_blob_create_uses_cached_lookup_only(
    ...
    def _fake_by_id_cached(cls, obj_id):  # noqa: ANN001
        seen["obj_id"] = obj_id
        return existing

    monkeypatch.setattr(StorageBlob, "by_id_cached", classmethod(_fake_by_id_cached))

    result = await storage_blob_handler.create(
        sha=sha, mime_type="text/plain", size_bytes=12
    )
...
async def test_storage_blob_create_builds_new_blob_on_cache_miss(
    ...
    result = await storage_blob_handler.create(
        sha=sha_input, mime_type=" ", size_bytes=0
    )
