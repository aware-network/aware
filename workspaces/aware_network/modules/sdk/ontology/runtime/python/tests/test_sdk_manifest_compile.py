...
sdk workspace_sdk {
    ...
    operation load_semantic_source {
        "Load semantic source."
        endpoint workspace.semantic_source.semantic_source
    }

    operation materialize_workspace {
        "Materialize a workspace package."
        endpoint workspace.materialize.materialize
    }

    operation build_workspace {
        "Build a workspace."
        endpoint workspace.build.build
    }

    operation test_revision {
        "Test a workspace revision."
        endpoint workspace.revision.test_revision
    }
}
""",
    )
...
def _create_product_sdk_package(
    tmp_path: Path,
    *,
    package_name: str,
    fqn_prefix: str,
    ...
    package_root = tmp_path / "sdks" / sdk_name
    _write(
        package_root / "aware.sdk.toml",
        f"""
aware_sdk = 1
...
""",
    ...
    api_lines = "\n".join(f"    api {api_ref}" for api_ref in api_refs)
    operation_blocks = "\n\n".join(
        f"""    operation {operation_name} {{
        endpoint {endpoint_ref}
    }}"""
        for operation_name, endpoint_ref in operations
    )
    _write(
        package_root / source_file,
        f"""\
sdk {sdk_name} {{
{api_lines}
...
def test_compile_sdk_workspace_builds_ontology_mode_plan(tmp_path: Path) -> None:
    ...
    api_view = operation.api_views[0]
    assert api_view.api_view_ref == "workspace.status_panel"
    assert api_view.api_ref == "workspace"
    assert api_view.view_name == "status_panel"
    assert api_view.order == 1
    assert operation.api_view_capability_endpoints == ()
    ...
    assert (
        payload["sdk_configs"][0]["operations"][0]["api_views"][0]["api_view_ref"]
        == "workspace.status_panel"
    )
    assert (
...
def test_compile_sdk_workspace_rejects_undeclared_api_view(tmp_path: Path) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace
...
sdk workspace_client {
    ...
    operation load_status {
        endpoint workspace.status.get
        view identity.status_panel
    }
}
""",
    )

    with pytest.raises(ValueError, match="undeclared api view"):
        _ = compile_sdk_workspace(toml_path=sdk_toml_path, repo_root=tmp_path)


def test_compile_sdk_workspace_rejects_api_view_without_operation_endpoint(
    tmp_path: Path,
) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace
...
""",
    ...
    with pytest.raises(ValueError, match="must bind an operation endpoint"):
        _ = compile_sdk_workspace(toml_path=sdk_toml_path, repo_root=tmp_path)
...
def test_compile_workspace_aware_sdk_source_builds_compile_plan(
    tmp_path: Path,
) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace
...
def test_compile_code_aware_sdk_source_builds_view_operation_plan() -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace
    ...
    assert result.compile_plan is not None
    sdk_config = result.compile_plan.sdk_configs[0]
    assert sdk_config.name == "code_sdk"
    operations = {operation.name: operation for operation in sdk_config.operations}

    select_code = operations["select_code"]
    assert select_code.api_endpoints[0].endpoint_ref == ("code.view_action.select_code")
    assert select_code.api_views[0].api_view_ref == "code.package_selector"
    assert select_code.api_views[0].view_name == "package_selector"
    assert select_code.api_view_capability_endpoints == ()

    select_section = operations["select_section"]
    assert select_section.api_endpoints[0].endpoint_ref == (
        "code.view_action.select_section"
    )
    assert select_section.api_views[0].api_view_ref == "code.editor"
    assert select_section.api_views[0].view_name == "editor"
    assert select_section.api_view_capability_endpoints == ()
...
def test_compile_skill_aware_sdk_source_builds_compile_plan(tmp_path: Path) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace

    sdk_toml_path = _create_product_sdk_package(
        tmp_path,
        package_name="skill-sdk",
        fqn_prefix="aware_skill_sdk",
        sdk_name="skill_sdk",
        api_refs=("skill",),
        source_file="skill_sdk.aware",
        operations=(("invoke_skill", "skill.invoke.invoke"),),
    )
...
def test_compile_network_aware_sdk_source_builds_compile_plan(
    tmp_path: Path,
) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace
    ...
    sdk_toml_path = _create_product_sdk_package(
        tmp_path,
        package_name="network-sdk",
        fqn_prefix="aware_network_sdk",
        sdk_name="network_sdk",
        api_refs=("network",),
        source_file="aware/network_sdk.aware",
        operations=(
            ("register_node", "network.node.register"),
            ("upsert_peer", "network.peer.upsert"),
            ("list_peers", "network.peer.list"),
            ("publish_hosted_service", "network.hosted_service.publish"),
            ("list_hosted_services", "network.hosted_service.list"),
            (
...
def test_compile_environment_aware_sdk_source_builds_compile_plan(
    tmp_path: Path,
) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace
    ...
    sdk_toml_path = _create_product_sdk_package(
        tmp_path,
        package_name="environment-sdk",
        fqn_prefix="aware_environment_sdk",
        sdk_name="environment_sdk",
        api_refs=("environment",),
        source_file="environment_sdk.aware",
        operations=(
            ("ensure_ready", "environment.ready.ensure_ready"),
            ("invoke_function", "environment.function_call.invoke_function"),
            ("run_program", "environment.program.run_program"),
            (
...
def test_compile_hub_aware_sdk_source_builds_compile_plan(tmp_path: Path) -> None:
    from aware_sdk_runtime.compile import compile_sdk_workspace

    sdk_toml_path = _create_product_sdk_package(
        tmp_path,
        package_name="hub-sdk",
        fqn_prefix="aware_hub_sdk",
        sdk_name="hub_sdk",
        api_refs=("hub",),
        source_file="aware/hub_sdk.aware",
        operations=(
            ("discover_public_map", "hub.public_map.discover"),
            (
                "discover_code_package_channel_heads",
                "hub.code_package_channel_heads.discover",
            ),
            ("publish_code_package", "hub.code_package.publish"),
            ("resolve_deployment_artifact", "hub.deployment_artifact.resolve"),
        ),
    )
