...
async def test_sdk_config_projection_module_proof(tmp_path: Path) -> None:
    ...
    api_view_observable_id = uuid5(
        NAMESPACE_URL,
        "sdk://tests/config/api-view-observable",
    )
    api_view_id = stable_api_view_id(
        api_id=workspace_api_id,
        object_projection_graph_observable_id=api_view_observable_id,
        name="status_panel",
    )
    ...
    operation_api_view_endpoint_id = (
        stable_sdk_operation_api_view_capability_endpoint_id(
            sdk_operation_id=operation_id,
            sdk_operation_api_capability_endpoint_id=operation_endpoint_id,
            api_view_id=api_view_id,
    ...
    with IsolatedMetaAwareRoot(tmp_path / "aware_root") as aware_root:
        ...
        with lane.activate(commit=True, publish=False):
            await operation.bind_api_view_capability_endpoint(
                sdk_operation_api_capability_endpoint_id=operation_endpoint_id,
                api_view_id=api_view_id,
                api_view_capability_endpoint_id=api_view_capability_endpoint_id,
                api_view_ref="workspace.status_panel",
                action_key="select_status",
                endpoint_ref="workspace.status.status",
            )
    ...
    assertions.expect_instance(operation_api_view_endpoint_id)
    ...
    assertions.expect_edge(
        source_id=operation_id,
        target_id=operation_api_view_endpoint_id,
        relationship_name="api_view_capability_endpoints",
    )
    assertions.expect_edge(
        source_id=operation_id,
    ...
    assertions.expect_primitive(
        instance_id=operation_api_view_endpoint_id,
        field_name="api_view_ref",
        expected="workspace.status_panel",
    )
    assertions.expect_primitive(
        instance_id=operation_api_view_endpoint_id,
        field_name="action_key",
        expected="select_status",
    )
    assertions.expect_primitive(
        instance_id=operation_api_view_endpoint_id,
        field_name="endpoint_ref",
    ...
    _expect_uuid_primitive(
        assertions,
        instance_id=operation_api_view_endpoint_id,
        field_name="sdk_operation_api_capability_endpoint_id",
        expected=operation_endpoint_id,
    )
    _expect_uuid_primitive(
        assertions,
        instance_id=operation_api_view_endpoint_id,
    ...
    _expect_uuid_primitive(
        assertions,
        instance_id=operation_api_view_endpoint_id,
        field_name="api_view_capability_endpoint_id",
        expected=api_view_capability_endpoint_id,
    )
    _expect_uuid_primitive(
        assertions,
