...
def test_sdk_operation_api_view_binding_resolves_api_owned_endpoint() -> None:
    from aware_api_runtime.view_bindings import (  # noqa: WPS433
        ApiViewCapabilityEndpointBinding,
        api_view_capability_endpoint_catalog_from_bindings,
    )
    from aware_sdk_runtime.models import (
        SdkOperationApiViewPlan,
    ...
    api_view_id = uuid4()
    endpoint_id = uuid4()
    api_view_endpoint_id = uuid4()
    catalog = api_view_capability_endpoint_catalog_from_bindings(
        bindings=(
            ApiViewCapabilityEndpointBinding(
                api_ref="code",
                api_view_ref="code.package_selector",
                endpoint_ref="code.view_action.select_code",
                action_key="select_code",
                api_view_id=api_view_id,
                api_capability_endpoint_id=endpoint_id,
                api_view_capability_endpoint_id=api_view_endpoint_id,
                description="Select Code source.",
            ),
        ),
    )
    operation = SdkOperationPlan(
        name="select_code",
        source_path="code_sdk.aware",
        api_endpoints=(
            SdkOperationEndpointPlan(
                name="select_code",
                endpoint_ref="code.view_action.select_code",
    ...
    resolved = sdk_materialization_service._resolve_operation_api_view_capability_endpoint_plans(
        operation=operation,
        api_view_endpoint_catalog=catalog,
    )

    binding = resolved.api_view_capability_endpoints[0]
    assert binding.api_view_ref == "code.package_selector"
    assert binding.endpoint_ref == "code.view_action.select_code"
    assert binding.action_key == "select_code"
    assert binding.api_view_id == api_view_id
    assert binding.api_view_capability_endpoint_id == api_view_endpoint_id


def test_sdk_api_view_resolution_loads_declared_api_package_compile_plan(
    tmp_path: Path,
) -> None:
    ...
    from aware_api_runtime.ir import (  # noqa: WPS433
        APICompilePlan,
        encode_api_compile_plan_payload,
    )
    from aware_api_runtime.models import (  # noqa: WPS433
    ...
    from aware_api_runtime.ontology_graph.ontology import (  # noqa: WPS433
        build_api_ontology_plans,
    )
    from aware_meta_ontology.graph.config.object_config_graph import (  # noqa: WPS433
        ObjectConfigGraph,
    )
    from aware_meta_ontology.graph.config.object_config_graph_identity import (  # noqa: WPS433
        ObjectConfigGraphIdentity,
    )
    from aware_meta_ontology.graph.projection.object_projection_graph import (  # noqa: WPS433
        ObjectProjectionGraph,
    )
    from aware_meta_ontology.graph.projection.object_projection_graph_identity import (  # noqa: WPS433
        ObjectProjectionGraphIdentity,
    )
    from aware_meta_ontology.graph.projection.object_projection_graph_observable import (  # noqa: WPS433
    ...
    from aware_sdk_runtime.models import (  # noqa: WPS433
        SdkConfigApiPlan,
        SdkConfigPlan,
        SdkOperationApiViewPlan,
        SdkOperationEndpointPlan,
    ...
    from aware_sdk_runtime.manifest.spec import (  # noqa: WPS433
        ...
        APIOwnership(
                    ...
                    endpoints=(
                        APICapabilityEndpointOwnership(
                            ...
                            request_config=APICapabilityEndpointRequestConfigOwnership(
                                class_ref="aware_code.SelectCodeRequest",
            ...
            views=(
                APIViewOwnership(
                    ...
                    observable_ref="CodePackage.codes",
                    state_model_ref="aware_code.CodePackageSelectorViewStateV1",
                    state_model_id=uuid4(),
                    view_ref="code.package_selector",
                    view_key="package_selector",
    ...
    plan = APICompilePlan(
        ...
        source_files=("bindings/code.apis.aware",),
    ...
    compile_plan_path.parent.mkdir(parents=True)
    compile_plan_path.write_text(
        json.dumps(encode_api_compile_plan_payload(plan=plan), indent=2),
        encoding="utf-8",
    )
    graph_id = uuid4()
    ...
    ocgi_id = stable_object_config_graph_identity_id(key="aware_code")
    ...
    observable = ObjectProjectionGraphObservable(
        id=stable_object_projection_graph_observable_id(
            ...
            observable_key="codes",
        ),
        object_projection_graph_identity_id=opgi_id,
        key="CodePackage:codes",
        observable_key="codes",
        kind="instance",
    )
    code_graph = ObjectConfigGraph(
        id=graph_id,
        name="aware_code",
        fqn_prefix="aware_code",
        hash="sha256:aware_code",
        language=CodeLanguage.aware,
    ...
    (compile_plan_path.parent / "api.accessible_dependency_graphs.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "graphs": [
    ...
    manifest_spec = AwareSdkTomlSpec(
        ...
        build=AwareSdkTomlBuildSpec(
            compilation_mode=AwareSdkCompilationMode.sdk_ontology
        ),
        dependencies=[
            AwareSdkTomlDependencySpec(
                kind=AwareSdkDependencyKind.api_package,
                package_name="code-service-api",
                version_number=1,
            ),
            AwareSdkTomlDependencySpec(
    ...
    plans = sdk_materialization_service._api_compile_plans_for_sdk_dependencies(
        workspace_root=tmp_path,
        manifest_spec=manifest_spec,
        required_api_refs={"code"},
    )

    assert len(plans) == 1
    dependency = plans[0]
    assert dependency.package_name == "code-service-api"
    assert dependency.compile_plan.package_name == "code-service-api"
    assert len(dependency.accessible_graphs) == 1
    view_endpoint = dependency.compile_plan.api_ontology[0].view_capability_endpoints[0]
    assert view_endpoint.action_key == "select_code"
    assert view_endpoint.endpoint_ref == "code.view_action.select_code"

    sdk_graph = ObjectConfigGraph(
        id=uuid4(),
        name="aware_code_sdk",
        fqn_prefix="aware_code_sdk",
        hash="sha256:aware_code_sdk",
        language=CodeLanguage.aware,
        object_config_graph_nodes=[],
        object_projection_graphs=[],
        object_config_graph_identity=ObjectConfigGraphIdentity(
            id=stable_object_config_graph_identity_id(key="aware_code_sdk"),
            key="aware_code_sdk",
            label="ocg:aware_code_sdk",
            object_projection_graph_identities=[],
        ),
    )
    resolved = sdk_materialization_service._resolve_sdk_config_api_view_capability_endpoint_plans(
        index=cast(
            Any,
            SimpleNamespace(
                ocg=sdk_graph,
        ...
        sdk_config=SdkConfigPlan(
            name="code_sdk",
            source_path="code_sdk.aware",
            apis=(SdkConfigApiPlan(api_ref="code", source_path="code_sdk.aware"),),
            operations=(
                SdkOperationPlan(
                    name="select_code",
                    source_path="code_sdk.aware",
    ...
    binding = resolved.operations[0].api_view_capability_endpoints[0]
    assert binding.api_view_ref == "code.package_selector"
    assert binding.endpoint_ref == "code.view_action.select_code"
    assert binding.action_key == "select_code"
...
def _create_interface_sdk_fixture(*, workspace_root: Path) -> Path:
    sdk_toml_path = workspace_root / "sdks" / "interface" / "aware" / "aware.sdk.toml"
    _write(
        sdk_toml_path,
        """
aware_sdk = 1
...
fqn_prefix = "aware_interface_sdk"
version_number = 1
...
""",
    ...
    _write(
        sdk_toml_path.parent / "interface_sdk.aware",
        """\
sdk interface_sdk {
    api interface
...
""",
    ...
    _write(
        workspace_root / "sdks" / "interface" / "python" / "pyproject.toml",
    ...
    _write(
        ...
        / "aware_interface_sdk"
        / "lib"
        / "aware_interface_sdk.dart",
        "library aware_interface_sdk;\n",
    )
    return sdk_toml_path
...
async def test_sdk_workspace_provider_reports_full_rebuild_fallback_contract(
    ...
    implementation_python_config_key = code_package_source_config_key(
        manifest_kind="pyproject_toml",
        surface="sdk",
    )
    implementation_python_config_id = stable_code_package_config_id(
        config_key=implementation_python_config_key,
    )
            ...
            implementation_code_package_refs=(
                {
                    ...
                    "code_package_config_id": implementation_python_config_id,
                    "code_package_config_key": implementation_python_config_key,
...
async def test_interface_sdk_materialization_provider_commits_language_packages(
    ...
    sdk_toml_path = _create_interface_sdk_fixture(workspace_root=workspace_root)
    ...
    from aware_api_ontology.stable_ids import stable_api_package_id  # noqa: WPS433
...
@pytest.mark.asyncio
async def test_sdk_domain_commit_oig_id_uses_shallow_identity_metadata(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    branch_id = uuid4()
    projection_hash = "sdk-projection"
    ...
    monkeypatch.setattr(sdk_materialization_service, "FSCommitStore", _Store)

    assert await sdk_materialization_service._object_instance_graph_commit_id_from_domain_commit(
        branch_id=branch_id,
        projection_hash=projection_hash,
...
@pytest.mark.asyncio
async def test_sdk_domain_commit_oig_id_uses_envelope_without_legacy_hydration(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    domain_commit_id = uuid4()
    identity_id = uuid4()
