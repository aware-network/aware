...
from aware_api_runtime.view_bindings import (
...
from ..models import (
    SdkConfigPlan,
    SdkOperationApiViewCapabilityEndpointPlan,
    SdkOperationPlan,
)
...
@dataclass(frozen=True, slots=True)
class _SdkApiDependencyCompilePlan:
    package_name: str
...
class SdkImplementationCodePackageMaterialization:
    ...
    def to_payload(self) -> dict[str, object]:
        code_package_config_key = code_package_source_config_key(
            manifest_kind=self.manifest_kind,
            surface=self.code_package_surface,
        )
        code_package_config_id = getattr(
            self.code_package,
            "code_package_config_id",
            None,
        ) or stable_code_package_config_id(config_key=code_package_config_key)
        return {
...
async def materialize_sdk_package_from_manifest(
    *,
    ...
    with _record_phase(
        phase_timings_s,
        "resolve_sdk_operation_api_view_capability_endpoints",
    ):
        canonical_sdk = _resolve_sdk_config_api_view_capability_endpoint_plans(
            index=index,
            workspace_root=spec.workspace_root,
            manifest_spec=spec.manifest_spec,
            sdk_config=spec.compile_plan.sdk_configs[0],
        )
...
def _resolve_sdk_config_api_view_capability_endpoint_plans(
    *,
    index: MetaGraphRuntimeIndex,
    workspace_root: Path,
    manifest_spec: AwareSdkTomlSpec,
    ...
    if not any(operation.api_views for operation in sdk_config.operations):
        return sdk_config

    required_api_refs = {
        api_view.api_ref.casefold()
        for operation in sdk_config.operations
        for api_view in operation.api_views
        if api_view.api_ref.strip()
    }
    api_compile_plan_dependencies = _api_compile_plans_for_sdk_dependencies(
        workspace_root=workspace_root,
        manifest_spec=manifest_spec,
        required_api_refs=required_api_refs,
    )
    api_view_endpoint_bindings: list[ApiViewCapabilityEndpointBinding] = []
    for dependency in api_compile_plan_dependencies:
        api_view_endpoint_bindings.extend(
            api_view_capability_endpoint_bindings_from_compile_plan(
                plan=dependency.compile_plan,
                index=index,
                accessible_graphs=dependency.accessible_graphs,
            )
        )

    available_api_refs = {
        binding.api_ref.casefold() for binding in api_view_endpoint_bindings
    }
    missing_api_refs = sorted(required_api_refs - available_api_refs)
    if missing_api_refs:
        raise RuntimeError(
            "SDK API view resolution requires declared API package compile plans "
            + "for every operation view api_ref: "
            + f"missing_api_refs={missing_api_refs!r}"
        )

    api_view_endpoint_catalog = api_view_capability_endpoint_catalog_from_bindings(
        bindings=api_view_endpoint_bindings,
    )
    return replace(
        sdk_config,
        operations=tuple(
            _resolve_operation_api_view_capability_endpoint_plans(
                operation=operation,
                api_view_endpoint_catalog=api_view_endpoint_catalog,
            )
            for operation in sdk_config.operations
        ),
    )
...
def _api_compile_plans_for_sdk_dependencies(
    *,
    workspace_root: Path,
    manifest_spec: AwareSdkTomlSpec,
    required_api_refs: set[str],
) -> tuple[_SdkApiDependencyCompilePlan, ...]:
    required_api_refs = {
        api_ref.casefold().strip() for api_ref in required_api_refs if api_ref.strip()
    }
    api_package_names = tuple(
        sorted(
            {
    ...
    if not api_package_names:
        raise RuntimeError(
            "SDK API view resolution requires at least one declared api_package "
            + "dependency in aware.sdk.toml."
        )

    plans: list[_SdkApiDependencyCompilePlan] = []
    for package_name in api_package_names:
        runtime_package_dir = (
            workspace_root / ".aware" / "api" / "runtime" / package_name
        )
        ...
        if not isinstance(payload, Mapping):
            raise RuntimeError(
                "SDK API view resolution expected api.compile_plan.json to contain "
                + f"an object: path={compile_plan_path}"
            )
        plan = decode_api_compile_plan_payload(payload=payload)
        if plan.package_name != package_name:
            raise RuntimeError(
                "SDK API view resolution found mismatched API compile-plan "
                + "package_name: "
                + f"declared={package_name!r} actual={plan.package_name!r} "
                + f"path={compile_plan_path}"
            )
        plan_api_refs = {
            api.name.casefold().strip()
            for api in plan.api_ownership
            if api.name.strip()
        }
        if required_api_refs and not (plan_api_refs & required_api_refs):
            continue
        accessible_graphs = load_api_accessible_dependency_graphs_from_runtime_artifact(
            runtime_package_dir=runtime_package_dir,
        )
        plans.append(
            _SdkApiDependencyCompilePlan(
                package_name=package_name,
                compile_plan=plan,
...
def _resolve_operation_api_view_capability_endpoint_plans(
    *,
    operation: SdkOperationPlan,
    api_view_endpoint_catalog: Mapping[
        tuple[str, str],
        ApiViewCapabilityEndpointBinding,
    ],
) -> SdkOperationPlan:
    if not operation.api_views:
        return operation

    resolved_plans: list[SdkOperationApiViewCapabilityEndpointPlan] = []
    endpoint_refs = tuple(endpoint.endpoint_ref for endpoint in operation.api_endpoints)
    for api_view in operation.api_views:
        bindings_for_view = [
            api_view_endpoint_catalog.get(
                (api_view.api_view_ref.casefold(), endpoint_ref.casefold())
            )
            for endpoint_ref in endpoint_refs
        ]
        resolved_for_view = [
            binding for binding in bindings_for_view if binding is not None
        ]
        if not resolved_for_view:
            raise RuntimeError(
                "SDK operation API view binding found no API-owned endpoint "
                + "for the operation endpoints: "
                + f"operation={operation.name!r} "
                + f"api_view_ref={api_view.api_view_ref!r} "
                + f"endpoint_refs={endpoint_refs!r}"
            )
        for binding in resolved_for_view:
            resolved_plans.append(
                SdkOperationApiViewCapabilityEndpointPlan(
                    api_view_ref=binding.api_view_ref,
                    ...
                    order=api_view.order,
                    description=binding.description or api_view.description,
                )
            )

    return replace(
        operation,
        api_view_capability_endpoints=tuple(
            sorted(
                resolved_plans,
                key=lambda item: (
                    item.order,
