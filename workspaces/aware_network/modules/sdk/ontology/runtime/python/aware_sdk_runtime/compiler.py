...
def _load_sdk_operation_definition(
    *,
    ...
    api_views_by_ref: dict[str, SdkOperationApiViewOwnership] = {}
    operation_dependencies_by_ref: dict[str, SdkOperationDependencyOwnership] = {}
    for child in _iter_operation_children(node=node):
        if child.type == "sdk_operation_view_def":
            api_view_ref = _qualified_text(child.child_by_field_name("view"))
            api_ref = _resolve_declared_api_ref(
                api_ref=api_view_ref,
                declared_api_refs=declared_api_refs,
            )
            if len(api_view_ref.split(".")) != 2:
                raise ValueError(
                    f"SDK declaration {sdk_name!r} operation {operation_name!r} has invalid view ref "
                    + f"{api_view_ref!r} in {source_path}; expected api.view"
                )
            if api_ref is None:
                raise ValueError(
                    f"SDK declaration {sdk_name!r} operation {operation_name!r} references undeclared api view "
                    + f"{api_view_ref!r} in {source_path}"
                )
            api_view_key = api_view_ref.casefold()
            if api_view_key in api_views_by_ref:
                raise ValueError(
                    f"SDK declaration {sdk_name!r} operation {operation_name!r} has duplicate api view "
                    + f"{api_view_ref!r} in {source_path}"
                )
            api_views_by_ref[api_view_key] = SdkOperationApiViewOwnership(
        ...
        if (
            _resolve_declared_api_ref(
                api_ref=endpoint_ref, declared_api_refs=declared_api_refs
            )
            is None
        ):
            raise ValueError(
    ...
    _validate_operation_views_target_operation_endpoints(
        sdk_name=sdk_name,
        operation_name=operation_name,
        source_path=source_path,
        api_views=tuple(api_views_by_ref.values()),
        endpoints=tuple(endpoints_by_ref.values()),
        declared_api_refs=declared_api_refs,
    )
    ...
    return SdkOperationOwnership(
        ...
        api_views=tuple(
            sorted(
                api_views_by_ref.values(),
                key=lambda item: (item.api_view_ref, item.source_path),
            )
        ),
...
def _resolve_declared_api_ref(
    *, api_ref: str, declared_api_refs: tuple[str, ...]
) -> str | None:
    matches = [
        declared_api_ref
        for declared_api_ref in declared_api_refs
        if api_ref == declared_api_ref or api_ref.startswith(declared_api_ref + ".")
    ]
    if not matches:
        return None
...
def _validate_operation_views_target_operation_endpoints(
    *,
    sdk_name: str,
    operation_name: str,
    source_path: Path,
    api_views: tuple[SdkOperationApiViewOwnership, ...],
    ...
    if not api_views:
        return
    endpoint_api_refs = {
        resolved_api_ref.casefold()
        for endpoint in endpoints
        for resolved_api_ref in (
            _resolve_declared_api_ref(
                api_ref=endpoint.endpoint_ref,
                declared_api_refs=declared_api_refs,
            ),
        )
        if resolved_api_ref is not None
    }
    for api_view in api_views:
        view_api_ref = _resolve_declared_api_ref(
            api_ref=api_view.api_view_ref,
            declared_api_refs=declared_api_refs,
        )
        if view_api_ref is None:
            raise ValueError(
                f"SDK declaration {sdk_name!r} operation {operation_name!r} references undeclared api view "
                + f"{api_view.api_view_ref!r} in {source_path}"
            )
        if view_api_ref.casefold() not in endpoint_api_refs:
            raise ValueError(
                f"SDK declaration {sdk_name!r} operation {operation_name!r} view "
                + f"{api_view.api_view_ref!r} must bind an operation endpoint for api "
                + f"{view_api_ref!r} in {source_path}"
            )
