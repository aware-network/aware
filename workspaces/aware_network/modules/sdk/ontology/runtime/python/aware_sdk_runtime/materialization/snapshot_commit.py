...
def _build_sdk_config_manifest_snapshot_objects(
    *,
    ...
    for operation_plan in operations:
        ...
        endpoints_by_ref: dict[str, SdkOperationApiCapabilityEndpoint] = {}
        for endpoint_plan in operation_plan.api_endpoints:
            ...
            endpoints_by_ref[(endpoint_plan.endpoint_ref or "").casefold()] = endpoint
        for view_endpoint_plan in operation_plan.api_view_capability_endpoints:
            endpoint_ref = (view_endpoint_plan.endpoint_ref or "").strip()
            endpoint = endpoints_by_ref.get(endpoint_ref.casefold())
            if endpoint is None:
                raise RuntimeError(
                    "SdkConfig snapshot API view binding references an endpoint "
                    + "that is not bound by the same operation: "
                    + f"sdk_config={normalized_name!r} "
                    + f"operation={operation_name!r} "
                    + f"endpoint_ref={endpoint_ref!r} "
                    + f"api_view_ref={view_endpoint_plan.api_view_ref!r}"
                )
            view_endpoint = _remember(
                objects_by_id,
                _sdk_operation_api_view_capability_endpoint_from_plan(
                    sdk_operation_id=operation.id,
                    sdk_operation_api_capability_endpoint_id=endpoint.id,
                    plan=view_endpoint_plan,
                ),
            )
            operation.api_view_capability_endpoints.append(view_endpoint)
        for dependency_plan in operation_plan.sdk_operation_dependencies:
            target_operation_ref = (dependency_plan.target_operation_ref or "").strip()
            ...
            operation_dependency = _remember(
                ...
                SdkOperationDependency(
                    ...
                    description=(dependency_plan.description or "").strip() or None,
                ),
            )
...
def _sdk_operation_api_view_capability_endpoint_from_plan(
    *,
    sdk_operation_id: UUID,
    sdk_operation_api_capability_endpoint_id: UUID,
    plan: SdkOperationApiViewCapabilityEndpointPlan,
) -> SdkOperationApiViewCapabilityEndpoint:
    api_view_ref = (plan.api_view_ref or "").strip()
    endpoint_ref = (plan.endpoint_ref or "").strip()
    action_key = (plan.action_key or "").strip()
    if not api_view_ref or not endpoint_ref or not action_key:
        raise RuntimeError(
            "SdkConfig snapshot API view endpoint binding requires "
            "api_view_ref, endpoint_ref, and action_key"
        )
    return SdkOperationApiViewCapabilityEndpoint(
        id=stable_sdk_operation_api_view_capability_endpoint_id(
            ...
            api_view_id=plan.api_view_id,
            api_view_capability_endpoint_id=(plan.api_view_capability_endpoint_id),
        ),
        sdk_operation_id=sdk_operation_id,
        sdk_operation_api_capability_endpoint_id=(
            sdk_operation_api_capability_endpoint_id
...
def _build_sdk_package_manifest_snapshot_objects(
    *,
    ...
    for ocg_package_ref in sorted(
        ...
        expected_hash = (
            ocg_package_ref.expected_hash_sha256 or ""
        ).strip().lower() or None
    ...
    for dependency_ref in sorted(
        ...
        expected_hash = (
            dependency_ref.expected_hash_sha256 or ""
        ).strip().lower() or None
        if expected_hash is not None and (
            len(expected_hash) != 64
            or any(ch not in "0123456789abcdef" for ch in expected_hash)
