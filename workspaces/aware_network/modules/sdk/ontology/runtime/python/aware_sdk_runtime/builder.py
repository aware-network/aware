...
def _build_sdk_operation_plan(
    *,
    ...
    return SdkOperationPlan(
        ...
        api_views=tuple(
            SdkOperationApiViewPlan(
                api_view_ref=api_view.api_view_ref,
                api_ref=_resolve_api_ref_for_view(
                    api_view_ref=api_view.api_view_ref,
                    apis=apis,
                ),
                view_name=_resolve_api_view_name(api_view_ref=api_view.api_view_ref),
                source_path=api_view.source_path,
                order=order,
                description=api_view.description,
            )
            for order, api_view in enumerate(operation.api_views, start=1)
        ),
...
def _resolve_api_ref_for_view(
    *, api_view_ref: str, apis: tuple[SdkApiOwnership, ...]
) -> str:
    matches = [
        api.api_ref
        for api in apis
        if api_view_ref == api.api_ref or api_view_ref.startswith(api.api_ref + ".")
    ]
    if not matches:
        raise ValueError(
            f"SDK compile plan cannot resolve api_ref for view {api_view_ref!r}"
        )
    return max(matches, key=len)


def _resolve_api_view_name(*, api_view_ref: str) -> str:
    parts = [part.strip() for part in api_view_ref.split(".") if part.strip()]
    if len(parts) != 2:
        raise ValueError(
            f"SDK compile plan cannot resolve view name for view {api_view_ref!r}"
        )
    return parts[-1]
        ...
        "operations": [
            {
                ...
                "api_views": [
                    {
                        "api_view_ref": api_view.api_view_ref,
                        "api_ref": api_view.api_ref,
                        "view_name": api_view.view_name,
                        "source_path": api_view.source_path,
                ...
                "api_view_capability_endpoints": [
                    ...
                    for binding in operation.api_view_capability_endpoints
                ],
