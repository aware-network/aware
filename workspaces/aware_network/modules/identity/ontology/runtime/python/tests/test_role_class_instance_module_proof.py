...
def _identity_root_class_config_id(index: MetaGraphRuntimeIndex) -> UUID:
    identity_opg = next(
...
@pytest.mark.asyncio
async def test_role_class_instance_portal_chain_registered(tmp_path: Path) -> None:
    repo_root = REPO_ROOT
    ...
    with IsolatedAwareRoot(
        ...
        assert any(
            p.reference_field_name == "class_instance_identity"
            and p.target_projection_hash == oigi_opg.projection_hash
            for p in role_portals
        )
        assert any(
            p.reference_field_name == "role_config_class_config"
...
@pytest.mark.asyncio
async def test_role_add_class_instance_module_proof(tmp_path: Path) -> None:
    repo_root = REPO_ROOT
    ...
    role_config_name = "aware.identity.role-class-instance.module-proof"
    role_config_id = stable_role_config_id(name=role_config_name)
    object_instance_graph_identity_id = uuid5(
        NAMESPACE_URL,
        "aware.identity.role-class-instance.module-proof:oigi",
    )
    class_instance_id = uuid5(
        NAMESPACE_URL,
        "aware.identity.role-class-instance.module-proof:class-instance",
    )
    ...
    with IsolatedAwareRoot(
        ...
        context = runtime.context
        assert context is not None
        class_config_id = _identity_root_class_config_id(context.index)

        role_config_class_config_id = stable_role_config_class_config_id(
        ...
        await run_meta_runtime_proof(
            ...
            calls=[
                ProofCall(
                    ...
                    args=[
                        role_config_name,
                        "Role policy for RoleClassInstance module proof.",
                    ],
                ...
                ProofCall(
                    ...
                    object_id=SourceObjectId(role_config_id),
        ...
        result, assertions = await run_meta_runtime_proof(
            runtime=runtime,
            lane=_identity_lane(branch_id=role_id),
            opg_name="Role",
            calls=[
                ProofCall(
                    target="constructor",
                    ...
                    function_name="create",
                    args=[role_config_id, object_instance_graph_identity_id],
                ...
                ProofCall(
                    ...
                    object_id=SourceObjectId(role_id),
                    args=[class_instance_identity_id, role_config_class_config_id],
                ),
            ],
        )

        assertions.expect_root(role_id)
        assertions.expect_instance(role_class_instance_id)
        assertions.expect_edge(source_id=role_id, target_id=role_class_instance_id)
        assertions.expect_primitive(
            instance_id=role_class_instance_id,
            field_name="role_id",
            expected=role_id,
        )
        assertions.expect_primitive(
            ...
            expected=class_instance_identity_id,
        )
        assertions.expect_primitive(
            instance_id=role_class_instance_id,
            field_name="role_config_class_config_id",
            expected=role_config_class_config_id,
        )
        assert result.root_object_id == role_id
