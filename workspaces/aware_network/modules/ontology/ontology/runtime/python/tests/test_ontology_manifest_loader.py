...
def _targets_by_role(spec: object) -> dict[str, object]:
    return {
        target.role: target
        for target in getattr(spec, "language_materialization_targets")
    }
...
def test_default_layout_does_not_enable_dart_materialization(
    ...
    assert "python_runtime_handlers_meta" not in roles
...
def test_runtime_targets_declare_runtime_code_package_binding(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _prepend_runtime_roots(monkeypatch=monkeypatch)
    ...
    spec = load_aware_ontology_toml_spec_from_text(
        toml_text=_base_ontology_toml(
            layout_text="\n".join(
                (
                    "[runtime]",
                    'manifest = "runtime/python/pyproject.toml"',
                    'project_name = "aware-demo"',
    ...
    targets = _targets_by_role(spec)
    runtime_target = targets["python_runtime_handlers_meta"]
    assert runtime_target.declared_code_package_manifest_kind == "pyproject_toml"
    assert runtime_target.declared_code_package_manifest_relative_path == (
        "runtime/python/pyproject.toml"
    )
    assert runtime_target.declared_code_package_package_root == "runtime/python"
    assert runtime_target.declared_code_package_sources_root == (
        "runtime/python/aware_demo"
    )
