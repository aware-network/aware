...
def _default_language_materialization_targets(
    *,
    ...
    if python_orm_models_dir:
        ...
        targets.append(
            AwareOntologyLanguageMaterializationTargetSpec(
                ...
                package_name=_python_distribution_package_name(orm_models_import_root),
    ...
    runtime_output_dir = (
        _runtime_language_materialization_output_dir(
            ontology=ontology,
            runtime=runtime,
            layout=layout_spec,
        )
    ...
    runtime_code_package_binding = _runtime_code_package_target_binding(
        runtime=runtime,
...
def _runtime_code_package_target_binding(
    *,
    runtime: AwareOntologyRuntimeSpec | None,
    import_root: str,
) -> dict[str, str]:
    ...
    if manifest.name != "pyproject.toml":
        raise AwareOntologyTomlError(
            "[runtime].manifest must point to pyproject.toml for runtime handler "
            f"CodePackage binding: {runtime.manifest!r}"
        )
    package_root = manifest.parent
    sources_root = package_root / import_root.replace(".", "/")
    return {
        "declared_code_package_manifest_kind": "pyproject_toml",
        "declared_code_package_manifest_relative_path": manifest.as_posix(),
        "declared_code_package_package_root": package_root.as_posix(),
        "declared_code_package_sources_root": sources_root.as_posix(),
    }
