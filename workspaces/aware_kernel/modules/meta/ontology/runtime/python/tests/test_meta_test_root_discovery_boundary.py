...
from _meta_runtime_test_paths import META_TESTS_ROOT


def test_meta_runtime_tests_do_not_import_legacy_root_discovery() -> None:
    offenders: list[str] = []
    for path in sorted(META_TESTS_ROOT.rglob("*.py")):
...
def _location(path: Path, lineno: int, reason: str) -> str:
    return f"{path.relative_to(META_TESTS_ROOT).as_posix()}:{lineno}:{reason}"
