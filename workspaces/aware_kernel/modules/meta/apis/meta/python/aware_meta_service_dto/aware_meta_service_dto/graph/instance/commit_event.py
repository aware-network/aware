...
class MetaCommitActionMetadata(BaseModel):
    """
    Canonical Meta commit fan-out DTOs.
    These payloads are API truth for remote-safe Meta commit subscribers.
    They do not grant graph mutation authority to subscribers: services must
    re-enter Meta APIs for follow-up graph mutations.
    """

    # Attributes
    call_target: MetaGraphFunctionCallTarget | None = Field(default=None)
...
class MetaRequiredReactionReceipt(BaseModel):
    # Attributes
    reaction_key: str
    status: str
...
class MetaCommitArtifactRef(BaseModel):
    # Attributes
    artifact_kind: str
    artifact_ref: str
...
class MetaCommitEventEnvelope(BaseModel):
    # Attributes
    event_id: UUID
    event_family: str = Field(default="meta.oig_commit")
    schema_version: int = Field(default=1)
    emitted_at_unix_ms: int
    meta_authority_id: str
    actor_id: UUID
    ...
    commit_action: MetaCommitActionMetadata | None = Field(default=None)
    required_meta_reactions: list[MetaRequiredReactionReceipt] = Field(default_factory=list)
    artifact_refs: list[MetaCommitArtifactRef] = Field(default_factory=list)
    metadata: JsonObject = Field(default_factory=JsonObject)


class MetaCommitSubscriptionRequest(BaseModel):
    # Attributes
    subscriber_id: str
...
class MetaCommitSubscriptionResponse(BaseModel):
    # Attributes
    subscriber_id: str
