...
from aware_api_runtime.manifest.spec import AwareApiTomlSpec


@dataclass(frozen=True, slots=True)
class APIWorkspaceSnapshot:
    repo_root: Path
    package_root: Path
    spec_path: Path
...
class APIWorkspace:
    ...
    def __init__(self, *, spec_path: str | Path, repo_root: str | Path | None = None):
        ...
        if not resolved_spec_path.exists():
            raise FileNotFoundError(f"aware.api.toml not found: {resolved_spec_path}")
    ...
    @classmethod
    def from_toml(cls, *, toml_path: str | Path, repo_root: str | Path | None = None) -> APIWorkspace:
    ...
    def build_snapshot(self) -> APIWorkspaceSnapshot:
        spec = load_aware_api_toml_spec(toml_path=self._spec_path)
        ...
        if not sources_root.exists():
            raise FileNotFoundError(f"API sources_dir does not exist: {sources_root} (from {self._spec_path})")
        if not sources_root.is_dir():
            raise NotADirectoryError(f"API sources_dir must be a directory: {sources_root}")
