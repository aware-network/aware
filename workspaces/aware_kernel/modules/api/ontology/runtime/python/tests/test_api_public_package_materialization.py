...
from aware_meta.graph.config.package_strategy import (  # noqa: E402
...
def test_api_dependency_service_protocol_declares_external_api_package(
    tmp_path: Path,
) -> None:
    root = tmp_path
    toml_path = _write_api_toml(
        root,
        api_dto_semantic_export=False,
        include_api_dependency=True,
    )
    _write_api_source(root)
...
def test_compile_api_workspace_materializes_dart_public_package_when_requested(
    ...
    result = compile_api_workspace(
        ...
        dependency_repo_roots=(KERNEL_WORKSPACE_ROOT,),
    )
...
def test_dart_public_package_includes_api_dto_export_only_classes(
    ...
    assert "aware_home_api.door.BundleReleaseIdentity" in class_fqns
...
def test_dart_runtime_package_spec_rewrites_path_dependencies_to_runtime_root(
    tmp_path: Path,
) -> None:
    ...
    final_package_root.mkdir(parents=True)
    runtime_package_root.mkdir(parents=True)
    aware_api_root.mkdir(parents=True)
    helpers_root.mkdir(parents=True)

    final_api_rel = Path(
        os.path.relpath(aware_api_root.resolve(), start=final_package_root.resolve())
    ).as_posix()
    final_helpers_rel = Path(
        os.path.relpath(helpers_root.resolve(), start=final_package_root.resolve())
    ).as_posix()

    package = ObjectConfigGraphPackageSpec(
        name="aware_interface_service_api",
        package_name="aware_interface_service_api",
        package_root=final_package_root,
        dependencies=[
            "freezed_annotation: ^3.0.0",
            f"aware_api:\n  path: {final_api_rel}",
            f"aware_model_helpers:\n  path: {final_helpers_rel}",
        ],
        optional_dependencies={"dev": ["build_runner: ^2.4.11"]},
        metadata={
            "repo_root": repo_root.as_posix(),
            "path_dependencies": {
                "aware_api": "workspaces/aware_kernel/modules/api/libs/api/dart",
                "aware_model_helpers": (
                    "workspaces/aware_kernel/libs/model_helpers/dart/"
                    "aware_model_helpers"
                ),
            },
        },
    ...
    runtime_api_rel = Path(
        os.path.relpath(aware_api_root.resolve(), start=runtime_package_root.resolve())
    ).as_posix()
    runtime_helpers_rel = Path(
        os.path.relpath(helpers_root.resolve(), start=runtime_package_root.resolve())
    ).as_posix()

    assert runtime_spec.package_root == runtime_package_root
    assert runtime_spec.dependencies == [
        "freezed_annotation: ^3.0.0",
        f"aware_api:\n  path: {runtime_api_rel}",
        f"aware_model_helpers:\n  path: {runtime_helpers_rel}",
    ]
    assert (
        runtime_package_root / runtime_helpers_rel
    ).resolve() == helpers_root.resolve()
    assert (
        runtime_package_root / runtime_api_rel
    ).resolve() == aware_api_root.resolve()
...
def test_refresh_api_workspace_from_runtime_artifacts_prunes_public_package_dto_graph_from_class_refs(
    ...
    assert class_fqns == {
        ...
        "aware_home_api.door.LockDoorContext",
    }
