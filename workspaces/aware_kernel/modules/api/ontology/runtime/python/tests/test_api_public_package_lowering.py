...
def test_emit_public_package_plan_artifact_and_build_lowering_handoff(
    tmp_path: Path,
) -> None:
    root = tmp_path
    ...
    assert runtime_artifacts.public_package_plan.path.exists()
    assert (
        runtime_artifacts.public_package_plan.relpath
        == "runtime/api.public_package_plan.json"
    )
    public_payload = runtime_artifacts.public_package_plan.path.read_text(
        encoding="utf-8"
    )
    assert '"materialization_source": "api"' in public_payload
    assert '"aware_package_kind": "api_public_package"' in public_payload
    assert '"discriminant": "home_devices.lock_door.lock_door"' in public_payload
