...
def test_build_api_public_package_plan_from_api_ontology(tmp_path: Path) -> None:
    root = tmp_path
    ...
    assert public_plan.schema_version == 1
    assert public_plan.package_name == "home-story-api"
    assert public_plan.fqn_prefix == "aware_home_story_api"
    assert (
        public_plan.backend_handoff.materialization_source == MaterializationSource.api
    )
    assert public_plan.backend_handoff.aware_package_kind == "api_public_package"
    assert (
        public_plan.backend_handoff.expected_renderer_profile
        == API_PUBLIC_PACKAGE_RENDERER_PROFILE
    )

    api_plan = public_plan.apis[0]
...
def test_build_api_public_package_plan_requires_request_config(tmp_path: Path) -> None:
    root = tmp_path
    toml_path = _write_api_toml(root)
    _write_api_source(root)
    ...
    try:
        _ = build_api_public_package_plan(
            package_name=compile_plan.package_name,
            fqn_prefix=compile_plan.fqn_prefix,
        ...
        raise AssertionError(
            "Expected missing request config to fail public package planning"
        )
