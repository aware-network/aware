...
def _build_render_job(root: Path):
    ...
    return build_api_service_protocol_render_job(
        handoff=handoff,
...
def test_build_api_service_protocol_render_inputs_from_render_job(
    tmp_path: Path,
) -> None:
    render_job = _build_render_job(tmp_path)

    lowered = build_api_service_protocol_render_inputs(render_job=render_job)
    ...
    assert tuple(ref.path.as_posix() for ref in refs) == (
        ...
        "runtime/api.external_python_type_index.json",
    )
...
def test_build_api_service_protocol_render_inputs_fails_closed_on_missing_artifact(
    tmp_path: Path,
) -> None:
    render_job = _build_render_job(tmp_path)
    ...
    with pytest.raises(
        ValueError, match="Missing service protocol package runtime artifacts"
    ):
        build_api_service_protocol_render_inputs(render_job=broken)
