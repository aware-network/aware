...
def test_build_api_service_protocol_render_job_from_lowering_handoff(
    tmp_path: Path,
) -> None:
    root = tmp_path
    ...
    render_job = build_api_service_protocol_render_job(
        ...
        target=ApiServiceProtocolRenderTarget(
            ...
            description="Generated service protocol package test package.",
        ),
    )
    ...
    assert materialization_config.name == "api-service-protocol-python"
    ...
    assert materialization_config.import_root == "aware_home_story_protocol"
    assert len(materialization_config.packages) == 1

    package_spec = materialization_config.packages[0]
    assert package_spec.name == "aware_home_story_protocol"
    assert package_spec.package_name == "aware_home_story_protocol"
    assert package_spec.package_root == root / "sdk" / "python"
    assert package_spec.import_root == "aware_home_story_protocol"
    ...
    assert package_spec.metadata == {
        ...
        "api_public_package_import_root": "aware_home_story_api",
        "dependency_import_roots": ["aware_home_story_api"],
        "root_export_refs": ["protocols.AwareHomeStoryServiceProtocol"],
    }
