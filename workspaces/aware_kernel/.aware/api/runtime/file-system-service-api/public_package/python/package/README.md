...
Public API boundary for canonical FileSystem observation, delta exchange, apply, and backend capability receipts.

## Install

```bash
pip install aware-file-system-service-api
```
...
api = AwareFileSystemServiceApiClient(AwareApiEndpointInvoker(...))
```
