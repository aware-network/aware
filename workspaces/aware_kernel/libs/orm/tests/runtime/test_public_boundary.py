...
def test_legacy_bootstrap_package_is_removed() -> None:
    repo_root = _repo_root()
    bootstrap_root = repo_root / "workspaces/aware_kernel/libs/orm/aware_orm/bootstrap"
    assert not (bootstrap_root / "__init__.py").exists()
    assert list(bootstrap_root.glob("*.py")) == []

    scan_roots = (
        repo_root / "workspaces/aware_kernel/libs/orm/aware_orm",
        repo_root / "workspaces/aware_kernel/libs/orm/tests/runtime",
        repo_root / "tools/test-runner",
    )
    allowed_paths = {
        "workspaces/aware_kernel/libs/orm/tests/runtime/test_public_boundary.py",
    }
    offenders: list[str] = []
    ...
    for root in scan_roots:
        ...
        for path in sorted(root.rglob("*.py")):
            ...
            if "/aware_orm/bootstrap/" in f"/{rel}" or rel in allowed_paths:
                continue
            text = path.read_text(encoding="utf-8")
            if "aware_orm.bootstrap" in text:
                offenders.append(rel)

    assert offenders == []


def test_legacy_mutation_log_module_is_removed() -> None:
    repo_root = _repo_root()
    mutation_log_path = (
        repo_root / "workspaces/aware_kernel/libs/orm/aware_orm/session/mutation_log.py"
    )
    assert not mutation_log_path.exists()
...
def test_built_wheel_metadata_and_import_boundary(tmp_path: Path) -> None:
    ...
    assert "aware_orm/session/mutation_log.py" not in wheel_files
    ...
    assert base_names == {"msgpack", "pydantic", "python-dotenv"}
    ...
    assert not any(
        name.endswith("/aware_orm/session/mutation_log.py") for name in sdist_files
    )
    assert not any(
...
assert importlib.util.find_spec("aware_orm.session.mutation_log") is None
...
def test_runtime_public_api_does_not_advertise_environment_bundle_install() -> None:
    ...
    assert importlib.util.find_spec("aware_orm.runtime.bundle_runtime_install") is None
...
def test_runtime_public_api_exposes_payload_loaders_not_bundle_wrappers() -> None:
    runtime_module = importlib.import_module("aware_orm.runtime")

    removed_bundle_wrappers = {
        "install_bindings_from_bundle",
        "install_sql_metadata_from_bundle",
        "install_relationship_strategies",
    }
    for name in removed_bundle_wrappers:
        assert name not in runtime_module.__all__
