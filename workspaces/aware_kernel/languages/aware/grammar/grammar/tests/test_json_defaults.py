...
def test_json_defaults_parse_as_structured_values(tmp_path: Path) -> None:
    ...
    attrs = {
        attr.name: attr
        for link in foo.class_config_attribute_configs
        for attr in [link.attribute_config]
    }
...
def test_decimal_defaults_preserve_signed_and_exponent_source_text(
    tmp_path: Path,
) -> None:
    CodeLanguagePluginRegistry.register(AWARE_CODE_PLUGIN)

    path = tmp_path / "domains" / "alpha" / "beta" / "decimal_defaults.aware"
    path.parent.mkdir(parents=True, exist_ok=True)
    _ = path.write_text(
        (
            "class ExactValues {\n"
            "    signed Decimal = -123.4500\n"
            "    exponent Decimal = 1.25e+3\n"
            "}\n"
        ),
        encoding="utf-8",
    )
    ...
    default_by_name = {
        attr.name: attr.default_value_text
        for section in code.code_sections
        if section.type == CodeSectionType.attribute
        and (attr := section.code_section_attribute) is not None
    }
    assert default_by_name["signed"] == "-123.4500"
    assert default_by_name["exponent"] == "1.25e+3"
